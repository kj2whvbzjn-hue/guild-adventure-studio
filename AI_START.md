...
- **ハードコード禁止:** システム上、固定値である必要が明示されているものを除き、値をSourceへ固定記述してはならない。Build番号、ID、パス、識別子、列挙値、状態値など、正本・設定・入力データから取得または導出できる値は必ずそこから取得・導出する。
- その時点のCurrent値を直接埋め込んでテストや処理を通す実装は禁止する。固定値が必要な場合は、システム契約上固定である根拠を確認してから使用する。
