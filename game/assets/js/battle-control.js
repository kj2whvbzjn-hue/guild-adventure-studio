...
function performBasicAttack(attacker,target,{prechecked=false}={}){
 ...
 const hitRate=currentBattleHitRatePercent(attacker,target),hitRoll=currentBattleRoll(attacker,target,'BASIC_ATTACK','hit',0)*100;
 if(hitRoll>=hitRate){queueSceneEvent({attackerId:attacker.id,targetId:target.id,attackerName:attacker.name,attackerSide:attacker.side,miss:true,damage:0});battle.log.push(`[Tick ${battle.tick}] ${attacker.name}の通常攻撃 → ${target.name} MISS（命中率${hitRate.toFixed(2)}%）`);typeof recordValidationEvent==='function'&&recordValidationEvent('basic_attack_miss',{source_id:attacker.id,target_id:target.id,accuracy:currentBattleAccuracy(attacker),evasion:currentBattleEvasion(target),hit_rate:hitRate,hit_roll:hitRoll});finishIfNeeded();return true;}
 const baseDamage=Math.max(0,effectiveAttackValue(attacker)),resistance=effectiveDamageResist(target),postResistance=Math.max(0,baseDamage*(1-resistance/100)),criticalRate=currentBattleCriticalRatePercent(attacker),criticalRoll=currentBattleRoll(attacker,target,'BASIC_ATTACK','critical',0)*100,critical=criticalRoll<criticalRate,criticalDamage=currentBattleCriticalDamagePercent(attacker),criticalMultiplier=critical?1+criticalDamage/100:1,finalDamage=Math.max(0,Math.floor(postResistance*criticalMultiplier)),shield=consumeShieldDamage(target,finalDamage,{sourceId:attacker.id,damageType:'basic_attack'}),damage=shield.hpDamage,before=target.hp;target.hp=Math.max(0,target.hp-damage);const applied=before-target.hp;
