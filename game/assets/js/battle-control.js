...
function simulateFormalAdventureBattle({party,formation,monsters,seed=1,maxTicks=200000}={}){
 ...
 try{
  ...
  const enemies=expanded.map((row,i)=>makeCombatant({id:`E${i}`,monsterId:row.monster_id,name:row.name,side:'敵',formationPosition:row.formationPosition,aiPolicy:row.aiPolicy,ownedSkillIds:clone(row.skillIds||[]),defaultSkillId:row.defaultSkillId,agi:row.agi,attack:row.attack,maxHp:row.maxHp,maxMp:row.formalGenerated?row.maxMp:100,mp:row.formalGenerated?row.maxMp:100,gauge:0,actions:0,order:100+i,lastActionTick:null}));
