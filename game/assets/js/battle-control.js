...
function resetBattle(context=null){pauseBattle();setBattleLaunchContext(context||standaloneBattleContext());sceneQueue=[];sceneBusy=false;battle={tick:0,actions:0,units:makeBattleUnits(),log:[],timer:null,running:false,runToken:battle.runToken,lastFrameAt:0,tickAccumulator:0,result:null,pendingResult:null,ending:false,reward:null,rewardApplied:false,validationMode:false,validationCaptureEvents:true,validationEvents:[],validationMeta:null};initializeBattleTieRolls();renderBattle();recordValidationEvent('battle_started',{seed:battleLaunchContext?.seed??battle.p0113TieSeed??null,source:battleLaunchContext?.source||'standalone_fixture'});if(typeof dispatchCurrentBattleStartPassiveReactives==='function')dispatchCurrentBattleStartPassiveReactives();ensureSceneUnits(true);setupTagSkillTestUI();populateTagSkillTestUI()}
function resetBattleSessionToIdle(){pauseBattle();clearBattleLaunchContext();sceneQueue=[];sceneBusy=false;sceneSignature='';sceneLastActionCount=0;battle={tick:0,actions:0,units:[],log:[],timer:null,running:false,runToken:battle.runToken,lastFrameAt:0,tickAccumulator:0,result:null,pendingResult:null,ending:false,reward:null,rewardApplied:false,validationMode:false,validationCaptureEvents:true,validationEvents:[],validationMeta:null};renderBattle();ensureSceneUnits(true);return battle}
...
function ensureBattleFormationSafePoint(reason='state_safe_point'){
 const changes=[];
 for(const side of ['味方','敵']){
  const alive=battle.units.filter(u=>u.alive&&u.side===side);if(!alive.length)continue;
  const front=alive.filter(u=>String(u.formationPosition||'FRONTLINE')==='FRONTLINE');if(front.length)continue;
  const back=alive.filter(u=>String(u.formationPosition||'FRONTLINE')==='BACKLINE');
  if(!back.length)continue;
  for(const u of back){u.formationPosition='FRONTLINE';changes.push({unit_id:u.id,side,from:'BACKLINE',to:'FRONTLINE'});}
 }
 if(changes.length&&typeof recordValidationEvent==='function')recordValidationEvent('formation_forced_advance',{reason,changes});
 return changes;
}
...
function formalAiCatalog(){return window.GKGameAICatalogLoader?.current?.()||null;}
function formalAiResolutionForActor(actor){
 if(!actor||!window.GKGameAISaveBridge||!window.GKGameAIBattleBridge)return null;
 return GKGameAISaveBridge.resolveRuntimeForActor({save:data,catalog:formalAiCatalog(),actor});
}
function formalAiRuntimeForActor(actor){return formalAiResolutionForActor(actor)?.runtime||null;}
function formalAiTargetContract(compiled){
 const tc=compiled?.definition?.runtimeContracts?.targetContract||compiled?.definition?.target;if(!tc||typeof tc!=='object'||Array.isArray(tc))return null;
 const side=String(tc.side||'').trim().toUpperCase(),range=String(tc.range||'').trim().toUpperCase();if(!side||!range)return null;
 return{side,range,...(tc.randomCount!=null?{randomCount:tc.randomCount}:{}),excludeSelf:tc.excludeSelf===true};
}
function formalAiTargetRequirement(actionId,compiled){
 const id=String(actionId||'');if(id==='attack')return'REQUIRED';if(!id.startsWith('skill:'))return'UNRESOLVED';
 const tc=formalAiTargetContract(compiled);if(!tc||!globalThis.GKSAIProgramValidator?.selectorRequirement)return'UNRESOLVED';
 return GKSAIProgramValidator.selectorRequirement({actionEvaluator:'action.skill',targetContract:tc,wait:false});
}
function formalAiReservationTargetMode(actionId,compiled){const requirement=formalAiTargetRequirement(actionId,compiled);return requirement==='REQUIRED'?'RESERVATION_FIXED':requirement==='FORBIDDEN'?'EXECUTION_PRECHECK':'UNRESOLVED';}
function formalAiSkillAccess(actor,skillId){
 const checker=window.GKGameSkillLoadout?.skillUseCheck;if(typeof checker!=='function')return{ok:false,reason:'LOADOUT_RUNTIME_UNAVAILABLE'};
 if(!actor?.characterId&&!actor?.monsterId)return{ok:false,reason:'FORMAL_AI_OWNER_KIND_REQUIRED'};
 const ownerKindPolicy={requireEquipped:!!actor.characterId};return checker({skills:actor.ownedSkillIds||[],skillLoadoutIds:actor.selectedSkillIds||[],equippedSkillId:actor.equippedSkillId||''},skillId,{...ownerKindPolicy,requireEquipped:false,requireSelected:!!actor.characterId});
}
function formalAiSkillState(actor,skillId){
 const id=String(skillId||'').trim(),skill=formalBattleSkillExact(id);if(!id||!skill)return{ready:false,usable:false,target_contract:null,reason:'SKILL_NOT_FOUND'};
 const compiled=compileSkillForRuntime(skill);if(!compiled?.ok)return{ready:false,usable:false,target_contract:null,reason:'SKILL_COMPILE_FAILED'};
 const targetContract=formalAiTargetContract(compiled);if(!targetContract)return{ready:false,usable:false,target_contract:null,reason:'SKILL_TARGET_CONTRACT_MISSING'};
 const access=formalAiSkillAccess(actor,id);if(!access.ok)return{ready:false,usable:false,target_contract:targetContract,reason:access.reason||'SKILL_ACCESS_BLOCKED'};
 const eligibility=typeof actionExecutionEligibility==='function'?actionExecutionEligibility(actor,{actionKind:'skill_action',skillId:id,cooldown:compiled.definition?.parameters?.cooldown,compiled}):{ok:false,reason:'ACTION_ELIGIBILITY_UNAVAILABLE'};
 if(!eligibility.ok)return{ready:false,usable:false,target_contract:targetContract,reason:eligibility.reason||'SKILL_NOT_READY'};
 const ctx={actor_id:String(actor.id||''),units:battle.units};let legal=[];try{legal=GKGameAIBattleBridge.legalCandidates(ctx,targetContract);}catch(_){legal=[];}
 return{ready:true,usable:legal.length>0,target_contract:targetContract,reason:legal.length?null:'NO_VALID_TARGET'};
}
function formalAiProjectedUnits(actor,runtime){
 const refs=new Set(((actor?.characterId?actor?.selectedSkillIds:actor?.ownedSkillIds)||[]).map(String));for(const instruction of runtime?.instructions||[]){const id=String(instruction?.params?.skill_id||'').trim();if(id)refs.add(id);}
...
function reserveFormalAiAction(actor,runtime,{threshold=actor.gauge,phase='rethink'}={}){
 ...
 if(actionId==='attack'){
  const target=battle.units.find(unit=>unit.alive&&unit.id===String(proposal.target_id||''));if(!target){actor.reservedAction=null;battle.log.push(`[Tick ${battle.tick}] ${actor.name}のFormal AI通常攻撃Targetが見つかりません`);return false;}
  actor.reservedAction={...base,actionId,type:'attack',targetId:target.id,targetResolution:'RESERVATION_FIXED',label:'通常攻撃',icon:'⚔️'};
 }else if(actionId.startsWith('skill:')){
  const skillId=actionId.slice(6),access=formalAiSkillAccess(actor,skillId);
  if(!access.ok){actor.reservedAction=null;battle.log.push(`[Tick ${battle.tick}] ${actor.name}のFormal AI指定Skillは現在使用できません — ${skillId} (${access.reason})`);typeof recordValidationEvent==='function'&&recordValidationEvent('formal_ai_skill_blocked',{source_id:actor.id,program_id:runtime.program_id,skill_id:skillId,reason:access.reason,selected_skill_ids:[...(actor.selectedSkillIds||[])],owned_skill_ids:[...(actor.ownedSkillIds||[])]});return false;}
  ...
  const compiled=compileSkillForRuntime(skill),targetContract=formalAiTargetContract(compiled),mode=formalAiReservationTargetMode(actionId,compiled);if(!compiled?.ok||!targetContract||mode==='UNRESOLVED'){actor.reservedAction=null;battle.log.push(`[Tick ${battle.tick}] ${actor.name}のFormal AI Target契約を解決できません — ${skillId}`);return false;}
  let target=null;
  if(mode==='RESERVATION_FIXED'){
   let legal=[];try{legal=GKGameAIBattleBridge.legalCandidates({actor_id:actor.id,units:battle.units},targetContract);}catch(_){legal=[];}
   target=legal.find(unit=>String(unit.id)===String(proposal.target_id||''))||null;if(!target){actor.reservedAction=null;battle.log.push(`[Tick ${battle.tick}] ${actor.name}のFormal AI固定Targetが予約時点で無効です — ${skillId}`);return false;}
  }else if(proposal.target_id!=null&&String(proposal.target_id)!==''){actor.reservedAction=null;battle.log.push(`[Tick ${battle.tick}] ${actor.name}のFormal AI targetless ActionにTargetが混入しました — ${skillId}`);return false;}
...
function precheckSkillExecution(actor,r,skill,target){
 if(r?.formalAi){const access=formalAiSkillAccess(actor,skill?.id);if(!access.ok)return{ok:false,reason:`Formal AI指定Skillは現在使用できません (${access.reason})`,code:access.reason||'SKILL_ACCESS_BLOCKED',skill,access};}
 ...
 const targetContract=formalAiTargetContract(compiled),conditionTarget=target||(targetContract?.side==='SELF'?actor:null),conditionResult=evaluateTaggedSkillConditions(actor,compiled,conditionTarget);if(!conditionResult.ok)return{ok:false,reason:'発動条件不成立',code:'CONDITION_FAILED',skill,compiled,conditionResult};
 ...
 const executionSnapshot={checkedAt:battle.tick,skillId:compiled.definition.id,targetIds:resolved.targets.map(x=>x.id),targetStates:resolved.targets.map(x=>({id:x.id,alive:x.alive!==false,hp:Number(x.hp)||0})),presentationTargetId:r?.targetId||target?.id||null,targetResolution:r?.targetResolution||null,rngStream:targetContract?.range==='RANDOM'?'BATTLE_EXECUTION':null,costs:(eligibility.costCheck?.costs||[]).map(x=>({...x})),conditionResult:JSON.parse(JSON.stringify(conditionResult))};
 return{ok:true,target:resolved.targets[0]||target||null,skill,compiled,actionKind:'skill',executionSnapshot};
}
function evaluateCandidateExecution(actor){
 ...
 if(r.type==='attack'){const target=battle.units.find(unit=>unit.id===String(r.targetId||''));if(!target)return{ok:false,reason:'候補対象が行動順到達時点で存在しません',code:'NO_VALID_TARGET'};if(actor.basicAttackUnavailableReason)return{ok:false,reason:actor.basicAttackUnavailableReason,code:'BASIC_ATTACK_USE_REQUIREMENT_FAILED'};if(!target.alive)return{ok:false,reason:'通常攻撃の対象が行動順到達時点で無効です',code:'NO_VALID_TARGET'};if(target.side===actor.side)return{ok:false,reason:'通常攻撃の対象が敵ではありません',code:'INVALID_ATTACK_TARGET'};const eligibility=typeof actionExecutionEligibility==='function'?actionExecutionEligibility(actor,{actionKind:'normal_action'}):{ok:true};if(!eligibility.ok)return{ok:false,reason:'行動不能',code:eligibility.reason||'ACTION_DISABLED',eligibility};return{ok:true,target,actionKind:'attack'};}
 if(r.type==='skill'){
  const skill=formalBattleSkillExact(r.skillId)||formalBattleSkill(r.skillId);if(!skill)return{ok:false,reason:'候補の正式Production Skillがありません',code:'NO_FORMAL_PRODUCTION_SKILL'};
  const compiled=compileSkillForRuntime(skill),mode=formalAiReservationTargetMode(`skill:${skill.id}`,compiled);if(!compiled?.ok||mode==='UNRESOLVED')return{ok:false,reason:'Skill Target契約を解決できません',code:'TARGET_CONTRACT_UNRESOLVED'};
  let target=null;if(mode==='RESERVATION_FIXED'){target=battle.units.find(unit=>unit.id===String(r.targetId||''));if(!target)return{ok:false,reason:'固定Targetが行動順到達時点で存在しません',code:'NO_VALID_TARGET'};}else if(r.targetId!=null&&String(r.targetId)!=='')return{ok:false,reason:'targetless Actionに固定Targetが混入しています',code:'TARGETLESS_RESERVATION_HAS_TARGET'};
  return precheckSkillExecution(actor,r,skill,target);
 }
...
function basicAttackStrikeProfiles(attacker){
 ...
 if(!rows)return[{weaponStyle:String(attacker?.weaponStyle||'single'),weaponSlot:null,weaponId:null,attack:Number(attacker?.attack)||0,accuracy:Number(attacker?.accuracy)||0,weaponCriticalRate:Number(attacker?.weaponCriticalRate)||0}];
 return rows.map((row,index)=>({weaponStyle:String(row?.weaponStyle||attacker?.weaponStyle||'single'),weaponSlot:row?.weaponSlot||null,weaponId:row?.weaponId||null,attack:Number.isFinite(Number(row?.attack))?Number(row.attack):Number(attacker?.attack)||0,accuracy:Number.isFinite(Number(row?.accuracy))?Number(row.accuracy):Number(attacker?.accuracy)||0,weaponCriticalRate:Number.isFinite(Number(row?.weaponCriticalRate))?Number(row.weaponCriticalRate):Number(attacker?.weaponCriticalRate)||0,strikeIndex:index}));
}
function performBasicAttack(attacker,target,{prechecked=false}={}){
 if(!target)return false;
 if(attacker?.basicAttackUnavailableReason){typeof recordValidationEvent==='function'&&recordValidationEvent('basic_attack_blocked',{source_id:attacker?.id||null,target_id:target?.id||null,reason:attacker.basicAttackUnavailableReason});battle.log.push(`[Tick ${battle.tick}] ${attacker.name}の通常攻撃は使用不可 — ${attacker.basicAttackUnavailableReason}`);return false}
 ...
 const profiles=basicAttackStrikeProfiles(attacker),dual=String(attacker?.weaponStyle||profiles[0]?.weaponStyle||'single')==='dual_wield',strikes=dual?profiles.slice(0,2):profiles.slice(0,1),triggerActionContext=typeof createTaggedTriggerActionContext==='function'?createTaggedTriggerActionContext(attacker,{definition:{id:'BASIC_ATTACK'}},null):null,basicAttackCompiled={definition:{id:'BASIC_ATTACK',name:'通常攻撃',target:{side:'enemy',range:'single'},logicOrder:['ATTACK']}};let processed=false;
 for(let strikeIndex=0;strikeIndex<strikes.length;strikeIndex++){
  if(!target.alive||battle.result||battle.pendingResult)break;
  const profile=strikes[strikeIndex],sourceView={...attacker,attack:profile.attack,accuracy:profile.accuracy,weaponCriticalRate:profile.weaponCriticalRate},criticalResolution=currentBattleCriticalResolution(sourceView),criticalRate=Math.max(0,Math.min(100,criticalResolution.final_critical_rate*100)),criticalRoll=currentBattleRoll(attacker,target,'BASIC_ATTACK','critical',strikeIndex)*100,critical=criticalRoll<criticalRate;processed=true;
  const hitBypass=critical?'CRITICAL_GUARANTEED_HIT':null,hitRate=critical?null:currentBattleHitRatePercent(sourceView,target),hitRoll=critical?null:currentBattleRoll(attacker,target,'BASIC_ATTACK','hit',strikeIndex)*100,reactiveEventContext={actionSourceId:attacker.id,targetId:target.id,skillId:'BASIC_ATTACK',hitIndex:strikeIndex,actionContext:triggerActionContext};if(triggerActionContext&&critical)queueCurrentBattlePassiveReactive(attacker,'critical',reactiveEventContext);
  if(!critical&&hitRoll>=hitRate){if(triggerActionContext)queueCurrentBattlePassiveReactive(target,'evade',reactiveEventContext);queueSceneEvent({attackerId:attacker.id,targetId:target.id,attackerName:attacker.name,attackerSide:attacker.side,miss:true,damage:0});battle.log.push(`[Tick ${battle.tick}] ${attacker.name}の通常攻撃${dual?` Hit${strikeIndex+1}`:''} → ${target.name} MISS（命中率${hitRate.toFixed(2)}%）`);typeof recordValidationEvent==='function'&&recordValidationEvent('basic_attack_miss',{source_id:attacker.id,target_id:target.id,weapon_style:profile.weaponStyle,weapon_slot:profile.weaponSlot,weapon_id:profile.weaponId,strike_index:strikeIndex,accuracy:currentBattleAccuracy(sourceView),evasion:currentBattleEvasion(target),critical_rate:criticalRate,critical_roll:criticalRoll,critical:false,hit_bypass:null,hit_rate:hitRate,hit_roll:hitRoll});if(triggerActionContext)runCurrentBattleHitSafePoint(attacker,target,basicAttackCompiled,{ok:false,miss:true,damage:0},{origin:'base',triggerActionContext,hitIndex:strikeIndex});continue;}
  if(triggerActionContext)queueCurrentBattlePassiveReactive(attacker,'hit_dealt',reactiveEventContext);const baseDamage=Math.max(0,currentBattleResolveDirectModifierTarget(effectiveAttackValue(sourceView),'PHYSICAL_DAMAGE',attacker).final_value),resistance=effectiveDamageResist(target),postResistance=Math.max(0,baseDamage*(1-resistance/100)),criticalDamage=currentBattleCriticalDamagePercent(attacker),criticalBonusReduction=currentBattleCriticalBonusDamageReduction(target),criticalBonus=critical?(criticalDamage/100)*(1-criticalBonusReduction):0,criticalMultiplier=1+criticalBonus,formationMultiplier=String(attacker?.formationPosition||'FRONTLINE')==='BACKLINE'?0.5:1,finalDamage=Math.max(0,Math.floor(postResistance*criticalMultiplier*formationMultiplier)),baseBlockRate=Number(target?.blockRate??target?.block_rate)||0,blockRate=Math.max(0,Math.min(1,currentBattleResolveDirectModifierTarget(baseBlockRate,'BLOCK_RATE',target).final_value)),blockRoll=blockRate>0?currentBattleRoll(attacker,target,'BASIC_ATTACK','block',strikeIndex):null,block=typeof resolveCurrentBlockDamage==='function'?resolveCurrentBlockDamage(target,finalDamage,blockRoll):{blocked:false,blockRate:0,blockRoll:null,blockDamageCutRate:0,damage:finalDamage};if(triggerActionContext&&block.blocked)queueCurrentBattlePassiveReactive(target,'block',reactiveEventContext);const shield=consumeShieldDamage(target,block.damage,{sourceId:attacker.id,damageType:'basic_attack'}),damage=shield.hpDamage,before=target.hp,projectedHp=before-damage,fatalInterrupt=projectedHp<=0&&typeof resolveFatalDamageInterrupt==='function'?resolveFatalDamageInterrupt(target,{actionSourceId:attacker.id,skillId:'BASIC_ATTACK',hitIndex:strikeIndex,projectedHp,actionContext:triggerActionContext,damageKind:'BASIC_ATTACK_DAMAGE'}):{triggered:false,projectedHp};target.hp=fatalInterrupt.triggered?fatalInterrupt.resolvedHp:Math.max(0,projectedHp);if(typeof evaluateLowHpPassivesAfterHpCommit==='function')evaluateLowHpPassivesAfterHpCommit(target,{reason:'BASIC_ATTACK_DAMAGE',sourceId:attacker.id,skillId:'BASIC_ATTACK'});const applied=before-target.hp;if(triggerActionContext)queueCurrentBattlePassiveReactive(target,'hit_received',reactiveEventContext);
  ...
  battle.log.push(`[Tick ${battle.tick}] ${attacker.name}の通常攻撃${dual?` Hit${strikeIndex+1}`:''} → ${target.name}に${applied}HPダメージ（基礎${baseDamage}、耐性${resistance}%、Crit${critical?'ON':'OFF'}、シールド吸収${shield.absorbed}、残HP ${target.hp}/${target.maxHp}）`);recordValidationEvent('basic_attack',{source_id:attacker.id,target_id:target.id,weapon_style:profile.weaponStyle,weapon_slot:profile.weaponSlot,weapon_id:profile.weaponId,strike_index:strikeIndex,accuracy:currentBattleAccuracy(sourceView),evasion:currentBattleEvasion(target),hit_bypass:hitBypass,hit_rate:hitRate,hit_roll:hitRoll,base_damage:baseDamage,resistance,post_resistance_damage:postResistance,weapon_critical_rate:criticalResolution.weapon_critical_rate,LUK:criticalResolution.LUK,base_critical_rate:criticalResolution.base_critical_rate,final_critical_rate:criticalResolution.final_critical_rate,critical_rate:criticalRate,critical_roll:criticalRoll,result:critical,critical,critical_contributions:criticalResolution.contributions,critical_damage:criticalDamage,critical_multiplier:criticalMultiplier,formation_position:String(attacker?.formationPosition||'FRONTLINE'),formation_multiplier:formationMultiplier,final_damage:finalDamage,block_rate:block.blockRate,block_roll:block.blockRoll,blocked:block.blocked,block_damage_cut_rate:block.blockDamageCutRate,blocked_damage:block.damage,fatal_interrupt:!!fatalInterrupt.triggered,fatal_passive_id:fatalInterrupt.passiveId||null,damage:applied,hp_before:before,hp_after:target.hp,shield_absorbed:shield.absorbed});
  if(target.hp<=0){if(typeof resetCombatantOnDeath==='function')resetCombatantOnDeath(target,{reason:'basic_attack',sourceId:attacker.id});else{target.alive=false;target.gauge=0;target.reservedAction=null;target.castingAction=null;}recordValidationEvent('basic_attack_ko',{source_id:attacker.id,target_id:target.id,weapon_style:profile.weaponStyle,weapon_slot:profile.weaponSlot,weapon_id:profile.weaponId,strike_index:strikeIndex});battle.log.push(`[Tick ${battle.tick}] ${target.name}は戦闘不能`);ensureBattleFormationSafePoint('post_basic_attack_hp_alive_mutation');}if(triggerActionContext)runCurrentBattleHitSafePoint(attacker,target,basicAttackCompiled,{ok:true,miss:false,damage:applied,shieldAbsorbed:shield.absorbed},{origin:'base',triggerActionContext,hitIndex:strikeIndex});if(target.hp<=0)break;
 }
...
function createFormalBattleSnapshot({battleId,settingsVersion,seed,formation}={}){
 ...
 return Boundary.createBattleSnapshot({battleId:String(battleId||`BATTLE-${String(seed)}`),settingsVersion:String(settingsVersion||APP_RUNTIME_GAME_BUILD||'unknown'),seed,actors,formation:clone(formation||[]),fixedOrder:formalBattleFixedOrder()});
}
...
function simulateFormalAdventureBattle({party,formation,monsters,seed=1,maxTicks=200000,battleId="",settingsVersion=""}={}){
 ...
 try{
  battleLaunchContext={formation:GKAdventureBattleCore.normalizeFormation(formation),monsters:clone(monsters||[]),seed,battle_id:String(battleId||''),settings_version:String(settingsVersion||APP_RUNTIME_GAME_BUILD||''),source:'adventure_questrun'};
  const allies=normalizedParty.map((row,i)=>makeCombatant({id:`A${i}`,characterId:String(row?.character_id||row?.id||''),name:String(row?.name||`Adventurer ${i+1}`),side:'味方',formationPosition:String(row?.formation_position||'FRONTLINE').toUpperCase()==='BACKLINE'?'BACKLINE':'FRONTLINE',ownedSkillIds:Array.isArray(row?.skills)?clone(row.skills):[],selectedSkillIds:Array.isArray(row?.selected_skill_ids)?clone(row.selected_skill_ids):[],equippedSkillId:String(row?.equipped_skill_id||row?.equippedSkillId||''),defaultSkillId:String(row?.selected_skill_ids?.[0]||''),agi:Math.floor(requiredFormalCombatNumber(row?.agi,`party[${i}].agi`,{min:0})),attack:Math.floor(requiredFormalCombatNumber(row?.attack??row?.atk,`party[${i}].attack`,{min:0})),accuracy:Math.max(0,Number(row?.accuracy)||0),evasion:Math.max(0,Number(row?.evasion)||0),magicAccuracy:Math.max(0,Number(row?.magic_accuracy??row?.magicAccuracy)||0),magicResistance:Math.max(0,Number(row?.magic_resistance??row?.magicResistance)||0),magicWeaponBonus:Math.max(0,Number(row?.magic_weapon_bonus??row?.magicWeaponBonus)||0),luk:Math.max(0,Number(row?.luk??row?.LUK)||0),weaponCriticalRate:Math.max(0,Number(row?.weapon_critical_rate??row?.weaponCriticalRate)||0),equipmentPerformance:clone(row?.equipment_performance||row?.equipmentPerformance||null),formalAiBinding:clone(row?.formal_ai_binding||row?.formalAiBinding||null),formalModifierContributions:Array.isArray(row?.formalModifierContributions)?clone(row.formalModifierContributions):[],formalPassiveRuntimeContracts:Array.isArray(row?.formalPassiveRuntimeContracts)?clone(row.formalPassiveRuntimeContracts):[],magicIncreaseRate:Number.isFinite(Number(row?.magic_increase_rate??row?.magicIncreaseRate))?Math.max(0,Number(row?.magic_increase_rate??row?.magicIncreaseRate)):undefined,criticalDamage:Number.isFinite(Number(row?.critical_damage??row?.criticalDamage))?Math.max(0,Number(row?.critical_damage??row?.criticalDamage)):undefined,damageResist:Number.isFinite(Number(row?.damage_resist??row?.damageResist))?Math.max(0,Number(row?.damage_resist??row?.damageResist)):undefined,statusResistance:row?.status_resistance??row?.statusResistance??{},maxHp:Math.floor(requiredFormalCombatNumber(row?.max_hp??row?.maxHp??row?.hp,`party[${i}].maxHp`,{min:1})),maxMp:Math.floor(requiredFormalCombatNumber(row?.max_mp??row?.maxMp,`party[${i}].maxMp`,{min:0})),mp:Math.floor(requiredFormalCombatNumber(row?.mp,`party[${i}].mp`,{min:0})),gauge:0,actions:0,order:i,lastActionTick:null}));
  ...
  const enemies=expanded.map((row,i)=>{if(row?.formalGenerated!==true)throw Object.assign(new Error(`Formal Adventure Battle requires Formal Monster combat values: ${row?.monster_id||i}`),{code:'FORMAL_ADVENTURE_MONSTER_REQUIRED',monster_id:row?.monster_id??null});return makeCombatant({id:`E${i}`,monsterId:row.monster_id,name:row.name,side:'敵',formationPosition:row.formationPosition,formalAiBinding:row.formalAiBinding?clone(row.formalAiBinding):null,ownedSkillIds:clone(row.skillIds||[]),agi:row.agi,attack:row.attack,accuracy:requiredFormalCombatNumber(row?.accuracy,`monster[${row?.monster_id||i}].accuracy`,{min:0}),maxHp:row.maxHp,maxMp:row.maxMp,mp:row.maxMp,gauge:0,actions:0,order:100+i,lastActionTick:null});});
  ...
  initializeBattleTieRolls(seed);battle.contractSnapshot=createFormalBattleSnapshot({battleId:battleLaunchContext.battle_id,settingsVersion:battleLaunchContext.settings_version,seed,formation:battleLaunchContext.formation});recordValidationEvent('battle_started',{seed,source:'adventure_questrun',battle_id:battleLaunchContext.battle_id||null,fixed_order:formalBattleFixedOrder()});if(typeof dispatchCurrentBattleStartPassiveReactives==='function')dispatchCurrentBattleStartPassiveReactives();
  ...
  const result=GKAdventureBattleCore.buildBattleResult({battle,context:battleLaunchContext});result.battle_id=String(battleLaunchContext.battle_id||"");result.battle_snapshot=clone(battle.contractSnapshot);result.reward={};return result;
...
window.GKGameAIReservationExecution=Object.freeze({targetRequirement:formalAiTargetRequirement,reservationTargetMode:formalAiReservationTargetMode,evaluateCandidateExecution});
