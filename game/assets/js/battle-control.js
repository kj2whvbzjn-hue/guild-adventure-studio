...
function performBasicAttack(attacker,target,{prechecked=false}={}){
 ...
 for(let strikeIndex=0;strikeIndex<strikes.length;strikeIndex++){
  ...
  const profile=strikes[strikeIndex],sourceView={...attacker,attack:profile.attack,accuracy:profile.accuracy,weaponCriticalRate:profile.weaponCriticalRate},hitRate=currentBattleHitRatePercent(sourceView,target),hitRoll=currentBattleRoll(attacker,target,'BASIC_ATTACK','hit',strikeIndex)*100;processed=true;
