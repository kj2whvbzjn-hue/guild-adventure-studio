...
function currentBattleModifierEntries(unit){const rows=unit?.formalModifierContributions;if(rows==null)return[];if(!Array.isArray(rows))throw Object.assign(new Error('Game Battle formalModifierContributionsは配列が必要です。'),{code:'GAME_BATTLE_MODIFIER_CONTRIBUTIONS_INVALID'});return rows}
...
const CURRENT_BATTLE_DIRECT_MODIFIER_TARGETS=Object.freeze(['BLOCK_RATE','BLOCK_PERFORMANCE','CRITICAL_DAMAGE','CRITICAL_BONUS_DAMAGE_REDUCTION','MP_COST_REDUCTION','COOLDOWN_REDUCTION','CAST_TIME_REDUCTION']);
function currentBattleDirectModifierTargetConnected(target){const key=String(target||'').toUpperCase();return CURRENT_BATTLE_DIRECT_MODIFIER_TARGETS.includes(key)||key.startsWith('STATUS_RESISTANCE:')}
...
function executeSkillRuntime(actor,target,skillSource,{manual=false,isFollowUp=false,origin=null,suppressDerived=false,derivedGeneration=0,skipExecutionEligibility=false,triggerActionContext=null,executionSnapshot=null}={}){
 ...
 for(const originalTarget of resolved.targets){
  ...
  for(const logic of compiled.definition.logicOrder){
   ...
   else if(logic==='ATTACK'){attackResult=executeRuntimeDamageRuntime(actor,actionTarget,compiled,{triggerActionContext:actionTriggerContext})||applyTaggedDamage(actor,actionTarget,calculateTaggedAttackDamage(actor,compiled.definition),compiled.definition);attackSucceeded=!!attackResult?.ok}
   ...
   else if(logic==='FOLLOW_UP'){followUpResult=executeRuntimeDamageRuntime(actor,actionTarget,compiled,{triggerActionContext:actionTriggerContext})||applyTaggedDamage(actor,actionTarget,calculateTaggedAttackDamage(actor,compiled.definition),compiled.definition);attackSucceeded=!!followUpResult?.ok}
