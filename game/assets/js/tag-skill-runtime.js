...
function compileTaggedSkill(skill){
 ...
 if(g.has('COVER')){
  ...
  if(coverLifetime==='persistent'){
   ...
   // Structured SkillではDURATIONはSHIELD等の別contract由来になり得る。COVER lifetimeはgenericRuntimeを正本とする。
   if(n.DURATION&&!structuredTargetControl)errors.push('COVER_LIFETIME=persistentではDURATIONを指定できません');
  }
  ...
  // 新仕様SkillはeffectContracts/applyContractsが複合可否の正本。旧tag-only Skillだけ従来のCOVER専用制約を維持する。
  if(mixed.length&&!genericRuntime)errors.push(`初回COVER定義は専用関係のみです: ${mixed.join(',')}`);
 }
 if(g.has('COUNTER')){
