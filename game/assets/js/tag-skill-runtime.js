...
function applyCurrentFormalDamage(attacker,target,baseDamage,compiled,contract,{hitIndex=0,actionContext=null}={}){
 ...
 const resistance=effectiveDamageResist(target),postResistance=Math.max(0,baseDamage*(1-resistance/100)),criticalDamage=currentBattleCriticalDamagePercent(attacker),criticalBonusReduction=typeof currentBattleCriticalBonusDamageReduction==='function'?currentBattleCriticalBonusDamageReduction(target):0,criticalBonus=critical?(criticalDamage/100)*(1-criticalBonusReduction):0,criticalMultiplier=1+criticalBonus,preElementDamage=Math.max(0,postResistance*criticalMultiplier),elementDamage=Array.isArray(contract.elementComponents)&&contract.elementComponents.length?currentBattleResolveElementDamage(target,preElementDamage,contract.elementComponents):{elemental:false,totalDamage:preElementDamage,components:[]},formationMultiplier=(()=>{const r=String(compiled?.definition?.target?.range||'').toLowerCase(),pos=String(attacker?.formationPosition||attacker?.formation_position||'FRONTLINE').toUpperCase();return pos==='BACKLINE'&&['single','front','random'].includes(r)?0.5:1})(),finalDamage=Math.max(0,Math.floor(elementDamage.totalDamage*formationMultiplier));
