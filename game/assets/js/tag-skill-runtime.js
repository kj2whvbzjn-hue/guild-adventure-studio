...
function applyCurrentFormalDamage(attacker,target,baseDamage,compiled,contract,{hitIndex=0}={}){
 ...
 const resistance=effectiveDamageResist(target),postResistance=Math.max(0,baseDamage*(1-resistance/100)),criticalDamage=currentBattleCriticalDamagePercent(attacker),criticalMultiplier=critical?1+criticalDamage/100:1,finalDamage=Math.max(0,Math.floor(postResistance*criticalMultiplier));
 const blockRate=Number(target?.blockRate??target?.block_rate)||0,blockRoll=blockRate>0?currentBattleRoll(attacker,target,compiled?.definition?.id,'block',hitIndex):null,block=resolveCurrentBlockDamage(target,finalDamage,blockRoll),before=target.hp,shield=consumeShieldDamage(target,block.damage,{sourceId:attacker.id,skillId:compiled.definition.id,damageType:'formal_skill'});target.hp=Math.max(0,target.hp-shield.hpDamage);const applied=before-target.hp;queueSceneEvent({attackerId:attacker.id,targetId:target.id,attackerName:attacker.name,attackerSide:attacker.side,miss:false,damage:applied});attacker.damageDealt+=applied;target.damageTaken+=applied;
...
function resolveCurrentBlockDamage(target,finalDamage,blockRoll=null){const blockRate=Number(target?.blockRate??target?.block_rate)||0,blockDamageCutRate=Number(target?.blockDamageCutRate??target?.block_damage_cut_rate)||0,blocked=blockRate>0&&blockRoll!=null&&blockRoll<blockRate,damage=blocked?Math.floor(finalDamage*(1-blockDamageCutRate)):finalDamage;return{blocked,blockRate,blockRoll,blockDamageCutRate,damage}}
