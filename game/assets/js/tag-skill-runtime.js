...
function compileSkillRuntime(skill){
 ...
 if(runtime.acquisitionConditionContracts!=null&&!Array.isArray(runtime.acquisitionConditionContracts))errors.push('runtimeContracts.acquisitionConditionContractsは配列が必要です');
...
function resolveTaggedTargets(actor,target,definition){
 ...
 if(range==='SINGLE'||range==='BACK'){
  ...
  const rawCount=definition.target.randomCount;if(rawCount!=null&&rawCount!==''&&(!Number.isInteger(Number(rawCount))||Number(rawCount)<1))return{ok:false,reason:'RANDOMには1以上のrandomCountが必要です',targets:[]};
  ...
  candidates=resolver.sampleTargetsWithReplacement(candidates,rawCount,rng);
...
function currentBattleHitRatePercent(attacker,target){const accuracy=currentBattleAccuracy(attacker),evasion=currentBattleEvasion(target);return typeof GKHitCriticalDomain!=='undefined'?GKHitCriticalDomain.physicalHitRatePercent({accuracy,evasion}):(evasion<=0?100:Math.max(0,(accuracy/evasion)*100))}
...
function currentBattleMagicHitRatePercent(attacker,target){const magicAccuracy=currentBattleMagicAccuracy(attacker),magicResistance=currentBattleMagicResistance(target);return typeof GKHitCriticalDomain!=='undefined'?GKHitCriticalDomain.magicalHitRatePercent({magicAccuracy,magicResistance}):(magicResistance<=0?100:Math.max(0,Math.min(100,(magicAccuracy/magicResistance)*100)))}
function currentBattleCriticalResolution(unit){const explicit=currentBattleFinite(unit?.criticalRate??unit?.critical_rate,NaN),weaponRate=Math.max(0,currentBattleFinite(unit?.weaponCriticalRate??unit?.weapon_critical_rate,0)),luck=Math.max(0,currentBattleFinite(unit?.luk??unit?.LUK,0)),baseRate=Number.isFinite(explicit)?Math.max(0,explicit)/100:(typeof GKHitCriticalDomain!=='undefined'?GKHitCriticalDomain.criticalBaseRate({weaponCriticalRate:weaponRate,luk:luck}):weaponRate*(1+luck/100)),resolved=currentBattleResolveModifierTarget(baseRate,'CRITICAL_RATE',unit),finalRate=Math.max(0,resolved.final_value);return{weapon_critical_rate:weaponRate,LUK:luck,base_critical_rate:baseRate,final_critical_rate:finalRate,contributions:resolved.contributions}}
...
function currentBattleCriticalDamagePercent(unit){const explicit=currentBattleFinite(unit?.criticalDamage??unit?.critical_damage,NaN),initial=typeof GKHitCriticalDomain!=='undefined'?GKHitCriticalDomain.INITIAL_CRITICAL_BONUS_DAMAGE_PERCENT:50,base=Number.isFinite(explicit)?Math.max(0,explicit):initial,resolved=currentBattleResolveDirectModifierTarget(base,'CRITICAL_DAMAGE',unit);return Math.max(0,Math.min(700,resolved.final_value))}
...
function resolveFatalDamageInterrupt(target,{actionSourceId=null,skillId=null,hitIndex=0,projectedHp=null,actionContext=null,damageKind='DAMAGE'}={}){
 ...
 for(const candidate of candidates){const {passiveId,contract}=candidate,cooldownTicks=contract.cooldownTicks,surviveHp=contract.surviveHp;if(!Number.isInteger(cooldownTicks)||cooldownTicks<0)throw Object.assign(new Error(`ON_FATAL_DAMAGE Passive ${passiveId}に有効なcooldownTicksがありません。`),{code:'FATAL_DAMAGE_PASSIVE_COOLDOWN_REQUIRED',passive_id:passiveId});const remaining=passiveCooldownRemaining(target,passiveId);if(remaining>0){typeof recordValidationEvent==='function'&&recordValidationEvent('fatal_damage_passive_skipped',{source_id:target.id,passive_id:passiveId,reason:'COOLDOWN',cooldown_remaining:remaining});continue}
  ...
  const chance=Object.prototype.hasOwnProperty.call(contract,'activationChance')?Number(contract.activationChance):1,chanceCheck=evaluateCurrentBattleTriggerChance(chance,()=>currentBattleRoll(target,{id:actionSourceId||''},passiveId,'activation:fatal_damage',hitIndex),'FATAL_DAMAGE_PASSIVE_ACTIVATION_CHANCE_INVALID',passiveId);if(!chanceCheck.passed){typeof recordValidationEvent==='function'&&recordValidationEvent('fatal_damage_passive_skipped',{source_id:target.id,passive_id:passiveId,reason:chanceCheck.reason,activation_chance:chanceCheck.activation_chance,activation_roll:chanceCheck.activation_roll});continue}const roll=chanceCheck.activation_roll;
...
function evaluateCurrentBattleTriggerChance(chance,randomFn,errorCode,passiveId){
 const engine=globalThis.GKSTriggerEngine,result=engine?.evaluateActivationChance?engine.evaluateActivationChance(chance,randomFn):null;
 if(!result?.ok)throw Object.assign(new Error(`Passive ${passiveId||''}のactivationChanceが不正です。`),{code:errorCode||'PASSIVE_ACTIVATION_CHANCE_INVALID',passive_id:passiveId||null,reason:result?.reason||'TRIGGER_ACTIVATION_CHANCE_INVALID'});
 return result;
}
...
function currentBattleRngPurpose(purpose){const raw=String(purpose||'').toLowerCase(),P=globalThis.GKRuntimeBoundaryContracts?.RNG_PURPOSES;if(!P)return null;if(raw==='hit')return P.HIT;if(raw==='critical')return P.CRITICAL;if(raw==='block')return P.BLOCK;if(raw==='range_random')return P.TARGET_SELECTION;if(raw.startsWith('activation:'))return P.PASSIVE_TRIGGER;return P.BATTLE_MISC}
function currentBattleRoll(source,target,skillId,purpose,index=0){const sequence=Math.max(0,Math.floor(Number(battle.formalRandomSequence)||0));battle.formalRandomSequence=sequence+1;const seed=String(battle.p0113TieSeed??'FORMAL-BATTLE');const text=`${seed}|${battle.tick}|${source?.id||''}|${target?.id||''}|${skillId||''}|${purpose}|${index}|${sequence}`;let hash;if(typeof p0113Hash32==='function')hash=p0113Hash32(text);else{hash=2166136261>>>0;for(let i=0;i<text.length;i++){hash^=text.charCodeAt(i);hash=Math.imul(hash,16777619)>>>0}}const fallback=()=>(hash>>>0)/4294967296,boundary=globalThis.GKRuntimeBoundaryContracts,rngPurpose=currentBattleRngPurpose(purpose);return boundary&&rngPurpose?boundary.runtimeDraw(rngPurpose,fallback,{seed,battle_tick:Number(battle.tick)||0,source_id:String(source?.id||''),target_id:String(target?.id||''),skill_id:String(skillId||''),raw_purpose:String(purpose||''),hit_index:Number(index)||0,sequence}):fallback()}
...
function currentBattleFormationDamageMultiplier(attacker,range){
 ...
 if(String(attacker?.formationPosition||attacker?.formation_position||'FRONTLINE').toUpperCase()==='BACKLINE'&&['single','front','random'].includes(normalized))return 0.5;
 return 1;
}
...
function applyCurrentFormalDamage(attacker,target,baseDamage,compiled,contract,{hitIndex=0,actionContext=null}={}){
 ...
 const criticalResolution=currentBattleCriticalResolution(attacker),criticalRate=Math.max(0,Math.min(100,criticalResolution.final_critical_rate*100)),magical=contract.damageType==='MAGICAL';
 const hitResolution=typeof GKHitCriticalDomain!=='undefined'?GKHitCriticalDomain.resolveHit({criticalRatePercent:criticalRate,damageType:magical?'MAGICAL':'PHYSICAL',accuracy:currentBattleAccuracy(attacker),evasion:currentBattleEvasion(target),magicAccuracy:currentBattleMagicAccuracy(attacker),magicResistance:currentBattleMagicResistance(target),drawCritical:()=>currentBattleRoll(attacker,target,compiled?.definition?.id,'critical',hitIndex),drawHit:()=>currentBattleRoll(attacker,target,compiled?.definition?.id,'hit',hitIndex)}):(()=>{const criticalRoll=currentBattleRoll(attacker,target,compiled?.definition?.id,'critical',hitIndex)*100,critical=criticalRoll<criticalRate,hitRate=critical?null:(magical?currentBattleMagicHitRatePercent(attacker,target):currentBattleHitRatePercent(attacker,target)),hitRoll=critical?null:currentBattleRoll(attacker,target,compiled?.definition?.id,'hit',hitIndex)*100;return{critical,critical_roll:criticalRoll,hit_rate_percent:hitRate,hit_roll:hitRoll,rng_consumption:{critical:1,hit:critical?0:1,total:critical?1:2}}})();
 const critical=hitResolution.critical,criticalRoll=hitResolution.critical_roll,hitBypass=critical?'CRITICAL_GUARANTEED_HIT':null,hitRate=critical?null:hitResolution.hit_rate_percent,hitRoll=critical?null:hitResolution.hit_roll;
 ...
 if(!critical&&hitRoll>=hitRate){if(actionContext)queueCurrentBattlePassiveReactive(target,'evade',reactiveEventContext);queueSceneEvent({attackerId:attacker.id,targetId:target.id,attackerName:attacker.name,attackerSide:attacker.side,miss:true,damage:0});typeof recordValidationEvent==='function'&&recordValidationEvent('attack_missed',{source_id:attacker.id,target_id:target.id,skill_id:compiled?.definition?.id||null,hit_index:hitIndex,accuracy:currentBattleAccuracy(attacker),evasion:currentBattleEvasion(target),magic_accuracy:magical?currentBattleMagicAccuracy(attacker):null,magic_resistance:magical?currentBattleMagicResistance(target):null,critical_rate:criticalRate,critical_roll:criticalRoll,critical:false,hit_bypass:null,hit_rate:hitRate,hit_roll:hitRoll,rng_consumption:hitResolution.rng_consumption});battle.log.push(`[Tick ${battle.tick}] [TAG][MISS] ${attacker.name}の${compiled.definition.name} → ${target.name}（命中率${hitRate.toFixed(2)}%）`);return{ok:false,miss:true,damage:0,baseDamage,hitIndex,critical:false,criticalRate,criticalRoll,hitBypass:null,hitRate,hitRoll}}
 if(actionContext)queueCurrentBattlePassiveReactive(attacker,'hit_dealt',reactiveEventContext);
 const resistance=effectiveDamageResist(target),criticalDamage=currentBattleCriticalDamagePercent(attacker),criticalBonusReduction=typeof currentBattleCriticalBonusDamageReduction==='function'?currentBattleCriticalBonusDamageReduction(target):0,formationMultiplier=(()=>{const r=String(compiled?.definition?.target?.range||'').toLowerCase(),pos=String(attacker?.formationPosition||attacker?.formation_position||'FRONTLINE').toUpperCase();return pos==='BACKLINE'&&['single','front','random'].includes(r)?0.5:1})(),damageDefense=typeof GKDamageDefenseDomain!=='undefined'?GKDamageDefenseDomain.resolveFinalDamage({baseDamage,damageResistance:resistance,critical,criticalBonusDamagePercent:criticalDamage,criticalBonusReduction,elementShares:Array.isArray(contract.elementComponents)?contract.elementComponents.map(row=>({element:row.element,share:row.share*100})):null,elementResistances:Object.fromEntries(CURRENT_DAMAGE_ELEMENTS.map(element=>[element,currentBattleElementResistance(target,element)])),formationMultiplier}):null,postResistance=damageDefense?damageDefense.post_resistance_damage:Math.max(0,baseDamage*(1-resistance/100)),criticalMultiplier=damageDefense?damageDefense.critical_multiplier:1+(critical?(criticalDamage/100)*(1-criticalBonusReduction):0),preElementDamage=damageDefense?damageDefense.pre_element_damage:Math.max(0,postResistance*criticalMultiplier),elementDamage=damageDefense?{elemental:damageDefense.element_damage.elemental,totalDamage:damageDefense.element_damage.total_damage,components:damageDefense.element_damage.components.map(row=>({element:row.element,share:row.share_percent/100,damage_before_resistance:row.damage_before_resistance,resistance:row.resistance_percent,damage_after_resistance:row.damage_after_resistance}))}:Array.isArray(contract.elementComponents)&&contract.elementComponents.length?currentBattleResolveElementDamage(target,preElementDamage,contract.elementComponents):{elemental:false,totalDamage:preElementDamage,components:[]},finalDamage=damageDefense?damageDefense.final_damage:Math.max(0,Math.floor(elementDamage.totalDamage*formationMultiplier));
...
const DOT_STACK_TYPES={poison:{id:'poison',label:'毒'}};
let dotStackSequence=0;
function resolveDotType(compiled){if(compiled.parsed?.generalTags?.has('毒属性'))return DOT_STACK_TYPES.poison;return{id:'generic-dot',label:'DOT'}}
...
function applyTaggedDot(source,target,compiled,lifecyclePolicy=null){
 ...
 if(!lifecyclePolicy){typeof recordValidationEvent==='function'&&recordValidationEvent('dot_lifecycle_rejected',{source_id:source.id,target_id:target.id,skill_id:compiled.definition.id,reason:'DOT_LIFECYCLE_REQUIRED'});return{ok:false,reason:'DOT_LIFECYCLE_REQUIRED'}}
...
function resolveCoverIntervention(attacker,originalTarget,incomingCompiled,context={}){
 ...
 if(!['single','back','random'].includes(String(incomingCompiled?.definition?.target?.range||'').toLowerCase())){
 ...
 const candidates=ensureCoverEffects(originalTarget).map(effect=>({effect,source:battle.units.find(x=>x.id===effect.sourceId)})).filter(x=>x.source?.alive&&x.source.id!==originalTarget.id);if(!candidates.length)return{target:originalTarget,covered:false,effect:null};
 candidates.sort((a,b)=>b.effect.priority-a.effect.priority||(Number(b.source.battleTieRoll)||0)-(Number(a.source.battleTieRoll)||0)||String(a.source.id).localeCompare(String(b.source.id)));const {effect,source:coverSource}=candidates[0];
...
function evaluateSkillUseRequirements(unit,compiled){const contracts=Array.isArray(compiled?.definition?.useRequirementContracts)?compiled.definition.useRequirementContracts:[],evaluator=globalThis.GKSkillUseConditionRuntime?.evaluateEquipmentTagRequirements;if(typeof evaluator==='function')return evaluator(unit,contracts);const owned=new Set((Array.isArray(unit?.equipmentTagIds)?unit.equipmentTagIds:[]).map(String));for(const [i,r] of contracts.entries()){const allTags=Array.isArray(r?.allTags)?r.allTags:[],anyTags=Array.isArray(r?.anyTags)?r.anyTags:[],missingAllTags=allTags.filter(tag=>!owned.has(String(tag))),anyOk=!anyTags.length||anyTags.some(tag=>owned.has(String(tag)));if(missingAllTags.length||!anyOk)return{ok:false,reason:'USE_REQUIREMENT_FAILED',requirement_index:i,type:'EQUIPMENT_TAGS',missingAllTags,anyTags:[...anyTags],ownedTags:[...owned]}}return{ok:true,ownedTags:[...owned]}}
