...
function applyCurrentFormalDamage(attacker,target,baseDamage,compiled,contract,{hitIndex=0}={}){
 ...
 const resistance=effectiveDamageResist(target),postResistance=Math.max(0,baseDamage*(1-resistance/100)),criticalResolution=currentBattleCriticalResolution(attacker),criticalRate=Math.max(0,Math.min(100,criticalResolution.final_critical_rate*100)),criticalRoll=currentBattleRoll(attacker,target,compiled?.definition?.id,'critical',hitIndex)*100,critical=criticalRoll<criticalRate,criticalDamage=currentBattleCriticalDamagePercent(attacker),criticalMultiplier=critical?1+criticalDamage/100:1,finalDamage=Math.max(0,Math.floor(postResistance*criticalMultiplier));
 ...
 if(target.hp<=0){resetCombatantOnDeath(target,{reason:'formal_skill_damage',sourceId:attacker.id});recordModifierSourceDefeated(target);battle.log.push(`[Tick ${battle.tick}] ${target.name}は戦闘不能`)}finishIfNeeded();return{ok:true,miss:false,damage:applied,baseDamage,resistance,postResistanceDamage:postResistance,critical,criticalRate,criticalRoll,criticalDamage,finalDamage,blocked:block.blocked,blockRate:block.blockRate,blockRoll:block.blockRoll,blockDamageCutRate:block.blockDamageCutRate,blockedDamage:block.damage,shieldAbsorbed:shield.absorbed,beforeHp:before,afterHp:target.hp,hitIndex,hitRate,hitRoll};
}
