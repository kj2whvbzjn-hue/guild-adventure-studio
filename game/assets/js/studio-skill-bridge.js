...
function r06FormalSkillSource(skill){
 const candidates=[
  ['master',skill],
  ['params.skill_generation.skill',skill?.params?.skill_generation?.skill],
  ['skill',skill?.skill],
  ['formalSkill',skill?.formalSkill],
  ['compiledSkill',skill?.compiledSkill]
 ];
 for(const [path,candidate] of candidates){
  if(!candidate||typeof candidate!=='object'||Array.isArray(candidate))continue;
  if(candidate.runtimeContracts)return{path,candidate};
  if(candidate.schemaVersion!=null&&candidate.trigger&&candidate.target&&Array.isArray(candidate.effects))return{path,candidate};
 }
 return{path:null,candidate:null};
}
function r06MasterSkillShape(skill){
 const nested=skill?.params?.skill_generation?.skill;
 return{top_level_keys:Object.keys(skill||{}).sort(),has_runtimeContracts:!!skill?.runtimeContracts,has_nested_generation_skill:!!(nested&&typeof nested==='object'),nested_generation_skill_keys:nested&&typeof nested==='object'?Object.keys(nested).sort():[]};
}
function compileStudioMasterSkillForRuntime(skill){
 const found=r06FormalSkillSource(skill);
 if(!found.candidate)return{ok:false,skill:null,authored:null,source:null,input_source:null,input_shape:r06MasterSkillShape(skill),errors:['正式Skill原稿をMasterレコードから取得できません']};
 const sourceSkill=found.candidate;
 if(sourceSkill.runtimeContracts){
  const prepared={...sourceSkill,id:String(skill?.id||sourceSkill.id||''),name:String(skill?.name||sourceSkill.name||'')};
  return{ok:true,skill:prepared,authored:null,source:'runtimeContracts',input_source:found.path,input_shape:r06MasterSkillShape(skill)};
 }
 ...
 if(!compiler?.compileSkill)return{ok:false,skill:null,authored:null,source:null,input_source:found.path,input_shape:r06MasterSkillShape(skill),errors:['GKSSkillCompilerがありません']};
 if(!registry)return{ok:false,skill:null,authored:null,source:null,input_source:found.path,input_shape:r06MasterSkillShape(skill),errors:['Skill Registryが読み込まれていません']};
 const compileInput={...sourceSkill,id:String(skill?.id||sourceSkill.id||''),name:String(skill?.name||sourceSkill.name||'')};
 const authored=compiler.compileSkill(compileInput,registry);
 if(!authored?.ok||!authored?.compiledSkill)return{ok:false,skill:null,authored,source:null,input_source:found.path,input_shape:r06MasterSkillShape(skill),errors:(authored?.errors||[]).map(x=>x?.message||x?.code||String(x))};
 return{ok:true,skill:{...authored.compiledSkill,source:'studio_master_localstorage',environment:'production'},authored,source:'formal_compiler',input_source:found.path,input_shape:r06MasterSkillShape(skill)};
}
...
function runR06MasterStructuredRuntimeFinalRegression(){
 ...
 if(!source.ok)return{schema_version:'1.0.0',build:'GA-B486.123',generated_at:new Date().toISOString(),test:{id:'R06-MASTER-STRUCTURED-RUNTIME-FINAL-001',mode:'master_registered_structured_skill_composite_runtime_final'},source,compile_results:[],cases:[],summary:{master_skill_count:0,compile_passed_count:0,runtime_passed_count:0,runtime_case_count:0,composite_case_count:0,passed:false,errors:[source.error||'Studio Masterを取得できません']}};
 ...
 return{schema_version:'1.1.0',build:'GA-B486.123',generated_at:new Date().toISOString(),test:{id:'R06-MASTER-STRUCTURED-RUNTIME-FINAL-001',mode:'master_registered_structured_skill_composite_runtime_final',entrypoint:'game/index.html'},source:{status:'loaded',project_id:source.project_id,storage:'Studio current project localStorage',skill_prefix:R06_FINAL_MASTER_SKILL_PREFIX,master_skill_count:rows.length},compile_results,cases,summary:{master_skill_count:rows.length,compile_passed_count:compile_results.filter(x=>x.compiled_ok&&x.runtime_contract_connected).length,runtime_contracts_count:runtimeContractsCount,no_contract_count:noContractCount,runtime_passed_count:cases.filter(x=>x.passed).length,runtime_case_count:cases.length,composite_case_count:compositeCount,passed:errors.length===0,errors}};
}
