...
async function tagTestRunAuraRuntimeJson(){
 ...
 const report={schema_version:'1.0.0',build:'GA-B486.51',generated_at:new Date().toISOString(),test:{id:'TAG-AURA-RUNTIME-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunAuraRuntimeJson'},current_spec:{task_id:'P01-06',stage:'runtime_connection_v1',runtime_application:true,source_dependency:true,stacking:'highest',source_death:'immediate_disable',source_revive:'re_evaluate_and_restore',target_death:'inactive_while_dead',target_revive:'re_evaluate',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},source:{studio_status:studioSkillBridge.status,data_version:studioSkillBridge.data_version},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.51-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function runActivationPriorityRuntimeValidation(){
 ...
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.51',generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.51-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.51',generated_at:new Date().toISOString(),validation_patch:'P01-13-VAL-2',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-002',mode:'validation_runtime',entrypoint:'game/index.html'},passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 const neutral=findTagSkill('SKL-TEST-ATTACK'),cases=[];const seed='P01-13-VAL2-SEED-001';
 ...
 const prep=(useSeed=seed)=>{pauseBattle();resetBattle();battle.validationMode=false;battle.validationCaptureEvents=true;battle.validationSimultaneousTieRoll=true;battle.validationEvents=[];battle.tick=0;const allies=ensureValidationTargets('味方',3),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.actions=0;u.reservedAction=null;u.lastReservation=null;u.statusEffects=[];u.cooldowns={};u.maxMp=100;u.mp=100;u.defaultSkillId=neutral.id}const rolls=assignBattleTieRolls(useSeed,battle.units);return{a:allies[0],b:allies[1],c:allies[2],target:enemies[0],rolls}};
 ...
 add('SIMULTANEOUS-TIEROLL-UNIQUE','戦闘開始時に参加者へ一意のseed付きtie rollを一度だけ割当',()=>{const f=prep(),errors=[],vals=f.rolls.map(x=>x.tie_roll);if(new Set(vals).size!==vals.length)errors.push('tie roll not unique');if(f.rolls.length!==battle.units.length)errors.push('not all participants assigned');return{battle_seed:battle.p0113TieSeed,rolls:f.rolls,errors}});
 add('SIMULTANEOUS-TIEROLL-REPRODUCIBLE','同じseed・同じ参加者なら同じtie rollを再現',()=>{const f1=prep(),r1=JSON.stringify(f1.rolls);const f2=prep(),r2=JSON.stringify(f2.rolls),errors=[];if(r1!==r2)errors.push('same seed produced different rolls');return{battle_seed:seed,first:f1.rolls,second:f2.rolls,errors}});
 add('SIMULTANEOUS-TIEROLL-PRIMARY-TIEBREAK','同じACTIVATION_PRIORITYならゲージ超過量・AGI・actor orderではなくtie roll高値を優先',()=>{const f=prep(),errors=[];const pair=[f.a,f.b].sort((x,y)=>y.battleTieRoll-x.battleTieRoll),winner=pair[0],loser=pair[1];winner.agi=1;winner.gauge=GAUGE_MAX;loser.agi=999;loser.gauge=GAUGE_MAX+999;reserveSameTick([loser,winner]);processTicks(1);const o=resultOrder();if(o.fixed[0]?.source_id!==winner.id)errors.push('tie roll winner not first fixed');if(o.committed[0]!==winner.id)errors.push('tie roll winner not first committed');return{battle_seed:seed,winner:{actor_id:winner.id,tie_roll:winner.battleTieRoll},loser:{actor_id:loser.id,tie_roll:loser.battleTieRoll},fixed_order:o.fixed,committed_order:o.committed,errors}});
 add('SIMULTANEOUS-TIEROLL-STABLE-BATTLE','戦闘終了まで同じtie rollを使い続ける',()=>{const f=prep(),errors=[],before=f.rolls.map(x=>({actor_id:x.actor_id,tie_roll:x.tie_roll}));const pair=[f.a,f.b].sort((x,y)=>y.battleTieRoll-x.battleTieRoll);for(const u of pair){u.gauge=GAUGE_MAX;reserveAction(u);u.reservedAction.executeAt=1}processTicks(1);battle.validationEvents=[];for(const u of pair){u.gauge=GAUGE_MAX;reserveAction(u);u.reservedAction.executeAt=2}processTicks(1);const after=battle.units.map(u=>({actor_id:u.id,tie_roll:u.battleTieRoll})).sort((a,b)=>a.actor_id.localeCompare(b.actor_id));const beforeSorted=[...before].sort((a,b)=>a.actor_id.localeCompare(b.actor_id));if(JSON.stringify(beforeSorted)!==JSON.stringify(after))errors.push('tie roll changed during battle');return{battle_seed:seed,before:beforeSorted,after,errors}});
 battle.validationCaptureEvents=false;battle.validationSimultaneousTieRoll=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.51',generated_at:new Date().toISOString(),validation_patch:'P01-13-VAL-2',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-002',mode:'validation_runtime',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,neutral_skill_id:neutral?.id||null},current_spec:{task_id:'P01-13',stage:'validation_v2',formal_runtime_change:false,candidate_only:true,candidate_order:['activation_priority_desc','battle_tie_roll_desc'],battle_tie_roll:{seeded:true,assigned_once_at_battle_start:true,stable_until_battle_end:true,collision_rule:'reroll_colliding_actors_until_unique'},legacy_tie_keys_removed:['gauge_overflow','AGI','actor_order']},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunSimultaneousActivationOrderJson(){pauseBattle();const report=runSimultaneousActivationOrderValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-simultaneous-activation-order-runtime-device-validation-GA-B486.51-P01-13-VAL2-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-13 SIMULTANEOUS ORDER VAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL CHANGE] NO\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}

function runCounterRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.51',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-002',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-07',stage:'runtime_v1_1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.51-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
