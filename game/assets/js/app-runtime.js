...
function equipmentMagicContributionChannels(character){if(!window.GKSFormalContribution)throw new Error('Formal Contribution resolverを読み込めません。');normalizeCharacterEquipmentState(character);const rows=[];for(const slot of ['weapon1','weapon2']){const ref=character.equipment[slot];if(!ref)continue;const equipment=equipmentDefinition(ref);if(!equipment||equipmentIsShield(equipment))continue;rows.push({base_item_type:equipmentBaseItemType(equipment),modified:{magic_weapon_bonus:Number(equipment.bonuses?.magicWeaponBonus)||0}})}return window.GKSFormalContribution.splitMagicWeaponContribution(rows)}
function characterBattleValues(character){const cfg=requireFormalRuntimeSettings().battle_actor,b=equipmentBonus(character),magic=equipmentMagicContributionChannels(character);return{maxHp:evaluateRuntimeFormula(cfg.max_hp,character)+b.maxHp,maxMp:evaluateRuntimeFormula(cfg.max_mp,character)+b.maxMp,attack:evaluateRuntimeFormula(cfg.attack,character)+b.attack,agi:evaluateRuntimeFormula(cfg.agi,character)+b.agi,accuracy:b.accuracy,evasion:b.evasion,magicWeaponBonus:b.magicWeaponBonus,magicDamageWeaponBonus:magic.magic_damage_bonus,magicSupportWeaponBonus:magic.magic_support_bonus,baseCriticalRate:b.baseCriticalRate}}
...
function makeBattleUnits(){
 ...
 const allies=members.map((c,i)=>{const v=characterBattleValues(c),e2e=developerE2EOverrideSkillId(c.id),basicAttackProfiles=characterBasicAttackProfiles(c);return makeCombatant({id:`A${i}`,characterId:c.id,name:c.name,side:'味方',ownedSkillIds:clone(c.skills||[]),equippedSkillId:c.equippedSkillId||'',defaultSkillId:e2e||c.equippedSkillId||c.skills?.[0]||'',agi:v.agi,attack:v.attack,accuracy:v.accuracy,evasion:v.evasion,magicWeaponBonus:v.magicWeaponBonus,magicDamageWeaponBonus:v.magicDamageWeaponBonus,magicSupportWeaponBonus:v.magicSupportWeaponBonus,baseCriticalRate:v.baseCriticalRate,basicAttackProfiles,weaponStyle:characterWeaponStyle(c),maxHp:v.maxHp,maxMp:v.maxMp,gauge:0,actions:0,order:i,lastActionTick:null})});
