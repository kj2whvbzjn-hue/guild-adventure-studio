...
function renderCharacterSkillView(){
 const c=data.characters.find(x=>x.id===selectedId),title=$('characterSkillTitle'),current=$('characterSkillCurrent'),list=$('characterSkillList'),catalog=$('characterSkillCatalog');
 if(!title||!current||!list)return;
 if(!c){title.textContent='冒険者のスキル';current.innerHTML='<div class="skill-empty">冒険者を選択してください。</div>';list.innerHTML='';if(catalog)catalog.innerHTML='';return}
 if(window.GKGameSkillLoadout)GKGameSkillLoadout.normalizeCharacterSkillState(c,{fallback:[]});
 const equipped=findSkill(c.equippedSkillId),equippedCompiled=equipped?compileSkillForRuntime(equipped):null;title.textContent=`${c.name}のスキル`;
