...
function battleAiReevaluationStep(){return battleGaugeMax()*battleAiReevaluationRatio()}
...
function makeCombatant(base){const maxHp=finiteNumber(base?.maxHp,`${base?.id||base?.name||'combatant'}.maxHp`,{min:1}),maxMp=finiteNumber(base?.maxMp,`${base?.id||base?.name||'combatant'}.maxMp`,{min:0}),ownedSkillIds=window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeSkillIds(base.ownedSkillIds||base.skills,{fallback:[]}):[...new Set((base.ownedSkillIds||base.skills||[]).map(id=>String(id||'').trim()).filter(Boolean))],selectedSkillIds=window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeSkillIds(base.selectedSkillIds||base.selected_skill_ids||base.skillLoadoutIds,{fallback:[]}):[...new Set((base.selectedSkillIds||base.selected_skill_ids||base.skillLoadoutIds||[]).map(id=>String(id||'').trim()).filter(Boolean))],equippedSkillId=String(base.equippedSkillId||base.equipped_skill_id||'').trim(),defaultSkillId=String(base.defaultSkillId||(base.characterId?selectedSkillIds[0]:(equippedSkillId||ownedSkillIds[0]))||''),step=battleAiReevaluationStep();GAUGE_MAX=battleGaugeMax();return {...base,maxHp,formationPosition:normalizeFormationPosition(base.formationPosition??base.formation_position,{fallback:'FRONTLINE'}),hp:maxHp,maxMp,mp:Math.max(0,Math.min(maxMp,Number(base.mp??maxMp)||0)),alive:true,damageDealt:0,damageTaken:0,dotStacks:[],modifierStacks:[],reservedAction:null,lastReservation:null,castingAction:null,nextAiEvaluationGauge:step,lastAiEvaluationGauge:null,ownedSkillIds,selectedSkillIds,equippedSkillId,defaultSkillId}}
