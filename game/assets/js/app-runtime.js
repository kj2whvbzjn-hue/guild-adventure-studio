...
let data={saveVersion:SAVE_VERSION,schemaRevision:'1.5.0',gameVersion:'GA-B486.91',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],partyIds:[],selectedQuestId:'Q-001',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20}};let selectedId=null;
...
$('baseDepart').onclick=$('baseDepartSide').onclick=beginSelectedAdventureOrLegacy;
...
const mobileDepart=$('mobileDepart');if(mobileDepart)mobileDepart.onclick=async()=>{if(!data.partyIds.length){notify('遠征パーティを1人以上選んでください。','bad');setBaseView('party');return}await beginSelectedAdventureOrLegacy()};

loadAdventureContent().then(content=>{registerAdventureQuestCards(content);if(typeof renderExpeditionSetup==='function')renderExpeditionSetup();}).catch(()=>{});
...
function normalize(raw){
 ...
 raw.schemaRevision='1.5.0';raw.gameVersion='GA-B486.91';
 return raw;
}
...
function adventurePlaybackEventText(event){
 const type=String(event?.type||'event'),p=event?.payload&&typeof event.payload==='object'?event.payload:event;
 const actor=p.actor_name||p.actor||p.source_name||p.source||'',target=p.target_name||p.target||'';
 if(type==='battle_start')return'戦闘開始';if(type==='battle_end')return`戦闘終了${p.result?'：'+p.result:''}`;
 if(type==='action_start')return`${actor||'ユニット'} 行動開始`;if(type==='skill_cast')return`${actor||'ユニット'}${p.skill_name?'：'+p.skill_name:' がスキルを使用'}`;
 if(type==='damage')return`${target||'対象'}に${Number(p.amount??p.damage??0)||0}ダメージ`;if(type==='heal')return`${target||'対象'}を${Number(p.amount??0)||0}回復`;
 if(type==='status_apply')return`${target||'対象'}に状態付与${p.status?'：'+p.status:''}`;if(type==='status_remove')return`${target||'対象'}の状態解除${p.status?'：'+p.status:''}`;if(type==='ko')return`${target||actor||'ユニット'}が戦闘不能`;
 if(type==='hit')return`${actor||'攻撃'} → ${target||'対象'} 命中`;return type;
}
function renderAdventurePlaybackDetail(run,itemIndex){
 const detail=$('adventurePlaybackDetail');if(!detail)return;const item=run.timeline_result?.[Number(itemIndex)];if(!item){detail.classList.add('hidden');detail.innerHTML='';return;}
 ...
 if(view.kind==='scene'){
  ...
  body=`<b>ストーリーイベント</b><div class="small">Scene ${escapeHtml(scene.scene_id||item.ref_id||'')}</div>${dialogues.length?dialogues.map(d=>`<p><b>${escapeHtml(d.speaker||d.character_name||'')}</b> ${escapeHtml(d.text||d.body||'')}</p>`).join(''):'<p class="small">会話Snapshotはありません。</p>'}`;
 }else if(view.kind==='battle'){
  const br=view.detail||{},events=Array.isArray(br.playback_events)?br.playback_events:[];
  body=`<b>戦闘詳細</b><div class="small">${br.victory===false?'敗北':'勝利'} ／ 保存済みBattle Result</div><div class="adventure-detail-events">${events.length?events.map(e=>escapeHtml(adventurePlaybackEventText(e))).join('<br>'):'再生イベントはありません。'}</div>`;
 }else{
  const ev=view.detail||{};body=`<b>イベント詳細</b><div class="small">Event ${escapeHtml(ev.event_id||item.resolved_ref_id||item.ref_id||'')}</div><p>${ev.success===false?'失敗':'完了'}</p>`;
 }
 detail.innerHTML=body;detail.classList.remove('hidden');
}
function renderAdventurePlayback(nowMs=Date.now()){
 const panel=$('adventurePlaybackPanel');if(!panel)return null;const state=resumeAdventurePlayback(nowMs);
 ...
 const {run,playback}=state;panel.classList.remove('hidden');if($('eventBattle'))$('eventBattle').classList.add('hidden');if($('eventObserve'))$('eventObserve').classList.add('hidden');
 ...
 if($('adventurePlaybackClock'))$('adventurePlaybackClock').textContent=`${elapsed.toFixed(0)} / ${playback.duration_seconds.toFixed(0)}秒${playback.complete?' ／ 帰還可能':''}`;
 if($('adventurePlaybackProgress'))$('adventurePlaybackProgress').style.width=`${pct}%`;
 if($('eventNotice'))$('eventNotice').textContent=playback.complete?(run.final_result?.success?'冒険が完了しました。保存済み結果を確定して帰還できます。':'冒険は失敗しました。帰還すると今回の報酬は反映されません。'):'冒険は開始時刻基準で進行中です。別画面を見ていても時間は進みます。';
 const log=$('adventurePlaybackLog');if(log){const visible=playback.visible_timeline||[];log.innerHTML=visible.length?visible.map((item,i)=>{const actualIndex=(run.timeline_result||[]).indexOf(item),view=adventurePlaybackEntry(run,item);return`<div class="adventure-log-entry"><span class="time">${Number(item.at_seconds||0).toFixed(0)}s</span><span>${escapeHtml(view.title)}</span><button type="button" data-adventure-detail="${actualIndex}">見る</button></div>`}).join(''):'<div class="small">まだ表示可能な冒険ログはありません。</div>';log.querySelectorAll('[data-adventure-detail]').forEach(btn=>btn.onclick=()=>renderAdventurePlaybackDetail(run,btn.dataset.adventureDetail));}
 const ret=$('adventureReturn');if(ret){ret.disabled=!playback.complete;ret.textContent=playback.complete?'帰還して結果を確定':'冒険進行中';}
 return state;
}
function openAdventurePlayback(run){
 prepareEvent();const q=QUESTS.find(x=>String(x.id)===String(run?.quest_id))||selectedQuest();$('eventTitle').textContent=q?.name||run?.quest_id||'冒険';$('eventBody').textContent='Quest開始時に確定したQuestRunを再生しています。';setPhase('event');renderAdventurePlayback();
}
function returnFromAdventurePlayback(){
 ...
 render();notify(result.success===false?'冒険失敗。報酬を失って帰還しました。':'冒険結果を反映して帰還しました。',result.success===false?'warn':'ok');setPhase('base');setBaseView('home',{instant:true});
}
...
const ADVENTURE_EXPORT_FILES={quests:['../Export/quest/main_quests.json','../Export/quest/sub_quests.json','../Export/quest/event_quests.json'],chapters:'../Export/scenario/chapters.json',sections:'../Export/scenario/sections.json',scenes:'../Export/scenario/scenes.json',events:'../Export/event/events.json',monsters:'../Export/monster/monsters.json'};
let adventureContentCache=null;
async function fetchAdventureExport(url){const res=await fetch(url,{cache:'no-store'});if(!res.ok)throw new Error(`Adventure Export load failed: ${url} (${res.status})`);const json=await res.json();return Array.isArray(json?.data)?json.data:[];}
async function loadAdventureContent({force=false}={}){if(adventureContentCache&&!force)return adventureContentCache;const questLists=await Promise.all(ADVENTURE_EXPORT_FILES.quests.map(fetchAdventureExport));const [chapters,sections,scenes,events,monsters]=await Promise.all([ADVENTURE_EXPORT_FILES.chapters,ADVENTURE_EXPORT_FILES.sections,ADVENTURE_EXPORT_FILES.scenes,ADVENTURE_EXPORT_FILES.events,ADVENTURE_EXPORT_FILES.monsters].map(fetchAdventureExport));adventureContentCache={quests:questLists.flat(),chapters,sections,scenes,events,monsters};return adventureContentCache;}
function registerAdventureQuestCards(content){for(const q of content?.quests||[]){if(QUESTS.some(x=>String(x.id)===String(q.id)))continue;const links=q.links||{};if(!links.chapter_id||!links.section_id)continue;QUESTS.push({id:String(q.id),name:String(q.name||q.id),rank:'Story',stars:1,recommendedLevel:1,description:String(q.summary||''),reward:0,bonus:0,enemies:[],drops:[],adventureStory:true});}return QUESTS;}
...
function adventureEnemyBudget({quest,section}){return Math.max(0,Number(quest?.enemy_budget??section?.enemy_budget??0)||0);}
...
async function startSelectedQuestAdventure({forceReload=false}={}){if(!window.GKAdventureStorySystem||!window.GKAdventureBattleCore)return{started:false,reason:'adventure_runtime_unavailable'};const current=currentAdventureQuestRun();if(current)return{started:false,reason:'active_quest_run',run:current};let content;try{content=await loadAdventureContent({force:forceReload});}catch(error){return{started:false,reason:'export_load_failed',error};}const bundle=resolveAdventureBundle(content,data.selectedQuestId);if(!bundle)return{started:false,reason:'legacy_quest'};const partySnapshot=adventurePartySnapshot();const run=GKAdventureStorySystem.simulateQuest({quest:bundle.quest,section:bundle.section,chapter:bundle.chapter,scenes:bundle.scenes,events:bundle.events,monsters:bundle.monsters,partySnapshot,flags:clone(data.flags||{}),checkEventCondition:adventureEventCondition,resolveEvent:adventureEventResult,enemyBudget:adventureEnemyBudget,simulateBattle:args=>simulateAdventureBattle(args,bundle,partySnapshot)});const stored=storeAdventureQuestRun(run);return{started:true,run:stored,bundle};}
async function beginSelectedAdventureOrLegacy(){if(!data.partyIds.length){notify('遠征パーティを1人以上選んでください。','bad');return}const result=await startSelectedQuestAdventure();if(!result.started){if(result.reason==='active_quest_run'){openAdventurePlayback(result.run);return}prepareEvent();setPhase('event');return}openAdventurePlayback(result.run);}
...
function renderExpeditionSetup(){
 ...
 const qs=$('questSummary'),q=selectedQuest();if(qs){const activeLabel=adventurePlaybackLabel();qs.textContent=`選択中：${q.name} ／ 敵 ${q.enemies.length}体 ／ 編成人数 ${data.partyIds.length}人${activeLabel?' ／ '+activeLabel:''}`;}
...
$('clearBtn').onclick=()=>{if(!confirm('正式版Phase Aの全データを初期化しますか？'))return;data={saveVersion:1,schemaRevision:'1.5.0',gameVersion:'GA-B486.91',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],partyIds:[],selectedQuestId:'Q-001',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20}};selectedId=null;persist();render();notify('全データを初期化しました。','warn')};
...
function runActivationPriorityRuntimeValidation(){
 ...
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.91',generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.91-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runBattleEndEffectClearValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.91',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 ...
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.91',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',test:{id:'TAG-BATTLE-END-EFFECT-CLEAR-003',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},current_spec:{task_id:'P01-14',stage:'formal_candidate_v1',formal_runtime_enabled:true,battle_end_clear:['reserved_action','shield_effects','status_effects','cover_effects','dot_stacks','modifier_stacks','cooldowns']},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunBattleEndEffectClearJson(){pauseBattle();const report=runBattleEndEffectClearValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-battle-end-effect-clear-formal-runtime-device-validation-GA-B486.91-P01-14-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-14 BATTLE END CLEAR FORMAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}`;renderBattle();return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.91',generated_at:new Date().toISOString(),formal_candidate:'P01-13-FORMAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-004',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.91-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
