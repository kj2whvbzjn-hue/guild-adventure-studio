...
function resolveFormalModifierRows(base,target,rows){const baseValue=Number.isFinite(Number(base))?Number(base):0,targetStat=String(target||'').toUpperCase(),contributions=[];for(const row of Array.isArray(rows)?rows:[]){if(String(row?.target_stat||'').toUpperCase()!==targetStat)continue;const mode=String(row?.mode||'').toUpperCase(),raw=Number(row?.raw_modifier_value);if(!Number.isFinite(raw))throw Object.assign(new Error(`Game ${targetStat} Contribution valueが不正です。`),{code:'GAME_MODIFIER_VALUE_INVALID',target_stat:targetStat});let resolved=0;if(mode==='RELATIVE_PERCENT')resolved=baseValue*raw;else if(mode==='FLAT_ADD'||mode==='ADDITIVE_POINT')resolved=raw;else if(mode==='SUBTRACTIVE_POINT')resolved=-raw;else throw Object.assign(new Error(`Game modifier operation ${mode||'(empty)'}は未対応です。`),{code:'GAME_MODIFIER_OPERATION_UNSUPPORTED',target_stat:targetStat,operation:mode});contributions.push({...row,target_stat:targetStat,base_value_used:baseValue,resolved_contribution:resolved})}return{base_value:baseValue,final_value:baseValue+contributions.reduce((sum,row)=>sum+row.resolved_contribution,0),contributions}}
...
function mountAdventureBattleDetailPlayback(detail,battleResult){
 ...
 const speedValue=()=>Math.max(1,Math.min(4,Number(detail.querySelector('[data-adventure-battle-speed]')?.value)||1));
 ...
 detail.querySelector('[data-adventure-battle-speed]')?.addEventListener('change',()=>{if(!running)return;stopAdventureBattleDetailPlayback();schedule();});
