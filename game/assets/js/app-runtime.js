...
const formalEquipmentCatalog=new Map();
...
function legacyEquipmentRecord(ref){const e=EQUIPMENT[String(ref||'')];if(!e)return null;return{id:String(ref),name:String(ref),source:'legacy_fixture',slot:e.slot,slotLabel:FORMAL_EQUIPMENT_SLOT_LABEL[e.slot]||e.slot,rarity:e.rarity||'common',description:e.description||'',required:{STR:0,DEX:0,INT:0,VIT:0,MND:0,AGI:0},bonuses:{attack:Number(e.attack)||0,maxHp:Number(e.maxHp)||0,maxMp:Number(e.maxMp)||0,agi:Number(e.agi)||0,accuracy:0,evasion:0,magicWeaponBonus:0,baseCriticalRate:0}}}
function equipmentDefinition(ref){const id=String(ref||'');return formalEquipmentCatalog.get(id)||legacyEquipmentRecord(id)}
...
function equipmentRequirementCheck(character,equipment){
 ...
 const stats=character?.stats&&typeof character.stats==='object'?character.stats:{},missing=[];for(const stat of STATS){const required=Math.max(0,Number(equipment.required?.[stat])||0),actual=Math.max(0,Number(stats[stat])||0);if(actual<required)missing.push({stat,required,actual});}
 return{ok:missing.length===0,reason:missing.length?'EQUIPMENT_REQUIREMENTS_NOT_MET':'OK',slot:equipment.slot,missing};
}
...
function equipInventoryItemToCharacter(character,inventoryIndex){
 const index=Number(inventoryIndex);if(!character||!Number.isInteger(index)||index<0||index>=data.inventory.length)return{ok:false,reason:'INVALID_EQUIP_REQUEST'};
 ...
 character.equipment=character.equipment&&typeof character.equipment==='object'?character.equipment:{weapon:null,armor:null,accessory:null};const previous=character.equipment[equipment.slot]||null;character.equipment[equipment.slot]=ref;if(previous)data.inventory.push(previous);data.inventory.splice(index,1);return{ok:true,equipment,slot:equipment.slot,previous,ref};
}
...
window.GKGameEquipmentRuntime=Object.freeze({bridge:formalEquipmentBridge,normalizeRecord:normalizeFormalEquipmentRecord,definition:equipmentDefinition,requirementCheck:equipmentRequirementCheck,bonusLabel:equipmentBonusLabel,requirementLabel:equipmentRequirementLabel,load:loadFormalEquipmentDefinitions});
let data={saveVersion:SAVE_VERSION,schemaRevision:'1.6.0',gameVersion:'GA-B486.201',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],aiPrograms:[],aiLayouts:[],aiPresets:[],partyIds:[],selectedQuestId:'',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},flags:{},quest_progress:{completed_quest_ids:[],unlocked_quest_ids:[]},quest_resources:{},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20,stone_selection_by_quest:{}}};let selectedId=null;
...
loadFormalEquipmentDefinitions().then(()=>{if(typeof render==='function')render();});
...
function makeCharacter(name,job){const c={id:uid(),name,level:1,job,stats:Object.fromEntries(STATS.map(s=>[s,10])),skills:[...(window.GKGameSkillLoadout?.DEFAULT_SKILL_IDS||['SKL-TEST-ATTACK','SKL-TEST-HEAVY','SKL-TEST-POISON'])],equippedSkillId:'SKL-TEST-ATTACK',formalAiBinding:null,equipment:{weapon:null,armor:null,accessory:null},jobHistory:[{job,level:1,at:new Date().toISOString()}],growthHistory:[],createdAt:new Date().toISOString()};return window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeCharacterSkillState(c):c}
function normalize(raw){
 ...
 raw.characters.forEach(c=>{
  ...
  if(window.GKGameSkillLoadout)GKGameSkillLoadout.normalizeCharacterSkillState(c);else{c.skills=Array.isArray(c.skills)?[...new Set(c.skills.map(id=>String(id||'').trim()).filter(Boolean))]:['SKL-TEST-ATTACK','SKL-TEST-HEAVY','SKL-TEST-POISON'];if(!c.skills.length)c.skills=['SKL-TEST-ATTACK','SKL-TEST-HEAVY','SKL-TEST-POISON'];c.equippedSkillId=c.skills.includes(String(c.equippedSkillId||''))?String(c.equippedSkillId):c.skills[0];}
 ...
 raw.schemaRevision='1.6.0';raw.gameVersion='GA-B486.201';
 return raw;
}
function persist(){
...
function adventurePartySnapshot(){return data.partyIds.map(id=>data.characters.find(c=>c.id===id)).filter(Boolean).slice(0,6).map(c=>{const b=equipmentBonus(c),maxMp=Math.max(0,100+b.maxMp);return{character_id:c.id,id:c.id,name:c.name,job:c.job,level:c.level,max_hp:100+c.stats.VIT*20+c.level*10+b.maxHp,max_mp:maxMp,mp:maxMp,attack:10+c.stats.STR*3+c.level*2+b.attack,agi:Math.max(1,c.stats.AGI+b.agi),skills:clone(c.skills||[]),equipped_skill_id:c.equippedSkillId||''};});}
...
function render(){
 ...
 $('detail').innerHTML=`<div class="row"><div><div class="name">${escapeHtml(c.name)}</div><span class="tag">Lv ${c.level} / 50</span><span class="tag">現在職：${c.job}</span></div></div><div class="stats">${STATS.map(s=>`<div class="stat"><span class="small">${s}</span><b>${c.stats[s]}</b></div>`).join('')}</div><h3>装着スキル</h3><div class="small">${escapeHtml(findSkill(c.equippedSkillId)?.name||'未装着')}</div><h3>装備</h3><div class="small">${Object.values(c.equipment||{}).filter(Boolean).map(equipmentDisplayName).join(' / ')||'なし'}（攻撃 +${equipmentBonus(c).attack} / HP +${equipmentBonus(c).maxHp} / MP +${equipmentBonus(c).maxMp} / AGI ${equipmentBonus(c).agi>=0?'+':''}${equipmentBonus(c).agi}）</div><h3>転職履歴</h3><div class="small">${c.jobHistory.map(h=>`Lv${h.level} ${h.job}`).join(' → ')}</div><h3>直近の成長</h3><div class="small">${c.growthHistory.slice(-5).reverse().map(g=>`Lv${g.toLevel}: ${g.gained.length?g.gained.join(', '):'能力上昇なし'}`).join('<br>')||'まだレベルアップしていません。'}</div>`;
...
function renderCharacterSkillView(){
 ...
 if(window.GKGameSkillLoadout)GKGameSkillLoadout.normalizeCharacterSkillState(c);
...
function equipmentBonus(c){return Object.values(c.equipment||{}).filter(Boolean).reduce((a,ref)=>{const e=equipmentDefinition(ref),b=e?.bonuses;if(b){a.attack+=Number(b.attack)||0;a.maxHp+=Number(b.maxHp)||0;a.maxMp+=Number(b.maxMp)||0;a.agi+=Number(b.agi)||0;a.accuracy+=Number(b.accuracy)||0;a.evasion+=Number(b.evasion)||0;a.magicWeaponBonus+=Number(b.magicWeaponBonus)||0;a.baseCriticalRate+=Number(b.baseCriticalRate)||0}return a},{attack:0,maxHp:0,maxMp:0,agi:0,accuracy:0,evasion:0,magicWeaponBonus:0,baseCriticalRate:0})}
...
function renderExpeditionSetup(){
 ...
 const inv=$('inventoryList');if(inv){
  inv.innerHTML=data.inventory.length?data.inventory.map((ref,i)=>{const e=equipmentDefinition(ref);if(!e)return `<div class="unit loot-card rarity-common"><b>★ ${escapeHtml(ref)}</b> <span class="tag">正式装備定義未読込</span><div class="small">Reward ResultのIDはSaveへ保持されています。Equipment Exportを取得できるまで装備しません。状態: ${escapeHtml(formalEquipmentBridge.status)}</div></div>`;const rarity=RARITY[e.rarity||'common']||RARITY.common,formal=e.source==='studio_export',eligible=data.characters.map(c=>({c,gate:equipmentRequirementCheck(c,e)}));return `<div class="unit loot-card rarity-${e.rarity||'common'}"><b>${formal?'◆':rarity.label} ${escapeHtml(e.name)}</b> <span class="tag">${formal?'Formal Export':rarity.name}</span><span class="tag">${escapeHtml(e.slotLabel||e.slot||'部位未定義')}</span><div class="small">${escapeHtml(equipmentBonusLabel(e))}<br>装備条件: ${escapeHtml(equipmentRequirementLabel(e))}${e.description?`<br>${escapeHtml(e.description)}`:''}</div>${e.slot?`<label>装備先<select data-equip-index="${i}"><option value="">選択</option>${eligible.map(({c,gate})=>`<option value="${c.id}" ${gate.ok?'':'disabled'}>${escapeHtml(c.name)}${gate.ok?'':`（${escapeHtml(gate.missing.map(x=>`${x.stat} ${x.actual}/${x.required}`).join(', ')||gate.reason)}）`}</option>`).join('')}</select></label>`:'<div class="small warn">この装備の部位は現在のGame Runtimeでは未接続です。IDを保持します。</div>'}</div>`}).join(''):'<p class="small">装備はまだありません。依頼を達成して戦利品を集めましょう。</p>';
  inv.querySelectorAll('[data-equip-index]').forEach(el=>el.onchange=()=>{if(!el.value)return;const c=data.characters.find(x=>x.id===el.value),result=equipInventoryItemToCharacter(c,Number(el.dataset.equipIndex));if(!result.ok){const detail=result.missing?.length?result.missing.map(x=>`${x.stat} ${x.actual}/${x.required}`).join(', '):result.reason;notify(`装備できません: ${detail}`,'bad');renderExpeditionSetup();return}persist();render();notify(`${c.name}が${result.equipment.name}を装備しました。`)})
 }
 renderGuildSummary();
 refreshMobileHome();
...
$('clearBtn').onclick=()=>{if(!confirm('正式版Phase Aの全データを初期化しますか？'))return;data={saveVersion:SAVE_VERSION,schemaRevision:'1.6.0',gameVersion:'GA-B486.201',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],aiPrograms:[],aiLayouts:[],aiPresets:[],partyIds:[],selectedQuestId:'',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},flags:{},quest_progress:{completed_quest_ids:[],unlocked_quest_ids:[]},quest_resources:{},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20,stone_selection_by_quest:{}}};selectedId=null;persist();render();notify('全データを初期化しました。','warn')};
...
async function tagTestRunAuraRuntimeJson(){
 ...
 const report={schema_version:'1.0.0',build:'GA-B486.201',generated_at:new Date().toISOString(),test:{id:'TAG-AURA-RUNTIME-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunAuraRuntimeJson'},current_spec:{task_id:'P01-06',stage:'runtime_connection_v1',runtime_application:true,source_dependency:true,stacking:'highest',source_death:'immediate_disable',source_revive:'re_evaluate_and_restore',target_death:'inactive_while_dead',target_revive:'re_evaluate',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},source:{studio_status:studioSkillBridge.status,data_version:studioSkillBridge.data_version},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.201-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function makeCombatant(base){const maxMp=Math.max(0,Number(base.maxMp??100)||0),ownedSkillIds=window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeSkillIds(base.ownedSkillIds||base.skills,{fallback:[]}):[...new Set((base.ownedSkillIds||base.skills||[]).map(id=>String(id||'').trim()).filter(Boolean))],equippedSkillId=String(base.equippedSkillId||base.equipped_skill_id||'').trim(),defaultSkillId=String(base.defaultSkillId||equippedSkillId||ownedSkillIds[0]||'SKL-TEST-ATTACK');return {...base,hp:base.maxHp,maxMp,mp:Math.max(0,Math.min(maxMp,Number(base.mp??maxMp)||0)),alive:true,damageDealt:0,damageTaken:0,dotStacks:[],modifierStacks:[],reservedAction:null,lastReservation:null,ownedSkillIds,equippedSkillId,defaultSkillId}}
function makeBattleUnits(){
 const members=data.partyIds.map(id=>data.characters.find(c=>c.id===id)).filter(Boolean).slice(0,6);
 const allies=members.map((c,i)=>{const b=equipmentBonus(c),e2e=developerE2EOverrideSkillId(c.id);return makeCombatant({id:`A${i}`,characterId:c.id,name:c.name,side:'味方',ownedSkillIds:clone(c.skills||[]),equippedSkillId:c.equippedSkillId||'',defaultSkillId:e2e||c.equippedSkillId||c.skills?.[0]||'SKL-TEST-ATTACK',agi:Math.max(1,c.stats.AGI+b.agi),attack:10+c.stats.STR*3+c.level*2+b.attack,maxHp:100+c.stats.VIT*20+c.level*10+b.maxHp,maxMp:Math.max(0,100+b.maxMp),gauge:0,actions:0,order:i,lastActionTick:null})});
...
function runActivationPriorityRuntimeValidation(){
 ...
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.201',generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.201-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runBattleEndEffectClearValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.201',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 ...
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.201',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',test:{id:'TAG-BATTLE-END-EFFECT-CLEAR-003',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},current_spec:{task_id:'P01-14',stage:'formal_candidate_v1',formal_runtime_enabled:true,battle_end_clear:['reserved_action','shield_effects','status_effects','cover_effects','dot_stacks','modifier_stacks','cooldowns']},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunBattleEndEffectClearJson(){pauseBattle();const report=runBattleEndEffectClearValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-battle-end-effect-clear-formal-runtime-device-validation-GA-B486.201-P01-14-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-14 BATTLE END CLEAR FORMAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}`;renderBattle();return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.201',generated_at:new Date().toISOString(),formal_candidate:'P01-13-FORMAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-004',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
...
function runCounterRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.201',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-002',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-07',stage:'runtime_v1_1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.201-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
