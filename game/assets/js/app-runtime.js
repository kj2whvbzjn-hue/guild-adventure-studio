...
function growthRowsForAptitude(value){const rows=requireFormalRuntimeSettings().growth.outcomes_by_aptitude[String(value)];if(!rows)throw new Error(`適性値 ${value} の成長テーブルがありません。`);return rows}
function aptitudeExpected(v){const rows=growthRowsForAptitude(v),total=rows.reduce((a,x)=>a+x.weight,0);return(rows.reduce((a,x)=>a+x.amount*x.weight,0)/total).toFixed(2)}
function rollGrowth(v){const rows=growthRowsForAptitude(v),total=rows.reduce((a,x)=>a+x.weight,0),Boundary=window.GKRuntimeBoundaryContracts;let roll=(Boundary?Boundary.runtimeDraw(Boundary.RNG_PURPOSES.GROWTH,Math.random,{aptitude:Number(v)}):Math.random())*total;for(const row of rows){roll-=row.weight;if(roll<0)return row.amount}return rows[rows.length-1].amount}
...
$('levelBtn').onclick=()=>{try{const c=data.characters.find(x=>x.id===selectedId),cfg=requireFormalRuntimeSettings();if(!c||c.level>=cfg.character.max_level)return;if(!window.GKGameSkillAcquisitionRuntime)throw new Error('Skill Acquisition Runtimeを読み込めません。');const progression=skillProgressionBalance(),job=jobDefinition(c.job);if(!job)throw new Error(`正式Jobが見つかりません: ${c.job}`);const gained=[],growth={};STATS.forEach(stat=>{const amount=rollGrowth(job.aptitudes[stat]);growth[stat]=amount;if(amount>0){c.stats[stat]+=amount;gained.push(`${stat} +${amount}`)}});const from=c.level;c.level++;const skillPointResult=GKGameSkillAcquisitionRuntime.grantLevelUpSkillPoints(c,progression,{levels:1});c.growthHistory.push({fromLevel:from,toLevel:c.level,job:job.id,growth,gained,skillPointsGained:skillPointResult.gained,ruleRevision:cfg.growth.rule_revision,at:new Date().toISOString()});autoSave();render();notify(`${c.name}がLv${c.level}になりました。Skill Point +${skillPointResult.gained}${gained.length?' / 上昇: '+gained.join(', '):' / 能力値上昇なし'}`)}catch(error){notify(`レベルアップできません: ${error.message}`,'bad')}};
