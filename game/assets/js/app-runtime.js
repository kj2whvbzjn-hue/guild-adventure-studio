...
function setupTagSkillTestUI(){
 ...
 if(runCounterRuntimeJson)runCounterRuntimeJson.onclick=tagTestRunCounterRuntimeJson;if(runCoverRuntimeJson)runCoverRuntimeJson.onclick=tagTestRunCoverRuntimeJson;if(runActionDisabledRuntimeJson)runActionDisabledRuntimeJson.onclick=tagTestRunActionDisabledRuntimeJson;
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.41-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
