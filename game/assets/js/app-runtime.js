...
async function tagTestRunAuraRuntimeJson(){
 ...
 const report={schema_version:'1.0.0',build:'GA-B486.52',generated_at:new Date().toISOString(),test:{id:'TAG-AURA-RUNTIME-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunAuraRuntimeJson'},current_spec:{task_id:'P01-06',stage:'runtime_connection_v1',runtime_application:true,source_dependency:true,stacking:'highest',source_death:'immediate_disable',source_revive:'re_evaluate_and_restore',target_death:'inactive_while_dead',target_revive:'re_evaluate',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},source:{studio_status:studioSkillBridge.status,data_version:studioSkillBridge.data_version},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.52-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function runActivationPriorityRuntimeValidation(){
 ...
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.52',generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.52-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.52',generated_at:new Date().toISOString(),validation_patch:'P01-13-VAL-3',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-003',mode:'validation_runtime',entrypoint:'game/index.html'},passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 const neutral=findTagSkill('SKL-TEST-ATTACK'),cases=[];const seed='P01-13-VAL3-SEED-001';
 ...
 const prep=(useSeed=seed,hashFn=p0113Hash32)=>{pauseBattle();resetBattle();battle.validationMode=false;battle.validationCaptureEvents=true;battle.validationSimultaneousTieRoll=true;battle.validationEvents=[];battle.tick=0;const allies=ensureValidationTargets('味方',3),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.actions=0;u.reservedAction=null;u.lastReservation=null;u.statusEffects=[];u.cooldowns={};u.maxMp=100;u.mp=100;u.defaultSkillId=neutral.id}const rolls=assignBattleTieRolls(useSeed,battle.units,hashFn);return{a:allies[0],b:allies[1],c:allies[2],target:enemies[0],rolls}};
 ...
 const collisionHash=(text,actorId,round)=>{if((actorId==='A-MOD-2'||actorId==='A-MOD-3')&&round===0)return 424241;return p0113Hash32(text)};
 add('SIMULTANEOUS-TIEROLL-FORCED-COLLISION','強制衝突した参加者だけ再ロールして一意化',()=>{const f=prep(seed,collisionHash),errors=[];const by=Object.fromEntries(f.rolls.map(x=>[x.actor_id,x]));if(by['A-MOD-2']?.reroll_round!==0)errors.push('first colliding actor unexpectedly rerolled');if(by['A-MOD-3']?.reroll_round!==1)errors.push('second colliding actor did not reroll once');if(by['A-MOD-2']?.tie_roll===by['A-MOD-3']?.tie_roll)errors.push('collision unresolved');return{battle_seed:seed,forced_collision_roll:424242,rolls:f.rolls,errors}});
 add('SIMULTANEOUS-TIEROLL-NONCOLLIDER-STABLE','非衝突参加者は再ロールされない',()=>{const f=prep(seed,collisionHash),errors=[];const non=f.rolls.filter(x=>!['A-MOD-2','A-MOD-3'].includes(x.actor_id));if(non.some(x=>x.reroll_round!==0))errors.push('non-colliding actor rerolled');return{battle_seed:seed,non_colliding:non,errors}});
 add('SIMULTANEOUS-TIEROLL-COLLISION-REPRODUCIBLE','同じseedなら衝突と再ロール結果も再現',()=>{const f1=prep(seed,collisionHash),r1=JSON.stringify(f1.rolls);const f2=prep(seed,collisionHash),r2=JSON.stringify(f2.rolls),errors=[];if(r1!==r2)errors.push('forced collision reroll not reproducible');return{battle_seed:seed,first:f1.rolls,second:f2.rolls,errors}});
 add('SIMULTANEOUS-TIEROLL-ORDER-AFTER-REROLL','再ロール後の最終tie rollで同優先度順を確定',()=>{const f=prep(seed,collisionHash),errors=[];const pair=[f.a,f.b].sort((x,y)=>y.battleTieRoll-x.battleTieRoll),winner=pair[0];for(const u of pair){u.gauge=GAUGE_MAX;reserveAction(u);u.reservedAction.executeAt=1}processTicks(1);const o=resultOrder();if(o.fixed[0]?.source_id!==winner.id)errors.push('reroll winner not first fixed');if(o.committed[0]!==winner.id)errors.push('reroll winner not first committed');return{battle_seed:seed,winner:{actor_id:winner.id,tie_roll:winner.battleTieRoll},fixed_order:o.fixed,committed_order:o.committed,errors}});
 battle.validationCaptureEvents=false;battle.validationSimultaneousTieRoll=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.52',generated_at:new Date().toISOString(),validation_patch:'P01-13-VAL-3',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-003',mode:'validation_runtime',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,neutral_skill_id:neutral?.id||null},current_spec:{task_id:'P01-13',stage:'validation_v3_collision',formal_runtime_change:false,candidate_only:true,candidate_order:['activation_priority_desc','battle_tie_roll_desc'],battle_tie_roll:{seeded:true,assigned_once_at_battle_start:true,stable_until_battle_end:true,collision_rule:'reroll_colliding_actors_until_unique'},forced_collision_validation:true},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunSimultaneousActivationOrderJson(){pauseBattle();const report=runSimultaneousActivationOrderValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-simultaneous-activation-order-runtime-device-validation-GA-B486.52-P01-13-VAL3-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-13 SIMULTANEOUS ORDER VAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL CHANGE] NO\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}

function runCounterRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.52',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-002',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-07',stage:'runtime_v1_1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.52-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
