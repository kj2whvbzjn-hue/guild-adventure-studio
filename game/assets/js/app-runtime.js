...
function normalize(raw){
 ...
 raw.schemaRevision='1.5.0';raw.gameVersion='GA-B486.180';
 return raw;
}
...
const ADVENTURE_EXPORT_FILES={manifest:'../Export/manifest.json',quests:['../Export/quest/main_quests.json','../Export/quest/sub_quests.json','../Export/quest/event_quests.json'],chapters:'../Export/scenario/chapters.json',sections:'../Export/scenario/sections.json',scenes:'../Export/scenario/scenes.json',events:'../Export/event/events.json',monsters:'../Export/monster/monsters.json',tablets:'../Export/stone/stones.json',maps:'../Export/world/maps.json',explorationOutcomes:'../Export/exploration/outcomes.json',adventureSettings:'../Export/system/adventure_settings.json',dropTables:'../Export/system/drop_tables.json'};
...
async function loadAdventureContent({force=false}={}){if(adventureContentCache&&!force)return adventureContentCache;const questDocs=await Promise.all(ADVENTURE_EXPORT_FILES.quests.map(fetchAdventureExport));const [chapters,sections,scenes,events,monsters,tablets,maps,explorationOutcomes,adventureSettings,dropTables,manifest]=await Promise.all([... [ADVENTURE_EXPORT_FILES.chapters,ADVENTURE_EXPORT_FILES.sections,ADVENTURE_EXPORT_FILES.scenes,ADVENTURE_EXPORT_FILES.events,ADVENTURE_EXPORT_FILES.monsters,ADVENTURE_EXPORT_FILES.tablets,ADVENTURE_EXPORT_FILES.maps,ADVENTURE_EXPORT_FILES.explorationOutcomes,ADVENTURE_EXPORT_FILES.adventureSettings,ADVENTURE_EXPORT_FILES.dropTables].map(fetchAdventureExport),fetchAdventureManifest()]);const documents=[...questDocs,chapters,sections,scenes,events,monsters,tablets,maps,explorationOutcomes,adventureSettings,dropTables];assertAdventureExportVersionConsistency(manifest,documents);adventureContentCache={quests:questDocs.flatMap(x=>x.data),chapters:chapters.data,sections:sections.data,scenes:scenes.data,events:events.data,monsters:monsters.data,tablets:tablets.data,maps:maps.data,explorationOutcomes:explorationOutcomes.data,adventureSettings:adventureSettings.data,dropTables:dropTables.data,manifest};return adventureContentCache;}
...
function adventureQuestEnvironmentNeeds(content,quest){
 ...
 for(const box of (Array.isArray(quest?.boxes)?quest.boxes:[]))for(const zoneKey of ['event_zone_before_pre','event_zone_pre_to_mid','event_zone_mid_to_post','event_zone_after_post'])for(const placement of (Array.isArray(box?.[zoneKey])?box[zoneKey]:[])){
  if(placement?.kind==='fixed_event'){const e=eventById.get(String(placement?.event_id||placement?.ref_id||''));if(e&&['battle','exploration'].includes(String(e.type||'')))types.add(String(e.type));}
...
function validateAdventureEncounterOverride(content,quest){
 ...
 for(const box of (Array.isArray(quest?.boxes)?quest.boxes:[]))for(const zoneKey of ['event_zone_before_pre','event_zone_pre_to_mid','event_zone_mid_to_post','event_zone_after_post'])for(const placement of (Array.isArray(box?.[zoneKey])?box[zoneKey]:[])){
  if(placement?.kind==='random_event'&&placement?.encounter_override)return{ready:false,quest_id:String(quest?.id||''),box_id:String(box?.box_id||box?.id||''),code:'FORMAL_QUEST_RANDOM_OVERRIDE_INVALID'};
  if(placement?.kind!=='fixed_event'||!placement?.encounter_override)continue;const e=eventById.get(String(placement?.event_id||''));if(String(e?.type||'')!=='battle')return{ready:false,quest_id:String(quest?.id||''),box_id:String(box?.box_id||box?.id||''),code:'FORMAL_QUEST_OVERRIDE_NON_BATTLE'};
  const o=placement.encounter_override,mode=String(o.mode||'resolver');if(!['resolver','required_monsters','fixed_formation'].includes(mode))return{ready:false,quest_id:String(quest?.id||''),box_id:String(box?.box_id||box?.id||''),code:'FORMAL_QUEST_OVERRIDE_MODE_INVALID'};
  const rows=mode==='required_monsters'?o.required_monsters:mode==='fixed_formation'?o.formation:[];if(mode!=='resolver'&&(!Array.isArray(rows)||!rows.length))return{ready:false,quest_id:String(quest?.id||''),box_id:String(box?.box_id||box?.id||''),code:'FORMAL_QUEST_OVERRIDE_FORMATION_EMPTY'};for(const row of rows||[])if(!monsterIds.has(String(row?.monster_id||'')))return{ready:false,quest_id:String(quest?.id||''),box_id:String(box?.box_id||box?.id||''),event_id:String(placement?.event_id||''),code:'FORMAL_QUEST_OVERRIDE_MONSTER_MISSING'};
 }
 return{ready:true};
}
...
function simulateAdventureBattle({formation,seed,encounter_result},bundle,partySnapshot){if(!window.GKAdventureBattleCore?.simulateBasicBattle)throw new Error('Adventure Battle Core simulation is not loaded');const scaling=encounter_result?.battle_scaling||null,monsterMaster=scaling&&window.GKAdventureEncounterResolver?.applyBattleScaling?GKAdventureEncounterResolver.applyBattleScaling(bundle.monsters,scaling):bundle.monsters;return GKAdventureBattleCore.simulateBasicBattle({party:partySnapshot,formation,monsters:monsterMaster,seed});}
...
$('clearBtn').onclick=()=>{if(!confirm('正式版Phase Aの全データを初期化しますか？'))return;data={saveVersion:1,schemaRevision:'1.5.0',gameVersion:'GA-B486.180',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],partyIds:[],selectedQuestId:'Q-001',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},flags:{},quest_progress:{completed_quest_ids:[],unlocked_quest_ids:[]},quest_resources:{},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20,stone_selection_by_quest:{}}};selectedId=null;persist();render();notify('全データを初期化しました。','warn')};
...
async function tagTestRunAuraRuntimeJson(){
 ...
 const report={schema_version:'1.0.0',build:'GA-B486.180',generated_at:new Date().toISOString(),test:{id:'TAG-AURA-RUNTIME-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunAuraRuntimeJson'},current_spec:{task_id:'P01-06',stage:'runtime_connection_v1',runtime_application:true,source_dependency:true,stacking:'highest',source_death:'immediate_disable',source_revive:'re_evaluate_and_restore',target_death:'inactive_while_dead',target_revive:'re_evaluate',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},source:{studio_status:studioSkillBridge.status,data_version:studioSkillBridge.data_version},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.180-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function runActivationPriorityRuntimeValidation(){
 ...
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.180',generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.180-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runBattleEndEffectClearValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.180',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 ...
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.180',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',test:{id:'TAG-BATTLE-END-EFFECT-CLEAR-003',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},current_spec:{task_id:'P01-14',stage:'formal_candidate_v1',formal_runtime_enabled:true,battle_end_clear:['reserved_action','shield_effects','status_effects','cover_effects','dot_stacks','modifier_stacks','cooldowns']},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunBattleEndEffectClearJson(){pauseBattle();const report=runBattleEndEffectClearValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-battle-end-effect-clear-formal-runtime-device-validation-GA-B486.180-P01-14-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-14 BATTLE END CLEAR FORMAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}`;renderBattle();return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.180',generated_at:new Date().toISOString(),formal_candidate:'P01-13-FORMAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-004',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
...
function runCounterRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.180',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-002',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-07',stage:'runtime_v1_1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.180-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
