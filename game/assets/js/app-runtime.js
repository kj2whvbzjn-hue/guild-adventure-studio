...
const formalJobCatalog=new Map(),formalEquipmentCatalog=new Map();
...
function equipmentBonusFromRows(rows){const out={attack:0,maxHp:0,maxMp:0,agi:0,accuracy:0,evasion:0,magicWeaponBonus:0,baseCriticalRate:0,blockRate:0,blockDamageCutRate:0},blockOwners=[];for(const row of rows){const e=equipmentDefinition(row.ref),b=e?.bonuses;if(!b)continue;out.attack+=Number(b.attack)||0;out.maxHp+=Number(b.maxHp)||0;out.maxMp+=Number(b.maxMp)||0;out.agi+=Number(b.agi)||0;out.accuracy+=Number(b.accuracy)||0;out.evasion+=Number(b.evasion)||0;out.magicWeaponBonus+=Number(b.magicWeaponBonus)||0;if(!equipmentIsQuiver(e))out.baseCriticalRate+=Number(b.baseCriticalRate)||0;if((Number(b.blockRate)||0)>0||(Number(b.blockDamageCutRate)||0)>0)blockOwners.push(b)}if(blockOwners.length>1)throw Object.assign(new Error('Block性能を持つEquipmentを複数装備できません。'),{code:'EQUIPMENT_MULTI_BLOCK_SOURCE_FORBIDDEN'});if(blockOwners.length===1){out.blockRate=Number(blockOwners[0].blockRate)||0;out.blockDamageCutRate=Number(blockOwners[0].blockDamageCutRate)||0}return out}
...
function makeBattleUnits(){
 ...
 const allies=members.map((c,i)=>{const v=characterBattleValues(c),e2e=developerE2EOverrideSkillId(c.id),basicAttackProfiles=characterBasicAttackProfiles(c);return makeCombatant({id:`A${i}`,characterId:c.id,name:c.name,side:'味方',ownedSkillIds:clone(c.skills||[]),equippedSkillId:c.equippedSkillId||'',defaultSkillId:e2e||c.equippedSkillId||c.skills?.[0]||'',agi:v.agi,attack:v.attack,accuracy:v.accuracy,evasion:v.evasion,magicWeaponBonus:v.magicWeaponBonus,baseCriticalRate:v.baseCriticalRate,blockRate:v.blockRate,blockDamageCutRate:v.blockDamageCutRate,equipmentTagIds:characterEquipmentTagIds(c),basicAttackUnavailableReason:bowQuiverActionRequirementReason(c),basicAttackProfiles,weaponStyle:characterWeaponStyle(c),maxHp:v.maxHp,maxMp:v.maxMp,gauge:0,actions:0,order:i,lastActionTick:null})});
