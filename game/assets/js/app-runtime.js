...
function normalizeFormalRuntimeSettingsRecord(record){
 ...
 const newGame=objectValue(runtime.new_game,'game_runtime.new_game'),roster=Array.isArray(newGame.starter_roster)?newGame.starter_roster.map((row,i)=>({name:String(row?.name||'').trim(),job_id:String(row?.job_id||'').trim()})):null;if(!roster||roster.some(x=>!x.name||!x.job_id))throw new Error('game_runtime.new_game.starter_roster が不正です。');
