...
async function tagTestRunAuraRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.57-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.57-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runBattleEndEffectClearValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.57',generated_at:new Date().toISOString(),validation_patch:'P01-14-VAL-2',passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 ...
 const defeatAllEnemies=f=>{for(const e of battle.units.filter(x=>x.side==='敵')){e.hp=0;e.alive=false}};
 add('BATTLE-END-TERMINATION-ESTABLISHED','全敵戦闘不能でfinishIfNeededが終了判定へ入る',()=>{const f=prep(),errors=[];defeatAllEnemies(f);const before={ally_alive:battle.units.some(u=>u.alive&&u.side==='味方'),enemy_alive:battle.units.some(u=>u.alive&&u.side==='敵')};const returned=finishIfNeeded();if(before.enemy_alive)errors.push('enemy still alive before finish');if(returned!==true)errors.push('finishIfNeeded returned false');if(battle.pendingResult!=='味方勝利'&&battle.result!=='味方勝利')errors.push(`result not established: pending=${battle.pendingResult} result=${battle.result}`);return{pre_finish:before,finish_returned:returned,pending_result:battle.pendingResult,result:battle.result,errors}});
 add('BATTLE-END-CLEAR-TRANSIENT-STATE','終了成立後に残る一時状態を横断確認',()=>{const f=prep(),before=snap(f.u),errors=[];defeatAllEnemies(f);finishIfNeeded();const after=snap(f.u);for(const k of ['reserved_action','shield_effects','status_effects','cover_effects','dot_stacks','modifier_stacks','cooldowns'])if(after[k])errors.push(`${k} remains=${after[k]}`);return{before,after,pending_result:battle.pendingResult,result:battle.result,errors}});
 add('BATTLE-END-CLEAR-IDEMPOTENT','終了成立後の再呼出しで状態が再生成されない',()=>{const f=prep(),errors=[];defeatAllEnemies(f);finishIfNeeded();const once=snap(f.u);finishIfNeeded();const twice=snap(f.u);if(JSON.stringify(once)!==JSON.stringify(twice))errors.push('second finish changed state');return{once,twice,pending_result:battle.pendingResult,result:battle.result,errors}});
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.57',generated_at:new Date().toISOString(),validation_patch:'P01-14-VAL-2',test:{id:'TAG-BATTLE-END-EFFECT-CLEAR-002',mode:'termination_condition_validation',entrypoint:'game/index.html'},current_spec:{task_id:'P01-14',formal_runtime_change:false,candidate_only:true,must_clear:['reserved_action','shield_effects','status_effects','cover_effects','dot_stacks','modifier_stacks','cooldowns'],val1_correction:'all enemy units are now defeated before finishIfNeeded'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunBattleEndEffectClearJson(){pauseBattle();const report=runBattleEndEffectClearValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-battle-end-effect-clear-validation-GA-B486.57-P01-14-VAL2-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-14 BATTLE END CLEAR VAL-2] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;renderBattle();return report}
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.57-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
