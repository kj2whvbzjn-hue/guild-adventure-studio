...
function characterBattleValues(character){const b=equipmentBonus(character),weapon=characterPrimaryWeaponBonus(character),mods=characterFormalModifierContributions(character),baseHp=finiteNumber(character?.base_hp,`${character?.id||character?.name||'character'}.base_hp`,{integer:true,min:1}),baseMp=finiteNumber(character?.base_mp,`${character?.id||character?.name||'character'}.base_mp`,{integer:true,min:0}),agi=finiteNumber(character?.stats?.AGI,`${character?.id||character?.name||'character'}.stats.AGI`,{integer:true,min:0}),baseMaxHp=baseHp+b.maxHp,baseMaxMp=baseMp+b.maxMp,maxHp=resolveFormalModifierRows(baseMaxHp,'MAX_HP',mods).final_value,maxMp=resolveFormalModifierRows(baseMaxMp,'MAX_MP',mods).final_value;return{maxHp:Math.max(1,maxHp),maxMp:Math.max(0,maxMp),attack:weapon.attack,agi,accuracy:weapon.accuracy,evasion:b.evasion,magicAccuracy:Math.max(0,weapon.magicAccuracy),magicResistance:Math.max(0,b.magicResistance),magicWeaponBonus:weapon.magicWeaponBonus,weaponCriticalRate:weapon.weaponCriticalRate,luk:Math.max(0,Number(character?.stats?.LUK)||0),blockRate:b.blockRate,blockDamageCutRate:b.blockDamageCutRate}}
...
function formalEquipmentNumber(record,key){const p=record?.params&&typeof record.params==='object'?record.params:{};const stats=p.stats&&typeof p.stats==='object'?p.stats:{};const raw=record?.[key]??p[key]??stats[key];return Number.isFinite(Number(raw))?Number(raw):0}
function normalizeFormalEquipmentRecord(record){
 ...
 const id=String(record.id||'').trim(),name=String(record.name||'').trim();if(!id||!name)return null;
 const p=record.params&&typeof record.params==='object'?record.params:{},input=record.generation?.generation_input&&typeof record.generation.generation_input==='object'?record.generation.generation_input:{};
 const armorSlot=String(record.armor_slot??p.armor_slot??input.armor_slot??'').trim(),explicitSlot=String(record.slot??p.slot??'').trim(),kind=String(input.kind??record.kind??p.kind??'').trim().toLowerCase();
 let slot='';
 if(explicitSlot){
 ...
 if(!slot){if(kind==='weapon'||record.required_str!=null||record.required_dex!=null||record.required_int!=null)slot='weapon';else if(armorSlot)slot=FORMAL_EQUIPMENT_SLOT_MAP[armorSlot]||'';else if(kind==='armor')slot='armor';}
 ...
 const bonuses={attack:formalEquipmentNumber(record,'attack'),maxHp:formalEquipmentNumber(record,'hp_bonus')||formalEquipmentNumber(record,'maxHp')||formalEquipmentNumber(record,'hp'),maxMp:formalEquipmentNumber(record,'mp_bonus')||formalEquipmentNumber(record,'maxMp')||formalEquipmentNumber(record,'mp'),agi:formalEquipmentNumber(record,'agi'),accuracy:formalEquipmentNumber(record,'accuracy'),evasion:formalEquipmentNumber(record,'evasion'),magicAccuracy:formalEquipmentNumber(record,'magic_accuracy'),magicResistance:formalEquipmentNumber(record,'magic_resistance'),magicWeaponBonus:formalEquipmentNumber(record,'magic_weapon_bonus'),weaponCriticalRate:formalEquipmentNumber(record,'weapon_critical_rate'),blockRate:formalEquipmentNumber(record,'block_rate'),blockDamageCutRate:formalEquipmentNumber(record,'block_damage_cut_rate')};
 const rarity=String(record.rarity??p.rarity??'').trim();return{...record,id,name,source:'studio_export',slot,slotLabel:FORMAL_EQUIPMENT_TARGET_LABEL[slot]||armorSlot||slot||'未定義',armorSlot,rarity,required,bonuses};
}
...
function equipmentBonusLabel(equipment){const b=equipment?.bonuses||{},rows=[];if(b.attack)rows.push(`攻撃 +${b.attack}`);if(b.maxHp)rows.push(`HP +${b.maxHp}`);if(b.maxMp)rows.push(`MP +${b.maxMp}`);if(b.agi)rows.push(`AGI ${b.agi>=0?'+':''}${b.agi}`);if(b.accuracy)rows.push(`命中値 +${b.accuracy}`);if(b.evasion)rows.push(`回避値 +${b.evasion}`);if(b.magicWeaponBonus)rows.push(`魔法武器Contribution +${b.magicWeaponBonus}`);if(b.weaponCriticalRate)rows.push(`武器Crit値 ${Math.round(b.weaponCriticalRate*10000)/100}%`);return rows.length?rows.join(' / '):'数値補正なし'}
...
function render(){
 ...
 $('detail').innerHTML=`<div class="row"><div><div class="name">${escapeHtml(c.name)}</div><span class="tag">Lv ${c.level}${maxLevel?` / ${maxLevel}`:''}</span><span class="tag">現在職：${escapeHtml(jobDisplayName(c.job))}</span></div></div><div class="stats">${STATS.map(s=>`<div class="stat"><span class="small">${s}</span><b>${c.stats[s]}</b></div>`).join('')}</div><h3>装着スキル</h3><div class="small">${escapeHtml(findSkill(c.equippedSkillId)?.name||'未装着')}</div><h3>装備</h3><div class="small">${characterEquipmentEntries(c).map(x=>`${x.label}: ${equipmentDisplayName(x.ref)}`).join(' / ')||'なし'}（攻撃 +${equipmentBonus(c).attack} / HP +${equipmentBonus(c).maxHp} / MP +${equipmentBonus(c).maxMp} / AGI ${equipmentBonus(c).agi>=0?'+':''}${equipmentBonus(c).agi}）</div><h3>転職履歴</h3><div class="small">${c.jobHistory.map(h=>`Lv${h.level} ${escapeHtml(jobDisplayName(h.job))}`).join(' → ')}</div><h3>直近の成長</h3><div class="small">${c.growthHistory.slice(-5).reverse().map(g=>`Lv${g.toLevel}: ${g.gained.length?g.gained.join(', '):'能力上昇なし'}`).join('<br>')||'まだレベルアップしていません。'}</div>`;
