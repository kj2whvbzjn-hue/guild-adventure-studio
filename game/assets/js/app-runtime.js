...
function setupTagSkillTestUI(){
 ...
 if(runCounterRuntimeJson)runCounterRuntimeJson.onclick=tagTestRunCounterRuntimeJson;if(runR04GenericCounterRuntimeJson)runR04GenericCounterRuntimeJson.onclick=tagTestRunR04GenericCounterRuntimeJson;if(runR04GenericHealRuntimeJson)runR04GenericHealRuntimeJson.onclick=tagTestRunR04GenericHealRuntimeJson;if(runR04GenericShieldRuntimeJson)runR04GenericShieldRuntimeJson.onclick=tagTestRunR04GenericShieldRuntimeJson;if(runR04GenericShieldBoundaryRuntimeJson)runR04GenericShieldBoundaryRuntimeJson.onclick=tagTestRunR04GenericShieldBoundaryRuntimeJson;if(runR04GenericFollowUpRuntimeJson)runR04GenericFollowUpRuntimeJson.onclick=tagTestRunR04GenericFollowUpRuntimeJson;if(runR04GenericAuraRuntimeJson)runR04GenericAuraRuntimeJson.onclick=tagTestRunR04GenericAuraRuntimeJson;if(runCoverRuntimeJson)runCoverRuntimeJson.onclick=tagTestRunCoverRuntimeJson;if(runActionDisabledRuntimeJson)runActionDisabledRuntimeJson.onclick=tagTestRunActionDisabledRuntimeJson;if(runCooldownRuntimeJson)runCooldownRuntimeJson.onclick=tagTestRunCooldownRuntimeJson;if(runCostRuntimeJson)runCostRuntimeJson.onclick=tagTestRunCostRuntimeJson;
...
function runCooldownRuntimeRegression(){
 ...
 add('COOLDOWN-RUNTIME-START','発動成立時にCOOLDOWN=300開始',()=>{const f=prep(),a=f.allies[0],t=f.enemies[0],er=[];const r=executeTaggedSkill(a,t,skills.cool),rem=skillCooldownRemaining(a,skills.cool.id),started=ev('cooldown_started');if(!r.ok)er.push('発動失敗');if(rem!==300)er.push(`remaining=${rem}`);if(started.length!==1||started[0].expires_at!==300)er.push('cooldown_started不正');return{remaining:rem,events:[...battle.validationEvents],errors:er}});
 add('COOLDOWN-RUNTIME-BLOCK','CD中は同一スキル実行不可',()=>{const f=prep(),a=f.allies[0],t=f.enemies[0],er=[];executeTaggedSkill(a,t,skills.cool);const hp=t.hp,r=executeTaggedSkill(a,t,skills.cool);if(r.ok||r.reason!=='COOLDOWN')er.push(`result=${r.reason||r.ok}`);if(t.hp!==hp)er.push('CD中に効果発生');if(skillCooldownRemaining(a,skills.cool.id)!==300)er.push('CDが再開始された');return{remaining:skillCooldownRemaining(a,skills.cool.id),events:[...battle.validationEvents],errors:er}});
 add('COOLDOWN-RUNTIME-EXPIRE-REUSE','Tick300で満了し再使用可能',()=>{const f=prep(),a=f.allies[0],t=f.enemies[0],er=[];executeTaggedSkill(a,t,skills.cool);processTicks(299);if(skillCooldownRemaining(a,skills.cool.id)!==1)er.push('Tick299残1でない');const blocked=executeTaggedSkill(a,t,skills.cool);if(blocked.ok)er.push('Tick299で再使用');processTicks(1);if(skillCooldownRemaining(a,skills.cool.id)!==0)er.push('Tick300で満了しない');const reused=executeTaggedSkill(a,t,skills.cool);if(!reused.ok)er.push('満了後再使用不可');if(skillCooldownRemaining(a,skills.cool.id)!==300)er.push('再使用後CD300でない');return{tick:battle.tick,remaining_after_reuse:skillCooldownRemaining(a,skills.cool.id),events:[...battle.validationEvents],errors:er}});
 add('COOLDOWN-RUNTIME-ZERO','COOLDOWN未指定/0は状態を作らない',()=>{const f=prep(),a=f.allies[0],t=f.enemies[0],er=[];executeTaggedSkill(a,t,skills.attack);executeTaggedSkill(a,t,skills.attack);if(Object.keys(ensureCooldownState(a)).length)er.push('CD0でstate生成');return{cooldowns:{...ensureCooldownState(a)},events:[...battle.validationEvents],errors:er}});
...
function runCostRuntimeRegression(){
 ...
 add('COST-RUNTIME-INVALID-TARGET-NO-CONSUME','対象不成立ならMP/CDなし',()=>{const f=prep(),er=[],before=f.actor.mp;f.target.hp=0;f.target.alive=false;const r=executeTaggedSkill(f.actor,f.target,skill);if(r.ok)er.push('無効対象で発動');if(f.actor.mp!==before)er.push('MP消費');if(skillCooldownRemaining(f.actor,skill.id)!==0)er.push('CD開始');return{mp_before:before,mp_after:f.actor.mp,events:[...battle.validationEvents],errors:er}});
 add('COST-RUNTIME-ACTION-DISABLED-NO-CONSUME','行動不能ならMP/CDなし',()=>{const f=prep(),er=[],before=f.actor.mp;f.actor.actionDisabled=true;const r=executeTaggedSkill(f.actor,f.target,skill);if(r.ok)er.push('行動不能で発動');if(f.actor.mp!==before)er.push('MP消費');if(skillCooldownRemaining(f.actor,skill.id)!==0)er.push('CD開始');return{mp_before:before,mp_after:f.actor.mp,events:[...battle.validationEvents],errors:er}});
 add('COST-RUNTIME-SECOND-USE-CD-BLOCK','初回消費後のCD中再使用では追加消費なし',()=>{const f=prep(),er=[];executeTaggedSkill(f.actor,f.target,skill);const afterFirst=f.actor.mp,r2=executeTaggedSkill(f.actor,f.target,skill);if(r2.ok)er.push('CD中再発動');if(f.actor.mp!==afterFirst)er.push('CD中に追加消費');if(ev('cost_consumed').length!==1)er.push(`cost_events=${ev('cost_consumed').length}`);return{mp_after_first:afterFirst,mp_after_second:f.actor.mp,events:[...battle.validationEvents],errors:er}});
 const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.101',generated_at:new Date().toISOString(),test:{id:'TAG-COST-RUNTIME-001',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version,cost_skill_id:skill.id},current_spec:{task_id:'P01-11',stage:'runtime_v1',runtime_application:true,formal_cost_type:'mp',extensible_cost_structure:true,reservation_consumes:false,action_order_checks_cost:true,activation_established_consumes:true,activation_established_starts_cooldown:true,insufficient_cost_effect:false,casting_connection:'deferred_existing_casting_foundation_not_present',formal_integrated_spec:'戦闘仕様 正式統合版 v5'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunCostRuntimeJson(){pauseBattle();const report=runCostRuntimeRegression();report.test={...report.test,mode:'device_runtime_validation',trigger:'tagTestRunCostRuntimeJson'};const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-cost-runtime-device-validation-GA-B486.101-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COST RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}

function runActionDisabledRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.101',generated_at:new Date().toISOString(),test:{id:'TAG-ACTION-DISABLED-RUNTIME-001',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-09',stage:'runtime_v1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
 const req=id=>{const x=findTagSkill(id);if(!x)throw new Error(`Studio正式スキル不足: ${id}`);if(x.source!=='studio_export'||(x.environment||'production')!=='production')throw new Error(`Studio production由来ではありません: ${id}`);return x};
 ...
 const applyDisabled=(source,target)=>executeTaggedSkill(source,target,skills.disabled,{skipExecutionEligibility:true});
 const ev=t=>battle.validationEvents.filter(x=>x.type===t);
 ...
 add('ACTION-DISABLED-RUNTIME-COUNTER-BLOCK','STATUS行動不能でCOUNTER停止',()=>{const f=prep(),def=f.allies[0],atk=f.enemies[0],source=f.enemies[1],er=[];def.counterSkillId=skills.counter.id;applyDisabled(source,def);battle.validationEvents=[];executeTaggedSkill(atk,def,skills.attack);if(ev('counter_triggered').length)er.push('COUNTERが発動');if(!ev('counter_skipped').some(x=>x.reason==='ACTION_DISABLED'))er.push('ACTION_DISABLED skipなし');return{events:[...battle.validationEvents],errors:er}});
 add('ACTION-DISABLED-RUNTIME-FOLLOW-UP-BLOCK','STATUS行動不能でFOLLOW_UP停止',()=>{const f=prep(),initiator=f.allies[0],follower=f.allies[1],target=f.enemies[0],source=f.enemies[1],er=[];executeTaggedSkill(initiator,target,skills.poison);battle.validationEvents=[];follower.followUpSkillIds=[skills.follow.id];applyDisabled(source,follower);battle.validationEvents=[];dispatchConditionalFollowUps(initiator,target,{trigger:'ALLY_ATTACK',originSkillId:skills.attack.id,derivedGeneration:0});if(ev('follow_up_triggered').length)er.push('FOLLOW_UPが発動');if(!ev('follow_up_skipped').some(x=>x.reason==='ACTION_DISABLED'))er.push('ACTION_DISABLED skipなし');return{events:[...battle.validationEvents],errors:er}});
 add('ACTION-DISABLED-RUNTIME-DOT-CONTINUES','既存DOTは行動不能中も継続',()=>{const f=prep(),actor=f.allies[0],enemy=f.enemies[0],er=[];executeTaggedSkill(enemy,actor,skills.poison);applyDisabled(enemy,actor);const hp=actor.hp;battle.tick=100;processDotStacks();if(actor.hp>=hp)er.push('DOTが停止');return{hp_before:hp,hp_after:actor.hp,dot_count:ensureDotStackList(actor).length,events:[...battle.validationEvents],errors:er}});
 add('ACTION-DISABLED-RUNTIME-SHIELD-CONTINUES','既存SHIELDは行動不能中も継続',()=>{const f=prep(),actor=f.allies[0],ally=f.allies[1],enemy=f.enemies[0],er=[];executeTaggedSkill(ally,actor,skills.shield);applyDisabled(enemy,actor);const hp=actor.hp,shieldBefore=shieldTotal(actor);executeTaggedSkill(enemy,actor,skills.attack);if(actor.hp!==hp)er.push('シールドが機能せずHP減少');if(shieldTotal(actor)>=shieldBefore)er.push('シールドが消費されていない');return{hp_before:hp,hp_after:actor.hp,shield_before:shieldBefore,shield_after:shieldTotal(actor),events:[...battle.validationEvents],errors:er}});
...
function runActivationPriorityRuntimeValidation(){
 ...
 add('ACTIVATION-PRIORITY-COMPILE','整数優先度を正式コンパイル',()=>{const errors=[],hc=compileTaggedSkill(high),lc=compileTaggedSkill(low);if(!hc.ok||hc.definition.parameters.activationPriority!==100)errors.push('high compile');if(!lc.ok||lc.definition.parameters.activationPriority!==-100)errors.push('low compile');return{high_priority:hc.definition.parameters.activationPriority,low_priority:lc.definition.parameters.activationPriority,errors}});
 ...
 add('ACTIVATION-PRIORITY-DECIMAL-REJECT','小数優先度を拒否',()=>{const bad=findTagSkill('ACTIVATION-PRIORITY-VALIDATION-DECIMAL'),c=compileTaggedSkill(bad),errors=[];if(c.ok)errors.push('decimal accepted');return{compiled_ok:c.ok,compiler_errors:c.errors,errors}});
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.101',generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.101-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runBattleEndEffectClearValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.101',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 ...
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.101',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',test:{id:'TAG-BATTLE-END-EFFECT-CLEAR-003',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},current_spec:{task_id:'P01-14',stage:'formal_candidate_v1',formal_runtime_enabled:true,battle_end_clear:['reserved_action','shield_effects','status_effects','cover_effects','dot_stacks','modifier_stacks','cooldowns']},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunBattleEndEffectClearJson(){pauseBattle();const report=runBattleEndEffectClearValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-battle-end-effect-clear-formal-runtime-device-validation-GA-B486.101-P01-14-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-14 BATTLE END CLEAR FORMAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}`;renderBattle();return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.101',generated_at:new Date().toISOString(),formal_candidate:'P01-13-FORMAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-004',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 const neutral=findTagSkill('SKL-TEST-ATTACK'),high=findTagSkill('SKL-ACTIVATION-PRIORITY-HIGH'),cases=[];const seed='P01-13-FORMAL1-SEED-001';
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.101-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}

async function runR04GenericCounterDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint='game/index.html',cases=[];
 const add=(id,label,run)=>{try{const detail=run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[String(e?.message||e)]})}};
 const genericSkill={schemaVersion:1,id:'R04-DEVICE-GENERIC-COUNTER',name:'R04 Generic Counter Device',trigger:{type:'ON_HIT_RECEIVED',scope:'SELF',priority:7},target:{side:'ENEMY',range:'SINGLE'},effects:[{type:'DAMAGE',power:100,damageType:'PHYSICAL'}],resource:{mpCost:0,cooldown:0}};
 let generated=null,fixture=null,fixtureIndex=-1;
 try{
  if(!window.GKSGenericSkillBridge?.compileForLegacy)throw new Error('GKSGenericSkillBridge.compileForLegacy がありません');
  generated=await window.GKSGenericSkillBridge.compileForLegacy(genericSkill,{legacyCompile:compileTaggedSkill});
  if(!generated?.ok)throw new Error(`Generic compile failed: ${(generated?.errors||[]).map(x=>x.code||x.message||String(x)).join(', ')}`);
  fixture={...generated.legacySkill,source:'device_fixture',environment:'validation'};
  if(findTagSkill(fixture.id))throw new Error(`fixture id collision: ${fixture.id}`);
  TAG_SKILLS.push(fixture);fixtureIndex=TAG_SKILLS.length-1;
  const compiled=compileTaggedSkill(fixture),contract=compiled.definition?.genericRuntime?.triggerContract||null;
  add('R04-GENERIC-COUNTER-CONTRACT','Generic→Legacy Adapter→COUNTER Trigger契約',()=>{const errors=[];if(!compiled.ok)errors.push(`legacy compile failed: ${(compiled.errors||[]).join(', ')}`);if(contract?.type!=='ON_HIT_RECEIVED')errors.push(`trigger type=${contract?.type||null}`);if(contract?.engineEvent!=='hit_received')errors.push(`engine event=${contract?.engineEvent||null}`);if(contract?.dispatchMode!=='LEGACY_COUNTER_ADAPTER')errors.push(`dispatch mode=${contract?.dispatchMode||null}`);return{generic_skill:genericSkill,legacy_skill:fixture,trigger_contract:contract,errors}});
  const incoming=findTagSkill('SKL-TEST-ATTACK');if(!incoming)throw new Error('Studio正式SKL-TEST-ATTACKがありません');
  const prep=()=>{pauseBattle();resetBattle();battle.validationMode=true;battle.validationCaptureEvents=true;battle.validationEvents=[];const allies=ensureValidationTargets('味方',2),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.counterSkillId=null;u.counterDisabled=false;u.shieldEffects=[];u.statusEffects=[];u.followUpSkillIds=[]}return{defender:allies[0],ally:allies[1],attacker:enemies[0]}};
  add('R04-GENERIC-COUNTER-RUNTIME-BASIC','Game本番RuntimeでGeneric COUNTER発動',()=>{const f=prep(),errors=[];f.defender.counterSkillId=fixture.id;const hp=f.attacker.hp,result=executeTaggedSkill(f.attacker,f.defender,incoming,{origin:'base'}),genericEvents=battle.validationEvents.filter(x=>x.type==='generic_trigger_dispatched'&&x.counter_skill_id===fixture.id),counterEvents=battle.validationEvents.filter(x=>x.type==='counter_triggered'&&x.counter_skill_id===fixture.id);if(!result.ok)errors.push(`incoming failed: ${result.reason||result.stage||'unknown'}`);if(genericEvents.length!==1)errors.push(`generic_trigger_dispatched=${genericEvents.length}`);if(counterEvents.length!==1)errors.push(`counter_triggered=${counterEvents.length}`);if(f.attacker.hp>=hp)errors.push('攻撃者へ反撃ダメージがありません');return{attacker_hp_before:hp,attacker_hp_after:f.attacker.hp,generic_trigger_count:genericEvents.length,counter_trigger_count:counterEvents.length,events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-COUNTER-RUNTIME-ZERO-DAMAGE','シールド全吸収でもGeneric COUNTER発動',()=>{const f=prep(),errors=[];f.defender.counterSkillId=fixture.id;f.defender.shieldEffects=[{id:'R04-DEVICE-SHIELD',sequence:1,sourceId:f.defender.id,skillId:'R04-DEVICE',skillName:'R04 Device Shield',amount:9999,remaining:9999,appliedAt:0,expiresAt:9999}];const hp=f.defender.hp;executeTaggedSkill(f.attacker,f.defender,incoming,{origin:'base'});const genericEvents=battle.validationEvents.filter(x=>x.type==='generic_trigger_dispatched'&&x.counter_skill_id===fixture.id);if(f.defender.hp!==hp)errors.push(`defender hp ${hp}->${f.defender.hp}`);if(genericEvents.length!==1)errors.push(`generic_trigger_dispatched=${genericEvents.length}`);return{defender_hp_before:hp,defender_hp_after:f.defender.hp,generic_trigger_count:genericEvents.length,events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-COUNTER-RUNTIME-DERIVED-BLOCK','派生攻撃からGeneric COUNTERを再発火しない',()=>{const f=prep(),errors=[];f.defender.counterSkillId=fixture.id;executeTaggedSkill(f.attacker,f.defender,incoming,{origin:'follow_up',derivedGeneration:1});const genericEvents=battle.validationEvents.filter(x=>x.type==='generic_trigger_dispatched'&&x.counter_skill_id===fixture.id),counterEvents=battle.validationEvents.filter(x=>x.type==='counter_triggered'&&x.counter_skill_id===fixture.id),chainBlocks=battle.validationEvents.filter(x=>x.type==='follow_up_chain_blocked'&&x.skill_id===incoming.id);if(genericEvents.length)errors.push(`generic_trigger_dispatched=${genericEvents.length}`);if(counterEvents.length)errors.push(`counter_triggered=${counterEvents.length}`);if(chainBlocks.length!==1)errors.push(`follow_up_chain_blocked=${chainBlocks.length}`);return{generic_trigger_count:genericEvents.length,counter_trigger_count:counterEvents.length,follow_up_chain_block_count:chainBlocks.length,events:[...battle.validationEvents],errors}});
 }catch(e){cases.push({id:'R04-GENERIC-COUNTER-SETUP',label:'Generic COUNTER実機fixture準備',passed:false,errors:[String(e?.message||e)]})}
 finally{battle.validationCaptureEvents=false;if(fixtureIndex>=0&&TAG_SKILLS[fixtureIndex]?.id===fixture?.id)TAG_SKILLS.splice(fixtureIndex,1)}
 const errors=cases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:gameBuild,generated_at:new Date().toISOString(),test:{id:'R04-GENERIC-COUNTER-DEVICE-001',mode:'generic_production_runtime_device_validation',entrypoint,trigger:'tagTestRunR04GenericCounterRuntimeJson'},pipeline:['generic_skill','generic_skill_compiler','legacy_adapter','compileTaggedSkill','Game executeTaggedSkill','GKSTriggerEngine','COUNTER runtime'],cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
async function tagTestRunR04GenericCounterRuntimeJson(){
 pauseBattle();if(studioSkillBridge.status!=='loaded')await loadStudioSkillDefinitions();const report=await runR04GenericCounterDeviceValidation(),errors=report.summary.errors||[],build=report.build||'UNKNOWN';
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`r04-generic-counter-runtime-device-validation-${build}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[R04 GENERIC COUNTER RUNTIME] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;renderBattle();return report;
}


async function runR04GenericShieldDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint='game/index.html',cases=[];
 const add=(id,label,run)=>{try{const detail=run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[String(e?.message||e)]})}};
 const genericSkill={schemaVersion:1,id:'R04-DEVICE-GENERIC-SHIELD',name:'R04 Generic Shield Device',trigger:{type:'ON_USE',scope:'SELF'},target:{side:'ALLY',range:'SINGLE'},effects:[{type:'APPLY',effectId:'BARRIER',power:100,duration:300}],resource:{mpCost:0,cooldown:0}};
 let generated=null,fixture=null,fixtureIndex=-1;
 try{
  if(!window.GKSGenericSkillBridge?.compileForLegacy)throw new Error('GKSGenericSkillBridge.compileForLegacy がありません');
  generated=await window.GKSGenericSkillBridge.compileForLegacy(genericSkill,{legacyCompile:compileTaggedSkill});
  ...
  if(findTagSkill(fixture.id))throw new Error(`fixture id collision: ${fixture.id}`);
  TAG_SKILLS.push(fixture);fixtureIndex=TAG_SKILLS.length-1;
  const compiled=compileTaggedSkill(fixture),contract=compiled.definition?.genericRuntime?.applyContracts?.find(x=>x?.kind==='SHIELD')||null,triggerContract=compiled.definition?.genericRuntime?.triggerContract||null;
  add('R04-GENERIC-SHIELD-CONTRACT','Generic→Legacy Adapter→ON_USE SHIELD契約',()=>{const errors=[];if(!compiled.ok)errors.push(`legacy compile failed: ${(compiled.errors||[]).join(', ')}`);if(triggerContract?.type!=='ON_USE')errors.push(`trigger type=${triggerContract?.type||null}`);if(compiled.definition?.parameters?.shield!==100)errors.push(`shield=${compiled.definition?.parameters?.shield}`);if(compiled.definition?.parameters?.shieldDuration!==300)errors.push(`duration=${compiled.definition?.parameters?.shieldDuration}`);if(contract?.lifecycle?.consumeRule!=='FIFO')errors.push(`consumeRule=${contract?.lifecycle?.consumeRule||null}`);return{generic_skill:genericSkill,legacy_skill:fixture,trigger_contract:triggerContract,shield_apply_contract:contract,errors}});
  const prep=()=>{pauseBattle();resetBattle();battle.validationMode=true;battle.validationCaptureEvents=true;battle.validationEvents=[];const allies=ensureValidationTargets('味方',2),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.reservedAction=null;u.statusEffects=[];u.dotStacks=[];u.shieldEffects=[]}return{actor:allies[0],target:allies[1],enemy:enemies[0]}};
  add('R04-GENERIC-SHIELD-RUNTIME-APPLY','Game本番RuntimeでGeneric SHIELD付与',()=>{const f=prep(),errors=[];const result=executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'}),events=battle.validationEvents.filter(x=>x.type==='shield_added'&&x.skill_id===fixture.id);if(!result.ok)errors.push(`execute failed: ${result.reason||result.stage||'unknown'}`);if(result.shieldResult?.amount!==100)errors.push(`amount=${result.shieldResult?.amount}`);if(result.shieldResult?.duration!==300)errors.push(`duration=${result.shieldResult?.duration}`);if(shieldTotal(f.target)!==100)errors.push(`shield total=${shieldTotal(f.target)}`);if(events.length!==1)errors.push(`shield_added=${events.length}`);return{shield_result:result.shieldResult,shield_total:shieldTotal(f.target),events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-SHIELD-RUNTIME-ABSORB','Generic SHIELDがGame本番ダメージを吸収',()=>{const f=prep(),errors=[];const grant=executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'});if(!grant.ok)errors.push(`grant failed: ${grant.reason||grant.stage||'unknown'}`);const before=f.target.hp,damage=applyTaggedDamage(f.enemy,f.target,60,{id:'R04-DEVICE-DAMAGE-60',name:'R04 Device Damage 60',parameters:{damage:60}}),absorbs=battle.validationEvents.filter(x=>x.type==='shield_absorbed'&&x.target_id===f.target.id);if(damage.shieldAbsorbed!==60)errors.push(`absorbed=${damage.shieldAbsorbed}`);if(damage.damage!==0)errors.push(`hp damage=${damage.damage}`);if(f.target.hp!==before)errors.push(`hp ${before}->${f.target.hp}`);if(shieldTotal(f.target)!==40)errors.push(`shield remainder=${shieldTotal(f.target)}`);if(absorbs.length!==1)errors.push(`shield_absorbed=${absorbs.length}`);return{target_hp_before:before,target_hp_after:f.target.hp,damage_result:damage,shield_remaining:shieldTotal(f.target),events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-SHIELD-RUNTIME-ZERO-DAMAGE','Generic SHIELD全吸収でHPダメージ0',()=>{const f=prep(),errors=[];executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'});const before=f.target.hp,damage=applyTaggedDamage(f.enemy,f.target,100,{id:'R04-DEVICE-DAMAGE-100',name:'R04 Device Damage 100',parameters:{damage:100}});if(damage.shieldAbsorbed!==100)errors.push(`absorbed=${damage.shieldAbsorbed}`);if(damage.damage!==0)errors.push(`hp damage=${damage.damage}`);if(f.target.hp!==before)errors.push(`hp ${before}->${f.target.hp}`);if(shieldTotal(f.target)!==0)errors.push(`shield remainder=${shieldTotal(f.target)}`);return{target_hp_before:before,target_hp_after:f.target.hp,damage_result:damage,shield_remaining:shieldTotal(f.target),events:[...battle.validationEvents],errors}});
 }catch(e){cases.push({id:'R04-GENERIC-SHIELD-SETUP',label:'Generic SHIELD実機fixture準備',passed:false,errors:[String(e?.message||e)]})}
 finally{battle.validationCaptureEvents=false;if(fixtureIndex>=0&&TAG_SKILLS[fixtureIndex]?.id===fixture?.id)TAG_SKILLS.splice(fixtureIndex,1)}
 const errors=cases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:gameBuild,generated_at:new Date().toISOString(),test:{id:'R04-GENERIC-SHIELD-DEVICE-001',mode:'generic_production_runtime_device_validation',entrypoint,trigger:'tagTestRunR04GenericShieldRuntimeJson'},pipeline:['generic_skill','generic_skill_compiler','legacy_adapter','compileTaggedSkill','Game executeTaggedSkill','generic APPLY lifecycle','SHIELD runtime'],cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
async function tagTestRunR04GenericShieldRuntimeJson(){
 pauseBattle();if(studioSkillBridge.status!=='loaded')await loadStudioSkillDefinitions();const report=await runR04GenericShieldDeviceValidation(),errors=report.summary.errors||[],build=report.build||'UNKNOWN';
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`r04-generic-shield-runtime-device-validation-${build}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[R04 GENERIC SHIELD RUNTIME] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;renderBattle();return report;
}
async function runR04GenericShieldBoundaryDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint='game/index.html',cases=[];
 const add=(id,label,run)=>{try{const detail=run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[String(e?.message||e)]})}};
 const genericSkill={schemaVersion:1,id:'R04-DEVICE-GENERIC-SHIELD-BOUNDARY',name:'R04 Generic Shield Boundary Device',trigger:{type:'ON_USE',scope:'SELF'},target:{side:'ALLY',range:'SINGLE'},effects:[{type:'APPLY',effectId:'BARRIER',power:100,duration:300}],resource:{mpCost:0,cooldown:0}};
 let generated=null,fixture=null,fixtureIndex=-1;
 try{
  if(!window.GKSGenericSkillBridge?.compileForLegacy)throw new Error('GKSGenericSkillBridge.compileForLegacy がありません');
  generated=await window.GKSGenericSkillBridge.compileForLegacy(genericSkill,{legacyCompile:compileTaggedSkill});
  ...
  if(findTagSkill(fixture.id))throw new Error(`fixture id collision: ${fixture.id}`);
  TAG_SKILLS.push(fixture);fixtureIndex=TAG_SKILLS.length-1;
  const compiled=compileTaggedSkill(fixture),contract=compiled.definition?.genericRuntime?.applyContracts?.find(x=>x?.kind==='SHIELD')||null;
  if(!compiled.ok)throw new Error(`legacy compile failed: ${(compiled.errors||[]).join(', ')}`);
  if(contract?.lifecycle?.removeOnDeath!==true||contract?.lifecycle?.removeOnBattleEnd!==true||contract?.lifecycle?.stackRule!=='STACK'||contract?.lifecycle?.consumeRule!=='FIFO')throw new Error('Generic SHIELD lifecycle契約がC2境界要件と一致しません');
  const prep=()=>{pauseBattle();resetBattle();battle.validationMode=true;battle.validationCaptureEvents=true;battle.validationEvents=[];const allies=ensureValidationTargets('味方',2),enemies=ensureValidationTargets('敵',2);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.reservedAction=null;u.statusEffects=[];u.dotStacks=[];u.modifierStacks=[];u.shieldEffects=[]}return{actor:allies[0],target:allies[1],enemy:enemies[0],enemy2:enemies[1]}};
  add('R04-GENERIC-SHIELD-BOUNDARY-STACK','Generic SHIELD複数付与はSTACKで加算',()=>{const f=prep(),errors=[];const first=executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'}),second=executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'}),effects=ensureShieldEffects(f.target),added=battle.validationEvents.filter(x=>x.type==='shield_added'&&x.skill_id===fixture.id);if(!first.ok||!second.ok)errors.push('2回付与のどちらかが失敗');if(effects.length!==2)errors.push(`shield layers=${effects.length}`);if(shieldTotal(f.target)!==200)errors.push(`shield total=${shieldTotal(f.target)}`);if(added.length!==2)errors.push(`shield_added=${added.length}`);return{shield_total:shieldTotal(f.target),shield_layers:effects.map(x=>({id:x.id,remaining:x.remaining,sequence:x.sequence,consume_rule:x.lifecyclePolicy?.consumeRule||null})),shield_added_count:added.length,events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-SHIELD-BOUNDARY-FIFO','複数Generic SHIELDはFIFOで消費',()=>{const f=prep(),errors=[];executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'});executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'});const before=ensureShieldEffects(f.target).map(x=>x.id),damage=applyTaggedDamage(f.enemy,f.target,120,{id:'R04-DEVICE-DAMAGE-120',name:'R04 Device Damage 120',parameters:{damage:120}}),absorb=battle.validationEvents.find(x=>x.type==='shield_absorbed'&&x.skill_id==='R04-DEVICE-DAMAGE-120'),after=ensureShieldEffects(f.target);const consumed=absorb?.consumed||[];if(damage.shieldAbsorbed!==120)errors.push(`absorbed=${damage.shieldAbsorbed}`);if(consumed.length!==2)errors.push(`consumed layers=${consumed.length}`);if(consumed[0]?.shield_id!==before[0]||consumed[0]?.absorbed!==100||consumed[0]?.remaining!==0)errors.push('FIFO first layer mismatch');if(consumed[1]?.shield_id!==before[1]||consumed[1]?.absorbed!==20||consumed[1]?.remaining!==80)errors.push('FIFO second layer mismatch');if(after.length!==1||after[0]?.id!==before[1]||after[0]?.remaining!==80)errors.push('FIFO final layer mismatch');return{layer_ids_before:before,damage_result:damage,consumed,shield_total_after:shieldTotal(f.target),shield_layers_after:after.map(x=>({id:x.id,remaining:x.remaining})),events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-SHIELD-BOUNDARY-DEATH-CLEANUP','戦闘不能時にGeneric SHIELDをcleanup',()=>{const f=prep(),errors=[];executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'});const before=shieldTotal(f.target),resetResult=resetCombatantOnDeath(f.target,{reason:'device_validation',sourceId:f.enemy.id}),death=battle.validationEvents.filter(x=>x.type==='generic_apply_lifecycle_death_cleanup'&&x.target_id===f.target.id),reset=battle.validationEvents.filter(x=>x.type==='unit_death_reset'&&x.target_id===f.target.id);if(before!==100)errors.push(`shield before=${before}`);if(!resetResult.ok)errors.push(`reset failed=${resetResult.reason||'unknown'}`);if(f.target.alive||f.target.hp!==0)errors.push(`target alive=${f.target.alive} hp=${f.target.hp}`);if(shieldTotal(f.target)!==0||ensureShieldEffects(f.target).length!==0)errors.push(`shield remains=${shieldTotal(f.target)}`);if(death.length!==1)errors.push(`death cleanup=${death.length}`);if(death[0]?.cleared?.shields!==1)errors.push(`cleared shields=${death[0]?.cleared?.shields}`);if(reset.length!==1)errors.push(`unit_death_reset=${reset.length}`);return{shield_total_before:before,reset_result:resetResult,target_alive:f.target.alive,target_hp:f.target.hp,shield_total_after:shieldTotal(f.target),death_cleanup_count:death.length,death_cleanup:death[0]||null,events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-SHIELD-BOUNDARY-BATTLE-END','戦闘終了時にGeneric SHIELDをcleanup',()=>{const f=prep(),errors=[];executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'});for(const u of battle.units){if(u.side==='敵'){u.hp=0;u.alive=false}}const ended=finishIfNeeded(),clear=battle.validationEvents.filter(x=>x.type==='shield_cleared'&&x.target_id===f.target.id&&x.reason==='battle_end'),lifecycle=battle.validationEvents.filter(x=>x.type==='generic_apply_lifecycle_cleanup'&&x.reason==='battle_end');if(!ended)errors.push('finishIfNeeded=false');if(!battle.pendingResult&&!battle.result)errors.push('戦闘終了結果がありません');if(shieldTotal(f.target)!==0||ensureShieldEffects(f.target).length!==0)errors.push(`shield remains=${shieldTotal(f.target)}`);if(clear.length!==1)errors.push(`shield_cleared=${clear.length}`);if(lifecycle.length!==1)errors.push(`generic_apply_lifecycle_cleanup=${lifecycle.length}`);return{finish_if_needed:ended,pending_result:battle.pendingResult,result:battle.result,shield_total_after:shieldTotal(f.target),shield_cleared_count:clear.length,lifecycle_cleanup_count:lifecycle.length,events:[...battle.validationEvents],errors}});
 }catch(e){cases.push({id:'R04-GENERIC-SHIELD-BOUNDARY-SETUP',label:'Generic SHIELD境界fixture準備',passed:false,errors:[String(e?.message||e)]})}
 finally{battle.validationCaptureEvents=false;if(fixtureIndex>=0&&TAG_SKILLS[fixtureIndex]?.id===fixture?.id)TAG_SKILLS.splice(fixtureIndex,1)}
 const errors=cases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:gameBuild,generated_at:new Date().toISOString(),test:{id:'R04-GENERIC-SHIELD-BOUNDARY-DEVICE-001',mode:'generic_production_runtime_boundary_device_validation',entrypoint,trigger:'tagTestRunR04GenericShieldBoundaryRuntimeJson'},pipeline:['generic_skill','generic_skill_compiler','legacy_adapter','compileTaggedSkill','Game executeTaggedSkill','generic APPLY lifecycle','SHIELD runtime'],boundary_scope:['STACK','FIFO','DEATH_CLEANUP','BATTLE_END_CLEANUP'],cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
async function tagTestRunR04GenericShieldBoundaryRuntimeJson(){
 pauseBattle();if(studioSkillBridge.status!=='loaded')await loadStudioSkillDefinitions();const report=await runR04GenericShieldBoundaryDeviceValidation(),errors=report.summary.errors||[],build=report.build||'UNKNOWN';
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`r04-generic-shield-boundary-runtime-device-validation-${build}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[R04 GENERIC SHIELD BOUNDARY] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;renderBattle();return report;
}
async function runR04GenericFollowUpDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint='game/index.html',cases=[];
 const add=(id,label,run)=>{try{const detail=run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[String(e?.message||e)]})}};
 const makeGeneric=(id,priority)=>({schemaVersion:1,id,name:`R04 Generic Follow Up Device ${priority}`,trigger:{type:'ON_ALLY_ATTACK',scope:'SELF',priority},conditions:[{property:'TARGET_POISONED',scope:'TARGET',operator:'=',value:true}],target:{side:'ENEMY',range:'SINGLE'},effects:[{type:'DAMAGE',power:80,damageType:'PHYSICAL'}],resource:{mpCost:0,cooldown:0}});
 const genericSkill=makeGeneric('R04-DEVICE-GENERIC-FOLLOW-UP',7),genericHigh=makeGeneric('R04-DEVICE-GENERIC-FOLLOW-UP-HIGH',20);
 const fixtures=[];
 try{
  if(!window.GKSGenericSkillBridge?.compileForLegacy)throw new Error('GKSGenericSkillBridge.compileForLegacy がありません');
  for(const generic of [genericSkill,genericHigh]){
   const generated=await window.GKSGenericSkillBridge.compileForLegacy(generic,{legacyCompile:compileTaggedSkill});
   if(!generated?.ok)throw new Error(`Generic compile failed ${generic.id}: ${(generated?.errors||[]).map(x=>x.code||x.message||String(x)).join(', ')}`);
   const fixture={...generated.legacySkill,source:'device_fixture',environment:'validation'};
   if(findTagSkill(fixture.id))throw new Error(`fixture id collision: ${fixture.id}`);
   TAG_SKILLS.push(fixture);fixtures.push(fixture);
  }
  const fixture=fixtures[0],highFixture=fixtures[1],compiled=compileTaggedSkill(fixture),triggerContract=compiled.definition?.genericRuntime?.triggerContract||null,conditionContract=compiled.definition?.genericRuntime?.conditionContracts?.find(x=>x?.property==='TARGET_POISONED')||null;
  add('R04-GENERIC-FOLLOW-UP-CONTRACT','Generic→Legacy Adapter→FOLLOW_UP Trigger/Condition契約',()=>{const errors=[];if(!compiled.ok)errors.push(`legacy compile failed: ${(compiled.errors||[]).join(', ')}`);if(triggerContract?.type!=='ON_ALLY_ATTACK')errors.push(`trigger type=${triggerContract?.type||null}`);if(triggerContract?.engineEvent!=='ally_attack')errors.push(`engine event=${triggerContract?.engineEvent||null}`);if(triggerContract?.dispatchMode!=='LEGACY_FOLLOW_UP_ADAPTER')errors.push(`dispatch mode=${triggerContract?.dispatchMode||null}`);if(triggerContract?.priority!==7)errors.push(`priority=${triggerContract?.priority}`);if(conditionContract?.enginePredicate!=='target_poisoned'||conditionContract?.expected!==true)errors.push('TARGET_POISONED condition contract mismatch');if(!compiled.definition?.logicOrder?.includes('FOLLOW_UP'))errors.push('FOLLOW_UP logic missing');return{generic_skill:genericSkill,legacy_skill:fixture,trigger_contract:triggerContract,condition_contract:conditionContract,errors}});
  const incoming=findTagSkill('SKL-TEST-ATTACK'),poison=findTagSkill('SKL-TEST-POISON');if(!incoming||!poison)throw new Error('Studio正式ATTACK/POISON fixtureがありません');
  const poisonCompiled=compileTaggedSkill(poison);if(!poisonCompiled.ok)throw new Error(`POISON compile failed: ${(poisonCompiled.errors||[]).join(', ')}`);
  const prep=()=>{pauseBattle();resetBattle();battle.validationMode=true;battle.validationCaptureEvents=true;battle.validationEvents=[];const allies=ensureValidationTargets('味方',3),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.reservedAction=null;u.counterSkillId=null;u.counterDisabled=false;u.followUpSkillIds=[];u.shieldEffects=[];u.statusEffects=[];u.dotStacks=[];u.modifierStacks=[]}const target=enemies[0];target.maxHp=Math.max(target.maxHp,5000);target.hp=target.maxHp;return{initiator:allies[0],follower:allies[1],follower2:allies[2],target}};
  add('R04-GENERIC-FOLLOW-UP-RUNTIME-POISONED','毒状態の対象へGame本番RuntimeでGeneric FOLLOW_UP発動',()=>{const f=prep(),errors=[];f.follower.followUpSkillIds=[fixture.id];applyTaggedDot(f.initiator,f.target,poisonCompiled);battle.validationEvents=[];const hp=f.target.hp,result=executeTaggedSkill(f.initiator,f.target,incoming,{origin:'base'}),generic=battle.validationEvents.filter(x=>x.type==='generic_trigger_dispatched'&&x.skill_id===fixture.id),condition=battle.validationEvents.filter(x=>x.type==='generic_condition_resolved'&&x.skill_id===fixture.id),triggered=battle.validationEvents.filter(x=>x.type==='follow_up_triggered'&&x.skill_id===fixture.id),followDamage=battle.validationEvents.filter(x=>x.type==='attack'&&x.skill_id===fixture.id),chain=battle.validationEvents.filter(x=>x.type==='follow_up_chain_blocked'&&x.skill_id===fixture.id);if(!result.ok)errors.push(`base attack failed: ${result.reason||result.stage||'unknown'}`);if(generic.length!==1)errors.push(`generic_trigger_dispatched=${generic.length}`);if(condition.length!==1)errors.push(`generic_condition_resolved=${condition.length}`);if(triggered.length!==1)errors.push(`follow_up_triggered=${triggered.length}`);if(followDamage.length!==1)errors.push(`follow_up attack=${followDamage.length}`);if(chain.length!==1)errors.push(`follow_up_chain_blocked=${chain.length}`);if(f.target.hp>=hp)errors.push('対象HPが減少していません');return{target_hp_before:hp,target_hp_after:f.target.hp,generic_trigger_count:generic.length,generic_condition_count:condition.length,follow_up_trigger_count:triggered.length,follow_up_attack_count:followDamage.length,chain_block_count:chain.length,events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-FOLLOW-UP-RUNTIME-CONDITION-FALSE','毒なしではGeneric FOLLOW_UPをCondition Engineが抑止',()=>{const f=prep(),errors=[];f.follower.followUpSkillIds=[fixture.id];const result=executeTaggedSkill(f.initiator,f.target,incoming,{origin:'base'}),generic=battle.validationEvents.filter(x=>x.type==='generic_trigger_dispatched'&&x.skill_id===fixture.id),skipped=battle.validationEvents.filter(x=>x.type==='follow_up_skipped'&&x.skill_id===fixture.id&&x.reason==='CONDITION_POISONED_FALSE'),triggered=battle.validationEvents.filter(x=>x.type==='follow_up_triggered'&&x.skill_id===fixture.id),followDamage=battle.validationEvents.filter(x=>x.type==='attack'&&x.skill_id===fixture.id);if(!result.ok)errors.push(`base attack failed: ${result.reason||result.stage||'unknown'}`);if(generic.length!==1)errors.push(`generic_trigger_dispatched=${generic.length}`);if(skipped.length!==1)errors.push(`CONDITION_POISONED_FALSE=${skipped.length}`);if(triggered.length)errors.push(`follow_up_triggered=${triggered.length}`);if(followDamage.length)errors.push(`follow_up attack=${followDamage.length}`);return{generic_trigger_count:generic.length,condition_skip_count:skipped.length,follow_up_trigger_count:triggered.length,follow_up_attack_count:followDamage.length,events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-FOLLOW-UP-RUNTIME-PRIORITY','複数Generic FOLLOW_UPはpriority降順で本番実行',()=>{const f=prep(),errors=[];f.follower.followUpSkillIds=[fixture.id];f.follower2.followUpSkillIds=[highFixture.id];applyTaggedDot(f.initiator,f.target,poisonCompiled);battle.validationEvents=[];executeTaggedSkill(f.initiator,f.target,incoming,{origin:'base'});const order=battle.validationEvents.find(x=>x.type==='follow_up_order_fixed'),triggered=battle.validationEvents.filter(x=>x.type==='follow_up_triggered'&&(x.skill_id===fixture.id||x.skill_id===highFixture.id)),ids=order?.order?.map(x=>x.skill_id)||[];if(ids.length<2)errors.push(`order size=${ids.length}`);if(ids[0]!==highFixture.id||ids[1]!==fixture.id)errors.push(`priority order=${ids.join(' > ')}`);if(triggered.length!==2)errors.push(`follow_up_triggered=${triggered.length}`);if(triggered[0]?.skill_id!==highFixture.id||triggered[1]?.skill_id!==fixture.id)errors.push(`trigger order=${triggered.map(x=>x.skill_id).join(' > ')}`);return{order:order?.order||[],triggered:triggered.map(x=>({source_id:x.source_id,skill_id:x.skill_id,priority:x.priority})),events:[...battle.validationEvents],errors}});
 }catch(e){cases.push({id:'R04-GENERIC-FOLLOW-UP-SETUP',label:'Generic FOLLOW_UP実機fixture準備',passed:false,errors:[String(e?.message||e)]})}
 finally{battle.validationCaptureEvents=false;for(let i=fixtures.length-1;i>=0;i--){const idx=TAG_SKILLS.findIndex(x=>x.id===fixtures[i].id);if(idx>=0)TAG_SKILLS.splice(idx,1)}}
 const errors=cases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:gameBuild,generated_at:new Date().toISOString(),test:{id:'R04-GENERIC-FOLLOW-UP-DEVICE-001',mode:'generic_production_runtime_device_validation',entrypoint,trigger:'tagTestRunR04GenericFollowUpRuntimeJson'},pipeline:['generic_skill','generic_skill_compiler','legacy_adapter','compileTaggedSkill','Game executeTaggedSkill','GKSTriggerEngine','GKSConditionEngine','FOLLOW_UP runtime'],cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
async function tagTestRunR04GenericFollowUpRuntimeJson(){
 pauseBattle();if(studioSkillBridge.status!=='loaded')await loadStudioSkillDefinitions();const report=await runR04GenericFollowUpDeviceValidation(),errors=report.summary.errors||[],build=report.build||'UNKNOWN';
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`r04-generic-follow-up-runtime-device-validation-${build}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[R04 GENERIC FOLLOW UP RUNTIME] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;renderBattle();return report;
}

async function runR04GenericAuraDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint='game/index.html',cases=[];
 const add=(id,label,run)=>{try{const detail=run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[String(e?.message||e)]})}};
 const genericSkill={schemaVersion:1,id:'R04-DEVICE-GENERIC-AURA',name:'R04 Generic Aura Device',trigger:{type:'WHILE_SOURCE_ALIVE',scope:'SELF',priority:7},conditions:[],target:{side:'ALLY',range:'ALL'},effects:[{type:'APPLY',effectId:'ATK_UP',power:18}],resource:{mpCost:0,cooldown:0}};
 let generated=null,fixture=null,fixtureIndex=-1;
 try{
  if(!window.GKSGenericSkillBridge?.compileForLegacy)throw new Error('GKSGenericSkillBridge.compileForLegacy がありません');
  generated=await window.GKSGenericSkillBridge.compileForLegacy(genericSkill,{legacyCompile:compileTaggedSkill});
  ...
  if(findTagSkill(fixture.id))throw new Error(`fixture id collision: ${fixture.id}`);
  TAG_SKILLS.push(fixture);fixtureIndex=TAG_SKILLS.length-1;
  const compiled=compileTaggedSkill(fixture),contract=compiled.definition?.genericRuntime?.triggerContract||null,aura=compiled.definition?.genericRuntime?.auraEffectContract||null;
  add('R04-GENERIC-AURA-CONTRACT','Generic→Legacy Adapter→AURA Trigger/Effect契約',()=>{const errors=[];if(!compiled.ok)errors.push(`legacy compile failed: ${(compiled.errors||[]).join(', ')}`);if(contract?.type!=='WHILE_SOURCE_ALIVE')errors.push(`trigger type=${contract?.type||null}`);if(contract?.engineEvent!=='aura_evaluate')errors.push(`engine event=${contract?.engineEvent||null}`);if(contract?.dispatchMode!=='LEGACY_AURA_ADAPTER')errors.push(`dispatch mode=${contract?.dispatchMode||null}`);if(contract?.priority!==7)errors.push(`priority=${contract?.priority}`);if(aura?.kind!=='BUFF'||aura?.modifierStat!=='ATK'||aura?.power!==18)errors.push('aura effect contract mismatch');if(aura?.sourceDependent!==true)errors.push('sourceDependent=false');if(!compiled.definition?.logicOrder?.includes('AURA'))errors.push('AURA logic missing');return{generic_skill:genericSkill,legacy_skill:fixture,trigger_contract:contract,aura_effect_contract:aura,errors}});
  const prep=()=>{pauseBattle();resetBattle();battle.validationMode=true;battle.validationCaptureEvents=true;battle.validationEvents=[];const allies=ensureValidationTargets('味方',2),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.reservedAction=null;u.auraSkillIds=[];u.modifierStacks=[];u.statusEffects=[];u.dotStacks=[];u.shieldEffects=[]}return{source:allies[0],target:allies[1],enemy:enemies[0]}};
  add('R04-GENERIC-AURA-RUNTIME-SOURCE-ALIVE','発生源生存中にGame本番RuntimeでGeneric AURA適用',()=>{const f=prep(),errors=[];f.source.auraSkillIds=[fixture.id];const entries=activeAuraEntries(f.target,'BUFF','ATK'),winner=resolveEffectiveAuraEntry(entries),power=effectiveAuraPower(f.target,'BUFF','ATK');if(entries.length!==1)errors.push(`active aura entries=${entries.length}`);if(power!==18)errors.push(`effective power=${power}`);if(winner?.skillId!==fixture.id)errors.push(`winner=${winner?.skillId||null}`);if(winner?.genericTrigger!==true)errors.push('genericTrigger marker missing');if(effectiveAuraPower(f.enemy,'BUFF','ATK')!==0)errors.push('enemy received ally aura');return{source:{id:f.source.id,alive:f.source.alive,auraSkillIds:[...f.source.auraSkillIds]},target:{id:f.target.id,alive:f.target.alive,effective_atk_aura:power},enemy_effective_atk_aura:effectiveAuraPower(f.enemy,'BUFF','ATK'),active_auras:entries,winner,errors}});
 }catch(e){cases.push({id:'R04-GENERIC-AURA-SETUP',label:'Generic AURA実機fixture準備',passed:false,errors:[String(e?.message||e)]})}
 finally{battle.validationCaptureEvents=false;if(fixtureIndex>=0&&TAG_SKILLS[fixtureIndex]?.id===fixture?.id)TAG_SKILLS.splice(fixtureIndex,1)}
 const errors=cases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:gameBuild,generated_at:new Date().toISOString(),test:{id:'R04-GENERIC-AURA-DEVICE-001',mode:'generic_production_runtime_device_validation',entrypoint,trigger:'tagTestRunR04GenericAuraRuntimeJson'},pipeline:['generic_skill','generic_skill_compiler','legacy_adapter','compileTaggedSkill','Game activeAuraEntries/effectiveAuraPower','GKSTriggerEngine','AURA runtime'],scope:['CONTRACT','SOURCE_ALIVE_APPLY'],cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
async function tagTestRunR04GenericAuraRuntimeJson(){
 pauseBattle();if(studioSkillBridge.status!=='loaded')await loadStudioSkillDefinitions();const report=await runR04GenericAuraDeviceValidation(),errors=report.summary.errors||[],build=report.build||'UNKNOWN';
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`r04-generic-aura-runtime-device-validation-${build}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[R04 GENERIC AURA RUNTIME E1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;renderBattle();return report;
}

async function runR04GenericHealDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint='game/index.html',cases=[];
 const add=(id,label,run)=>{try{const detail=run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[String(e?.message||e)]})}};
 const genericSkill={schemaVersion:1,id:'R04-DEVICE-GENERIC-HEAL',name:'R04 Generic Heal Device',trigger:{type:'ON_USE',scope:'SELF'},target:{side:'ALLY',range:'SINGLE'},effects:[{type:'HEAL',power:120}],resource:{mpCost:0,cooldown:0}};
 let generated=null,fixture=null,fixtureIndex=-1;
 try{
  if(!window.GKSGenericSkillBridge?.compileForLegacy)throw new Error('GKSGenericSkillBridge.compileForLegacy がありません');
  generated=await window.GKSGenericSkillBridge.compileForLegacy(genericSkill,{legacyCompile:compileTaggedSkill});
  ...
  TAG_SKILLS.push(fixture);fixtureIndex=TAG_SKILLS.length-1;
  const compiled=compileTaggedSkill(fixture),contract=compiled.definition?.genericRuntime?.triggerContract||null;
  add('R04-GENERIC-HEAL-CONTRACT','Generic→Legacy Adapter→ON_USE HEAL契約',()=>{const errors=[];if(!compiled.ok)errors.push(`legacy compile failed: ${(compiled.errors||[]).join(', ')}`);if(contract?.type!=='ON_USE')errors.push(`trigger type=${contract?.type||null}`);if(contract?.engineEvent!=='use')errors.push(`engine event=${contract?.engineEvent||null}`);if(contract?.dispatchMode!=='RESOLVE_ONLY')errors.push(`dispatch mode=${contract?.dispatchMode||null}`);if(compiled.definition?.parameters?.heal!==120)errors.push(`heal=${compiled.definition?.parameters?.heal}`);return{generic_skill:genericSkill,legacy_skill:fixture,trigger_contract:contract,errors}});
  const prep=()=>{pauseBattle();resetBattle();battle.validationMode=true;battle.validationCaptureEvents=true;battle.validationEvents=[];const allies=ensureValidationTargets('味方',2),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.reservedAction=null;u.statusEffects=[];u.dotStacks=[];u.shieldEffects=[]}return{actor:allies[0],target:allies[1],enemy:enemies[0]}};
  add('R04-GENERIC-HEAL-RUNTIME-BASIC','Game本番RuntimeでGeneric HEAL発動',()=>{const f=prep(),errors=[];f.target.hp=Math.max(1,f.target.maxHp-200);const before=f.target.hp,result=executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'}),events=battle.validationEvents.filter(x=>x.type==='heal'&&x.skill_id===fixture.id);if(!result.ok)errors.push(`execute failed: ${result.reason||result.stage||'unknown'}`);if(result.healResult?.healed!==120)errors.push(`healed=${result.healResult?.healed}`);if(f.target.hp!==before+120)errors.push(`hp ${before}->${f.target.hp}`);if(events.length!==1)errors.push(`heal events=${events.length}`);return{target_hp_before:before,target_hp_after:f.target.hp,heal_result:result.healResult,heal_event_count:events.length,events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-HEAL-RUNTIME-CLAMP','最大HPを超えて回復しない',()=>{const f=prep(),errors=[];f.target.hp=f.target.maxHp-30;const before=f.target.hp,result=executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'});if(!result.ok)errors.push(`execute failed: ${result.reason||result.stage||'unknown'}`);if(f.target.hp!==f.target.maxHp)errors.push(`hp=${f.target.hp}/${f.target.maxHp}`);if(result.healResult?.healed!==30)errors.push(`healed=${result.healResult?.healed}`);if(result.healResult?.overheal!==90)errors.push(`overheal=${result.healResult?.overheal}`);return{target_hp_before:before,target_hp_after:f.target.hp,max_hp:f.target.maxHp,heal_result:result.healResult,events:[...battle.validationEvents],errors}});
  add('R04-GENERIC-HEAL-RUNTIME-DEAD-REJECT','戦闘不能対象を回復対象にしない',()=>{const f=prep(),errors=[];f.target.hp=0;f.target.alive=false;const result=executeTaggedSkill(f.actor,f.target,fixture,{origin:'base'}),healEvents=battle.validationEvents.filter(x=>x.type==='heal'&&x.skill_id===fixture.id);if(result.ok)errors.push('戦闘不能対象へのHEALが成功');if(result.stage!=='target')errors.push(`stage=${result.stage||null}`);if(f.target.hp!==0)errors.push(`dead hp=${f.target.hp}`);if(healEvents.length)errors.push(`heal events=${healEvents.length}`);return{result:{ok:result.ok,stage:result.stage||null,reason:result.reason||null},target_hp:f.target.hp,target_alive:f.target.alive,heal_event_count:healEvents.length,events:[...battle.validationEvents],errors}});
 }catch(e){cases.push({id:'R04-GENERIC-HEAL-SETUP',label:'Generic HEAL実機fixture準備',passed:false,errors:[String(e?.message||e)]})}
 finally{battle.validationCaptureEvents=false;if(fixtureIndex>=0&&TAG_SKILLS[fixtureIndex]?.id===fixture?.id)TAG_SKILLS.splice(fixtureIndex,1)}
 const errors=cases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:gameBuild,generated_at:new Date().toISOString(),test:{id:'R04-GENERIC-HEAL-DEVICE-001',mode:'generic_production_runtime_device_validation',entrypoint,trigger:'tagTestRunR04GenericHealRuntimeJson'},pipeline:['generic_skill','generic_skill_compiler','legacy_adapter','compileTaggedSkill','Game executeTaggedSkill','HEAL runtime'],cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
async function tagTestRunR04GenericHealRuntimeJson(){
 pauseBattle();if(studioSkillBridge.status!=='loaded')await loadStudioSkillDefinitions();const report=await runR04GenericHealDeviceValidation(),errors=report.summary.errors||[],build=report.build||'UNKNOWN';
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`r04-generic-heal-runtime-device-validation-${build}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[R04 GENERIC HEAL RUNTIME] ${report.summary.passed?'PASS':'FAIL'}
[CASES] ${report.summary.passed_count}/${report.summary.case_count}
[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;renderBattle();return report;
}
