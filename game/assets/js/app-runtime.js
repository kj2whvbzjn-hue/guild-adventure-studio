...
function normalizeSkillLoadoutSettings(value){const source=objectValue(value,'game_runtime.skill_loadout');return{active_skill_slots:finiteNumber(source.active_skill_slots,'game_runtime.skill_loadout.active_skill_slots',{integer:true,min:0}),passive_slots:finiteNumber(source.passive_slots,'game_runtime.skill_loadout.passive_slots',{integer:true,min:0})}}
...
function normalizeFormalRuntimeSettingsRecord(record){
 ...
 const skillProgression=runtime.skill_progression==null?null:normalizeSkillProgressionSettings(runtime.skill_progression),skillLoadout=normalizeSkillLoadoutSettings(runtime.skill_loadout);
...
function skillLoadoutBalance(){const value=requireFormalRuntimeSettings().skill_loadout;if(!value)throw new Error('Skill Loadout BalanceがGame Dataに設定されていません。');return value}
...
function rollGrowth(v){const rows=growthRowsForAptitude(v),total=rows.reduce((a,x)=>a+x.weight,0);let roll=Math.random()*total;for(const row of rows){roll-=row.weight;if(roll<0)return row.amount}return rows[rows.length-1].amount}
...
function characterFormalPassiveSelection(character){if(!window.GKSFormalContribution)throw new Error('Formal Contribution Resolverを読み込めません。');const selected=Array.isArray(character?.passiveLoadoutIds)?character.passiveLoadoutIds:[];return window.GKSFormalContribution.validatePassiveSelection([...formalPassiveCatalog.values()],selected,characterAbilityStats(character),{requireRuntimeContracts:true})}
...
function normalizeCharacterEquipmentState(character){
 ...
 if(character.passiveIds!=null&&(!Array.isArray(character.passiveIds)||character.passiveIds.some(id=>!String(id||'').trim())))throw new Error(`${character.name||character.id||'キャラクター'}のpassiveIdsが不正です。`);if(window.GKGameSkillLoadout)GKGameSkillLoadout.loadoutState(character,skillLoadoutBalance());characterFormalPassiveSelection(character);
...
function initializeCharacterSkillPassiveAiState(character){const cfg=requireFormalRuntimeSettings().character,skills=[...cfg.starter_skill_ids];character.skills=skills;character.equippedSkillId=cfg.starter_equipped_skill_id||skills[0]||'';character.skillPoints=cfg.initial_skill_points;character.skillPointSpend={skill:{},passive:{}};character.passiveIds=[...cfg.starter_passive_ids];character.skillLoadoutIds=[];character.passiveLoadoutIds=[];character.formalAiBinding=null;return window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeCharacterSkillState(character,{fallback:[]}):character}
...
function normalize(raw){
 ...
 raw.characters.forEach(c=>{
  ...
  if(window.GKGameSkillLoadout)GKGameSkillLoadout.loadoutState(c,skillLoadoutBalance());
...
function characterAiBindingRows(){return window.GKGameAISaveBridge?.availableCharacterBindings?GKGameAISaveBridge.availableCharacterBindings(data,currentFormalAiCatalog()):[];}
function characterAiBindingLabel(c){if(!window.GKGameAISaveBridge)return'AI機能未読込';const state=GKGameAISaveBridge.characterBindingAuthority(data,currentFormalAiCatalog(),c?.id);if(state.status==='unbound')return'AI未設定';if(state.status==='ambiguous')return'AI ID衝突';if(state.status==='unresolved')return'AI参照切れ';return`${state.source==='developer'?'開発者作成':'プレイヤー作成'} / ${state.program?.name||state.binding?.program_id||''}`;}
function renderCharacterAiBindingPanel(c){
 const host=$('characterAiBindingPanel');if(!host)return;if(!c){host.innerHTML='';return;}
 const catalog=currentFormalAiCatalog(),state=window.GKGameAISaveBridge?GKGameAISaveBridge.characterBindingAuthority(data,catalog,c.id):{status:'unresolved'},rows=characterAiBindingRows();
 const current=state.binding?`${state.binding.program_id}|${state.binding.layout_id}`:'';
 const options=rows.map(row=>{const key=`${row.binding.program_id}|${row.binding.layout_id}`,kind=row.source==='developer'?'開発者作成':'プレイヤー作成',name=row.program?.name||row.binding.program_id;return`<option value="${escapeHtml(key)}" ${current===key?'selected':''} ${row.ambiguous?'disabled':''}>${escapeHtml(`${kind}: ${name}${row.ambiguous?'（ID衝突）':''}`)}</option>`;}).join('');
 host.innerHTML=`<h3>AIプログラム装着</h3><div class="small">プレイヤー作成AIはGame Saveが正本です。開発者作成AIはStudio Formal Exportを利用し、GameではProgram graphを編集しません。</div><div class="row"><label>装着Program<select id="characterAiBindingSelect"><option value="">未装着</option>${options}</select></label><button id="characterAiBindingApply" type="button">装着/変更</button><button id="characterAiBindingClear" type="button" ${state.status==='unbound'?'disabled':''}>解除</button></div><div class="small">現在: ${escapeHtml(characterAiBindingLabel(c))}</div>`;
 const select=$('characterAiBindingSelect'),apply=$('characterAiBindingApply'),clear=$('characterAiBindingClear');
 if(apply)apply.onclick=()=>{try{const raw=String(select?.value||'');if(!raw){const result=GKGameAISaveBridge.clearCharacterBinding(data,c.id);data=result.save;autoSave();render();notify(`${c.name}のAI装着を解除しました。`,'ok');return;}const [program_id,layout_id]=raw.split('|'),result=GKGameAISaveBridge.equipCharacterBinding(data,c.id,{program_id,layout_id},catalog);data=result.save;autoSave();render();notify(`${c.name}へ${result.source==='developer'?'開発者作成':'プレイヤー作成'}AIを装着しました。`,'ok');}catch(error){notify(String(error?.message||error),'bad');}};
 if(clear)clear.onclick=()=>{try{const result=GKGameAISaveBridge.clearCharacterBinding(data,c.id);data=result.save;autoSave();render();notify(`${c.name}のAI装着を解除しました。`,'ok');}catch(error){notify(String(error?.message||error),'bad');}};
}
function render(){
 ...
 $('detail').innerHTML=`<div class="row"><div><div class="name">${escapeHtml(c.name)}</div><span class="tag">Lv ${c.level}${maxLevel?` / ${maxLevel}`:''}</span><span class="tag">現在職：${escapeHtml(jobDisplayName(c.job))}</span></div></div><div class="stats">${STATS.map(s=>`<div class="stat"><span class="small">${s}</span><b>${c.stats[s]}</b></div>`).join('')}</div><h3>Skill Loadout</h3><div class="small">${(c.skillLoadoutIds||[]).map(id=>escapeHtml(findSkill(id)?.name||id)).join(' / ')||'未選択'}</div><h3>Passive選択</h3><div class="small">${(c.passiveLoadoutIds||[]).map(id=>escapeHtml(formalPassiveCatalog.get(id)?.name||id)).join(' / ')||'未選択'}</div><h3>装備</h3><div class="small">${characterEquipmentEntries(c).map(x=>`${x.label}: ${equipmentDisplayName(x.ref)}`).join(' / ')||'なし'}（攻撃 +${characterEquipmentContribution(c).attack} / HP +${characterEquipmentContribution(c).hpBonus} / MP +${characterEquipmentContribution(c).mpBonus}）</div><h3>転職履歴</h3><div class="small">${c.jobHistory.map(h=>`Lv${h.level} ${escapeHtml(jobDisplayName(h.job))}`).join(' → ')}</div><h3>直近の成長</h3><div class="small">${c.growthHistory.slice(-5).reverse().map(g=>`Lv${g.toLevel}: ${g.gained.length?g.gained.join(', '):'能力上昇なし'}`).join('<br>')||'まだレベルアップしていません。'}</div>`;
 renderCharacterAiBindingPanel(c);
...
function renderCharacterSkillView(){
 ...
 let loadout=null,loadoutError='';try{loadout=window.GKGameSkillLoadout?GKGameSkillLoadout.loadoutState(c,skillLoadoutBalance()):null}catch(error){loadoutError=String(error?.message||error)}
 const activeCapacity=loadout?.balance?.active_skill_slots??0,passiveCapacity=loadout?.balance?.passive_slots??0,activeIds=new Set(loadout?.activeSkillIds||[]),selectedPassiveIds=new Set(loadout?.passiveIds||[]);
 const activeNames=[...activeIds].map(id=>findSkill(id)?.name||id),passiveNames=[...selectedPassiveIds].map(id=>formalPassiveCatalog.get(id)?.name||id);
 current.innerHTML=`<div class="skill-loadout-current"><b>Active Skill Loadout ${activeIds.size}/${activeCapacity}</b><div class="small">${escapeHtml(activeNames.join(' / ')||'未選択')}</div><b>Passive選択 ${selectedPassiveIds.size}/${passiveCapacity}</b><div class="small">${escapeHtml(passiveNames.join(' / ')||'未選択')}</div>${loadoutError?`<div class="small bad">${escapeHtml(loadoutError)}</div>`:''}</div>`;
 const ownedRows=(c.skills||[]).map(id=>({id,skill:findSkill(id)}));
 list.innerHTML=ownedRows.length?ownedRows.map(row=>{
  const skill=row.skill,compiled=skill?compileSkillForRuntime(skill):null,selected=activeIds.has(row.id),respec=progression&&window.GKGameSkillRespecRuntime?GKGameSkillRespecRuntime.previewIndividual(data,c,'skill',row.id,progression):{ok:false,reason:progressionError||'RESPEC_RUNTIME_UNAVAILABLE'},refund=Number(respec?.skillPointsRefund)||0;
  ...
  const full=!selected&&activeIds.size>=activeCapacity;
  return `<div class="skill-choice ${selected?'selected':''}"><div><b>${escapeHtml(skill?.name||row.id)}</b><span class="tag">${escapeHtml(row.id)}</span><span class="tag">${selected?'Loadout選択中':'所持'}</span><div class="small">${detail}</div>${tags.length?`<div class="skill-tags">${tags.map(tag=>`<span class="tag">${escapeHtml(tag)}</span>`).join('')}</div>`:''}<div class="small">習得条件: ${escapeHtml(skill?skillProgressionRequirementText(skill,'skill'):'正式定義未読込')}</div><div class="small">使用条件: ${escapeHtml(useText)}</div><div class="small">Respec: ${respec.ok?`${respec.cost} G / ${refund} SP返還`:escapeHtml(skillProgressionFailureText(respec))}</div></div><div class="row"><button type="button" class="${selected?'good':'primary'}" data-toggle-active-skill="${escapeHtml(row.id)}" ${!skill||!compiled?.ok||full?'disabled':''}>${selected?'Loadoutから外す':'Loadoutに追加'}</button><button type="button" data-respec-skill="${escapeHtml(row.id)}" ${respec.ok?'':'disabled'}>Respec</button></div></div>`;
 }).join(''):'<div class="skill-empty">所持Skillがありません。</div>';
 list.querySelectorAll('[data-toggle-active-skill]').forEach(btn=>btn.onclick=()=>{let balance;try{balance=skillLoadoutBalance()}catch(error){notify(error.message,'bad');return}const id=btn.dataset.toggleActiveSkill,result=window.GKGameSkillLoadout?GKGameSkillLoadout.toggleActiveSkill(c,id,balance):{ok:false,reason:'LOADOUT_RUNTIME_UNAVAILABLE'};if(!result.ok){notify(result.reason==='LOADOUT_FULL'?'Active Skill枠が上限です。':`Skill Loadoutを変更できません: ${result.reason||'INVALID_SKILL'}`,'bad');return}autoSave();render();notify(`${findSkill(id)?.name||id}をActive Skill Loadout${result.selected?'へ追加':'から解除'}しました。`)});
 ...
 if(passiveList){let effectiveIds=new Set(),passiveStateError='';try{effectiveIds=new Set(characterFormalPassiveSelection(c).activePassives.map(row=>row.id))}catch(error){passiveStateError=String(error?.message||error)}const passives=[...formalPassiveCatalog.values()];passiveList.innerHTML=passives.length?passives.map(passive=>{const owned=(c.passiveIds||[]).includes(passive.id),selected=owned&&selectedPassiveIds.has(passive.id),active=selected&&effectiveIds.has(passive.id),preview=!owned&&progression&&window.GKGameSkillAcquisitionRuntime?GKGameSkillAcquisitionRuntime.previewAcquirePassive(c,passive,progression):{ok:false,reason:owned?'PASSIVE_ALREADY_OWNED':progressionError||'ACQUISITION_RUNTIME_UNAVAILABLE'},cost=progression&&window.GKGameSkillAcquisitionRuntime?GKGameSkillAcquisitionRuntime.acquisitionCost(passive,'passive',progression):null,respec=owned&&progression&&window.GKGameSkillRespecRuntime?GKGameSkillRespecRuntime.previewIndividual(data,c,'passive',passive.id,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval}):{ok:false,reason:'PASSIVE_NOT_OWNED'},full=owned&&!selected&&selectedPassiveIds.size>=passiveCapacity;return `<div class="skill-choice ${selected?'selected':''}"><div><b>${escapeHtml(passive.name)}</b><span class="tag">${escapeHtml(passive.id)}</span><span class="tag">${owned?(selected?(active?'選択中・有効':'選択中・条件未成立'):'所持・未選択'):'未習得'}</span><div class="small">常時効果: ${escapeHtml(passiveEffectText(passive))}</div><div class="small">習得条件: ${escapeHtml(skillProgressionRequirementText(passive,'passive'))}</div><div class="small">必要Skill Point: ${cost??'—'}</div>${owned?`<div class="small">Respec: ${respec.ok?`${respec.cost} G / ${respec.skillPointsRefund} SP返還`:escapeHtml(skillProgressionFailureText(respec))}</div>`:`<div class="small">${preview.ok?'習得可能':escapeHtml(skillProgressionFailureText(preview))}</div>`}</div>${owned?`<div class="row"><button type="button" class="${selected?'good':'primary'}" data-toggle-passive="${escapeHtml(passive.id)}" ${full?'disabled':''}>${selected?'選択解除':'選択する'}</button><button type="button" data-respec-passive="${escapeHtml(passive.id)}" ${respec.ok?'':'disabled'}>Respec</button></div>`:`<button type="button" class="primary" data-acquire-passive="${escapeHtml(passive.id)}" ${preview.ok?'':'disabled'}>習得する</button>`}</div>`}).join(''):`<div class="skill-empty">正式Passive Masterがありません。${passiveStateError?` ${escapeHtml(passiveStateError)}`:''}</div>`;passiveList.querySelectorAll('[data-toggle-passive]').forEach(btn=>btn.onclick=()=>{let balance;try{balance=skillLoadoutBalance()}catch(error){notify(error.message,'bad');return}const id=btn.dataset.togglePassive,result=window.GKGameSkillLoadout?GKGameSkillLoadout.togglePassive(c,id,balance):{ok:false,reason:'LOADOUT_RUNTIME_UNAVAILABLE'};if(!result.ok){notify(result.reason==='LOADOUT_FULL'?'Passive枠が上限です。':`Passive選択を変更できません: ${result.reason||'INVALID_PASSIVE'}`,'bad');return}autoSave();render();notify(`${formalPassiveCatalog.get(id)?.name||id}をPassive選択${result.selected?'へ追加':'から解除'}しました。`)});passiveList.querySelectorAll('[data-acquire-passive]').forEach(btn=>btn.onclick=()=>{if(!progression||!window.GKGameSkillAcquisitionRuntime)return;const passive=formalPassiveCatalog.get(btn.dataset.acquirePassive),result=GKGameSkillAcquisitionRuntime.acquirePassive(c,passive,progression);if(!result.ok){notify(skillProgressionFailureText(result),'bad');return}autoSave();render();notify(`${passive.name}を習得しました。Skill Point -${result.skillPointsSpent}。`,'ok')});passiveList.querySelectorAll('[data-respec-passive]').forEach(btn=>btn.onclick=()=>{if(!progression||!window.GKGameSkillRespecRuntime)return;const id=btn.dataset.respecPassive,preview=GKGameSkillRespecRuntime.previewIndividual(data,c,'passive',id,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval});if(!preview.ok){notify(skillProgressionFailureText(preview),'bad');return}if(!confirm(`PassiveをRespecします。${preview.cost} Gを消費し、${preview.skillPointsRefund} SPを返還します。消費したGoldは戻りません。`))return;const result=GKGameSkillRespecRuntime.respecIndividual(data,c,'passive',id,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval});if(!result.ok){notify(skillProgressionFailureText(result),'bad');return}autoSave();render();notify(`PassiveをRespecしました。Skill Point +${result.skillPointsRefund} / ${result.cost} G消費。`,'ok')})}
...
function characterAiEditorSummary(c){if(!window.GKGameAISaveBridge)return'AI機能未読込';const state=GKGameAISaveBridge.characterBindingAuthority(data,currentFormalAiCatalog(),c?.id);if(state.status!=='resolved')return state.status==='ambiguous'?'AI ID衝突':state.status==='unresolved'?'AI参照切れ':'AI未設定';return `${state.source==='developer'?'開発者作成':'プレイヤー作成'} / ${state.program?.nodes?.length||0}チップ`; }
...
function openAiEditorFor(c){
 ...
 const bindingState=GKGameAISaveBridge.characterBindingAuthority(data,currentFormalAiCatalog(),c.id),saved=bindingState.source==='player'?GKGameAISaveBridge.loadForCharacter(data,c.id):null;
 if(bindingState.source==='developer')notify('開発者作成AIはGameで編集しません。新しいプレイヤー作成AIとして編集を開始します。','warn');
