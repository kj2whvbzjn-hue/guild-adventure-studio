...
function normalizeFormalRuntimeSettingsRecord(record){
 ...
 const starterSkillIds=stringArray(character.starter_skill_ids,'game_runtime.character.starter_skill_ids'),starterEquipped=String(character.starter_equipped_skill_id||'').trim();if(starterEquipped&&!starterSkillIds.includes(starterEquipped))throw new Error('starter_equipped_skill_id は starter_skill_ids に含めてください。');
...
function initializeCharacterSkillPassiveAiState(character){const cfg=requireFormalRuntimeSettings().character,skills=[...cfg.starter_skill_ids];character.skills=skills;character.equippedSkillId=cfg.starter_equipped_skill_id||skills[0]||'';character.skillPoints=Number.isInteger(character.skillPoints)&&character.skillPoints>=0?character.skillPoints:0;character.skillPointSpend={skill:{},passive:{}};character.passiveIds=[];character.formalAiBinding=null;return window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeCharacterSkillState(character,{fallback:[]}):character}
