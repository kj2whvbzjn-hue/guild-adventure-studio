...
const FORMAL_EQUIPMENT_SLOT_MAP=Object.freeze({'鎧':'armor','頭':'head','手':'gloves','手袋':'gloves','足':'feet','アミュレット':'amulet','指輪1':'ring1','指輪2':'ring2','ベルト':'belt'});
const FORMAL_EQUIPMENT_SLOT_LABEL=Object.freeze({weapon:'武器',armor:'鎧',head:'頭',gloves:'手袋',feet:'足',amulet:'アミュレット',ring1:'指輪1',ring2:'指輪2',belt:'ベルト',accessory:'アクセサリ'});
...
function normalizeFormalEquipmentRecord(record){
 ...
 let slot=explicitSlot;if(!slot){if(kind==='weapon'||record.required_str!=null||record.required_dex!=null||record.required_int!=null)slot='weapon';else if(armorSlot)slot=FORMAL_EQUIPMENT_SLOT_MAP[armorSlot]||'';else if(kind==='armor')slot='armor';}
...
function normalizeCharacterEquipmentState(character){
 ...
 const source=character.equipment&&typeof character.equipment==='object'&&!Array.isArray(character.equipment)?character.equipment:{};
 const normalized={...source};
 if(!Object.prototype.hasOwnProperty.call(normalized,'weapon1'))normalized.weapon1=normalized.weapon||null;
 if(!Object.prototype.hasOwnProperty.call(normalized,'weapon2'))normalized.weapon2=null;
 delete normalized.weapon;
 character.equipment=normalized;
 const marker=String(character.equipmentTwoHandedSlot||'').trim();
 if(!['weapon1','weapon2'].includes(marker)||!normalized[marker])character.equipmentTwoHandedSlot=null;
 else{const other=marker==='weapon1'?'weapon2':'weapon1';if(normalized[other])character.equipmentTwoHandedSlot=null;}
 return character;
}
...
function equipmentBonusLabel(equipment){const b=equipment?.bonuses||{},rows=[];if(b.attack)rows.push(`攻撃 +${b.attack}`);if(b.maxHp)rows.push(`HP +${b.maxHp}`);if(b.maxMp)rows.push(`MP +${b.maxMp}`);if(b.agi)rows.push(`AGI ${b.agi>=0?'+':''}${b.agi}`);if(b.accuracy)rows.push(`命中値 +${b.accuracy}（戦闘式未接続）`);if(b.evasion)rows.push(`回避値 +${b.evasion}（戦闘式未接続）`);if(b.magicWeaponBonus)rows.push(`魔法武器値 +${b.magicWeaponBonus}（戦闘式未接続）`);if(b.baseCriticalRate)rows.push(`基礎Crit値 ${Math.round(b.baseCriticalRate*10000)/100}%（戦闘式未接続）`);return rows.length?rows.join(' / '):'数値補正なし'}
...
function weaponSlotLabel(slot){return slot==='weapon1'?'武器枠1':slot==='weapon2'?'武器枠2':slot}
function characterEquipmentEntries(character){normalizeCharacterEquipmentState(character);const two=character.equipmentTwoHandedSlot;return Object.entries(character.equipment||{}).filter(([,ref])=>ref).map(([slot,ref])=>({slot,ref,label:slot===two?`${weaponSlotLabel(slot)}＋${weaponSlotLabel(slot==='weapon1'?'weapon2':'weapon1')}（両手持ち）`:weaponSlotLabel(slot)}));}
function removeEquippedWeaponState(character){normalizeCharacterEquipmentState(character);const returned=[];for(const slot of ['weapon1','weapon2']){const ref=character.equipment[slot];if(ref)returned.push(ref);character.equipment[slot]=null;}character.equipmentTwoHandedSlot=null;return returned;}
function unequipCharacterEquipmentSlot(character,slot){normalizeCharacterEquipmentState(character);if(!character?.equipment)return[];if(['weapon1','weapon2'].includes(slot)&&character.equipmentTwoHandedSlot){return removeEquippedWeaponState(character)}const ref=character.equipment[slot];if(!ref)return[];character.equipment[slot]=null;return[ref];}
function equipmentEquipTargets(character,equipment){
 ...
 if(gate.handsRequired===2)return[{value:`${id}|twohand`,label:`${character.name}（両手持ち / 武器枠1+2）`,disabled:false,gate}];
 return['weapon1','weapon2'].map(slot=>{const current=character.equipmentTwoHandedSlot?character.equipment[character.equipmentTwoHandedSlot]:character.equipment[slot];return{value:`${id}|${slot}`,label:`${character.name} / ${weaponSlotLabel(slot)}${current?`（入替: ${equipmentDisplayName(current)}）`:''}`,disabled:false,gate}});
}
function equipInventoryItemToCharacter(character,index,targetSlot=''){
 ...
 if(equipment.slot==='weapon'){
  if(gate.handsRequired===2){returned.push(...removeEquippedWeaponState(character));character.equipment.weapon1=ref;character.equipment.weapon2=null;character.equipmentTwoHandedSlot='weapon1';targetSlot='weapon1';}
  else{
   const slot=['weapon1','weapon2'].includes(targetSlot)?targetSlot:'weapon1';
   if(character.equipmentTwoHandedSlot)returned.push(...removeEquippedWeaponState(character));
   const previous=character.equipment[slot];if(previous)returned.push(previous);character.equipment[slot]=ref;character.equipmentTwoHandedSlot=null;targetSlot=slot;
  }
 }else{
  const slot=equipment.slot,previous=character.equipment[slot]||null;if(previous)returned.push(previous);character.equipment[slot]=ref;targetSlot=slot;
 }
...
window.GKGameEquipmentRuntime=Object.freeze({bridge:formalEquipmentBridge,normalizeRecord:normalizeFormalEquipmentRecord,definition:equipmentDefinition,requirementCheck:equipmentRequirementCheck,bonusLabel:equipmentBonusLabel,requirementLabel:equipmentRequirementLabel,normalizeCharacterState:normalizeCharacterEquipmentState,equipTargets:equipmentEquipTargets,load:loadFormalEquipmentDefinitions});
let data={saveVersion:SAVE_VERSION,schemaRevision:'1.6.0',gameVersion:'GA-B486.207',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],aiPrograms:[],aiLayouts:[],aiPresets:[],partyIds:[],selectedQuestId:'',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},flags:{},quest_progress:{completed_quest_ids:[],unlocked_quest_ids:[]},quest_resources:{},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20,stone_selection_by_quest:{}}};let selectedId=null;
...
if(formalGameBridge.status==='loaded'){try{data=normalize(data);persist()}catch(error){formalGameBridge.status='failed';formalGameBridge.errors.push(String(error?.message||error));notify(`Formal Game Runtime整合エラー: ${error.message}`,'bad')}}if(typeof render==='function')render();
...
function normalize(raw){
 ...
 raw.schemaRevision='1.6.0';raw.gameVersion='GA-B486.207';
 return raw;
}
function persist(){
...
$('clearBtn').onclick=()=>{if(!confirm('正式版Phase Aの全データを初期化しますか？'))return;data={saveVersion:SAVE_VERSION,schemaRevision:'1.6.0',gameVersion:'GA-B486.207',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],aiPrograms:[],aiLayouts:[],aiPresets:[],partyIds:[],selectedQuestId:'',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},flags:{},quest_progress:{completed_quest_ids:[],unlocked_quest_ids:[]},quest_resources:{},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20,stone_selection_by_quest:{}}};selectedId=null;persist();render();notify('全データを初期化しました。','warn')};
...
async function tagTestRunAuraRuntimeJson(){
 ...
 const report={schema_version:'1.0.0',build:'GA-B486.207',generated_at:new Date().toISOString(),test:{id:'TAG-AURA-RUNTIME-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunAuraRuntimeJson'},current_spec:{task_id:'P01-06',stage:'runtime_connection_v1',runtime_application:true,source_dependency:true,stacking:'highest',source_death:'immediate_disable',source_revive:'re_evaluate_and_restore',target_death:'inactive_while_dead',target_revive:'re_evaluate',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},source:{studio_status:studioSkillBridge.status,data_version:studioSkillBridge.data_version},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.207-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function makeBattleUnits(){
 ...
 const allies=members.map((c,i)=>{const v=characterBattleValues(c),e2e=developerE2EOverrideSkillId(c.id);return makeCombatant({id:`A${i}`,characterId:c.id,name:c.name,side:'味方',ownedSkillIds:clone(c.skills||[]),equippedSkillId:c.equippedSkillId||'',defaultSkillId:e2e||c.equippedSkillId||c.skills?.[0]||'',agi:v.agi,attack:v.attack,maxHp:v.maxHp,maxMp:v.maxMp,gauge:0,actions:0,order:i,lastActionTick:null})});
...
function runActivationPriorityRuntimeValidation(){
 ...
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.207',generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.207-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runBattleEndEffectClearValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.207',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 ...
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.207',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',test:{id:'TAG-BATTLE-END-EFFECT-CLEAR-003',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},current_spec:{task_id:'P01-14',stage:'formal_candidate_v1',formal_runtime_enabled:true,battle_end_clear:['reserved_action','shield_effects','status_effects','cover_effects','dot_stacks','modifier_stacks','cooldowns']},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunBattleEndEffectClearJson(){pauseBattle();const report=runBattleEndEffectClearValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-battle-end-effect-clear-formal-runtime-device-validation-GA-B486.207-P01-14-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-14 BATTLE END CLEAR FORMAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}`;renderBattle();return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.207',generated_at:new Date().toISOString(),formal_candidate:'P01-13-FORMAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-004',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
...
function runCounterRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.207',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-002',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-07',stage:'runtime_v1_1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.207-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
