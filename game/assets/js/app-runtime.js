...
async function tagTestRunAuraRuntimeJson(){
 ...
 const report={schema_version:'1.0.0',build:'GA-B486.50',generated_at:new Date().toISOString(),test:{id:'TAG-AURA-RUNTIME-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunAuraRuntimeJson'},current_spec:{task_id:'P01-06',stage:'runtime_connection_v1',runtime_application:true,source_dependency:true,stacking:'highest',source_death:'immediate_disable',source_revive:'re_evaluate_and_restore',target_death:'inactive_while_dead',target_revive:'re_evaluate',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},source:{studio_status:studioSkillBridge.status,data_version:studioSkillBridge.data_version},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.50-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function runActivationPriorityRuntimeValidation(){
 ...
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.50',generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.50-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.50',generated_at:new Date().toISOString(),validation_patch:'P01-13-VAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-001',mode:'validation_runtime',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-13',stage:'validation_v1',formal_runtime_change:false,candidate_only:true,tie_rule_source:'existing_due_order',formal_tie_rule:'deferred_by_V9'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
 const neutral=findTagSkill('SKL-TEST-ATTACK');
 const cases=[];const add=(id,label,run)=>{try{const detail=run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[String(e?.message||e)]})}};
 const prep=()=>{pauseBattle();resetBattle();battle.validationMode=false;battle.validationActivationPriority=false;battle.validationCaptureEvents=true;battle.validationEvents=[];battle.tick=0;const allies=ensureValidationTargets('味方',3),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.actions=0;u.reservedAction=null;u.lastReservation=null;u.statusEffects=[];u.cooldowns={};u.maxMp=100;u.mp=100;u.defaultSkillId=neutral.id}return{a:allies[0],b:allies[1],c:allies[2],target:enemies[0]}};
 const reserveSameTick=(actors,tick=1)=>{for(const u of actors){reserveAction(u);u.reservedAction.executeAt=tick}};
 const resultOrder=()=>({fixed:(battle.validationEvents.find(x=>x.type==='activation_order_fixed')?.order||[]).map(x=>x.source_id),committed:battle.validationEvents.filter(x=>x.type==='action_execution_committed').map(x=>x.source_id)});
 add('SIMULTANEOUS-ORDER-OVERFLOW','同優先度ではゲージ超過量の既存順を候補として維持',()=>{const f=prep(),errors=[];f.a.agi=10;f.b.agi=10;f.a.gauge=GAUGE_MAX+100;f.b.gauge=GAUGE_MAX;reserveSameTick([f.b,f.a]);processTicks(1);const order=resultOrder();if(order.fixed[0]!==f.a.id)errors.push('overflow winner not first in fixed order');if(order.committed[0]!==f.a.id)errors.push('overflow winner not first committed');return{candidate_keys:{a:{gauge_before_tick:GAUGE_MAX+100,agi:10,order:f.a.order},b:{gauge_before_tick:GAUGE_MAX,agi:10,order:f.b.order}},fixed_order:order.fixed,committed_order:order.committed,errors}});
 add('SIMULTANEOUS-ORDER-AGI','同優先度・同Tick到達値ではAGIの既存順を候補として維持',()=>{const f=prep(),errors=[];f.a.agi=20;f.b.agi=10;f.a.gauge=GAUGE_MAX;f.b.gauge=GAUGE_MAX+10;reserveSameTick([f.b,f.a]);processTicks(1);const order=resultOrder();if(order.fixed[0]!==f.a.id)errors.push('AGI winner not first in fixed order');if(order.committed[0]!==f.a.id)errors.push('AGI winner not first committed');return{post_increment_gauge_tie:true,candidate_keys:{a:{agi:20,order:f.a.order},b:{agi:10,order:f.b.order}},fixed_order:order.fixed,committed_order:order.committed,errors}});
 add('SIMULTANEOUS-ORDER-ACTOR-ORDER','完全同条件ではactor orderの既存順を候補として維持',()=>{const f=prep(),errors=[];f.a.agi=10;f.b.agi=10;f.a.gauge=GAUGE_MAX;f.b.gauge=GAUGE_MAX;const expected=[f.a,f.b].sort((x,y)=>x.order-y.order)[0];reserveSameTick([f.b,f.a]);processTicks(1);const order=resultOrder();if(order.fixed[0]!==expected.id)errors.push('actor order winner not first in fixed order');if(order.committed[0]!==expected.id)errors.push('actor order winner not first committed');return{expected_first:expected.id,candidate_keys:{a:{agi:10,order:f.a.order},b:{agi:10,order:f.b.order}},fixed_order:order.fixed,committed_order:order.committed,errors}});
 add('SIMULTANEOUS-ORDER-FIXED-COMMITTED','同一Tickの固定順と確定順が一致',()=>{const f=prep(),errors=[];f.a.agi=10;f.b.agi=10;f.c.agi=10;f.a.gauge=GAUGE_MAX+50;f.b.gauge=GAUGE_MAX+20;f.c.gauge=GAUGE_MAX;reserveSameTick([f.c,f.b,f.a]);processTicks(1);const order=resultOrder();if(JSON.stringify(order.fixed)!==JSON.stringify(order.committed))errors.push('fixed and committed differ');return{fixed_order:order.fixed,committed_order:order.committed,errors}});
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.50',generated_at:new Date().toISOString(),validation_patch:'P01-13-VAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-001',mode:'validation_runtime',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,neutral_skill_id:neutral?.id||null},current_spec:{task_id:'P01-13',stage:'validation_v1',formal_runtime_change:false,candidate_only:true,formal_tie_rule:'deferred_by_V9',candidate_order:['executeAt_asc','gauge_overflow_desc','agi_desc','actor_order_asc'],activation_priority_primary:true,same_tick_fixed_once:true,next_tick_redecides:true},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunSimultaneousActivationOrderJson(){pauseBattle();const report=runSimultaneousActivationOrderValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-simultaneous-activation-order-runtime-device-validation-GA-B486.50-P01-13-VAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-13 SIMULTANEOUS ORDER VAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL CHANGE] NO\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}

function runCounterRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.50',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-002',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-07',stage:'runtime_v1_1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.50-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
