...
function tagTestRunCounterRuntimeJson(){
 ...
 const requireSkill=id=>{const x=findTagSkill(id);if(!x)throw new Error(`Studio正式スキル不足: ${id}`);return x};
 const counter=requireSkill('SKL-COUNTER-ATTACK-100'),single=requireSkill('SKL-TEST-ATTACK'),area=requireSkill('SKL-COUNTER-TEST-INCOMING-ALL-60');
 const cases=[];const add=(id,label,run)=>{try{const detail=run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[e.message]})}};
 const prep=()=>{resetBattle();battle.validationMode=true;battle.validationEvents=[];const allies=ensureValidationTargets('味方',2),enemies=ensureValidationTargets('敵',2);for(const u of battle.units){u.counterSkillId=null;u.counterDisabled=false;u.hp=u.maxHp;u.alive=true;u.shieldEffects=[]}return{defender:allies[0],ally:allies[1],attacker:enemies[0],enemy2:enemies[1]}};
 ...
 add('COUNTER-RUNTIME-BATTLE-END','元攻撃で戦闘終了確定なら反撃しない',()=>{const f=prep();f.defender.side='敵';f.attacker.side='味方';f.defender.counterSkillId=counter.id;f.defender.hp=1;for(const u of battle.units){if(u.side==='敵'&&u.id!==f.defender.id){u.hp=0;u.alive=false}}executeTaggedSkill(f.attacker,f.defender,single,{origin:'base'});const ev=battle.validationEvents.filter(x=>x.type==='counter_triggered'),er=[];if(ev.length)er.push('戦闘終了確定後に反撃');if(!battle.pendingResult&&!battle.result)er.push('戦闘終了が確定していません');return{pending_result:battle.pendingResult,result:battle.result,events:[...battle.validationEvents],errors:er}});
 ...
 const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));const report={schema_version:'1.0.0',build:'GA-B486.31',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-001',mode:'device_runtime_validation',entrypoint:'game/index.html',trigger:'tagTestRunCounterRuntimeJson'},source:{data_version:studioSkillBridge.dataVersion||null,counter_skill_id:counter.id,area_fixture_skill_id:area.id},current_spec:{task_id:'P01-07',stage:'runtime_v1',trigger:'hit',incoming_direct_single_only:true,shield_zero_damage_counter:true,defender_alive_required:true,battle_end_blocks_counter:true,counter_target:'attacker',attack_definition:'existing ATTACK pipeline',counter_and_follow_up_chain:false,action_disabled_blocks_counter:true},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.31-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
