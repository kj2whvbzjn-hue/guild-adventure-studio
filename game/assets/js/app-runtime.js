...
function normalizeRuntimeFormula(value,path){const f=objectValue(value,path),stats=f.stats&&typeof f.stats==='object'?f.stats:{};const normalized={base:finiteNumber(f.base,`${path}.base`),level_coefficient:finiteNumber(f.level_coefficient,`${path}.level_coefficient`),stats:{},rounding:String(f.rounding||'').trim(),min:f.min==null?null:finiteNumber(f.min,`${path}.min`),max:f.max==null?null:finiteNumber(f.max,`${path}.max`)};if(!['floor','round','ceil','trunc'].includes(normalized.rounding))throw new Error(`${path}.rounding が不正です。`);if(normalized.min!=null&&normalized.max!=null&&normalized.max<normalized.min)throw new Error(`${path}.max は min 以上にしてください。`);for(const [stat,coef] of Object.entries(stats)){if(!STATS.includes(stat))throw new Error(`${path}.stats.${stat} は未対応能力値です。`);normalized.stats[stat]=finiteNumber(coef,`${path}.stats.${stat}`);}return normalized}
...
function evaluateRuntimeFormula(formula,character){let value=Number(formula.base);for(const [stat,coef] of Object.entries(formula.stats||{}))value+=(Number(character?.stats?.[stat])||0)*Number(coef);value+=(Number(character?.level)||0)*Number(formula.level_coefficient);if(formula.rounding==='floor')value=Math.floor(value);else if(formula.rounding==='ceil')value=Math.ceil(value);else if(formula.rounding==='round')value=Math.round(value);else value=Math.trunc(value);if(formula.min!=null)value=Math.max(Number(formula.min),value);if(formula.max!=null)value=Math.min(Number(formula.max),value);return value}
function characterBattleValues(character){const cfg=requireFormalRuntimeSettings().battle_actor,b=equipmentBonus(character),mods=characterFormalModifierContributions(character),baseMaxHp=evaluateRuntimeFormula(cfg.max_hp,character)+b.maxHp,baseMaxMp=evaluateRuntimeFormula(cfg.max_mp,character)+b.maxMp,maxHp=resolveFormalModifierRows(baseMaxHp,'MAX_HP',mods).final_value,maxMp=resolveFormalModifierRows(baseMaxMp,'MAX_MP',mods).final_value;return{maxHp:Math.max(1,maxHp),maxMp:Math.max(0,maxMp),attack:evaluateRuntimeFormula(cfg.attack,character)+b.attack,agi:evaluateRuntimeFormula(cfg.agi,character)+b.agi,accuracy:b.accuracy,evasion:b.evasion,magicAccuracy:Math.max(0,b.magicAccuracy),magicResistance:Math.max(0,b.magicResistance),magicWeaponBonus:b.magicWeaponBonus,weaponCriticalRate:b.weaponCriticalRate,luk:Math.max(0,Number(character?.stats?.LUK)||0),blockRate:b.blockRate,blockDamageCutRate:b.blockDamageCutRate}}
...
const formalDefinitionsReady=Promise.all([loadFormalGameDefinitions(),loadFormalEquipmentDefinitions(),loadFormalPassiveDefinitions()]);
...
function characterBasicAttackProfiles(character){
 ...
 if(style==='weapon_shield'){
  ...
  return[{weaponStyle:'weapon_shield',weaponSlot:slot,weaponId:ref,attack:baseAttack+(Number(b.attack)||0),accuracy:baseAccuracy+(Number(b.accuracy)||0),weaponCriticalRate:Number(b.weaponCriticalRate)||0}];
 }
 if(style==='bow_quiver'){const state=characterBowQuiverState(character);if(!state.bow||!state.quiver)throw new Error(`${character.name||character.id||'キャラクター'}のbow_quiver構成が不正です。`);const nonWeapon=equipmentBonusFromRows(allRows.filter(row=>!['weapon1','weapon2'].includes(row.slot))),bb=state.bow.bonuses||{};return[{weaponStyle:'bow_quiver',weaponSlot:state.bowSlot,weaponId:state.bowRef,attack:evaluateRuntimeFormula(cfg.attack,character)+nonWeapon.attack+(Number(bb.attack)||0),accuracy:nonWeapon.accuracy+(Number(bb.accuracy)||0),weaponCriticalRate:Number(bb.weaponCriticalRate)||0}]};
 if(style!=='dual_wield')return[{weaponStyle:style,weaponSlot:twoHandedWeaponRef(character)?'weapon1':(character.equipment.weapon1?'weapon1':character.equipment.weapon2?'weapon2':null),weaponId:twoHandedWeaponRef(character)||character.equipment.weapon1||character.equipment.weapon2||null,...(()=>{const v=characterBattleValues(character);return{attack:v.attack,accuracy:v.accuracy,weaponCriticalRate:v.weaponCriticalRate}})()}];
 const nonWeapon=equipmentBonusFromRows(allRows.filter(row=>!['weapon1','weapon2'].includes(row.slot))),baseAttack=evaluateRuntimeFormula(cfg.attack,character)+nonWeapon.attack,baseAccuracy=nonWeapon.accuracy,profiles=[];
 for(const slot of ['weapon1','weapon2']){const ref=character.equipment[slot];if(!ref)continue;const b=equipmentDefinition(ref)?.bonuses||{};profiles.push({weaponStyle:'dual_wield',weaponSlot:slot,weaponId:ref,attack:baseAttack+(Number(b.attack)||0),accuracy:baseAccuracy+(Number(b.accuracy)||0),weaponCriticalRate:Number(b.weaponCriticalRate)||0});}
 return profiles.length?profiles:[{weaponStyle:'single',weaponSlot:null,weaponId:null,attack:baseAttack,accuracy:baseAccuracy,weaponCriticalRate:0}];
}
...
function makeBattleUnits(){
 ...
 const enemies=expanded.map((e,i)=>makeCombatant({id:`E${i}`,monsterId:e.monster_id,name:e.name,side:'敵',formationPosition:e.formationPosition,aiPolicy:e.aiPolicy,ownedSkillIds:clone(e.skillIds||[]),defaultSkillId:e.defaultSkillId,agi:e.agi,attack:e.attack,maxHp:e.maxHp,gauge:0,actions:0,order:100+i,lastActionTick:null}));
