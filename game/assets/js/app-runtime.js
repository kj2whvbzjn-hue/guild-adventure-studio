...
function normalize(raw){
 ...
 raw.schemaRevision='1.5.0';raw.gameVersion='GA-B486.98';
 return raw;
}
...
function registerAdventureQuestCards(content){adventureQuestCatalog=(content?.quests||[]).filter(q=>{const links=q.links||{};return Boolean(q?.id&&links.chapter_id&&links.section_id)}).map(q=>({id:String(q.id),name:String(q.name||q.id),rank:'Story',stars:1,recommendedLevel:Math.max(1,Math.floor(Number(q.recommended_level)||1)),description:String(q.summary||''),reward:0,bonus:0,enemies:[],drops:[],adventureStory:true}));for(const q of adventureQuestCatalog){const i=QUESTS.findIndex(x=>String(x.id)===q.id);if(i>=0)QUESTS[i]=q;else QUESTS.push(q);}return adventureQuestCatalog;}
...
$('clearBtn').onclick=()=>{if(!confirm('正式版Phase Aの全データを初期化しますか？'))return;data={saveVersion:1,schemaRevision:'1.5.0',gameVersion:'GA-B486.98',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],partyIds:[],selectedQuestId:'Q-001',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},flags:{},quest_progress:{completed_quest_ids:[],unlocked_quest_ids:[]},quest_resources:{},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20}};selectedId=null;persist();render();notify('全データを初期化しました。','warn')};
