...
const APP_RUNTIME_GAME_BUILD=String(window.GA_PROJECT_CONFIG?.gameBuild||'').trim();
...
const FORMAL_EQUIPMENT_EXPORT_URL=window.GA_PROJECT_CONFIG?.equipmentExportUrl||'../Export/equipment/equipment.json';
const FORMAL_MOD_EXPORT_URL=window.GA_PROJECT_CONFIG?.equipmentModExportUrl||'../Export/equipment/mods.json';
const FORMAL_PASSIVE_EXPORT_URL=window.GA_PROJECT_CONFIG?.passiveExportUrl||'../Export/master/passives.json';
const formalJobCatalog=new Map(),formalEquipmentCatalog=new Map(),formalModCatalog=new Map();
let formalJobDomainCatalog=null;
let formalRuntimeSettings=null;
...
const formalEquipmentBridge={status:'idle',source_url:FORMAL_EQUIPMENT_EXPORT_URL,mods_url:FORMAL_MOD_EXPORT_URL,schema_version:null,data_version:null,generated_by:null,loaded_at:null,imported_ids:[],imported_mod_ids:[],errors:[]};
const formalPassiveCatalog=new Map(),formalPassiveBridge={status:'idle',source_url:FORMAL_PASSIVE_EXPORT_URL,schema_version:null,data_version:null,generated_by:null,loaded_at:null,imported_ids:[],errors:[]};
...
function normalizeStarterEquipmentConfig(value,path){if(value==null)return{};const source=objectValue(value,path),unknown=Object.keys(source).filter(slot=>!CHARACTER_EQUIPMENT_SLOTS.includes(slot));if(unknown.length)throw new Error(`${path} に未対応slotがあります: ${unknown.join(', ')}`);const out={};for(const slot of CHARACTER_EQUIPMENT_SLOTS){if(!Object.prototype.hasOwnProperty.call(source,slot))continue;const id=String(source[slot]||'').trim();if(id)out[slot]=id}return out}
function normalizeFormalJobRecord(record){
 if(!record||typeof record!=='object'||Array.isArray(record)||record.enabled===false)return null;
 const id=String(record.id||'').trim(),name=String(record.name||'').trim();if(!id||!name)return null;
 const p=record.params&&typeof record.params==='object'?record.params:{},source=p.aptitudes&&typeof p.aptitudes==='object'?p.aptitudes:(p.stats&&typeof p.stats==='object'?p.stats:record),aptitudes={};
 for(const stat of STATS){const raw=source[stat]??source[stat.toLowerCase()];aptitudes[stat]=finiteNumber(raw,`${id}.aptitudes.${stat}`,{integer:true,min:0});}
 return{...record,id,name,source:'studio_export',aptitudes};
}
function normalizeSkillProgressionCostMap(value,path){if(value==null)return{};const source=objectValue(value,path),out={};for(const[id,raw]of Object.entries(source)){const key=String(id||'').trim();if(!key)throw new Error(`${path} に空IDがあります。`);out[key]=finiteNumber(raw,`${path}.${key}`,{integer:true,min:1})}return out}
function normalizeSkillProgressionSettings(value){const source=objectValue(value,'game_runtime.skill_progression'),acquisition=objectValue(source.acquisition_cost,'game_runtime.skill_progression.acquisition_cost'),respec=objectValue(source.respec_gold,'game_runtime.skill_progression.respec_gold');return{skill_points_per_level:finiteNumber(source.skill_points_per_level,'game_runtime.skill_progression.skill_points_per_level',{integer:true,min:1}),acquisition_cost:{skill_default:finiteNumber(acquisition.skill_default,'game_runtime.skill_progression.acquisition_cost.skill_default',{integer:true,min:1}),passive_default:finiteNumber(acquisition.passive_default,'game_runtime.skill_progression.acquisition_cost.passive_default',{integer:true,min:1}),skill_by_id:normalizeSkillProgressionCostMap(acquisition.skill_by_id,'game_runtime.skill_progression.acquisition_cost.skill_by_id'),passive_by_id:normalizeSkillProgressionCostMap(acquisition.passive_by_id,'game_runtime.skill_progression.acquisition_cost.passive_by_id')},respec_gold:{skill:finiteNumber(respec.skill,'game_runtime.skill_progression.respec_gold.skill',{integer:true,min:1}),passive:finiteNumber(respec.passive,'game_runtime.skill_progression.respec_gold.passive',{integer:true,min:1}),all:finiteNumber(respec.all,'game_runtime.skill_progression.respec_gold.all',{integer:true,min:1})}}}
function normalizeFormalRuntimeSettingsRecord(record){
 if(!record||typeof record!=='object'||Array.isArray(record)||record.enabled===false)return null;
 const p=record.params&&typeof record.params==='object'?record.params:{},runtime=p.game_runtime;if(!runtime)return null;objectValue(runtime,`${record.id||'settings'}.params.game_runtime`);
 const character=objectValue(runtime.character,'game_runtime.character'),initialStats=objectValue(character.initial_stats,'game_runtime.character.initial_stats'),normalizedStats={};for(const stat of STATS)normalizedStats[stat]=finiteNumber(initialStats[stat],`game_runtime.character.initial_stats.${stat}`,{integer:true,min:0});
 const starterSkillIds=stringArray(character.starter_skill_ids,'game_runtime.character.starter_skill_ids'),starterEquipped=String(character.starter_equipped_skill_id||'').trim(),initialSkillPoints=finiteNumber(character.initial_skill_points,'game_runtime.character.initial_skill_points',{integer:true,min:0}),starterPassiveIds=stringArray(character.starter_passive_ids,'game_runtime.character.starter_passive_ids'),initialFormationPosition=String(character.initial_formation_position||'').trim().toUpperCase();if(starterEquipped&&!starterSkillIds.includes(starterEquipped))throw new Error('starter_equipped_skill_id は starter_skill_ids に含めてください。');if(!FORMATION_POSITIONS.includes(initialFormationPosition))throw new Error('game_runtime.character.initial_formation_position が不正です。');
 const growth=objectValue(runtime.growth,'game_runtime.growth'),outcomeMap=objectValue(growth.outcomes_by_aptitude,'game_runtime.growth.outcomes_by_aptitude'),rankMap=objectValue(growth.rank_by_aptitude,'game_runtime.growth.rank_by_aptitude'),normalizedOutcomes={};for(const [key,rows] of Object.entries(outcomeMap)){if(!Array.isArray(rows)||!rows.length)throw new Error(`growth.outcomes_by_aptitude.${key} が空です。`);normalizedOutcomes[String(key)]=rows.map((row,i)=>({amount:finiteNumber(row?.amount,`growth.outcomes_by_aptitude.${key}[${i}].amount`,{integer:true,min:0}),weight:finiteNumber(row?.weight,`growth.outcomes_by_aptitude.${key}[${i}].weight`,{min:0})}));if(!normalizedOutcomes[String(key)].some(x=>x.weight>0))throw new Error(`growth.outcomes_by_aptitude.${key} のweight合計が0です。`)}
 const newGame=objectValue(runtime.new_game,'game_runtime.new_game'),roster=Array.isArray(newGame.starter_roster)?newGame.starter_roster.map((row,i)=>({name:String(row?.name||'').trim(),job_id:String(row?.job_id||'').trim(),starter_equipment:normalizeStarterEquipmentConfig(row?.starter_equipment,`game_runtime.new_game.starter_roster[${i}].starter_equipment`)})):null;if(!roster||roster.some(x=>!x.name||!x.job_id))throw new Error('game_runtime.new_game.starter_roster が不正です。');
 const partyIndexes=Array.isArray(newGame.party_member_indexes)?newGame.party_member_indexes.map((x,i)=>finiteNumber(x,`game_runtime.new_game.party_member_indexes[${i}]`,{integer:true,min:0})):null;if(!partyIndexes)throw new Error('game_runtime.new_game.party_member_indexes が必要です。');if(new Set(partyIndexes).size!==partyIndexes.length)throw new Error('game_runtime.new_game.party_member_indexes に重複があります。');if(partyIndexes.some(i=>i>=roster.length))throw new Error('game_runtime.new_game.party_member_indexes が starter_roster 範囲外です。');
 const battleFlow=objectValue(runtime.battle_flow,'game_runtime.battle_flow'),actionGauge=objectValue(battleFlow.action_gauge,'game_runtime.battle_flow.action_gauge'),casting=objectValue(battleFlow.casting,'game_runtime.battle_flow.casting');
 const equipmentRules=objectValue(runtime.equipment_rules,'game_runtime.equipment_rules'),weaponRules=objectValue(equipmentRules.weapon,'game_runtime.equipment_rules.weapon');
 const gaugeMax=finiteNumber(actionGauge.max,'game_runtime.battle_flow.action_gauge.max',{min:Number.EPSILON}),aiReevaluationRatio=finiteNumber(actionGauge.ai_reevaluation_ratio,'game_runtime.battle_flow.action_gauge.ai_reevaluation_ratio',{min:Number.EPSILON,max:1}),successfulConsumeRatio=finiteNumber(actionGauge.successful_action_consume_ratio,'game_runtime.battle_flow.action_gauge.successful_action_consume_ratio',{min:0,max:1}),failedConsumeRatio=finiteNumber(actionGauge.failed_execution_consume_ratio,'game_runtime.battle_flow.action_gauge.failed_execution_consume_ratio',{min:0,max:1}),defaultSkillDurationTicks=finiteNumber(casting.default_skill_duration_ticks,'game_runtime.battle_flow.casting.default_skill_duration_ticks',{integer:true,min:0}),weaponStrTwoHandMultiplier=finiteNumber(weaponRules.str_requirement_two_hand_multiplier,'game_runtime.equipment_rules.weapon.str_requirement_two_hand_multiplier',{min:1});
 const rarities=runtime.rarities&&typeof runtime.rarities==='object'?runtime.rarities:{};
 const skillProgression=runtime.skill_progression==null?null:normalizeSkillProgressionSettings(runtime.skill_progression),skillLoadout=objectValue(runtime.skill_loadout,'game_runtime.skill_loadout'),activeSkillSlots=finiteNumber(skillLoadout.active_skill_slots,'game_runtime.skill_loadout.active_skill_slots',{integer:true,min:1}),passiveSlots=finiteNumber(skillLoadout.passive_slots,'game_runtime.skill_loadout.passive_slots',{integer:true,min:1});
 const initialLevel=finiteNumber(character.initial_level,'game_runtime.character.initial_level',{integer:true,min:1}),maxLevel=finiteNumber(character.max_level,'game_runtime.character.max_level',{integer:true,min:1}),initialHp=finiteNumber(character.initial_hp,'game_runtime.character.initial_hp',{integer:true,min:1}),initialMp=finiteNumber(character.initial_mp,'game_runtime.character.initial_mp',{integer:true,min:0}),partyMax=finiteNumber(character.party_max_size,'game_runtime.character.party_max_size',{integer:true,min:1});if(maxLevel<initialLevel)throw new Error('character.max_level は initial_level 以上にしてください。');if(partyIndexes.length>partyMax)throw new Error('party_member_indexes 件数が party_max_size を超えています。');
 return{id:String(record.id||'').trim(),name:String(record.name||record.id||'').trim(),character:{initial_level:initialLevel,max_level:maxLevel,initial_hp:initialHp,initial_mp:initialMp,initial_stats:normalizedStats,party_max_size:partyMax,starter_skill_ids:starterSkillIds,starter_equipped_skill_id:starterEquipped,initial_skill_points:initialSkillPoints,starter_passive_ids:starterPassiveIds,initial_formation_position:initialFormationPosition},growth:{outcomes_by_aptitude:normalizedOutcomes,rank_by_aptitude:Object.fromEntries(Object.entries(rankMap).map(([k,v])=>[String(k),String(v)])),rule_revision:String(growth.rule_revision||record.id||'').trim()},battle_flow:{action_gauge:{max:gaugeMax,ai_reevaluation_ratio:aiReevaluationRatio,successful_action_consume_ratio:successfulConsumeRatio,failed_execution_consume_ratio:failedConsumeRatio},casting:{default_skill_duration_ticks:defaultSkillDurationTicks}},equipment_rules:{weapon:{str_requirement_two_hand_multiplier:weaponStrTwoHandMultiplier}},skill_progression:skillProgression,skill_loadout:{active_skill_slots:activeSkillSlots,passive_slots:passiveSlots},new_game:{starting_gold:finiteNumber(newGame.starting_gold,'game_runtime.new_game.starting_gold',{integer:true,min:0}),starter_roster:roster,party_member_indexes:partyIndexes,starter_inventory_ids:stringArray(newGame.starter_inventory_ids,'game_runtime.new_game.starter_inventory_ids')},rarities};
}

function jobDefinition(ref){if(!window.GKJobTransferDomain||!formalJobDomainCatalog)return null;return GKJobTransferDomain.resolveJob(formalJobDomainCatalog,ref)}
function currentGuildEditState(){if(!window.GKJobTransferDomain)return{ok:false,code:'JOB_TRANSFER_DOMAIN_UNAVAILABLE',message:'Job Transfer Domainを読み込めません。'};return GKJobTransferDomain.evaluateGuildEditState({phase:currentPhase,activeQuestRunId:String(data?.adventure?.active_quest_run_id||'')})}
function jobDisplayName(ref){return jobDefinition(ref)?.name||String(ref||'')}
function requireFormalRuntimeSettings(){if(formalGameBridge.status!=='loaded'||!formalRuntimeSettings)throw new Error(`Formal Game Runtime設定を読み込めません。状態: ${formalGameBridge.status}${formalGameBridge.errors.length?` / ${formalGameBridge.errors.join(' / ')}`:''}`);return formalRuntimeSettings}
function skillProgressionBalance(){const value=requireFormalRuntimeSettings().skill_progression;if(!value)throw new Error('Skill Point / Respec BalanceがGame Dataに設定されていません。');return value}
function partyMaxSize(){return formalRuntimeSettings?.character?.party_max_size||null}
function skillLoadoutBalance(){const row=formalRuntimeSettings?.skill_loadout;if(!row)throw new Error('Formal Skill Loadout設定を読み込めません。');return row}
function characterSelectedActiveSkillIds(character){if(!window.GKGameSkillLoadout?.selectedSkillIds)throw new Error('Formal Skill Loadout Runtimeを読み込めません。');return GKGameSkillLoadout.selectedSkillIds(character,{capacity:skillLoadoutBalance().active_skill_slots})}
function battleFlowConfig(){return requireFormalRuntimeSettings().battle_flow}
function battleGaugeMax(){return battleFlowConfig().action_gauge.max}
function battleAiReevaluationRatio(){return battleFlowConfig().action_gauge.ai_reevaluation_ratio}
function battleAiReevaluationGaugeInterval(){return battleGaugeMax()*battleAiReevaluationRatio()}
function battleAiReevaluationIntervalTicks(){return battleAiReevaluationGaugeInterval()}
function battleGaugeConsumeAmount(kind='success'){const cfg=battleFlowConfig().action_gauge,ratio=kind==='failed'?cfg.failed_execution_consume_ratio:cfg.successful_action_consume_ratio;return battleGaugeMax()*ratio}
function battleDefaultSkillCastingTicks(){return battleFlowConfig().casting.default_skill_duration_ticks}
function weaponStrTwoHandRequirementMultiplier(){return requireFormalRuntimeSettings().equipment_rules.weapon.str_requirement_two_hand_multiplier}
function rarityView(key){const row=formalRuntimeSettings?.rarities?.[String(key||'')]||{};return{label:String(row.label||''),name:String(row.name||key||'未設定')}}
function characterPrimaryWeaponBonus(character){normalizeCharacterEquipmentState(character);const style=characterWeaponStyle(character);let ref=null;if(style==='two_hand')ref=twoHandedWeaponRef(character);else if(style==='bow_quiver')ref=characterBowQuiverState(character).bowRef;else{for(const slot of ['weapon1','weapon2']){const candidate=character.equipment[slot];if(!candidate)continue;const equipment=equipmentDefinition(candidate);if(equipmentIsShield(equipment)||equipmentIsQuiver(equipment))continue;ref=candidate;break;}}const b=equipmentDefinition(ref)?.bonuses||{};return{ref:ref||null,attack:Number(b.attack)||0,accuracy:Number(b.accuracy)||0,magicAccuracy:Number(b.magicAccuracy)||0,magicWeaponBonus:Number(b.magicWeaponBonus)||0,weaponCriticalRate:Number(b.weaponCriticalRate)||0}}
function characterBattleValues(character){const equipmentContribution=characterEquipmentContribution(character),weapon=characterPrimaryWeaponBonus(character),mods=characterFormalModifierContributions(character),baseHp=finiteNumber(character?.base_hp,`${character?.id||character?.name||'character'}.base_hp`,{integer:true,min:1}),baseMp=finiteNumber(character?.base_mp,`${character?.id||character?.name||'character'}.base_mp`,{integer:true,min:0}),agi=finiteNumber(character?.stats?.AGI,`${character?.id||character?.name||'character'}.stats.AGI`,{integer:true,min:0}),baseMaxHp=baseHp+equipmentContribution.hpBonus,baseMaxMp=baseMp+equipmentContribution.mpBonus,maxHp=resolveFormalModifierRows(baseMaxHp,'MAX_HP',mods).final_value,maxMp=resolveFormalModifierRows(baseMaxMp,'MAX_MP',mods).final_value;return{maxHp:Math.max(1,maxHp),maxMp:Math.max(0,maxMp),attack:weapon.attack,agi,accuracy:weapon.accuracy,evasion:equipmentContribution.evasion,magicAccuracy:Math.max(0,weapon.magicAccuracy),magicResistance:Math.max(0,equipmentContribution.magicResistance),magicWeaponBonus:weapon.magicWeaponBonus,weaponCriticalRate:weapon.weaponCriticalRate,luk:Math.max(0,Number(character?.stats?.LUK)||0),blockRate:equipmentContribution.blockRate,blockDamageCutRate:equipmentContribution.blockDamageCutRate}}
...
async function loadFormalGameDefinitions(){
 formalGameBridge.status='loading';formalGameBridge.errors=[];
 try{const [jobResponse,settingsResponse]=await Promise.all([fetch(FORMAL_JOB_EXPORT_URL,{cache:'no-store'}),fetch(FORMAL_RUNTIME_SETTINGS_EXPORT_URL,{cache:'no-store'})]);if(!jobResponse.ok)throw new Error(`Job Export HTTP ${jobResponse.status}`);if(!settingsResponse.ok)throw new Error(`Runtime Settings HTTP ${settingsResponse.status}`);const jobPayload=await jobResponse.json(),settingsPayload=await settingsResponse.json(),jobRows=envelopeData(jobPayload),settingRows=envelopeData(settingsPayload);if(!Array.isArray(jobRows))throw new Error('Job Export data が配列ではありません。');if(!Array.isArray(settingRows))throw new Error('Adventure Settings Export data が配列ではありません。');const next=new Map(),names=new Set();for(const raw of jobRows){const job=normalizeFormalJobRecord(raw);if(!job)continue;if(next.has(job.id))throw new Error(`Job ID duplicated: ${job.id}`);if(names.has(job.name))throw new Error(`Job name duplicated: ${job.name}`);next.set(job.id,job);names.add(job.name)}if(!next.size)throw new Error('Formal Job Masterが0件です。');if(window.GKFormalRuntimeGate){const settingsValidation=GKFormalRuntimeGate.validateAdventureSettingsPayload(settingsPayload,{canonicalId:'ADV-0001'});GKFormalRuntimeGate.assertValid(settingsValidation,{code:'FORMAL_RUNTIME_SETTINGS_INVALID'});}const settings=settingRows.map(normalizeFormalRuntimeSettingsRecord).filter(Boolean);if(settings.length!==1)throw new Error(`game_runtimeを持つAdventure Settingsは1件必要です。現在 ${settings.length}件です。`);const dataVersions=[jobPayload?.data_version,settingsPayload?.data_version].filter(Boolean);if(new Set(dataVersions).size>1)throw new Error(`Job/Runtime SettingsのData Version不一致: ${dataVersions.join(' / ')}`);for(const row of settings[0].new_game.starter_roster)if(!next.has(row.job_id))throw new Error(`starter_rosterのJobが存在しません: ${row.job_id}`);formalJobCatalog.clear();formalJobDomainCatalog=window.GKJobTransferDomain?GKJobTransferDomain.normalizeCatalog(next,{statKeys:STATS}):null;for(const [id,row] of next)formalJobCatalog.set(id,row);formalRuntimeSettings=settings[0];formalGameBridge.status='loaded';formalGameBridge.schema_version=settingsPayload?.schema_version||jobPayload?.schema_version||null;formalGameBridge.data_version=dataVersions[0]||null;formalGameBridge.generated_by=settingsPayload?.generated_by||jobPayload?.generated_by||null;formalGameBridge.loaded_at=new Date().toISOString();formalGameBridge.job_ids=[...formalJobCatalog.keys()];formalGameBridge.settings_id=formalRuntimeSettings.id;renderFormalJobControls();return formalGameBridge;}catch(error){formalJobCatalog.clear();formalJobDomainCatalog=null;formalRuntimeSettings=null;formalGameBridge.status='failed';formalGameBridge.errors=[String(error?.message||error)];renderFormalJobControls();return formalGameBridge;}
}
const CHARACTER_EQUIPMENT_SLOTS=Object.freeze(['weapon1','weapon2','head','armor','gloves','feet','amulet','ring1','ring2','belt']);
const CHARACTER_EQUIPMENT_SLOT_LABEL=Object.freeze({weapon1:'武器1',weapon2:'武器2',head:'頭',armor:'鎧',gloves:'手袋',feet:'足',amulet:'アミュレット',ring1:'指輪1',ring2:'指輪2',belt:'ベルト'});
const FORMAL_EQUIPMENT_SLOT_MAP=Object.freeze({'weapon1':'weapon','weapon2':'weapon','ring1':'ring','ring2':'ring','鎧':'armor','頭':'head','手':'gloves','手袋':'gloves','足':'feet','アミュレット':'amulet','護符':'amulet','指輪':'ring','指輪1':'ring','指輪2':'ring','ベルト':'belt'});
const FORMAL_EQUIPMENT_TARGET_LABEL=Object.freeze({weapon:'武器',ring:'指輪',head:'頭',armor:'鎧',gloves:'手袋',feet:'足',amulet:'アミュレット',belt:'ベルト'});
function formalEquipmentNumber(record,key){const raw=record?.[key];if(raw==null)return 0;const value=Number(raw);if(!Number.isFinite(value))throw Object.assign(new Error(`Equipment ${record?.id||'(unknown)'}.${key} が数値ではありません。`),{code:'GAME_EQUIPMENT_FORMAL_NUMBER_INVALID',equipment_id:String(record?.id||''),field:String(key||'')});return value}
function normalizeFormalEquipmentRecord(record){
 if(!record||typeof record!=='object'||Array.isArray(record))return null;
 if(Object.prototype.hasOwnProperty.call(record,'base_critical_rate'))throw Object.assign(new Error(`Equipment ${record.id||'(unknown)'}は廃止field base_critical_rateを使用できません。`),{code:'GAME_EQUIPMENT_BASE_CRITICAL_RATE_FORBIDDEN',equipment_id:String(record.id||'')});
 const legacyParams=record.params&&typeof record.params==='object'&&!Array.isArray(record.params)?record.params:{},legacyStats=legacyParams.stats&&typeof legacyParams.stats==='object'&&!Array.isArray(legacyParams.stats)?legacyParams.stats:{};
 const forbiddenFields=['base_attack','base_defense','defense','base_critical_rate','maxHp','maxMp','hp','mp','agi'];for(const field of forbiddenFields){if(Object.prototype.hasOwnProperty.call(record,field)||Object.prototype.hasOwnProperty.call(legacyParams,field)||Object.prototype.hasOwnProperty.call(legacyStats,field))throw Object.assign(new Error(`Equipment ${record.id||'(unknown)'}は廃止field ${field}を使用できません。`),{code:'GAME_EQUIPMENT_LEGACY_FIELD_FORBIDDEN',equipment_id:String(record.id||''),field});}
 const id=String(record.id||'').trim(),name=String(record.name||'').trim();if(!id||!name)return null;
 const input=record.generation?.generation_input&&typeof record.generation.generation_input==='object'?record.generation.generation_input:{};
 const armorSlot=String(record.armor_slot??input.armor_slot??'').trim(),explicitSlot=String(record.slot??'').trim(),kind=String(input.kind??record.kind??'').trim().toLowerCase(),baseItemType=String(record.generation?.base_item_type??input.base_item_type??'').trim();
 let slot='';
 if(explicitSlot){
  const normalizedExplicit=FORMAL_EQUIPMENT_SLOT_MAP[explicitSlot]||explicitSlot;
  if(normalizedExplicit==='weapon'||normalizedExplicit==='ring'||['head','armor','gloves','feet','amulet','belt'].includes(normalizedExplicit))slot=normalizedExplicit;
 }
 if(!slot){if(kind==='weapon'||record.required_str!=null||record.required_dex!=null||record.required_int!=null)slot='weapon';else if(armorSlot)slot=FORMAL_EQUIPMENT_SLOT_MAP[armorSlot]||'';else if(kind==='accessory')slot=FORMAL_EQUIPMENT_SLOT_MAP[baseItemType]||'';else if(kind==='armor')slot='armor';}
 const required={STR:formalEquipmentNumber(record,'required_str'),DEX:formalEquipmentNumber(record,'required_dex'),INT:formalEquipmentNumber(record,'required_int'),VIT:formalEquipmentNumber(record,'required_vit'),MND:formalEquipmentNumber(record,'required_mnd'),AGI:formalEquipmentNumber(record,'required_agi')};
 const bonuses={attack:formalEquipmentNumber(record,'attack'),hpBonus:formalEquipmentNumber(record,'hp_bonus'),mpBonus:formalEquipmentNumber(record,'mp_bonus'),accuracy:formalEquipmentNumber(record,'accuracy'),evasion:formalEquipmentNumber(record,'evasion'),magicAccuracy:formalEquipmentNumber(record,'magic_accuracy'),magicResistance:formalEquipmentNumber(record,'magic_resistance'),magicWeaponBonus:formalEquipmentNumber(record,'magic_weapon_bonus'),weaponCriticalRate:formalEquipmentNumber(record,'weapon_critical_rate'),blockRate:formalEquipmentNumber(record,'block_rate'),blockDamageCutRate:formalEquipmentNumber(record,'block_damage_cut_rate')};
 const rarity=String(record.rarity??'').trim();return{...record,id,name,source:'studio_export',slot,slotLabel:FORMAL_EQUIPMENT_TARGET_LABEL[slot]||armorSlot||slot||'未定義',armorSlot,rarity,required,bonuses};
}
function equipmentDefinition(ref){return formalEquipmentCatalog.get(String(ref||''))||null}
function weaponBaseItemRuntimeSnapshot(ref){
 const row=equipmentDefinition(ref);if(!row)return null;
 const type=equipmentBaseItemType(row);if(!type)return null;
 return{id:row.id,base_item_type:type,item_level:Number(row.item_level),required:{...row.required},bonuses:{...row.bonuses},generation:clone(row.generation||null)};
}
...
const CHARACTER_WEAPON_STYLES=Object.freeze(['single','dual_wield','two_hand','weapon_shield','bow_quiver']);
function characterAbilityStats(character){return character?.stats&&typeof character.stats==='object'?character.stats:{}}
function characterFormalPassiveSelection(character){if(!window.GKSFormalContribution)throw new Error('Formal Contribution Resolverを読み込めません。');return window.GKSFormalContribution.validatePassiveSelection([...formalPassiveCatalog.values()],character?.passiveIds||[],characterAbilityStats(character),{requireRuntimeContracts:true})}
function characterCombatCapabilities(character){if(!window.GKSCombatCapabilityRuntime)throw new Error('Combat Capability Runtimeを読み込めません。');const selection=characterFormalPassiveSelection(character);return new Set(window.GKSCombatCapabilityRuntime.activeCapabilitiesFromFormalPassives(selection.activePassives))}
...
function equipmentIsBow(equipment){return['弓','大弓'].includes(equipmentBaseItemType(equipment))}
function equipmentIsQuiver(equipment){return equipmentBaseItemType(equipment)==='矢筒'}
function equipmentIsBowQuiverPair(a,b){return(!!a&&!!b)&&((equipmentIsBow(a)&&equipmentIsQuiver(b))||(equipmentIsQuiver(a)&&equipmentIsBow(b)))}
...
function inferCharacterWeaponStyle(character){
 ...
 if(w1&&w2){const e1=equipmentDefinition(w1),e2=equipmentDefinition(w2);if(equipmentIsBowQuiverPair(e1,e2))return'bow_quiver';if(equipmentIsShield(e1)||equipmentIsShield(e2))return'weapon_shield';if(!characterCanDualWield(character))throw Object.assign(new Error(`${character?.name||character?.id||'キャラクター'}は二刀流パッシブを所持していません。`),{code:'DUAL_WIELD_PASSIVE_REQUIRED',capability:'DUAL_WIELD'});return'dual_wield';}
 return'single';
}
...
function normalizeCharacterEquipmentState(character){
 ...
 if(!source)throw new Error(`${character.name||character.id||'キャラクター'}のequipmentが不正です。`);
 const keys=Object.keys(source);
 const unknown=keys.filter(slot=>!CHARACTER_EQUIPMENT_SLOTS.includes(slot));
 const missing=CHARACTER_EQUIPMENT_SLOTS.filter(slot=>!Object.prototype.hasOwnProperty.call(source,slot));
 ...
 if(character.passiveIds!=null&&(!Array.isArray(character.passiveIds)||character.passiveIds.some(id=>!String(id||'').trim())))throw new Error(`${character.name||character.id||'キャラクター'}のpassiveIdsが不正です。`);characterFormalPassiveSelection(character);
 character.equipment=Object.fromEntries(CHARACTER_EQUIPMENT_SLOTS.map(slot=>[slot,source[slot]||null]));
 if(character.weaponStyle!=null){const style=String(character.weaponStyle),w1=!!character.equipment.weapon1,w2=!!character.equipment.weapon2,count=Number(w1)+Number(w2);if(style==='single'&&count>1)throw new Error(`${character.name||character.id||'キャラクター'}のsingle weaponStyleに武器が2本あります。`);if(style==='dual_wield'&&!characterCanDualWield(character))throw Object.assign(new Error(`${character.name||character.id||'キャラクター'}のdual_wieldには二刀流パッシブが必要です。`),{code:'DUAL_WIELD_PASSIVE_REQUIRED',capability:'DUAL_WIELD'});if(style==='dual_wield'&&count!==2)throw new Error(`${character.name||character.id||'キャラクター'}のdual_wieldは武器2本が必要です。`);if(style==='two_hand'&&count!==1)throw new Error(`${character.name||character.id||'キャラクター'}のtwo_handは武器1本を両手使用する状態が必要です。`);if(style==='weapon_shield'&&count!==2)throw new Error(`${character.name||character.id||'キャラクター'}のweapon_shieldは武器枠2件が必要です。`);if(style==='bow_quiver'){const e1=equipmentDefinition(character.equipment.weapon1),e2=equipmentDefinition(character.equipment.weapon2);if(count!==2||!equipmentIsBowQuiverPair(e1,e2))throw new Error(`${character.name||character.id||'キャラクター'}のbow_quiverは弓/大弓と矢筒が1件ずつ必要です。`);}}
 return character;
}
function setCharacterWeaponStyle(character,style){if(!CHARACTER_WEAPON_STYLES.includes(style))throw new Error(`weaponStyleが不正です: ${style}`);if(style==='dual_wield'&&!characterCanDualWield(character))throw Object.assign(new Error(`${character.name||character.id||'キャラクター'}のdual_wieldには二刀流パッシブが必要です。`),{code:'DUAL_WIELD_PASSIVE_REQUIRED',capability:'DUAL_WIELD'});character.weaponStyle=style;return style}
...
function equipmentRequirementCheck(character,equipment){
 ...
 for(const stat of STATS){
  const required=Math.max(0,Number(equipment.required?.[stat])||0),actual=Math.max(0,Number(stats[stat])||0);if(actual>=required)continue;
  if(equipment.slot==='weapon'&&stat==='STR'){
   const multiplier=weaponStrTwoHandRequirementMultiplier(),limit=actual*multiplier;
   if(required<=limit){twoHanded=true;continue;}
   missing.push({stat,required,actual,twoHandLimit:limit});continue;
  }
  missing.push({stat,required,actual});
 }
 return{ok:missing.length===0,reason:missing.length?'EQUIPMENT_REQUIREMENTS_NOT_MET':'OK',slot:equipment.slot,missing,handsRequired:equipment.slot==='weapon'?(twoHanded?2:1):null,twoHanded};
}
function equipmentRequirementLabel(equipment){const rows=Object.entries(equipment?.required||{}).filter(([,v])=>Number(v)>0);return rows.length?rows.map(([k,v])=>`${k} ${v}`).join(' / '):'条件なし'}
function equipmentBonusLabel(equipment){const b=equipment?.bonuses||{},rows=[];if(b.attack)rows.push(`攻撃 +${b.attack}`);if(b.hpBonus)rows.push(`HP +${b.hpBonus}`);if(b.mpBonus)rows.push(`MP +${b.mpBonus}`);if(b.accuracy)rows.push(`命中値 +${b.accuracy}`);if(b.evasion)rows.push(`回避値 +${b.evasion}`);if(b.magicWeaponBonus)rows.push(`魔法武器Contribution +${b.magicWeaponBonus}`);if(b.weaponCriticalRate)rows.push(`武器Crit値 ${Math.round(b.weaponCriticalRate*10000)/100}%`);return rows.length?rows.join(' / '):'数値補正なし'}
...
function starterEquipmentDefinition(ref,path){const equipment=equipmentDefinition(ref);if(!equipment)throw new Error(`${path} のEquipmentが正式Masterに存在しません: ${ref}`);return equipment}
function applyStarterEquipmentConfiguration(character,configured,path='starter_equipment'){
 initializeCharacterInventoryEquipmentState(character);const source=configured&&typeof configured==='object'&&!Array.isArray(configured)?configured:{};
 for(const slot of CHARACTER_EQUIPMENT_SLOTS.slice(2)){
  const ref=String(source[slot]||'').trim();if(!ref)continue;const equipment=starterEquipmentDefinition(ref,`${path}.${slot}`),validSlot=equipment.slot==='ring'?['ring1','ring2'].includes(slot):equipment.slot===slot;if(!validSlot)throw new Error(`${path}.${slot} のEquipment部位が一致しません: ${ref} (${equipment.slot||'未定義'})`);const gate=equipmentRequirementCheck(character,equipment);if(!gate.ok)throw new Error(`${path}.${slot} の装備条件を満たしません: ${ref} / ${equipmentRequirementFailureLabel(gate)}`);character.equipment[slot]=ref;
 }
 const weaponSlots=['weapon1','weapon2'],weaponRows=weaponSlots.map(slot=>({slot,ref:String(source[slot]||'').trim()})).filter(row=>row.ref);
 if(!weaponRows.length)return character;
 const prepared=weaponRows.map(row=>{const equipment=starterEquipmentDefinition(row.ref,`${path}.${row.slot}`);if(equipment.slot!=='weapon')throw new Error(`${path}.${row.slot} はWeaponではありません: ${row.ref}`);const gate=equipmentRequirementCheck(character,equipment);if(!gate.ok)throw new Error(`${path}.${row.slot} の装備条件を満たしません: ${row.ref} / ${equipmentRequirementFailureLabel(gate)}`);return{...row,equipment,gate}});
 if(prepared.length===1){const row=prepared[0];if(equipmentIsBow(row.equipment)||equipmentIsQuiver(row.equipment))throw new Error(`${path} の弓系初期装備はBow+Quiverの2枠構成が必要です。`);if(equipmentIsShield(row.equipment))throw new Error(`${path} の盾は攻撃武器との2枠構成が必要です。`);if(row.gate.handsRequired===2){character.equipment.weapon1=row.ref;character.equipment.weapon2=null;setCharacterWeaponStyle(character,'two_hand')}else{character.equipment[row.slot]=row.ref;setCharacterWeaponStyle(character,'single')}return character}
 if(prepared.some(row=>row.gate.handsRequired===2))throw new Error(`${path} で両手持ちEquipmentと別Weaponを同時に指定できません。`);
 const [first,second]=prepared,e1=first.equipment,e2=second.equipment;character.equipment.weapon1=first.slot==='weapon1'?first.ref:second.ref;character.equipment.weapon2=first.slot==='weapon2'?first.ref:second.ref;
 if(equipmentIsBow(e1)||equipmentIsQuiver(e1)||equipmentIsBow(e2)||equipmentIsQuiver(e2)){if(!equipmentIsBowQuiverPair(e1,e2))throw new Error(`${path} の弓系初期装備はBow+Quiverの組合せにしてください。`);setCharacterWeaponStyle(character,'bow_quiver');return character}
 const shieldCount=[e1,e2].filter(equipmentIsShield).length;if(shieldCount){if(shieldCount!==1)throw new Error(`${path} では盾を2枚同時装備できません。`);setCharacterWeaponStyle(character,'weapon_shield');return character}
 if(!characterCanDualWield(character))throw new Error(`${path} の2武器構成には二刀流パッシブが必要です。`);setCharacterWeaponStyle(character,'dual_wield');return character
}
...
function refreshCharacterWeaponStyle(character){normalizeCharacterEquipmentState(character);const w1=String(character.equipment.weapon1||''),w2=String(character.equipment.weapon2||'');if(!w1&&!w2)return setCharacterWeaponStyle(character,'single');if(w1&&w2){const e1=equipmentDefinition(w1),e2=equipmentDefinition(w2);return setCharacterWeaponStyle(character,equipmentIsBowQuiverPair(e1,e2)?'bow_quiver':equipmentIsShield(e1)||equipmentIsShield(e2)?'weapon_shield':'dual_wield');}return setCharacterWeaponStyle(character,'single')}
...
function applyEquipmentToCharacterState(character,equipment,ref,targetSlot=''){
 ...
 if(equipment.slot==='weapon'){
  if(gate.handsRequired===2){returned.push(...removeEquippedWeaponState(character));character.equipment.weapon1=ref;character.equipment.weapon2=null;setCharacterWeaponStyle(character,'two_hand');targetSlot='twohand';}
   ...
   const otherRef=character.equipment[otherSlot],otherEquipment=equipmentDefinition(otherRef);if(otherRef&&!equipmentIsShield(equipment)&&!equipmentIsShield(otherEquipment)&&!equipmentIsBowQuiverPair(equipment,otherEquipment)&&!characterCanDualWield(character))return{ok:false,reason:'DUAL_WIELD_PASSIVE_REQUIRED',capability:'DUAL_WIELD',equipment,ref};
  ...
  if(!CHARACTER_EQUIPMENT_SLOTS.includes(slot))return{ok:false,reason:'EQUIPMENT_SLOT_UNSUPPORTED',equipment,ref};
  const previous=character.equipment[slot]||null;if(previous)returned.push(previous);character.equipment[slot]=ref;targetSlot=slot;
 }
 return{ok:true,equipment,slot:targetSlot,previous:returned[0]||null,returned,ref,handsRequired:gate.handsRequired,twoHanded:gate.twoHanded};
}
const EQUIPMENT_COMPARISON_FIELDS=Object.freeze([
 {key:'maxHp',label:'最大HP',format:'number'},
 {key:'maxMp',label:'最大MP',format:'number'},
 {key:'attack',label:'攻撃',format:'number'},
 {key:'accuracy',label:'命中',format:'number'},
 {key:'evasion',label:'回避',format:'number'},
 {key:'magicAccuracy',label:'魔法命中',format:'number'},
 {key:'magicResistance',label:'魔法抵抗',format:'number'},
 {key:'magicWeaponBonus',label:'魔法武器補正',format:'number'},
 {key:'weaponCriticalRate',label:'武器Critical率',format:'rate'},
 {key:'blockRate',label:'Block率',format:'rate'},
 {key:'blockDamageCutRate',label:'Block軽減率',format:'rate'}
]);
const EQUIPMENT_MOD_TARGET_LABEL=Object.freeze({MAX_HP:'最大HP',MAX_MP:'最大MP',PHYSICAL_DAMAGE:'物理Damage',MAGIC_DAMAGE:'魔法Damage',ACCURACY:'命中',MAGIC_ACCURACY:'魔法命中',EVASION:'回避',MAGIC_RESISTANCE:'魔法抵抗',BLOCK_RATE:'Block率',BLOCK_PERFORMANCE:'Block性能',CRITICAL_RATE:'Critical率',CRITICAL_DAMAGE:'Critical Damage',MP_COST_REDUCTION:'MP消費軽減',COOLDOWN_REDUCTION:'Cooldown短縮',CAST_TIME_REDUCTION:'CastTime短縮',ACTION_GAUGE_GAIN:'Action Gauge増加量'});
function equipmentComparisonNumber(value){const n=Number(value);if(!Number.isFinite(n))return 0;return Math.round(n*100)/100}
function equipmentComparisonValueText(value,format){const n=equipmentComparisonNumber(value);if(format==='rate')return`${Math.round(n*10000)/100}%`;return Number.isInteger(n)?String(n):String(n)}
function equipmentComparisonRows(beforeValues,afterValues){return EQUIPMENT_COMPARISON_FIELDS.map(def=>{const before=equipmentComparisonNumber(beforeValues?.[def.key]),after=equipmentComparisonNumber(afterValues?.[def.key]),delta=equipmentComparisonNumber(after-before),changed=Math.abs(delta)>1e-9,sign=delta>0?'+':'';return{key:def.key,label:def.label,format:def.format,before,after,delta,changed,direction:delta>0?'increase':delta<0?'decrease':'same',before_text:equipmentComparisonValueText(before,def.format),after_text:equipmentComparisonValueText(after,def.format),delta_text:changed?`${sign}${equipmentComparisonValueText(delta,def.format)}`:'±0'}})}
function equipmentModifierDisplayRows(equipment){if(!equipment)return[];if(!window.GKSFormalContribution)throw new Error('Formal Contribution Resolverを読み込めません。');return(Array.isArray(equipment.mod_ids)?equipment.mod_ids:[]).map(rawId=>{const id=String(rawId||'').trim(),candidate=formalModCatalog.get(id);if(!candidate)throw Object.assign(new Error(`Equipment ${equipment.id}が参照するGame MOD Candidateがありません: ${id}`),{code:'GAME_EQUIPMENT_MOD_CANDIDATE_MISSING',equipment_id:equipment.id,modifier_id:id});const checked=window.GKSFormalContribution.validateModCandidate(candidate),d=checked.definition,target=String(d.target||'').toUpperCase(),operation=String(d.operation||'').toUpperCase(),value=Number(checked.balance.value);let valueText=String(value);if(operation==='RELATIVE_PERCENT')valueText=`${value>=0?'+':''}${Math.round(value*10000)/100}%`;else if(operation==='FLAT_ADD')valueText=`${value>=0?'+':''}${equipmentComparisonNumber(value)}`;return{id,target,operation,value,label:`${EQUIPMENT_MOD_TARGET_LABEL[target]||target} ${valueText}`,detail:`${id} / ${operation}`}})}
function previewCharacterEquipmentEquip(character,ref,targetSlot=''){
 const equipment=equipmentDefinition(ref);if(!equipment)return{ok:false,reason:'EQUIPMENT_NOT_FOUND',reason_label:'正式装備定義がありません。',ref};
 const gate=equipmentRequirementCheck(character,equipment);if(!gate.ok)return{ok:false,...gate,equipment,ref,reason_label:equipmentRequirementFailureLabel(gate)};
 const beforeValues=characterBattleValues(character),candidate=JSON.parse(JSON.stringify(character)),applied=applyEquipmentToCharacterState(candidate,equipment,ref,targetSlot);if(!applied.ok)return{...applied,reason_label:equipmentRequirementFailureLabel(applied)};
 const afterValues=characterBattleValues(candidate),slotLabel=applied.slot==='twohand'?`${equipmentSlotLabel('weapon1')}＋${equipmentSlotLabel('weapon2')}（両手持ち）`:equipmentSlotLabel(applied.slot),warning=bowQuiverActionRequirementReason(candidate);
 return{...applied,ok:true,character_id:String(character?.id||''),character_name:String(character?.name||''),equipment_id:equipment.id,equipment_name:equipment.name,slot_label:slotLabel,before_values:beforeValues,after_values:afterValues,comparison_rows:equipmentComparisonRows(beforeValues,afterValues),modifiers:equipmentModifierDisplayRows(equipment),returned_items:applied.returned.map(id=>({id,name:equipmentDisplayName(id)})),action_warning:warning,action_warning_slot:bowQuiverMissingSlot(candidate),preview_character:candidate};
}
function equipInventoryItemToCharacter(character,index,targetSlot=''){
 const ref=String(data.inventory[index]||''),equipment=equipmentDefinition(ref),result=applyEquipmentToCharacterState(character,equipment,ref,targetSlot);if(!result.ok)return result;
 data.inventory.splice(index,1);if(result.returned.length)data.inventory.push(...result.returned);return result;
}
function formalModifierCandidateId(candidate){return String(candidate?.definition?.id??candidate?.id??'').trim()}
function replaceFormalModifierSourceCatalog(input){const {mods=[]}=input||{};if(!Array.isArray(mods))throw new Error('Formal MOD source catalogは配列で指定してください。');formalModCatalog.clear();for(const row of mods){const id=formalModifierCandidateId(row);if(!id)throw new Error('Formal MOD Candidate IDがありません。');if(formalModCatalog.has(id))throw new Error(`Formal MOD Candidate ID duplicated: ${id}`);formalModCatalog.set(id,clone(row))}return{mod_ids:[...formalModCatalog.keys()]}}
function formalModifierEntry(candidate,sourceType,sourceId){if(!window.GKSFormalContribution)throw new Error('Formal Contribution Resolverを読み込めません。');const checked=window.GKSFormalContribution.validateModCandidate(candidate),d=checked.definition;if(d.enabled===false)return null;if(d.effect_type!=='numeric_modifier')throw Object.assign(new Error(`Game Battle modifier source ${d.id}のeffect_typeは未対応です: ${d.effect_type}`),{code:'GAME_MODIFIER_SOURCE_EFFECT_UNSUPPORTED',modifier_id:d.id});return{source_type:String(sourceType).toUpperCase(),source_id:String(sourceId),modifier_id:String(d.id),target_stat:String(d.target).toUpperCase(),mode:String(d.operation).toUpperCase(),raw_modifier_value:Number(checked.balance.value)}}
function resolveFormalModifierRows(base,target,rows){if(!window.GKSFormalContribution?.resolveCombatStatContributions)throw new Error('Formal Combat Contribution Resolverを読み込めません。');return window.GKSFormalContribution.resolveCombatStatContributions(base,target,rows,{errorPrefix:'Game'})}
function characterFormalModifierContributions(character){if(!window.GKSFormalContribution)throw new Error('Formal Contribution Resolverを読み込めません。');const out=[];for(const row of characterEquipmentEntries(character)){const equipment=equipmentDefinition(row.ref);if(!equipment||equipmentIsQuiver(equipment))continue;for(const modId of Array.isArray(equipment.mod_ids)?equipment.mod_ids:[]){const candidate=formalModCatalog.get(String(modId));if(!candidate)throw Object.assign(new Error(`Equipment ${equipment.id}が参照するGame MOD Candidateがありません: ${modId}`),{code:'GAME_EQUIPMENT_MOD_CANDIDATE_MISSING',equipment_id:equipment.id,modifier_id:String(modId)});const entry=formalModifierEntry(candidate,'MOD',equipment.id);if(entry)out.push(entry)}}const selection=typeof characterFormalPassiveSelection==='function'?characterFormalPassiveSelection(character):{activePassives:[]};for(const passive of selection.activePassives){const refs=passive?.runtimeContracts?.modifierRefs?.modIds;if(!Array.isArray(refs))throw Object.assign(new Error(`Passive ${passive?.id||'(unknown)'}のruntimeContracts.modifierRefs.modIdsが不正です。`),{code:'GAME_PASSIVE_MOD_REFS_INVALID',passive_id:String(passive?.id||'')});for(const modId of refs){const candidate=formalModCatalog.get(String(modId));if(!candidate)throw Object.assign(new Error(`Passive ${passive.id}が参照するGame MOD Candidateがありません: ${modId}`),{code:'GAME_PASSIVE_MOD_CANDIDATE_MISSING',passive_id:passive.id,modifier_id:String(modId)});const entry=formalModifierEntry(candidate,'PASSIVE',passive.id);if(entry)out.push(entry)}}return out}
async function loadFormalEquipmentDefinitions(){
 formalEquipmentBridge.status='loading';formalEquipmentBridge.errors=[];
 try{const [equipmentResponse,modResponse]=await Promise.all([fetch(FORMAL_EQUIPMENT_EXPORT_URL,{cache:'no-store'}),fetch(FORMAL_MOD_EXPORT_URL,{cache:'no-store'})]);if(!equipmentResponse.ok)throw new Error(`Equipment HTTP ${equipmentResponse.status}`);if(!modResponse.ok)throw new Error(`Equipment MOD HTTP ${modResponse.status}`);const payload=await equipmentResponse.json(),modPayload=await modResponse.json(),rows=Array.isArray(payload)?payload:(Array.isArray(payload?.data)?payload.data:[]),modRows=Array.isArray(modPayload)?modPayload:(Array.isArray(modPayload?.data)?modPayload.data:[]),next=new Map(),errors=[];for(const raw of rows){const record=normalizeFormalEquipmentRecord(raw);if(!record){errors.push(`Equipment record invalid: ${String(raw?.id||'(no id)')}`);continue}if(next.has(record.id)){errors.push(`Equipment ID duplicated: ${record.id}`);continue}next.set(record.id,record)}if(errors.length)throw new Error(errors.join(' / '));formalEquipmentCatalog.clear();for(const [id,row] of next)formalEquipmentCatalog.set(id,row);replaceFormalModifierSourceCatalog({mods:modRows});formalEquipmentBridge.status='loaded';formalEquipmentBridge.schema_version=payload?.schema_version||null;formalEquipmentBridge.data_version=payload?.data_version||null;formalEquipmentBridge.generated_by=payload?.generated_by||null;formalEquipmentBridge.loaded_at=new Date().toISOString();formalEquipmentBridge.imported_ids=[...formalEquipmentCatalog.keys()];formalEquipmentBridge.imported_mod_ids=[...formalModCatalog.keys()];return formalEquipmentBridge;}catch(error){formalEquipmentCatalog.clear();formalModCatalog.clear();formalEquipmentBridge.status='failed';formalEquipmentBridge.errors=[String(error?.message||error)];return formalEquipmentBridge;}
}
async function loadFormalPassiveDefinitions(){
 formalPassiveBridge.status='loading';formalPassiveBridge.errors=[];try{const response=await fetch(FORMAL_PASSIVE_EXPORT_URL,{cache:'no-store'});if(!response.ok)throw new Error(`Passive HTTP ${response.status}`);const payload=await response.json(),rows=Array.isArray(payload)?payload:(Array.isArray(payload?.data)?payload.data:[]),next=new Map();for(const raw of rows){const checked=window.GKSFormalContribution.validateFormalPassive(raw,{requireRuntimeContracts:true});if(next.has(checked.id))throw new Error(`Passive ID duplicated: ${checked.id}`);next.set(checked.id,checked)}formalPassiveCatalog.clear();for(const [id,row] of next)formalPassiveCatalog.set(id,row);formalPassiveBridge.status='loaded';formalPassiveBridge.schema_version=payload?.schema_version||null;formalPassiveBridge.data_version=payload?.data_version||null;formalPassiveBridge.generated_by=payload?.generated_by||null;formalPassiveBridge.loaded_at=new Date().toISOString();formalPassiveBridge.imported_ids=[...formalPassiveCatalog.keys()];return formalPassiveBridge}catch(error){formalPassiveCatalog.clear();formalPassiveBridge.status='failed';formalPassiveBridge.errors=[String(error?.message||error)];return formalPassiveBridge}
}
window.GKGameJobService=Object.freeze({version:'GS-03-2',list:()=>formalJobDomainCatalog?[...formalJobDomainCatalog.rows]:[],resolve:jobDefinition,evaluateGuildEditState:currentGuildEditState});
window.GKGameFormalConfig=Object.freeze({bridge:formalGameBridge,jobs:formalJobCatalog,passives:formalPassiveCatalog,passiveBridge:formalPassiveBridge,load:loadFormalGameDefinitions,loadPassives:loadFormalPassiveDefinitions,jobDefinition,jobDisplayName,partyMaxSize,characterBattleValues,battleFlowConfig,battleGaugeMax,battleAiReevaluationRatio,battleAiReevaluationIntervalTicks,battleGaugeConsumeAmount,battleDefaultSkillCastingTicks,weaponStrTwoHandRequirementMultiplier,skillProgressionBalance});
window.GKGameEquipmentRuntime=Object.freeze({bridge:formalEquipmentBridge,slots:CHARACTER_EQUIPMENT_SLOTS,slotLabels:CHARACTER_EQUIPMENT_SLOT_LABEL,slotLabel:equipmentSlotLabel,weaponStyles:CHARACTER_WEAPON_STYLES,weaponStyle:characterWeaponStyle,normalizeRecord:normalizeFormalEquipmentRecord,definition:equipmentDefinition,weaponBaseItemRuntimeSnapshot,requirementCheck:equipmentRequirementCheck,requirementFailureLabel:equipmentRequirementFailureLabel,bonusLabel:equipmentBonusLabel,requirementLabel:equipmentRequirementLabel,normalizeCharacterState:normalizeCharacterEquipmentState,equipmentContribution:characterEquipmentContribution,equipTargets:equipmentEquipTargets,previewEquip:previewCharacterEquipmentEquip,modifierDisplay:equipmentModifierDisplayRows,basicAttackProfiles:characterBasicAttackProfiles,bowQuiverMissingSlot,bowQuiverActionRequirementReason,equipmentTagIds:characterEquipmentTagIds,combatCapabilities:characterCombatCapabilities,canDualWield:characterCanDualWield,replaceModifierSourceCatalog:replaceFormalModifierSourceCatalog,modifierContributions:characterFormalModifierContributions,load:loadFormalEquipmentDefinitions});
...
async function loadFormalAiCatalog(){if(!window.GKGameAICatalogLoader?.prime)return null;try{return await GKGameAICatalogLoader.prime();}catch(error){console.warn('[Formal AI Catalog]',String(error?.message||error));return null;}}
const formalDefinitionsReady=Promise.all([loadFormalGameDefinitions(),loadFormalEquipmentDefinitions(),loadFormalPassiveDefinitions(),loadFormalAiCatalog()]);
...
function setPhase(next,options={}){
 ...
 if(next==='title'&&typeof refreshTitleBackupRecovery==='function')refreshTitleBackupRecovery();
...
function initializeNewGameInventoryAndEquipment(candidate,cfg){
 ...
 const starterEquipmentInstances=GKNewGameDomain.buildStarterEquipmentInstances(candidate.characters,cfg.starter_roster,{issueId:issueNewGameEquipmentInstanceId,equipmentExists:id=>formalEquipmentCatalog.has(String(id||''))});
 GKNewGameDomain.validateStarterEquipmentInstances(candidate.characters,cfg.starter_roster,starterEquipmentInstances,{equipmentExists:id=>formalEquipmentCatalog.has(String(id||''))});
}
...
function normalizeFormationPosition(value,{fallback='FRONTLINE'}={}){const v=String(value||'').toUpperCase();return FORMATION_POSITIONS.includes(v)?v:fallback}
function characterFormationPosition(character){return normalizeFormationPosition(character?.formation_position,{fallback:'FRONTLINE'})}
function setCharacterFormationPosition(character,value){if(!character)return null;character.formation_position=normalizeFormationPosition(value,{fallback:'FRONTLINE'});return character.formation_position}
function makeCharacterCore(name,jobRef,{now=new Date().toISOString()}={}){const cfg=requireFormalRuntimeSettings().character,job=jobDefinition(jobRef);if(!job)throw new Error(`正式Jobが見つかりません: ${jobRef}`);return{id:uid(),name,level:cfg.initial_level,job:job.id,base_hp:cfg.initial_hp,base_mp:cfg.initial_mp,stats:{...cfg.initial_stats},skillPoints:cfg.initial_skill_points,skillPointSpend:{skill:{},passive:{}},jobHistory:[{job:job.id,level:cfg.initial_level,at:now}],growthHistory:[],createdAt:now,formation_position:cfg.initial_formation_position}}
...
function initializeCharacterSkillPassiveAiState(character){const cfg=requireFormalRuntimeSettings().character,skills=[...cfg.starter_skill_ids];character.skills=skills;character.equippedSkillId=cfg.starter_equipped_skill_id||skills[0]||'';character.skillLoadoutIds=[];character.skillPoints=cfg.initial_skill_points;character.skillPointSpend={skill:{},passive:{}};character.passives=[...cfg.starter_passive_ids];character.passiveIds=[...cfg.starter_passive_ids];character.formalAiBinding=null;return window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeCharacterSkillState(character,{fallback:[]}):character}
...
function normalize(raw){
 ...
 raw.characters.forEach(c=>{
  const resolvedJob=jobDefinition(c.job);if(formalGameBridge.status==='loaded'&&!resolvedJob)throw new Error(`${c.name}のJob参照が正式Job Masterにありません: ${c.job}`);if(resolvedJob)c.job=resolvedJob.id;const maxLevel=formalRuntimeSettings?.character?.max_level;if(!c.id||typeof c.name!=='string'||!c.name.trim()||!Number.isInteger(c.level)||c.level<1||(maxLevel!=null&&c.level>maxLevel))throw new Error('キャラクターデータが不正です。');
  ...
  if(!Number.isInteger(c.base_hp)||c.base_hp<1)throw new Error(`${c.name}のbase_hpが不正です。`);
  if(!Number.isInteger(c.base_mp)||c.base_mp<0)throw new Error(`${c.name}のbase_mpが不正です。`);
  ...
  if(window.GKGameSkillAcquisitionRuntime)GKGameSkillAcquisitionRuntime.normalizeCharacterState(c);else{c.skillPoints=Number.isInteger(c.skillPoints)&&c.skillPoints>=0?c.skillPoints:0;c.skillPointSpend=c.skillPointSpend&&typeof c.skillPointSpend==='object'?c.skillPointSpend:{skill:{},passive:{}};c.passiveIds=Array.isArray(c.passiveIds)?[...new Set(c.passiveIds.map(id=>String(id||'').trim()).filter(Boolean))]:[];c.passives=Array.isArray(c.passives)?[...new Set(c.passives.map(id=>String(id||'').trim()).filter(Boolean))]:[...c.passiveIds];c.passiveIds=c.passiveIds.filter(id=>c.passives.includes(id));}
  if(c.passiveIds.length>skillLoadoutBalance().passive_slots)throw new Error(`${c.name}のPassive選択数が設定枠を超えています。`);
 if(window.GKGameSkillLoadout){const passiveCheck=GKGameSkillLoadout.validatePassiveLoadout(c,{capacity:skillLoadoutBalance().passive_slots,resolvePassive:id=>formalPassiveCatalog.get(id)||null,validateSelection:ids=>{GKSFormalContribution.validatePassiveSelection([...formalPassiveCatalog.values()],ids,characterAbilityStats(c),{requireRuntimeContracts:true});return{ok:true}}});if(!passiveCheck.ok)throw Object.assign(new Error(`${c.name}のPassive選択が不正です: ${passiveCheck.reason}`),{code:passiveCheck.reason,details:passiveCheck});}
 ...
 const maxParty=partyMaxSize();const eligibleParty=Array.isArray(raw.partyIds)?raw.partyIds.filter(id=>raw.characters.some(c=>c.id===id)):raw.characters.map(c=>c.id);raw.partyIds=maxParty?eligibleParty.slice(0,maxParty):eligibleParty;
 ...
 raw.characters.forEach(c=>{normalizeCharacterEquipmentState(c);c.formalAiBinding=window.GKGameAISaveBridge?GKGameAISaveBridge.normalizeBinding(c.formalAiBinding):null;});
 raw.schemaRevision='1.6.0';raw.gameVersion=APP_RUNTIME_GAME_BUILD;
 return raw;
}
function buildAutoSaveSnapshot(source){
 ...
 const questRunBefore=window.GKGameQuestRunSaveBridge?GKGameQuestRunSaveBridge.capture(staged):null;
 ...
 if(questRunBefore&&window.GKGameQuestRunSaveBridge)GKGameQuestRunSaveBridge.assertCapturedPreserved(questRunBefore,staged);
 ...
 if(questRunBefore&&window.GKGameQuestRunSaveBridge)GKGameQuestRunSaveBridge.assertCapturedPreserved(questRunBefore,snapshot);
 return snapshot;
}
...
function validateSavePayload(payload){
 ...
 if(!window.GKGameQuestRunSaveBridge||typeof GKGameQuestRunSaveBridge.assertStore!=='function')throw new Error('Save Integrity: QuestRun Save Bridge is unavailable.');
 GKGameQuestRunSaveBridge.assertStore(clone(parsed));
 return parsed;
}
...
function normalizeLoadedSave(source){
 ...
 if(window.GKGameQuestRunSaveBridge)GKGameQuestRunSaveBridge.assertPreserved(before,normalized);
...
function saveBoundaryWrite(operation,key,value,phase){const Boundary=window.GKRuntimeBoundaryContracts;if(Boundary)return Boundary.persistenceWrite(localStorage,{operation,key,value,boundary:'C05',phase,owner:'game/assets/js/app-runtime.js'});if(operation==='remove')localStorage.removeItem(key);else localStorage.setItem(key,String(value));return true}
function currentSaveBoundaryDescriptor(snapshot=data){const Boundary=window.GKRuntimeBoundaryContracts;if(!Boundary)return null;const state=snapshot&&typeof snapshot==='object'?snapshot:{};return Boundary.createSaveDescriptor({saveId:SAVE_KEY,saveVersion:SAVE_VERSION,schemaRevision:String(state.schemaRevision||''),gameVersion:String(state.gameVersion||APP_RUNTIME_GAME_BUILD)})}
...
function readCurrentAutoSaveBackup(){
 const raw=localStorage.getItem(SAVE_BACKUP_KEY);if(raw===null)return null;const root=parseSaveRoot(raw);if(Number(root.saveVersion)!==SAVE_VERSION)throw new Error(`Save Recovery: backup Save VersionがCurrentではありません (${root.saveVersion}).`);validateSavePayload(raw);return{raw,save:normalizeLoadedSave(root)};
}
function currentAutoSaveBackupInfo(){try{const candidate=readCurrentAutoSaveBackup();if(!candidate)return null;return{available:true,character_count:candidate.save.characters.length,updated_at:String(candidate.save.updatedAt||''),game_version:String(candidate.save.gameVersion||'')}}catch(error){return{available:false,error:String(error?.message||error)}}}
...
function restoreAutoSaveBackup(){
 ...
 try{const stageRecord=buildSaveStageRecord(candidate.raw);const verified=verifySaveStageRecord(stageRecord);if(verified!==candidate.raw)throw new Error('Save Recovery: staged backup changed before restore.');saveBoundaryWrite('set',SAVE_KEY,verified,'backup_restore_commit');mainSwitched=true;if(localStorage.getItem(SAVE_KEY)!==verified)throw new Error('Save Recovery: restored payload verification failed.');validateSavePayload(localStorage.getItem(SAVE_KEY));return candidate.save;}
 catch(error){if(mainSwitched){try{if(previous===null)saveBoundaryWrite('remove',SAVE_KEY,null,'backup_restore_rollback');else saveBoundaryWrite('set',SAVE_KEY,previous,'backup_restore_rollback')}catch(rollbackError){console.error('Save recovery main rollback failed',rollbackError)}}throw error;}
}
async function restoreBackupAndContinue(){
 ...
 try{restoreAutoSaveBackup();refreshTitleBackupRecovery();const ok=await continueGame();if(ok)notify('直前バックアップからセーブデータを復旧しました。','ok');return ok}catch(error){showSaveRecovery('load',error,{retry:restoreBackupAndContinue});return false}
}
...
function commitMigratedSave(raw,result,{sourceKey=SAVE_KEY}={}){
 ...
 try{
  saveBoundaryWrite('set',SAVE_MIGRATION_BACKUP_KEY,raw,'migration_backup');
  ...
  saveBoundaryWrite('set',SAVE_KEY,verifiedPayload,'migration_main_commit');
  ...
  try{if(previousCurrent===null)saveBoundaryWrite('remove',SAVE_KEY,null,'migration_main_rollback');else saveBoundaryWrite('set',SAVE_KEY,previousCurrent,'migration_main_rollback')}catch(rollbackError){console.error('Save migration rollback failed',rollbackError);}
  try{if(previousMigrationBackup===null)saveBoundaryWrite('remove',SAVE_MIGRATION_BACKUP_KEY,null,'migration_backup_rollback');else saveBoundaryWrite('set',SAVE_MIGRATION_BACKUP_KEY,previousMigrationBackup,'migration_backup_rollback')}catch(backupRollbackError){console.error('Save migration backup rollback failed',backupRollbackError);}
  throw error;
 }
}
...
function adventurePlaybackEventText(event,unitNames={}){
 ...
 if(type==='damage'){const barrier=Math.max(0,Number(p.shield_absorbed)||0);return`${target||'対象'}に${value}HPダメージ${barrier?`（BARRIER吸収 ${barrier}）`:''}${hpAfter===null?'':`（残HP ${hpAfter}）`}`;}if(type==='heal')return`${target||'対象'}を${value}回復${hpAfter===null?'':`（残HP ${hpAfter}）`}`;
...
function closeAdventurePlaybackDetail(){stopAdventureBattleDetailPlayback();stopAdventureSceneDetailPlayback();adventureSceneDetailState=null;adventurePlaybackDetailItemIndex=null;const detail=$('adventurePlaybackDetail');if(detail){detail.classList.add('hidden');detail.innerHTML='';}restoreAdventurePlaybackDetailPosition();}
let adventureBattleDetailTimer=0;
let adventureSceneDetailTimer=0;
let adventureSceneDetailState=null;
...
function stopAdventureSceneDetailPlayback(){if(adventureSceneDetailTimer){clearInterval(adventureSceneDetailTimer);adventureSceneDetailTimer=0;}}
function mountAdventureSceneDetailPlayback(detail,run,sceneIndex){
 stopAdventureSceneDetailPlayback();const scene=run?.scene_snapshots?.[Number(sceneIndex)]||{},dialogues=Array.isArray(scene.dialogues)?scene.dialogues:[],stateKey=`${String(run?.quest_run_id||'')}:${Number(sceneIndex)}`;if(!adventureSceneDetailState||adventureSceneDetailState.key!==stateKey)adventureSceneDetailState={key:stateKey,cursor:0,history:false};const state=adventureSceneDetailState;
 const persistRead=index=>{if(!dialogues.length)return;const before=GKAdventureStorySystem.sceneReadState(run,sceneIndex).read_count,after=GKAdventureStorySystem.markSceneDialogueRead(run,sceneIndex,index).read_count;if(after!==before)autoSave();};
 const draw=()=>{const current=detail.querySelector('[data-adventure-scene-current]'),counter=detail.querySelector('[data-adventure-scene-counter]'),history=detail.querySelector('[data-adventure-scene-history]'),auto=detail.querySelector('[data-adventure-scene-auto]');if(!current)return;if(!dialogues.length){current.innerHTML='<div class="small">会話Snapshotはありません。</div>';if(counter)counter.textContent='0 / 0';return;}state.cursor=Math.max(0,Math.min(dialogues.length-1,Number(state.cursor)||0));persistRead(state.cursor);const line=dialogues[state.cursor]||{};current.innerHTML=`<div class="adventure-scene-line"><b>${escapeHtml(line.speaker||line.character_name||'')}</b><div>${escapeHtml(line.text||line.body||'')}</div></div>`;if(counter)counter.textContent=`${state.cursor+1} / ${dialogues.length}`;detail.querySelector('[data-adventure-scene-next]')?.toggleAttribute('disabled',state.cursor>=dialogues.length-1);const read=GKAdventureStorySystem.sceneReadState(run,sceneIndex);detail.querySelector('[data-adventure-scene-skip]')?.toggleAttribute('disabled',state.cursor>=dialogues.length-1||state.cursor>=read.read_count);if(auto)auto.textContent=adventureSceneDetailTimer?'Auto停止':'Auto';if(history){history.classList.toggle('hidden',!state.history);history.innerHTML=state.history?GKAdventureStorySystem.sceneDialogueHistory(run,sceneIndex).map((row,i)=>`<div class="adventure-scene-line"><span class="small">${i+1}</span> <b>${escapeHtml(row.speaker||row.character_name||'')}</b><div>${escapeHtml(row.text||row.body||'')}</div></div>`).join(''):'';}};
 const next=()=>{if(state.cursor>=dialogues.length-1){stopAdventureSceneDetailPlayback();draw();return;}state.cursor++;draw();};
 detail.querySelector('[data-adventure-scene-next]')?.addEventListener('click',()=>{stopAdventureSceneDetailPlayback();next();});detail.querySelector('[data-adventure-scene-auto]')?.addEventListener('click',()=>{if(adventureSceneDetailTimer){stopAdventureSceneDetailPlayback();draw();return;}if(!dialogues.length)return;adventureSceneDetailTimer=setInterval(next,900);draw();});detail.querySelector('[data-adventure-scene-skip]')?.addEventListener('click',()=>{stopAdventureSceneDetailPlayback();const nextIndex=GKAdventureStorySystem.nextSceneDialogueIndex(run,sceneIndex,state.cursor,{skipRead:true});if(nextIndex>=0)state.cursor=nextIndex;draw();});detail.querySelector('[data-adventure-scene-history-toggle]')?.addEventListener('click',()=>{state.history=!state.history;draw();});draw();
}
function mountAdventureBattleDetailPlayback(detail,battleResult){
 stopAdventureBattleDetailPlayback();const events=Array.isArray(battleResult?.playback_events)?battleResult.playback_events:[],units=Array.isArray(battleResult?.unit_final_state)?battleResult.unit_final_state:[],unitNames=Object.fromEntries(units.map(u=>[String(u.id||''),String(u.name||u.id||'')]));let cursor=events.length?0:-1,running=false;
 const savedPresentationSettings=()=>globalThis.GKPlayerSettingsDomain?GKPlayerSettingsDomain.restoreFromSave(data):null;
 const speedValue=()=>{const selected=Number(detail.querySelector('[data-adventure-battle-speed]')?.value);if(Number.isFinite(selected)&&selected>0)return selected;return savedPresentationSettings()?.battle_speed||1};
 const draw=()=>{const current=detail.querySelector('[data-adventure-battle-current]'),history=detail.querySelector('[data-adventure-battle-events]'),counter=detail.querySelector('[data-adventure-battle-counter]'),play=detail.querySelector('[data-adventure-battle-play]'),skip=detail.querySelector('[data-adventure-battle-skip]');if(!current||!history)return;const e=events[cursor];current.innerHTML=e?`<div class="small">${escapeHtml(adventurePlaybackEventMeta(e))}</div><div>${escapeHtml(adventurePlaybackEventText(e,unitNames))}</div>`:'再生イベントはありません。';history.innerHTML=cursor>=0?events.slice(0,cursor+1).map((row,i)=>`<div class="adventure-battle-event ${i===cursor?'current':''}"><span>${i+1}</span><span class="small">${escapeHtml(Number.isFinite(Number(row?.at_tick))?`T${Number(row.at_tick)}`:'T-')}</span><span>${escapeHtml(adventurePlaybackEventText(row,unitNames))}</span></div>`).join(''):'<div class="small">再生イベントはありません。</div>';if(counter)counter.textContent=events.length?`${cursor+1} / ${events.length}`:'0 / 0';if(play){play.disabled=running||!events.length;play.textContent=running?'再生中':'再生';}if(skip)skip.disabled=!events.length||cursor>=events.length-1;history.querySelector('.current')?.scrollIntoView({block:'nearest'});};
 const stop=()=>{running=false;stopAdventureBattleDetailPlayback();draw();};
 const schedule=()=>{if(!running)return;if(cursor>=events.length-1){stop();return;}adventureBattleDetailTimer=setTimeout(()=>{adventureBattleDetailTimer=0;if(!running)return;cursor++;draw();schedule();},Math.max(100,Math.round(700/speedValue())));};
 detail.querySelector('[data-adventure-battle-prev]')?.addEventListener('click',()=>{running=false;stopAdventureBattleDetailPlayback();cursor=Math.max(0,cursor-1);draw();});detail.querySelector('[data-adventure-battle-next]')?.addEventListener('click',()=>{running=false;stopAdventureBattleDetailPlayback();cursor=Math.min(events.length-1,cursor+1);draw();});detail.querySelector('[data-adventure-battle-play]')?.addEventListener('click',()=>{if(running||!events.length)return;if(cursor>=events.length-1)cursor=0;running=true;draw();schedule();});
 const speedSelect=detail.querySelector('[data-adventure-battle-speed]');if(speedSelect){const saved=savedPresentationSettings();if(saved&&[...speedSelect.options].some(o=>Number(o.value)===saved.battle_speed))speedSelect.value=String(saved.battle_speed);speedSelect.addEventListener('change',()=>{if(!running)return;stopAdventureBattleDetailPlayback();schedule();});}
 detail.querySelector('[data-adventure-battle-skip]')?.addEventListener('click',()=>{running=false;stopAdventureBattleDetailPlayback();cursor=events.length?events.length-1:-1;draw();});draw();
}
function renderAdventurePlaybackDetail(run,itemIndex){
 ...
 if(view.kind==='scene'){
  ...
  body=`<div class="adventure-detail-head"><div><b>ストーリーイベント</b><div class="small">保存済み Scene Snapshot ／ ${escapeHtml(scene.scene_id||item.ref_id||'')}</div></div><button type="button" data-adventure-detail-close>閉じる</button></div><div class="adventure-scene-dialogues"><div data-adventure-scene-current>${dialogues.length?'':'<div class="small">会話Snapshotはありません。</div>'}</div><div class="adventure-battle-controls"><button type="button" class="primary" data-adventure-scene-next>会話送り</button><button type="button" data-adventure-scene-auto>Auto</button><button type="button" data-adventure-scene-skip>既読Skip</button><button type="button" data-adventure-scene-history-toggle>会話履歴</button><span class="small" data-adventure-scene-counter></span></div><div class="adventure-scene-history hidden" data-adventure-scene-history></div></div>`;
  ...
  body=`<div class="adventure-detail-head"><div><b>戦闘結果</b><div class="small">保存済みBattle Result / Playback Eventsのみを再生</div></div><button type="button" data-adventure-detail-close>閉じる</button></div><div class="adventure-battle-summary"><div class="adventure-battle-stat"><b>結果</b><div>${br.victory===false?'敗北':'勝利'}</div></div><div class="adventure-battle-stat"><b>行動数</b><div>${Number(stats.actions)||0}</div></div><div class="adventure-battle-stat"><b>味方与ダメージ</b><div>${Number(stats.ally_damage)||0}</div></div><div class="adventure-battle-stat"><b>敵与ダメージ</b><div>${Number(stats.enemy_damage)||0}</div></div></div>${enemySummary?`<div class="small"><b>出現モンスター</b>：${enemySummary}</div>`:''}<div class="small"><b>戦闘報酬</b>：${escapeHtml(adventureRewardDetail(br.reward))}</div>${units.length?`<div class="small">最終状態：${units.map(u=>`${escapeHtml(u.name||u.id||'unit')} HP ${Number(u.hp)||0}/${Number(u.max_hp)||0}`).join(' ／ ')}</div>`:''}<div class="adventure-battle-playback"><div class="adventure-battle-current" data-adventure-battle-current></div><div class="adventure-battle-controls"><button type="button" class="primary" data-adventure-battle-play>再生</button><label class="small">速度 <select data-adventure-battle-speed><option value="1" selected>標準</option><option value="2">×2</option><option value="4">×4</option></select></label><button type="button" data-adventure-battle-skip>戦闘Skip</button><button type="button" class="developer-only" data-adventure-battle-prev>前</button><button type="button" class="developer-only" data-adventure-battle-next>次</button><span class="small" data-adventure-battle-counter></span></div><div class="adventure-detail-events" data-adventure-battle-events></div></div>`;
 }else{
 ...
 detail.innerHTML=body;detail.classList.remove('hidden');placeAdventurePlaybackDetail(index,{scroll:true});if(view.kind==='scene')mountAdventureSceneDetailPlayback(detail,run,Number(item.result_index));if(view.kind==='battle')mountAdventureBattleDetailPlayback(detail,view.detail||{});detail.querySelector('[data-adventure-detail-close]')?.addEventListener('click',closeAdventurePlaybackDetail);
}
...
function reconcileFormalAdventureQuestSelection(){const quests=formalAdventureQuests();if(!quests.length){data.selectedQuestId='';return null}let selected=quests.find(q=>q.id===data.selectedQuestId);if(!selected){selected=quests[0];data.selectedQuestId=selected.id;autoSave()}return selected;}
...
function adventurePartyStartValidation(){const maxParty=partyMaxSize();if(!maxParty)return{ok:false,reason:'party_limit_unavailable'};const ids=(Array.isArray(data.partyIds)?data.partyIds:[]).map(String);if(!ids.length)return{ok:false,reason:'party_empty',max_party_size:maxParty};if(ids.length>maxParty)return{ok:false,reason:'party_size_exceeded',party_size:ids.length,max_party_size:maxParty};if(new Set(ids).size!==ids.length)return{ok:false,reason:'party_duplicate_member'};const missing=ids.filter(id=>!data.characters.some(c=>String(c?.id||'')===id));if(missing.length)return{ok:false,reason:'party_member_missing',missing_character_ids:missing};return{ok:true,party_size:ids.length,max_party_size:maxParty};}
function adventurePartySnapshot(){const maxParty=partyMaxSize(),ids=(Array.isArray(data.partyIds)?data.partyIds:[]).map(String);let validation={ok:true,party_size:ids.length,max_party_size:maxParty};if(!maxParty)validation={ok:false,reason:'party_limit_unavailable'};else if(!ids.length)validation={ok:false,reason:'party_empty',max_party_size:maxParty};else if(ids.length>maxParty)validation={ok:false,reason:'party_size_exceeded',party_size:ids.length,max_party_size:maxParty};else if(new Set(ids).size!==ids.length)validation={ok:false,reason:'party_duplicate_member'};else{const missing=ids.filter(id=>!data.characters.some(c=>String(c?.id||'')===id));if(missing.length)validation={ok:false,reason:'party_member_missing',missing_character_ids:missing};}if(!validation.ok){const error=new Error(`Adventure Party invalid: ${validation.reason}`);error.code=String(validation.reason||'PARTY_INVALID').toUpperCase();error.validation=validation;throw error;}return ids.map(id=>data.characters.find(c=>String(c?.id||'')===String(id))).map(c=>{if(window.GKGameSkillLoadout){const selection=GKGameSkillLoadout.validateBattleSkillSelection(c,{activeCapacity:skillLoadoutBalance().active_skill_slots,passiveCapacity:skillLoadoutBalance().passive_slots,resolvePassive:id=>formalPassiveCatalog.get(id)||null,validatePassiveSelection:ids=>{GKSFormalContribution.validatePassiveSelection([...formalPassiveCatalog.values()],ids,characterAbilityStats(c),{requireRuntimeContracts:true});return{ok:true}}});if(!selection.ok)throw Object.assign(new Error(`出撃Skill/Passive選択が不正です: ${selection.reason}`),{code:selection.reason,selection});}const v=characterBattleValues(c),selectedSkillIds=clone(c.skillLoadoutIds||[]);return{character_id:c.id,id:c.id,name:c.name,job:c.job,job_name:jobDisplayName(c.job),level:c.level,formation_position:(String(c?.formation_position||'').toUpperCase()==='BACKLINE'?'BACKLINE':'FRONTLINE'),max_hp:v.maxHp,max_mp:v.maxMp,mp:v.maxMp,attack:v.attack,agi:v.agi,accuracy:v.accuracy,evasion:v.evasion,magic_accuracy:v.magicAccuracy,magic_resistance:v.magicResistance,magic_weapon_bonus:v.magicWeaponBonus,weapon_critical_rate:v.weaponCriticalRate,luk:v.luk,formalModifierContributions:characterFormalModifierContributions(c),formalPassiveRuntimeContracts:(typeof characterFormalPassiveSelection==='function'?characterFormalPassiveSelection(c).activePassives:[]).map(passive=>({passiveId:String(passive.id||''),runtimeContracts:clone(passive.runtimeContracts)})),skills:clone(c.skills||[]),selected_skill_ids:clone(selectedSkillIds),equipped_skill_id:c.equippedSkillId||'',equipment_performance:{equipment:clone(c.equipment||{}),battle_values:{max_hp:v.maxHp,max_mp:v.maxMp,attack:v.attack,agi:v.agi,accuracy:v.accuracy,evasion:v.evasion,magic_accuracy:v.magicAccuracy,magic_resistance:v.magicResistance,magic_weapon_bonus:v.magicWeaponBonus,weapon_critical_rate:v.weaponCriticalRate,luk:v.luk}},formal_ai_binding:clone(c.formalAiBinding||null)};});}
...
function resolveAdventureEventReward(args,bundle){const resolver=window.GKAdventureRewardResolver?.resolveEventReward;if(typeof resolver!=='function')throw new Error('Adventure Reward Resolver is not loaded');return resolver({...clone(args||{}),monsters:clone(bundle?.monsters||[]),drop_tables:clone(bundle?.dropTables||[]),settings_version:String(bundle?.manifest?.data_version||formalGameBridge.data_version||APP_RUNTIME_GAME_BUILD)});}
function simulateAdventureBattle({formation,seed,encounter_result,battle_id,settings_version},bundle,partySnapshot){if(!window.GKGameFormalAdventureBattle?.simulate)throw new Error('Formal Adventure Battle simulation is not loaded');const scaling=encounter_result?.battle_scaling||null,monsterMaster=scaling&&window.GKAdventureEncounterResolver?.applyBattleScaling?GKAdventureEncounterResolver.applyBattleScaling(bundle.monsters,scaling):bundle.monsters;return GKGameFormalAdventureBattle.simulate({party:partySnapshot,formation,monsters:monsterMaster,seed,battleId:String(battle_id||''),settingsVersion:String(settings_version||formalGameBridge.data_version||APP_RUNTIME_GAME_BUILD)});}
...
function adventureQuestStartFailureMessage(result){if(result.reason==='formal_quest_unavailable')return'正式Story Questが選択されていません。StudioからQuestをExportしてください。';if(result.reason==='quest_prerequisite_missing')return`前提クエスト未達成：${(result.missing_prerequisite_ids||[]).join(', ')}`;if(result.reason==='quest_required_flag_missing')return`開始条件Flag不足：${(result.missing_required_flags||[]).join(', ')}`;if(result.reason==='insufficient_start_cost')return'開始コストまたは選択した石板が不足しています。';if(result.reason==='party_size_exceeded')return`遠征パーティは最大${result.max_party_size}人です。現在${result.party_size}人です。`;if(result.reason==='party_empty')return'遠征パーティを1人以上選んでください。';if(result.reason==='party_duplicate_member')return'同じ冒険者を遠征パーティへ重複登録できません。';if(result.reason==='party_member_missing')return'遠征パーティに存在しない冒険者が含まれています。';if(result.reason==='party_limit_unavailable')return'パーティ上限設定を読み込めません。';if(result.reason==='export_load_failed')return'ストーリーデータを読み込めませんでした。';if(result.reason==='simulation_failed_before_cost')return'冒険生成に失敗しました。石板・開始コストは消費していません。';if(result.reason==='quest_start_commit_failed')return'冒険開始の保存に失敗しました。開始前の状態へ戻しました。';return'冒険を開始できませんでした。';}
const QUEST_START_IMMUTABLE_SAVE_KEYS=Object.freeze(['characters','partyIds','inventory','aiPrograms','aiLayouts','aiPresets','selectedQuestId','gameSettings','tutorialProgress']);
function captureQuestStartImmutableState(source=data){const out={};for(const key of QUEST_START_IMMUTABLE_SAVE_KEYS)out[key]=clone(source?.[key]);return out;}
function assertQuestStartImmutableState(expected,source=data){const actual=captureQuestStartImmutableState(source);if(JSON.stringify(stableSaveValue(expected))!==JSON.stringify(stableSaveValue(actual))){const error=new Error('Quest開始処理が冒険者・Party・装備等の非Quest永続状態を変更しました。開始処理を中止します。');error.code='QUEST_START_OWNER_STATE_MUTATION';error.expected=expected;error.actual=actual;throw error;}return actual;}
async function startSelectedQuestAdventure({forceReload=false}={}){
 if(!window.GKAdventureStorySystem||!window.GKGameFormalAdventureBattle||!window.GKAdventureRewardResolver)return{started:false,reason:'adventure_runtime_unavailable'};const current=currentAdventureQuestRun();if(current)return{started:false,reason:'active_quest_run',run:current};const partyValidation=adventurePartyStartValidation();if(!partyValidation.ok)return{started:false,...partyValidation};let content;try{content=await loadAdventureContent({force:forceReload});}catch(error){return{started:false,reason:'export_load_failed',error};}const bundle=resolveAdventureBundle(content,data.selectedQuestId);if(!bundle)return{started:false,reason:'formal_quest_unavailable'};const startState=adventureQuestStartState(bundle.quest,bundle);if(startState.requirements.missing_prerequisite_ids.length)return{started:false,reason:'quest_prerequisite_missing',...startState.requirements};if(startState.requirements.missing_required_flags.length)return{started:false,reason:'quest_required_flag_missing',...startState.requirements};if(!startState.afford.ok)return{started:false,reason:'insufficient_start_cost',...startState.afford};
 const persistentBefore=clone(data),immutableBefore=captureQuestStartImmutableState(data),partySnapshot=adventurePartySnapshot();let run;try{run=GKAdventureStorySystem.simulateQuest({quest:bundle.quest,scenes:bundle.scenes,events:bundle.events,monsters:bundle.monsters,tablets:bundle.tablets,adventureSettings:bundle.adventureSettings,selectedStones:startState.selectedStones,difficultySnapshot:startState.difficulty,rewardScalingSnapshot:startState.difficulty.reward_scaling_snapshot,partySnapshot,flags:clone(data.flags||{}),startCostResult:{consumed:false,pending:true,cost:clone(startState.cost),base_cost:clone(startState.baseCost),stone_cost:clone(startState.stoneCost)},checkEventCondition:adventureEventCondition,resolveEvent:adventureEventResult,resolveBattleEncounter:adventureBattleResolverAvailable()?args=>resolveAdventureBattleEncounter(args,bundle):undefined,resolveExploration:adventureExplorationResolverAvailable()?args=>resolveAdventureExploration(args,bundle):undefined,resolveReward:args=>resolveAdventureEventReward(args,bundle),settingsVersion:String(bundle?.manifest?.data_version||formalGameBridge.data_version||APP_RUNTIME_GAME_BUILD),simulateBattle:args=>simulateAdventureBattle(args,bundle,partySnapshot)});run.quest_name=String(bundle.quest?.name||bundle.quest?.id||'');assertQuestStartImmutableState(immutableBefore,data);}catch(error){data=persistentBefore;return{started:false,reason:'simulation_failed_before_cost',error,cost:startState.cost};}
 try{const consumed=GKAdventureStorySystem.consumeQuestStartCost(data,startState.cost);if(!consumed.consumed){data=persistentBefore;return{started:false,...consumed}}run.start_cost_result={consumed:true,cost:clone(consumed.cost),base_cost:clone(startState.baseCost),stone_cost:clone(startState.stoneCost),selected_stones:clone(startState.selectedStones)};data.adventure=data.adventure||{};data.adventure.last_start_cost={quest_id:String(bundle.quest.id||''),consumed_at:new Date().toISOString(),cost:clone(consumed.cost),stone_cost:clone(startState.stoneCost)};const stored=storeAdventureQuestRun(run);assertQuestStartImmutableState(immutableBefore,data);autoSave();return{started:true,run:stored,bundle};}
 catch(error){data=persistentBefore;return{started:false,reason:'quest_start_commit_failed',error,cost:startState.cost};}
}
...
function currentFormalAiCatalog(){return window.GKGameAICatalogLoader?.current?.()||null;}
function render(){
 ...
 if($('changeJob'))$('changeJob').value=jobDefinition(c.job)?.id||'';const maxLevel=formalRuntimeSettings?.character?.max_level;
 $('detail').innerHTML=`<div class="row"><div><div class="name">${escapeHtml(c.name)}</div><span class="tag">Lv ${c.level}${maxLevel?` / ${maxLevel}`:''}</span><span class="tag">現在職：${escapeHtml(jobDisplayName(c.job))}</span></div></div><div class="stats">${STATS.map(s=>`<div class="stat"><span class="small">${s}</span><b>${c.stats[s]}</b></div>`).join('')}</div><h3>装着スキル</h3><div class="small">${escapeHtml(findSkill(c.equippedSkillId)?.name||'未装着')}</div><h3>装備</h3><div class="small">${characterEquipmentEntries(c).map(x=>`${x.label}: ${equipmentDisplayName(x.ref)}`).join(' / ')||'なし'}（攻撃 +${characterEquipmentContribution(c).attack} / HP +${characterEquipmentContribution(c).hpBonus} / MP +${characterEquipmentContribution(c).mpBonus}）</div><h3>転職履歴</h3><div class="small">${c.jobHistory.map(h=>`Lv${h.level} ${escapeHtml(jobDisplayName(h.job))}`).join(' → ')}</div><h3>直近の成長</h3><div class="small">${c.growthHistory.slice(-5).reverse().map(g=>`Lv${g.toLevel}: ${g.gained.length?g.gained.join(', '):'能力上昇なし'}`).join('<br>')||'まだレベルアップしていません。'}</div>`;
 $('levelBtn').disabled=!maxLevel||c.level>=maxLevel;
}
...
function skillProgressionRequirementText(record,kind){
 if(!window.GKGameSkillAcquisitionRuntime)return'習得条件を取得できません。';
 try{
  const rows=kind==='skill'?GKGameSkillAcquisitionRuntime.skillRequirements(record):GKGameSkillAcquisitionRuntime.passiveRequirements(record);
  return rows.length?rows.map(row=>`${row.stat} ${row.min}以上`).join(' / '):'条件なし';
 }catch(error){return`習得条件エラー: ${String(error?.message||error)}`;}
}
function skillUseRequirementText(skill){
 const rows=Array.isArray(skill?.runtimeContracts?.useRequirementContracts)?skill.runtimeContracts.useRequirementContracts:(Array.isArray(skill?.useRequirements)?skill.useRequirements:[]);
 if(!rows.length)return'追加の使用条件なし';
 return rows.map(row=>{const all=Array.isArray(row?.allTags)?row.allTags:[],any=Array.isArray(row?.anyTags)?row.anyTags:[],parts=[];if(all.length)parts.push(`すべて: ${all.join(', ')}`);if(any.length)parts.push(`いずれか: ${any.join(', ')}`);return parts.join(' / ')||String(row?.type||'条件あり')}).join(' / ');
}
function skillProgressionFailureText(result){
 const reason=String(result?.reason||'');
 if(reason==='SKILL_POINTS_SHORTAGE')return`Skill Point不足（必要 ${result?.cost??'—'} / 現在 ${result?.skillPointsBefore??'—'}）`;
 if(reason==='ABILITY_REQUIREMENT_NOT_MET'){const rows=Array.isArray(result?.missing)?result.missing:[];return rows.length?`能力値不足: ${rows.map(row=>`${row.stat} ${row.actual??'—'} / 必要 ${row.required}`).join(', ')}`:'能力値条件を満たしていません。';}
 ...
 if(reason==='PASSIVE_SLOT_CAPACITY_REACHED'||reason==='PASSIVE_LOADOUT_CAPACITY_EXCEEDED')return`Passive枠が上限です（${result?.count??'—'} / ${result?.capacity??'—'}）。`;
 if(reason==='PASSIVE_SERIES_DUPLICATE')return`同じPassive系列は同時に選択できません${result?.passiveSeriesId?`（${result.passiveSeriesId}）`:''}。`;
 if(reason==='PASSIVE_LOADOUT_NOT_OWNED')return'未習得Passiveは選択できません。';
...
function formalProductionSkillRows(){
 if(typeof SKILLS==='undefined')return[];
 return SKILLS.filter(skill=>skill?.source==='studio_export'&&(skill.environment||'production')==='production').filter(skill=>!window.GKGameSkillLoadout||GKGameSkillLoadout.formalProductionSkillCheck(skill,compileSkillForRuntime).ok);
}
function passiveEffectText(passive){
 const refs=passive?.runtimeContracts?.modifierRefs||{},caps=Array.isArray(refs.combatCapabilities)?refs.combatCapabilities:[],mods=Array.isArray(refs.modIds)?refs.modIds:[],effects=Array.isArray(refs.effectIds)?refs.effectIds:[],rows=[];
 if(caps.length)rows.push(`Capability: ${caps.join(', ')}`);if(mods.length)rows.push(`MOD: ${mods.join(', ')}`);if(effects.length)rows.push(`Effect: ${effects.join(', ')}`);
...
function passiveSelectionPolicy(character){return{capacity:skillLoadoutBalance().passive_slots,resolvePassive:id=>formalPassiveCatalog.get(id)||null,validateSelection:ids=>{try{if(!window.GKSFormalContribution)throw new Error('Formal Contribution Resolverを読み込めません。');GKSFormalContribution.validatePassiveSelection([...formalPassiveCatalog.values()],ids,characterAbilityStats(character),{requireRuntimeContracts:true});return{ok:true}}catch(error){return{ok:false,reason:error?.code||'PASSIVE_SELECTION_INVALID',detail:String(error?.message||error)}}}}}
async function runCharacterSkillTransaction(characterId,operation,mutate){return runPersistentTransaction(operation,(proposed)=>{const character=(proposed.characters||[]).find(row=>String(row?.id||'')===String(characterId||''));if(!character)throw new Error('Skill/Passive更新対象キャラクターが見つかりません。');const result=mutate(proposed,character);if(result===false||result?.ok===false){const error=new Error(skillProgressionFailureText(result));error.code=result?.reason||'SKILL_DOMAIN_MUTATION_REJECTED';error.result=result;throw error}return result;});}
function renderCharacterSkillView(){
 ...
 list.querySelectorAll('[data-select-skill]').forEach(btn=>btn.onclick=async()=>{const id=btn.dataset.selectSkill,want=btn.dataset.selected!=='1';try{await runCharacterSkillTransaction(c.id,'GS34_SELECT_ACTIVE',(save,character)=>GKGameSkillLoadout.setSkillSelected(character,id,want,{capacity:loadoutCfg.active_skill_slots,resolveSkill:findSkill,compileSkill:compileSkillForRuntime}));render();notify(`${findSkill(id)?.name||id}を${want?'戦闘選択へ追加':'戦闘選択から解除'}しました。`)}catch(error){notify(error?.message||'戦闘選択を変更できません。','bad')}});
 list.querySelectorAll('[data-respec-skill]').forEach(btn=>btn.onclick=()=>{if(!progression||!window.GKGameSkillRespecRuntime)return;const id=btn.dataset.respecSkill,preview=GKGameSkillRespecRuntime.previewIndividual(data,c,'skill',id,progression);if(!preview.ok){notify(skillProgressionFailureText(preview),'bad');return}if(!confirm(`SkillをRespecします。${preview.cost} Gを消費し、${preview.skillPointsRefund} SPを返還します。消費したGoldは戻りません。`))return;runCharacterSkillTransaction(c.id,'GS34_RESPEC_ACTIVE',(save,character)=>GKGameSkillRespecRuntime.respecIndividual(save,character,'skill',id,progression)).then(tx=>{render();notify(`SkillをRespecしました。Skill Point +${tx.mutation_result.skillPointsRefund} / ${tx.mutation_result.cost} G消費。`,'ok')}).catch(error=>notify(error?.message||'SkillをRespecできません。','bad'))});
 const productionSkills=formalProductionSkillRows();
 if(acquireList){acquireList.innerHTML=productionSkills.length?productionSkills.map(skill=>{const owned=(c.skills||[]).includes(skill.id),preview=!owned&&progression&&window.GKGameSkillAcquisitionRuntime?GKGameSkillAcquisitionRuntime.previewAcquireSkill(c,skill,progression):{ok:false,reason:owned?'SKILL_ALREADY_OWNED':progressionError||'ACQUISITION_RUNTIME_UNAVAILABLE'},cost=progression&&window.GKGameSkillAcquisitionRuntime?GKGameSkillAcquisitionRuntime.acquisitionCost(skill,'skill',progression):null;return `<div class="skill-choice"><div><b>${escapeHtml(skill.name)}</b><span class="tag">${escapeHtml(skill.id)}</span><div class="small">習得条件: ${escapeHtml(skillProgressionRequirementText(skill,'skill'))}</div><div class="small">使用条件: ${escapeHtml(skillUseRequirementText(skill))}</div><div class="small">必要Skill Point: ${cost??'—'} ／ ${owned?'習得済み':preview.ok?'習得可能':escapeHtml(skillProgressionFailureText(preview))}</div></div><button type="button" class="primary" data-acquire-skill="${escapeHtml(skill.id)}" ${preview.ok?'':'disabled'}>習得する</button></div>`}).join(''):'<div class="skill-empty">現在、正式Skill Masterに習得対象Skillはありません。</div>';acquireList.querySelectorAll('[data-acquire-skill]').forEach(btn=>btn.onclick=()=>{if(!progression||!window.GKGameSkillAcquisitionRuntime)return;const skill=findSkill(btn.dataset.acquireSkill);runCharacterSkillTransaction(c.id,'GS34_ACQUIRE_ACTIVE',(save,character)=>GKGameSkillAcquisitionRuntime.acquireSkill(character,skill,progression)).then(tx=>{render();notify(`${skill.name}を習得しました。Skill Point -${tx.mutation_result.skillPointsSpent}。`,'ok')}).catch(error=>notify(error?.message||'Skillを習得できません。','bad'))});}
 if(passiveList){let activeIds=new Set(),passiveStateError='';try{activeIds=new Set(characterFormalPassiveSelection(c).activePassives.map(row=>row.id))}catch(error){passiveStateError=String(error?.message||error)}const passives=[...formalPassiveCatalog.values()],selectedPassiveIds=new Set(c.passiveIds||[]);passiveList.innerHTML=passives.length?passives.map(passive=>{const owned=(c.passives||[]).includes(passive.id),selected=selectedPassiveIds.has(passive.id),active=selected&&activeIds.has(passive.id),preview=!owned&&progression&&window.GKGameSkillAcquisitionRuntime?GKGameSkillAcquisitionRuntime.previewAcquirePassive(c,passive,progression):{ok:false,reason:owned?'PASSIVE_ALREADY_OWNED':progressionError||'ACQUISITION_RUNTIME_UNAVAILABLE'},cost=progression&&window.GKGameSkillAcquisitionRuntime?GKGameSkillAcquisitionRuntime.acquisitionCost(passive,'passive',progression):null,respec=owned&&progression&&window.GKGameSkillRespecRuntime?GKGameSkillRespecRuntime.previewIndividual(data,c,'passive',passive.id,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval}):{ok:false,reason:'PASSIVE_NOT_OWNED'};let selectPreview={ok:true};if(owned&&!selected&&window.GKGameSkillLoadout){const candidate=clone(c);selectPreview=GKGameSkillLoadout.setPassiveSelected(candidate,passive.id,true,passiveSelectionPolicy(candidate));}return `<div class="skill-choice ${selected?'selected':''}"><div><b>${escapeHtml(passive.name)}</b><span class="tag">${escapeHtml(passive.id)}</span><span class="tag">${!owned?'未習得':selected?(active?'選択中・有効':'選択中・能力条件未成立'):'習得済み・未選択'}</span><div class="small">常時効果: ${escapeHtml(passiveEffectText(passive))}</div><div class="small">習得条件: ${escapeHtml(skillProgressionRequirementText(passive,'passive'))}</div><div class="small">必要Skill Point: ${cost??'—'}</div>${owned?`<div class="small">Respec: ${respec.ok?`${respec.cost} G / ${respec.skillPointsRefund} SP返還`:escapeHtml(skillProgressionFailureText(respec))}</div>`:`<div class="small">${preview.ok?'習得可能':escapeHtml(skillProgressionFailureText(preview))}</div>`}</div><div class="row">${owned?`<button type="button" class="${selected?'good':'primary'}" data-select-passive="${escapeHtml(passive.id)}" data-selected="${selected?'1':'0'}" ${!selected&&!selectPreview.ok?'disabled':''}>${selected?'選択解除':'戦闘選択'}</button><button type="button" data-respec-passive="${escapeHtml(passive.id)}" ${respec.ok?'':'disabled'}>Respec</button>`:`<button type="button" class="primary" data-acquire-passive="${escapeHtml(passive.id)}" ${preview.ok?'':'disabled'}>習得する</button>`}</div></div>`}).join(''):`<div class="skill-empty">正式Passive Masterがありません。${passiveStateError?` ${escapeHtml(passiveStateError)}`:''}</div>`;passiveList.querySelectorAll('[data-acquire-passive]').forEach(btn=>btn.onclick=async()=>{if(!progression||!window.GKGameSkillAcquisitionRuntime)return;const passive=formalPassiveCatalog.get(btn.dataset.acquirePassive);try{const tx=await runCharacterSkillTransaction(c.id,'GS34_ACQUIRE_PASSIVE',(save,character)=>GKGameSkillAcquisitionRuntime.acquirePassive(character,passive,progression));render();notify(`${passive.name}を習得しました。Skill Point -${tx.mutation_result.skillPointsSpent}。`,'ok')}catch(error){notify(error?.message||'Passiveを習得できません。','bad')}});passiveList.querySelectorAll('[data-select-passive]').forEach(btn=>btn.onclick=async()=>{const id=btn.dataset.selectPassive,want=btn.dataset.selected!=='1';try{await runCharacterSkillTransaction(c.id,'GS34_SELECT_PASSIVE',(save,character)=>GKGameSkillLoadout.setPassiveSelected(character,id,want,passiveSelectionPolicy(character)));render();notify(`${formalPassiveCatalog.get(id)?.name||id}を${want?'戦闘選択へ追加':'戦闘選択から解除'}しました。`,'ok')}catch(error){notify(error?.message||'Passive選択を変更できません。','bad')}});passiveList.querySelectorAll('[data-respec-passive]').forEach(btn=>btn.onclick=async()=>{if(!progression||!window.GKGameSkillRespecRuntime)return;const id=btn.dataset.respecPassive,preview=GKGameSkillRespecRuntime.previewIndividual(data,c,'passive',id,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval});if(!preview.ok){notify(skillProgressionFailureText(preview),'bad');return}if(!confirm(`PassiveをRespecします。${preview.cost} Gを消費し、${preview.skillPointsRefund} SPを返還します。消費したGoldは戻りません。`))return;try{const tx=await runCharacterSkillTransaction(c.id,'GS34_RESPEC_PASSIVE',(save,character)=>GKGameSkillRespecRuntime.respecIndividual(save,character,'passive',id,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval}));render();notify(`PassiveをRespecしました。Skill Point +${tx.mutation_result.skillPointsRefund} / ${tx.mutation_result.cost} G消費。`,'ok')}catch(error){notify(error?.message||'PassiveをRespecできません。','bad')}})}
 if(respecAllPanel){const preview=progression&&window.GKGameSkillRespecRuntime?GKGameSkillRespecRuntime.previewAll(data,c,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval}):{ok:false,reason:progressionError||'RESPEC_RUNTIME_UNAVAILABLE'};respecAllPanel.innerHTML=`<b>全Respec</b><div class="small">${preview.ok?`${preview.cost} Gを消費 / ${preview.skillPointsRefund} SP返還。消費したGoldは戻りません。`:escapeHtml(skillProgressionFailureText(preview))}</div><button type="button" id="characterRespecAll" ${preview.ok?'':'disabled'}>Skill / Passiveを全Respec</button>`;const btn=$('characterRespecAll');if(btn)btn.onclick=()=>{const check=GKGameSkillRespecRuntime.previewAll(data,c,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval});if(!check.ok){notify(skillProgressionFailureText(check),'bad');return}if(!confirm(`Skill / Passiveを全Respecします。${check.cost} Gを消費し、${check.skillPointsRefund} SPを返還します。消費したGoldは戻りません。`))return;runCharacterSkillTransaction(c.id,'GS34_RESPEC_ALL',(save,character)=>GKGameSkillRespecRuntime.respecAll(save,character,progression,{validateAfterRemoval:validateCharacterAfterPassiveRemoval})).then(tx=>{render();notify(`全Respecしました。Skill Point +${tx.mutation_result.skillPointsRefund} / ${tx.mutation_result.cost} G消費。`,'ok')}).catch(error=>notify(error?.message||'全Respecできません。','bad'))}}
 if(catalog){const candidates=productionSkills.filter(skill=>!c.skills.includes(skill.id));catalog.innerHTML=candidates.length?`<div class="small">開発者用の直接割当です。Player習得は上のSkill Point経路を使用します。</div>${candidates.map(skill=>`<button type="button" class="unit" data-assign-formal-skill="${skill.id}"><b>${escapeHtml(skill.name)}</b><span class="tag">${escapeHtml(skill.id)}</span><div class="small">この冒険者へ直接割当</div></button>`).join('')}`:'<div class="small">未割当の正式Production Skillはありません。</div>';catalog.querySelectorAll('[data-assign-formal-skill]').forEach(btn=>btn.onclick=()=>{const skill=findSkill(btn.dataset.assignFormalSkill),result=window.GKGameSkillLoadout?GKGameSkillLoadout.assignFormalProductionSkill(c,skill,compileSkillForRuntime):{ok:false,reason:'LOADOUT_RUNTIME_UNAVAILABLE'};if(!result.ok){notify(`正式Skillを割当できません: ${result.reason}`,'bad');return}autoSave();render();notify(`${c.name}へ${skill.name}を開発者割当しました。`)})}
}
...
function characterAiEditorSummary(c){if(!window.GKGameAISaveBridge)return'AI機能未読込';const saved=GKGameAISaveBridge.loadForCharacter(data,c?.id);return saved?`プレイヤーAI / ${saved.program?.nodes?.length||0}チップ`:'AI未設定';}
function renderExpeditionSetup(){
 const party=$('partyEditor');if(party){party.innerHTML=data.characters.map(c=>`<div class="unit"><label><input type="checkbox" data-party="${c.id}" ${data.partyIds.includes(c.id)?'checked':''}> <b>${escapeHtml(c.name)}</b> <span class="tag">${escapeHtml(jobDisplayName(c.job))}</span></label><div class="small"><b>隊列:</b> <select data-formation-position="${c.id}"><option value="FRONTLINE" ${characterFormationPosition(c)==='FRONTLINE'?'selected':''}>前衛</option><option value="BACKLINE" ${characterFormationPosition(c)==='BACKLINE'?'selected':''}>後衛</option></select></div><div><button type="button" data-open-ai="${c.id}">AIチップ編集</button> <span class="small">${characterAiEditorSummary(c)}</span></div><div class="small">装備: ${characterEquipmentEntries(c).map(row=>`${escapeHtml(row.label)}: ${escapeHtml(equipmentDisplayName(row.ref))} <button type="button" class="mini" data-unequip="${c.id}:${row.slot}">外す</button>`).join(' / ')||'なし'}</div></div>`).join('')||'<p class="small">冒険者を作成してください。</p>';party.querySelectorAll('[data-party]').forEach(el=>el.onchange=()=>{const maxParty=partyMaxSize();if(!maxParty){el.checked=false;notify('Formal Game Runtime設定を読み込めません。','bad');return}if(el.checked&&data.partyIds.length>=maxParty){el.checked=false;notify(`パーティは最大${maxParty}人です。`,'warn');return}data.partyIds=el.checked?[...data.partyIds,el.dataset.party]:data.partyIds.filter(id=>id!==el.dataset.party);autoSave();renderExpeditionSetup()});party.querySelectorAll('[data-formation-position]').forEach(el=>el.onchange=()=>{const c=data.characters.find(x=>x.id===el.dataset.formationPosition);if(!c)return;setCharacterFormationPosition(c,el.value);autoSave();renderExpeditionSetup()});party.querySelectorAll('[data-open-ai]').forEach(btn=>btn.onclick=()=>openAiEditorFor(data.characters.find(x=>x.id===btn.dataset.openAi)));party.querySelectorAll('[data-unequip]').forEach(btn=>btn.onclick=e=>{e.preventDefault();const [id,slot]=btn.dataset.unequip.split(':'),c=data.characters.find(x=>x.id===id);if(c&&c.equipment?.[slot]){const returned=unequipCharacterEquipmentSlot(c,slot);if(returned.length)data.inventory.push(...returned);autoSave();render();notify(`${c.name}の装備を外しました。`)}})}
...
function createCharacterFromForm(){
 ...
 try{
  ...
  if(!jobDefinition(job)){notify('正式Jobを選択してください。','bad');return}
  const c=makeCharacter(name,job);
  data.characters.push(c);
  const maxParty=partyMaxSize();if(maxParty&&data.partyIds.length<maxParty)data.partyIds.push(c.id);
  selectedId=c.id;
  if(nameInput)nameInput.value='';
  let saveError=null;
  try{autoSave()}catch(error){saveError=error;console.error('character save failed',error)}
...
$('levelBtn').onclick=async()=>{const selectedCharacter=data.characters.find(x=>x.id===selectedId),cfg=requireFormalRuntimeSettings();if(!selectedCharacter||selectedCharacter.level>=cfg.character.max_level)return;if(!window.GKGameSkillAcquisitionRuntime){notify('レベルアップできません: Skill Acquisition Runtimeを読み込めません。','bad');return}const characterId=selectedCharacter.id,now=new Date().toISOString(),progression=skillProgressionBalance();try{let summary=null;await runPersistentTransaction('character_level_up',draft=>{const target=(draft.characters||[]).find(x=>String(x?.id||'')===String(characterId));if(!target)throw new Error(`レベルアップ対象が存在しません: ${characterId}`);const job=jobDefinition(target.job);if(!job)throw new Error(`正式Jobが見つかりません: ${target.job}`);const result=resolveCharacterLevelUp(target,job,cfg);if(!result.ok){const error=new Error('最大Levelでは追加成長できません。');error.code='CHARACTER_MAX_LEVEL';throw error}target.stats={...result.next_stats};target.level=result.to_level;const skillPointResult=GKGameSkillAcquisitionRuntime.grantLevelUpSkillPoints(target,progression,{levels:1}),gained=STATS.filter(stat=>result.increments[stat]>0).map(stat=>`${stat} +${result.increments[stat]}`);target.growthHistory=Array.isArray(target.growthHistory)?target.growthHistory:[];target.growthHistory.push({fromLevel:result.from_level,toLevel:result.to_level,job:result.job_id,growth:{...result.increments},growthRolls:{...result.rolls},gained,skillPointsGained:skillPointResult.gained,ruleRevision:GKCharacterGrowthDomain.VERSION,at:now});summary={name:target.name,level:target.level,gained,skillPointsGained:skillPointResult.gained,job:result.job_id};return summary;},{transactionId:`LEVEL-UP-${characterId}-${Date.now()}`});render();notify(`${summary.name}がLv${summary.level}になりました。Skill Point +${summary.skillPointsGained}${summary.gained.length?' / 上昇: '+summary.gained.join(', '):' / 能力値上昇なし'}`)}catch(error){notify(`レベルアップできません: ${error.message}`,'bad')}};
...
async function confirmJobChange(next){
 const c=data.characters.find(x=>x.id===selectedId),job=jobDefinition(next),editState=currentGuildEditState();
 if(!c||!job)return;if(!editState.ok){notify(`転職できません: ${editState.message}`,'bad');return}
 const characterId=c.id,now=new Date().toISOString();
 try{
  let committed=null;
  await runPersistentTransaction('job_change',draft=>{
   const target=(draft.characters||[]).find(x=>String(x?.id||'')===String(characterId));
   const liveEditState=GKJobTransferDomain.evaluateGuildEditState({phase:currentPhase,activeQuestRunId:String(draft?.adventure?.active_quest_run_id||'')});
   const result=GKJobTransferDomain.prepareJobChange({character:target,nextJobRef:job.id,catalog:formalJobDomainCatalog,editState:liveEditState,now});
   if(!result.ok){const error=new Error(result.message);error.code=result.code;throw error}
   target.job=result.to_job.id;
   target.jobHistory=Array.isArray(target.jobHistory)?target.jobHistory:[];
   target.jobHistory.push({...result.history_entry});
   committed={name:target.name,from_job:result.from_job.id,to_job:result.to_job.id};
   return committed;
  },{transactionId:`JOB-CHANGE-${characterId}-${Date.now()}`});
  render();closeJobChangeModal();notify(`${committed.name}は${jobDisplayName(committed.from_job)}から${jobDisplayName(committed.to_job)}へ転職しました。次回以降の能力成長テーブルが変わります。`)
 }catch(error){notify(`転職を保存できません: ${error.message}`,'bad')}
}
...
if($('saveRecoveryRetry'))$('saveRecoveryRetry').onclick=retrySaveRecovery;if($('saveRecoverySafeReturn'))$('saveRecoverySafeReturn').onclick=safeReturnFromSaveRecovery;if($('titleRestoreBackup'))$('titleRestoreBackup').onclick=restoreBackupAndContinue;
...
function tagTestRunShieldJson(){const cases=[],allErrors=[];const run=(id,label,fn)=>{try{const c=fn();cases.push(c);if(!c.passed)allErrors.push(...c.errors.map(x=>`${id}: ${x}`))}catch(e){const msg=String(e?.message||e);cases.push(makeShieldCase({id,label,initialState:null,events:[],finalState:null,expectations:{},result:null,errors:[msg]}));allErrors.push(`${id}: ${msg}`)}};
 ...
 const report={schema_version:'1.3.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),test:{id:'TAG-SHIELD-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunShieldJson'},design_decisions:{stacking:'additive individual instances',consumption:'FIFO by appliedAt',dot_consumes_shield:true,death_clears:true,battle_end_clears:true},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:allErrors.length===0,errors:allErrors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-shield-device-validation-${APP_RUNTIME_GAME_BUILD}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[SHIELD DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;renderBattle();return report}
...
function tagTestRunStatusJson(){
 ...
 const report={schema_version:'1.1.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),test:{id:'TAG-STATUS-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunStatusJson'},design_decisions:{attack_hit_applies_status:true,application_rate:100,resistance_affects:'duration',resistance_cap_percent:75,duration_formula:'floor(base_duration * (1 - resistance/100))',minimum_duration_tick:1},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-status-device-validation-${APP_RUNTIME_GAME_BUILD}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[STATUS DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}`;return report;
}
...
function tagTestRunAuraJson(){
 ...
 const report={schema_version:'1.0.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),test:{id:'TAG-AURA-DEVICE-001',mode:'device_validation',entrypoint,trigger:'tagTestRunAuraJson'},current_spec:{task_id:'P01-06',stage:'tag_validation',runtime_application:false,supported_effects:['BUFF','DEBUFF'],supported_stats:['ATK','DEF','MAGIC_WEAPON_BONUS','STATUS_RESIST'],value_tag:'AURA_VALUE',target_tag:'AURA_TARGET=<ally|enemy>',scope_tag:'AURA_SCOPE=<all|self_and_allies|allies_excluding_self>',stacking:'highest only',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-device-validation-${APP_RUNTIME_GAME_BUILD}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
async function tagTestRunAuraRuntimeJson(){
 ...
 const report={schema_version:'1.0.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),test:{id:'TAG-AURA-RUNTIME-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunAuraRuntimeJson'},current_spec:{task_id:'P01-06',stage:'runtime_connection_v1',runtime_application:true,source_dependency:true,stacking:'highest',source_death:'immediate_disable',source_revive:'re_evaluate_and_restore',target_death:'inactive_while_dead',target_revive:'re_evaluate',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},source:{studio_status:studioSkillBridge.status,data_version:studioSkillBridge.data_version},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-${APP_RUNTIME_GAME_BUILD}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function setupTagSkillTestUI(){
 ...
 if(runDefeat)runDefeat.onclick=()=>{
  ...
  const compiled=compileSkillForRuntime(skill);
  if(!compiled.ok||!compiled.definition.logicOrder.includes('DOT')){$('tagTestResult').textContent=formatCompileResult(compiled)+'\n[JSON TEST] FAILED / DOTスキルを選択してください';return}
  if(!battle.units.some(x=>x.alive&&x.side===target.side&&x.id!==target.id)){battle.units.push(makeCombatant({id:'E-DUMMY',name:'検証用生存敵',side:target.side,agi:1,attack:1,maxHp:9999,gauge:0,actions:0,order:999,lastActionTick:null}))}
...
let GAUGE_MAX=null;
const STANDALONE_BATTLE_FIXTURE={source:'standalone_fixture',seed:486187,formation:[{monster_id:'STANDALONE-SLIME',count:1},{monster_id:'STANDALONE-WOLF',count:1}],monsters:[{id:'STANDALONE-SLIME',name:'検証スライム',formalAiBinding:{program_id:'AIP-0001',layout_id:'AIL-0001'},params:{maxHp:240,attack:22,agi:7}},{id:'STANDALONE-WOLF',name:'検証ウルフ',formalAiBinding:{program_id:'AIP-0001',layout_id:'AIL-0001'},params:{maxHp:300,attack:30,agi:12}}]};
...
function setBattleLaunchContext(context){battleLaunchContext=context?{formation:window.GKAdventureBattleCore?GKAdventureBattleCore.normalizeFormation(context.formation):clone(context.formation||[]),monsters:clone(context.monsters||[]),seed:context.seed??null,battle_id:String(context.battle_id||context.battleId||''),settings_version:String(context.settings_version||context.settingsVersion||formalGameBridge.data_version||APP_RUNTIME_GAME_BUILD),source:context.source||'standalone_fixture'}:null;return battleLaunchContext}
...
function makeCombatant(base){const maxHp=finiteNumber(base?.maxHp,`${base?.id||base?.name||'combatant'}.maxHp`,{min:1}),maxMp=finiteNumber(base?.maxMp,`${base?.id||base?.name||'combatant'}.maxMp`,{min:0}),ownedSkillIds=window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeSkillIds(base.ownedSkillIds||base.skills,{fallback:[]}):[...new Set((base.ownedSkillIds||base.skills||[]).map(id=>String(id||'').trim()).filter(Boolean))],selectedSkillIds=window.GKGameSkillLoadout?GKGameSkillLoadout.normalizeSkillIds(base.selectedSkillIds||base.selected_skill_ids||base.skillLoadoutIds,{fallback:[]}):[...new Set((base.selectedSkillIds||base.selected_skill_ids||base.skillLoadoutIds||[]).map(id=>String(id||'').trim()).filter(Boolean))],equippedSkillId=String(base.equippedSkillId||base.equipped_skill_id||'').trim(),defaultSkillId=String(base.defaultSkillId||(base.characterId?selectedSkillIds[0]:(equippedSkillId||ownedSkillIds[0]))||'');GAUGE_MAX=battleGaugeMax();return {...base,maxHp,formationPosition:normalizeFormationPosition(base.formationPosition??base.formation_position,{fallback:'FRONTLINE'}),hp:maxHp,maxMp,mp:Math.max(0,Math.min(maxMp,Number(base.mp??maxMp)||0)),alive:true,damageDealt:0,damageTaken:0,dotStacks:[],modifierStacks:[],reservedAction:null,lastReservation:null,castingAction:null,nextAiEvaluationTick:battleAiReevaluationIntervalTicks(),lastAiEvaluationTick:null,ownedSkillIds,selectedSkillIds,equippedSkillId,defaultSkillId}}
function makeBattleUnits(){
 ...
 const allies=members.map((c,i)=>{const v=characterBattleValues(c),e2e=developerE2EOverrideSkillId(c.id),basicAttackProfiles=characterBasicAttackProfiles(c),selectedSkillIds=characterSelectedActiveSkillIds(c);return makeCombatant({id:`A${i}`,characterId:c.id,name:c.name,side:'味方',formationPosition:characterFormationPosition(c),ownedSkillIds:clone(c.skills||[]),selectedSkillIds:clone(selectedSkillIds),equippedSkillId:c.equippedSkillId||'',legacySingleSkillCompatibility:{defaultSkillId:e2e||c.equippedSkillId||''},defaultSkillId:e2e||selectedSkillIds[0]||'',agi:v.agi,attack:v.attack,accuracy:v.accuracy,evasion:v.evasion,magicAccuracy:v.magicAccuracy,magicResistance:v.magicResistance,magicWeaponBonus:v.magicWeaponBonus,weaponCriticalRate:v.weaponCriticalRate,luk:v.luk,formalModifierContributions:characterFormalModifierContributions(c),formalPassiveRuntimeContracts:(typeof characterFormalPassiveSelection==='function'?characterFormalPassiveSelection(c).activePassives:[]).map(passive=>({passiveId:String(passive.id||''),runtimeContracts:clone(passive.runtimeContracts)})),blockRate:v.blockRate,blockDamageCutRate:v.blockDamageCutRate,equipmentTagIds:characterEquipmentTagIds(c),basicAttackUnavailableReason:bowQuiverActionRequirementReason(c),basicAttackProfiles,weaponStyle:characterWeaponStyle(c),maxHp:v.maxHp,maxMp:v.maxMp,gauge:0,actions:0,order:i,lastActionTick:null})});
 if(!allies.length)allies.push(makeCombatant({id:'A0',name:'検証剣士',side:'味方',ownedSkillIds:['SKL-TEST-POISON'],agi:11,attack:48,maxHp:360,gauge:0,actions:0,order:0,lastActionTick:null}));
 ...
 GKAdventureBattleCore.validateFormalFormationBindings(battleLaunchContext.formation,battleLaunchContext.monsters||[]);const expanded=GKAdventureBattleCore.expandFormation(battleLaunchContext.formation,battleLaunchContext.monsters||[]);
 const enemies=expanded.map((e,i)=>makeCombatant({id:`E${i}`,monsterId:e.monster_id,name:e.name,side:'敵',formationPosition:e.formationPosition,formalAiBinding:e.formalAiBinding?clone(e.formalAiBinding):null,ownedSkillIds:clone(e.skillIds||[]),agi:e.agi,attack:e.attack,maxHp:e.maxHp,maxMp:e.maxMp,mp:e.maxMp,gauge:0,actions:0,order:100+i,lastActionTick:null}));
...
function openAiEditorFor(c){
 ...
 const allowedSkillIds=characterSelectedActiveSkillIds(c);
 window.GKGameAIEditorUI.open({
  character:c,allowedSkillIds,notify,program:saved?.program||null,layout:saved?.layout||null,userPresets:GKGameAISaveBridge.userPresets(data),
  onSave:async({program,layout,projectData})=>{
   const staged=GKGameAISaveBridge.saveForCharacter(data,c.id,program,layout,{catalog:currentFormalAiCatalog()});
...
function tagTestRunCounterJson(){
 ...
 const report={schema_version:'1.1.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-DEVICE-001',mode:'device_validation',entrypoint,trigger:'tagTestRunCounterJson'},current_spec:{task_id:'P01-07',stage:'data_driven_tag_validation',runtime_application:false,trigger_tag:'COUNTER_TRIGGER=hit',target_tag:'COUNTER_TARGET=attacker',limit_tag:'COUNTER_LIMIT=1',priority_tag:'COUNTER_PRIORITY=<integer>',require_alive_tag:'COUNTER_REQUIRE_ALIVE=true',allow_zero_damage_tag:'COUNTER_ALLOW_ZERO_DAMAGE=true',incoming_area_attack:false,attack_definition:'existing ATTACK',critical:'existing ATTACK',passive_effects:'existing ATTACK pipeline',counter_chain:false,derived_origin_trigger:false,multiple_execution:false,duplicate_policy:'skill configuration side',dedicated_counter_damage_formula:false},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-device-validation-${APP_RUNTIME_GAME_BUILD}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report;
}
...
function runActivationPriorityRuntimeValidation(){
 ...
 battle.validationCaptureEvents=false;battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),formal_candidate:'P01-12-FORMAL-1',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high?.id||null,low_skill_id:low?.id||null},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-${APP_RUNTIME_GAME_BUILD}-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runBattleEndEffectClearValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 ...
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',test:{id:'TAG-BATTLE-END-EFFECT-CLEAR-003',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},current_spec:{task_id:'P01-14',stage:'formal_candidate_v1',formal_runtime_enabled:true,battle_end_clear:['reserved_action','shield_effects','status_effects','cover_effects','dot_stacks','modifier_stacks','cooldowns']},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunBattleEndEffectClearJson(){pauseBattle();const report=runBattleEndEffectClearValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-battle-end-effect-clear-formal-runtime-device-validation-${APP_RUNTIME_GAME_BUILD}-P01-14-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-14 BATTLE END CLEAR FORMAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}`;renderBattle();return report}
function runSimultaneousActivationOrderValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),formal_candidate:'P01-13-FORMAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-004',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
 ...
 battle.validationCaptureEvents=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),formal_candidate:'P01-13-FORMAL-1',test:{id:'TAG-SIMULTANEOUS-ACTIVATION-ORDER-004',mode:'formal_runtime_candidate',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-13',stage:'formal_candidate_v1',formal_runtime_enabled:true,formal_order:['activation_priority_desc','battle_tie_roll_desc'],battle_tie_roll:{seeded:true,assigned_once_at_battle_start:true,stable_until_battle_end:true,collision_rule:'reroll_colliding_actors_until_unique'},legacy_tie_keys_removed:['gauge_overflow','AGI','actor_order']},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunSimultaneousActivationOrderJson(){pauseBattle();const report=runSimultaneousActivationOrderValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-simultaneous-activation-order-formal-runtime-device-validation-${APP_RUNTIME_GAME_BUILD}-P01-13-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-13 SIMULTANEOUS ORDER FORMAL-1] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL RUNTIME] ENABLED\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
...
function runCounterRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:APP_RUNTIME_GAME_BUILD,generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-002',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-07',stage:'runtime_v1_1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-${APP_RUNTIME_GAME_BUILD}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
