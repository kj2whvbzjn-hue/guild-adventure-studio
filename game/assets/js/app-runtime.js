...
function jobDefinition(ref){const key=String(ref||'').trim();if(!key)return null;return formalJobCatalog.get(key)||[...formalJobCatalog.values()].find(x=>x.name===key)||null}
...
async function confirmJobChange(next){const c=data.characters.find(x=>x.id===selectedId),job=jobDefinition(next);if(!c||!job||job.id===c.job)return;const characterId=c.id,old=c.job,now=new Date().toISOString();try{await runPersistentTransaction('job_change',draft=>{const target=(draft.characters||[]).find(x=>String(x?.id||'')===String(characterId));if(!target)throw new Error(`転職対象が存在しません: ${characterId}`);if(!jobDefinition(job.id))throw new Error(`正式Jobが見つかりません: ${job.id}`);target.job=job.id;target.jobHistory=Array.isArray(target.jobHistory)?target.jobHistory:[];target.jobHistory.push({job:job.id,level:target.level,from:old,from_job:old,to_job:job.id,cost_gold:0,at:now});return draft;},{transactionId:`JOB-CHANGE-${characterId}-${Date.now()}`});render();closeJobChangeModal();notify(`${c.name}は${jobDisplayName(old)}から${job.name}へ転職しました。次回以降の能力成長テーブルが変わります。`)}catch(error){notify(`転職を保存できません: ${error.message}`,'bad')}}
