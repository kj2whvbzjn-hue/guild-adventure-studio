...
function recordValidationEvent(type,payload={}){
 ...
 if(!battle.validationMode&&battle.validationActivationPriority!==true)return;
...
async function tagTestRunAuraRuntimeJson(){
 ...
 const report={schema_version:'1.0.0',build:'GA-B486.48',generated_at:new Date().toISOString(),test:{id:'TAG-AURA-RUNTIME-DEVICE-001',mode:'device_validation',entrypoint:'game/index.html',trigger:'tagTestRunAuraRuntimeJson'},current_spec:{task_id:'P01-06',stage:'runtime_connection_v1',runtime_application:true,source_dependency:true,stacking:'highest',source_death:'immediate_disable',source_revive:'re_evaluate_and_restore',target_death:'inactive_while_dead',target_revive:'re_evaluate',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},source:{studio_status:studioSkillBridge.status,data_version:studioSkillBridge.data_version},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.48-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function runActionDisabledRuntimeRegression(){
 ...
 const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.48',generated_at:new Date().toISOString(),test:{id:'TAG-ACTION-DISABLED-RUNTIME-001',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,action_disabled_skill_id:skills.disabled.id},current_spec:{task_id:'P01-09',stage:'runtime_v1',runtime_application:true,execution_decision_point:'action_order_execution_eligibility_check',reservation_semantics:'presentation_only_no_execution_right',blocked_actions:['normal_action','skill_action','COUNTER','FOLLOW_UP'],continuous_effects_continue:['DOT','SHIELD','AURA','COVER'],canonical_source:'STATUS payload action_disabled=true',individual_status_names:'not_formalized_here'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActionDisabledRuntimeJson(){pauseBattle();const report=runActionDisabledRuntimeRegression();report.test={...report.test,mode:'device_runtime_validation',trigger:'tagTestRunActionDisabledRuntimeJson'};const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-action-disabled-runtime-device-validation-GA-B486.48-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[ACTION DISABLED RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}

function runActivationPriorityRuntimeValidation(){
 const req=id=>{const skill=findTagSkill(id);if(!skill)throw new Error(`${id}がありません`);return skill};
 const high=req('SKL-ACTIVATION-PRIORITY-HIGH'),low=req('SKL-ACTIVATION-PRIORITY-LOW');
 const cases=[],add=(id,label,fn)=>{try{const x=fn(),errors=x.errors||[];cases.push({id,label,...x,passed:errors.length===0})}catch(e){cases.push({id,label,errors:[String(e?.message||e)],passed:false})}};
 const prep=()=>{pauseBattle();resetBattle();battle.validationMode=false;battle.validationEvents=[];battle.tick=0;battle.validationActivationPriority=true;const allies=ensureValidationTargets('味方',2),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.actions=0;u.reservedAction=null;u.lastReservation=null;u.statusEffects=[];u.cooldowns={};u.maxMp=100;u.mp=100}return{highActor:allies[0],lowActor:allies[1],target:enemies[0]}};
 add('ACTIVATION-PRIORITY-COMPILE','整数優先度をコンパイル',()=>{const hc=compileTaggedSkill(high),lc=compileTaggedSkill(low),errors=[];if(!hc.ok||hc.definition.parameters.activationPriority!==100)errors.push('high compile');if(!lc.ok||lc.definition.parameters.activationPriority!==-100)errors.push('low compile');return{high_priority:hc.definition.parameters.activationPriority,low_priority:lc.definition.parameters.activationPriority,errors}});
 add('ACTIVATION-PRIORITY-SAME-TICK-HIGH-FIRST','同一Tickでは高優先度を先に固定',()=>{const f=prep(),errors=[];f.highActor.defaultSkillId=high.id;f.lowActor.defaultSkillId=low.id;f.highActor.gauge=GAUGE_MAX;f.lowActor.gauge=GAUGE_MAX;reserveAction(f.lowActor);reserveAction(f.highActor);f.lowActor.reservedAction.executeAt=1;f.highActor.reservedAction.executeAt=1;processTicks(1);const fixed=battle.validationEvents.find(x=>x.type==='activation_order_fixed'),committed=battle.validationEvents.filter(x=>x.type==='action_execution_committed');if(fixed?.order?.[0]?.source_id!==f.highActor.id)errors.push('high not first in fixed order');if(committed[0]?.source_id!==f.highActor.id)errors.push('high not first committed');return{fixed_order:fixed?.order||[],committed_order:committed.map(x=>x.source_id),errors}});
 add('ACTIVATION-PRIORITY-GATE-OFF-PRESERVES-CURRENT','検証ゲートOFFでは現行順を維持',()=>{const f=prep(),errors=[];battle.validationActivationPriority=false;f.highActor.defaultSkillId=high.id;f.lowActor.defaultSkillId=low.id;f.highActor.gauge=GAUGE_MAX;f.lowActor.gauge=GAUGE_MAX;f.highActor.agi=1;f.lowActor.agi=99;reserveAction(f.highActor);reserveAction(f.lowActor);f.highActor.reservedAction.executeAt=1;f.lowActor.reservedAction.executeAt=1;processTicks(1);if(battle.validationEvents.some(x=>x.type==='activation_order_fixed'))errors.push('gate off emitted priority order');return{priority_gate:false,errors}});
 ...
 battle.validationActivationPriority=false;const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));return{schema_version:'1.0.0',build:'GA-B486.48',generated_at:new Date().toISOString(),validation_patch:'P01-12-VAL-2',test:{id:'TAG-ACTIVATION-PRIORITY-RUNTIME-001',mode:'validation_runtime',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null,high_skill_id:high.id,low_skill_id:low.id},current_spec:{task_id:'P01-12',stage:'validation_v2',formal_runtime_enabled:false,provisional_tag:'ACTIVATION_PRIORITY=<integer>',higher_first:true,same_tick_order_fixed_once:true,next_tick_redecides:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-runtime-device-validation-GA-B486.48-P01-12-VAL2-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY VALIDATION] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] NO\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runCounterRuntimeRegression(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.48',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-RUNTIME-002',mode:'runtime_regression',entrypoint:'game/index.html'},source:{data_version:studioSkillBridge.data_version||null},current_spec:{task_id:'P01-07',stage:'runtime_v1_1'},cases:[],summary:{case_count:0,passed_count:0,failed_count:1,passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]}};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.48-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
