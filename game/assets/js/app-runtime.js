...
function normalize(raw){
 ...
 raw.schemaRevision='1.5.0';raw.gameVersion='GA-B486.179';
 return raw;
}
...
const ADVENTURE_EXPORT_FILES={manifest:'../Export/manifest.json',quests:['../Export/quest/main_quests.json','../Export/quest/sub_quests.json','../Export/quest/event_quests.json'],chapters:'../Export/scenario/chapters.json',sections:'../Export/scenario/sections.json',scenes:'../Export/scenario/scenes.json',events:'../Export/event/events.json',monsters:'../Export/monster/monsters.json',tablets:'../Export/stone/stones.json',maps:'../Export/world/maps.json',explorationOutcomes:'../Export/exploration/outcomes.json',adventureSettings:'../Export/system/adventure_settings.json'};
...
async function loadAdventureContent({force=false}={}){if(adventureContentCache&&!force)return adventureContentCache;const questDocs=await Promise.all(ADVENTURE_EXPORT_FILES.quests.map(fetchAdventureExport));const [chapters,sections,scenes,events,monsters,tablets,maps,explorationOutcomes,adventureSettings,manifest]=await Promise.all([... [ADVENTURE_EXPORT_FILES.chapters,ADVENTURE_EXPORT_FILES.sections,ADVENTURE_EXPORT_FILES.scenes,ADVENTURE_EXPORT_FILES.events,ADVENTURE_EXPORT_FILES.monsters,ADVENTURE_EXPORT_FILES.tablets,ADVENTURE_EXPORT_FILES.maps,ADVENTURE_EXPORT_FILES.explorationOutcomes,ADVENTURE_EXPORT_FILES.adventureSettings].map(fetchAdventureExport),fetchAdventureManifest()]);const documents=[...questDocs,chapters,sections,scenes,events,monsters,tablets,maps,explorationOutcomes,adventureSettings];assertAdventureExportVersionConsistency(manifest,documents);adventureContentCache={quests:questDocs.flatMap(x=>x.data),chapters:chapters.data,sections:sections.data,scenes:scenes.data,events:events.data,monsters:monsters.data,tablets:tablets.data,maps:maps.data,explorationOutcomes:explorationOutcomes.data,adventureSettings:adventureSettings.data,manifest};return adventureContentCache;}
...
$('clearBtn').onclick=()=>{if(!confirm('正式版Phase Aの全データを初期化しますか？'))return;data={saveVersion:1,schemaRevision:'1.5.0',gameVersion:'GA-B486.179',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),characters:[],partyIds:[],selectedQuestId:'Q-001',inventory:[],guild:{gold:0,victories:0,defeats:0,lastBattle:null},flags:{},quest_progress:{completed_quest_ids:[],unlocked_quest_ids:[]},quest_resources:{},adventure:{quest_runs:[],active_quest_run_id:'',history_limit:20,stone_selection_by_quest:{}}};selectedId=null;persist();render();notify('全データを初期化しました。','warn')};
...
async function tagTestRunAuraRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-runtime-device-validation-GA-B486.179-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[STUDIO] ${report.source.data_version||report.source.studio_status}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}
...
function tagTestRunCounterJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-device-validation-GA-B486.179-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report;
}
...
function tagTestRunActivationPriorityRuntimeJson(){pauseBattle();const report=runActivationPriorityRuntimeValidation();const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-formal-runtime-device-validation-GA-B486.179-P01-12-FORMAL1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY FORMAL] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] YES\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report}
function runBattleEndEffectClearValidation(){
 if(studioSkillBridge.status!=='loaded')return{schema_version:'1.0.0',build:'GA-B486.179',generated_at:new Date().toISOString(),formal_candidate:'P01-14-FORMAL-1',passed:false,errors:[`Studioデータ未読込: ${studioSkillBridge.status}`]};
...
function tagTestRunCounterRuntimeJson(){
 ...
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-runtime-device-validation-GA-B486.179-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER RUNTIME JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
