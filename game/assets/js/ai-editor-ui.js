(function(root,factory){
  ...
  let ui=null,activeCharacter=null,activeSession=null,catalog=null,currentCategory='condition',candidateContext=null,configContext=null,notify=()=>{},saveHandler=null,presetHandler=null,userPresets=[],issueCursor=0,lastEvaluation=null;
  ...
  function createSession(c,options){
    ...
    function updateParameters(instanceId,parameters){const node=program.nodes.find(row=>row.instance_id===instanceId);if(!node)throw new Error('AI部品が見つかりません。');const def=definitionFor(node);if(!def)throw new Error('AI部品Masterが見つかりません。');const errors=Adapter.validateParameters(def,parameters,c?.refs||{});if(errors.length)throw new Error(errors.join('\n'));commit(()=>{node.parameters=clone(parameters||{});});}
    function replace(instanceId,masterId,parameters){const node=program.nodes.find(row=>row.instance_id===instanceId);if(!node)throw new Error('AI部品が見つかりません。');const def=definitionById(c,masterId);if(!def)throw new Error('AI部品Masterが見つかりません。');const errors=Adapter.validateParameters(def,parameters,c?.refs||{});if(errors.length)throw new Error(errors.join('\n'));commit(()=>{node.master_node_id=def.id;node.master_data_version=def.data_version;node.node_type=def.node_type;node.parameters=clone(parameters||{});});}
    ...
    function evaluate(){
      const baseProgram=Program.normalizeProgram(program),baseLayout=Layout.normalizeLayout(layout),data=projectData(c);let resolution,resolvedProgram,validation,generated=true,generationError='';
      try{resolution=Resolver.resolve(baseLayout,baseProgram,data);resolvedProgram=clone(baseProgram);resolvedProgram.edges=clone(resolution.edges||[]);validation=Validator.validate(resolvedProgram,data);}catch(error){generated=false;generationError=String(error?.message||error);resolution={valid:false,diagnostics:[{severity:'ERROR',code:'AI_PROGRAM_GENERATION_FAILED',message:generationError}],summary:{ERROR:1,WARNING:0,INFO:0},edges:[]};resolvedProgram=clone(baseProgram);validation={valid:false,issues:[],summary:{ERROR:0,WARNING:0,INFO:0}};}
    ...
    return Object.freeze({program:()=>clone(program),layout:()=>clone(layout),nodeAt,definitionFor,add,updateParameters,replace,rotate,remove,clear,undo:undoOnce,redo:redoOnce,canUndo:()=>undo.length>0,canRedo:()=>redo.length>0,parameterErrors:(def,values)=>Adapter.validateParameters(def,values,c?.refs||{}),descriptors:def=>Adapter.inputDescriptors(def,c?.refs||{}),catalog:()=>c,evaluate,replaceDocuments,loadDocuments});
  }
  ...
  function renderBoard(){
    if(!ui||!activeSession)return;const layout=activeSession.layout(),program=activeSession.program(),evaluation=activeSession.evaluate(),nodeSeverity=issueNodes(evaluation);
    ...
    for(let y=0;y<layout.height;y++)for(let x=0;x<layout.width;x++){
      const cell=document.createElement('button');cell.type='button';cell.className='ai-formal-cell';cell.dataset.x=x;cell.dataset.y=y;const chip=layout.chips.find(row=>row.x===x&&row.y===y);const node=chip?byId.get(chip.instance_id):null;
      if(node){const def=activeSession.definitionFor(node),summary=summaryFor(activeSession,node),entry=program.entry_node_id===node.instance_id,severity=nodeSeverity.get(node.instance_id);cell.classList.add('occupied');if(severity==='ERROR')cell.classList.add('ai-node-error');else if(severity==='WARNING')cell.classList.add('ai-node-warning');cell.dataset.nodeId=node.instance_id;cell.innerHTML=`<span class="ai-formal-chip ${esc(node.node_type)}">${portHtml(def,chip.rotation)}${entry?'<span class="ai-entry-badge">START</span>':''}<b>${esc(def?.name||node.master_node_id)}</b>${summary?`<small>${esc(summary)}</small>`:''}<small class="ai-rotation">${chip.rotation}°</small></span>`;cell.onclick=()=>openExistingConfig(node.instance_id);}else{cell.innerHTML='<span class="ai-formal-add">＋</span>';cell.setAttribute('aria-label',`空きセル ${x+1},${y+1}`);cell.onclick=()=>openCandidates({x,y});}ui.board.appendChild(cell);
    }
    ui.undo.disabled=!activeSession.canUndo();ui.redo.disabled=!activeSession.canRedo();renderValidationState(evaluation);ui.catalogNotice.classList.toggle('hidden',!!(catalog&&definitions(catalog).length));
  }
  function renderCandidateTabs(){ui.candidateTabs.innerHTML=Object.entries(CATEGORY_LABEL).map(([key,label])=>`<button type="button" data-ai-category="${key}" class="${currentCategory===key?'active':''}">${label}</button>`).join('');ui.candidateTabs.querySelectorAll('[data-ai-category]').forEach(btn=>btn.onclick=()=>{currentCategory=btn.dataset.aiCategory;renderCandidateTabs();renderCandidateList();});}
  function renderCandidateList(){const q=String(ui.candidateSearch.value||'').trim().toLowerCase();const rows=definitions(catalog).filter(row=>row.node_type===currentCategory&&(!q||[row.id,row.name,row.description,...(row.tags||[])].join(' ').toLowerCase().includes(q)));ui.candidateList.innerHTML=rows.map(row=>{const fields=Adapter.inputDescriptors(row,catalog?.refs||{}),req=fields.filter(f=>f.required).map(f=>f.label).join(' / ');return `<button type="button" class="ai-candidate-card ${esc(row.node_type)}" data-ai-master="${esc(row.id)}"><span class="ai-candidate-name">${esc(row.name)}</span><span class="ai-candidate-desc">${esc(row.description||'')}</span><span class="ai-candidate-meta">${req?`必須: ${esc(req)}`:'設定なし'} / ${row.node_type==='condition'?'TRUE・FALSE':row.node_type==='action'?'終端':'出口1'}</span></button>`;}).join('');ui.candidateEmpty.classList.toggle('hidden',rows.length>0);ui.candidateEmpty.textContent=definitions(catalog).length?'条件に一致するチップがありません。':'正式AI MasterがGameデータに配置されていません。';ui.candidateList.querySelectorAll('[data-ai-master]').forEach(btn=>btn.onclick=()=>{const def=definitionById(catalog,btn.dataset.aiMaster);if(!def)return;if(candidateContext?.replaceInstanceId)openConfig(def,{mode:'replace',instanceId:candidateContext.replaceInstanceId},{});else openNewConfig(btn.dataset.aiMaster);});}
  ...
  function renderConfigValidation(def){const values=valuesFromForm(),errors=activeSession.parameterErrors(def,values);ui.configErrors.innerHTML=errors.map(e=>`<div>${esc(e)}</div>`).join('');ui.configApply.disabled=errors.length>0;}
  ...
  function applyConfig(){if(!configContext)return;const def=definitionById(catalog,configContext.masterId);if(!def)return;const values=valuesFromForm(),errors=activeSession.parameterErrors(def,values);if(errors.length)return;try{if(configContext.mode==='new')activeSession.add(def.id,values,configContext.x,configContext.y);else if(configContext.mode==='replace')activeSession.replace(configContext.instanceId,def.id,values);else activeSession.updateParameters(configContext.instanceId,values);closeSubscreen();renderBoard();}catch(error){notify(String(error?.message||error),'bad');}}
  function replaceExisting(){if(!configContext?.instanceId)return;const node=activeSession.program().nodes.find(row=>row.instance_id===configContext.instanceId);if(!node)return;const chip=activeSession.layout().chips.find(row=>row.instance_id===node.instance_id);candidateContext={x:chip?.x||0,y:chip?.y||0,replaceInstanceId:node.instance_id};ui.candidateSearch.value='';currentCategory=node.node_type;renderCandidateTabs();renderCandidateList();showScreen('candidate');}
  ...
  function jumpToIssue(){if(!lastEvaluation?.issues?.length)return;const rows=lastEvaluation.issues.filter(row=>row.node_id||row.target_node_id);if(!rows.length)return;const row=rows[issueCursor%rows.length];issueCursor=(issueCursor+1)%rows.length;const id=String(row.node_id||row.target_node_id),cell=[...ui.board.querySelectorAll('[data-node-id]')].find(el=>el.dataset.nodeId===id);if(cell){cell.scrollIntoView({behavior:'smooth',block:'center',inline:'center'});cell.focus();}}
  async function open(options){const opts=options||{};activeCharacter=opts.character||null;notify=typeof opts.notify==='function'?opts.notify:()=>{};saveHandler=typeof opts.onSave==='function'?opts.onSave:null;presetHandler=typeof opts.onPresetAction==='function'?opts.onPresetAction:null;userPresets=clone(opts.userPresets||[]);if(!activeCharacter)return;if(!ui)bindDom();ui.title.textContent=`${activeCharacter.name||'冒険者'} — AIチップ編集`;ui.overlay.classList.add('open');ui.overlay.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';ui.state.textContent='正式AI Masterを読込中…';try{catalog=await Catalog.load({aiUrl:opts.aiUrl,skillUrl:opts.skillUrl,templateUrl:opts.templateUrl,tags:opts.tags,tag_categories:opts.tag_categories});}catch(error){catalog=Catalog.normalize([],[],[],[]);notify(String(error?.message||error),'bad');}const key=String(activeCharacter.id||'default');if(!sessions.has(key))sessions.set(key,createSession(catalog,{program:opts.program,layout:opts.layout,program_id:`AIP-DRAFT-${key.replace(/[^A-Za-z0-9_.-]/g,'_')}`,layout_id:'AIL-0001',name:`${activeCharacter.name||''} AI`}));activeSession=sessions.get(key);issueCursor=0;renderBoard();}
  function close(){if(!ui)return;showScreen('board');ui.overlay.classList.remove('open');ui.overlay.setAttribute('aria-hidden','true');document.body.style.overflow='';activeCharacter=null;activeSession=null;saveHandler=null;presetHandler=null;userPresets=[];}
  function resetSessions(characterId){if(characterId!=null)sessions.delete(String(characterId));else sessions.clear();}
  function bindDom(){const $=id=>document.getElementById(id);ui={overlay:$('aiEditor'),title:$('aiEditorTitle'),state:$('aiEditorState'),board:$('aiBoard'),boardWrap:$('aiBoard')?.parentElement,catalogNotice:$('aiCatalogNotice'),close:$('aiEditorClose'),save:$('aiEditorSave'),undo:$('aiUndo'),redo:$('aiRedo'),clear:$('aiClear'),issueJump:$('aiIssueJump'),presetOpen:$('aiPresetOpen'),preset:$('aiPresetScreen'),presetBack:$('aiPresetBack'),presetCreate:$('aiPresetCreate'),presetOfficial:$('aiPresetOfficial'),presetUser:$('aiPresetUser'),candidate:$('aiCandidateScreen'),candidateBack:$('aiCandidateBack'),candidateSearch:$('aiCandidateSearch'),candidateTabs:$('aiCandidateTabs'),candidateList:$('aiCandidateList'),candidateEmpty:$('aiCandidateEmpty'),config:$('aiConfigScreen'),configBack:$('aiConfigBack'),configTitle:$('aiConfigTitle'),configPort:$('aiConfigPort'),configBody:$('aiConfigBody'),configErrors:$('aiConfigErrors'),configApply:$('aiConfigApply'),configDelete:$('aiConfigDelete'),configReplace:$('aiConfigReplace'),configRotate:$('aiConfigRotate')};ui.close.onclick=close;ui.save.onclick=saveCurrent;ui.undo.onclick=()=>{if(activeSession?.undo())renderBoard();};ui.redo.onclick=()=>{if(activeSession?.redo())renderBoard();};ui.clear.onclick=()=>{if(activeSession&&confirm('編集中の盤面をすべて消去しますか？')){activeSession.clear();renderBoard();}};ui.issueJump.onclick=jumpToIssue;ui.presetOpen.onclick=openPresets;ui.presetBack.onclick=closeSubscreen;ui.presetCreate.onclick=()=>presetAction('create');ui.candidateBack.onclick=closeSubscreen;ui.candidateSearch.oninput=renderCandidateList;ui.configBack.onclick=()=>{if(configContext?.mode==='new'||configContext?.mode==='replace')showScreen('candidate');else closeSubscreen();};ui.configApply.onclick=applyConfig;ui.configDelete.onclick=()=>{if(configContext?.instanceId&&confirm('このチップを削除しますか？')){activeSession.remove(configContext.instanceId);closeSubscreen();renderBoard();}};ui.configReplace.onclick=replaceExisting;ui.configRotate.onclick=()=>{if(configContext?.instanceId&&activeSession.rotate(configContext.instanceId,90)){closeSubscreen();renderBoard();}};}
