(function(root,factory){
  ...
  let ui=null,activeCharacter=null,activeSession=null,catalog=null,currentCategory='condition',candidateContext=null,configContext=null,moveContext=null,notify=()=>{},saveHandler=null,presetHandler=null,userPresets=[],issueCursor=0,lastEvaluation=null,savedDocumentSignature='',saveInFlight=false;
  ...
  function createSession(c,options){
    ...
    function remove(instanceId){if(!program.nodes.some(row=>row.instance_id===instanceId))return false;commit(()=>{program.nodes=program.nodes.filter(row=>row.instance_id!==instanceId);program.edges=program.edges.filter(edge=>edge.from?.node_id!==instanceId&&edge.to?.node_id!==instanceId);layout=Layout.removeChip(layout,instanceId);if(program.entry_node_id===instanceId)program.entry_node_id='';});return true;}
