(function(root,factory){
  ...
  function createSession(c,options){
    ...
    function evaluateDocuments(programValue,layoutValue){
      const baseProgram=Program.normalizeProgram(programValue),baseLayout=Layout.normalizeLayout(layoutValue),data=projectData(c);let resolution,resolvedProgram,validation,generated=true,generationError='';
    ...
    function previewNode(masterId,parameters,x,y,rotation=0){
      const def=definitionById(c,masterId);if(!def)throw new Error('AI部品Masterが見つかりません。');const errors=Adapter.validateParameters(def,parameters,c?.refs||{});if(errors.length)throw new Error(errors.join('\n'));assertFree(x,y);
    ...
    return Object.freeze({program:()=>clone(program),layout:()=>clone(layout),nodeAt,extensionAt,definitionFor,add,addExtension,updateParameters,replace,updateExtension,rotate,rotateExtension,moveNode,moveExtension,remove,removeExtension,clear,undo:undoOnce,redo:redoOnce,canUndo:()=>undo.length>0,canRedo:()=>redo.length>0,parameterErrors:(def,values)=>Adapter.validateParameters(def,values,c?.refs||{}),descriptors:def=>Adapter.inputDescriptors(def,c?.refs||{}),catalog:()=>c,evaluate,previewNode,previewExtension,previewMoveNode,previewMoveExtension,replaceDocuments,loadDocuments});
  }
  ...
  function renderBoard(){
    ...
    for(let y=0;y<layout.height;y++)for(let x=0;x<layout.width;x++){
      ...
      if(node){const def=activeSession.definitionFor(node),summary=summaryFor(activeSession,node),entry=program.entry_node_id===node.instance_id,severity=nodeSeverity.get(node.instance_id);cell.classList.add('occupied');if(severity==='ERROR')cell.classList.add('ai-node-error');else if(severity==='WARNING')cell.classList.add('ai-node-warning');if(moveContext?.kind==='node'&&moveContext.id===node.instance_id)cell.classList.add('ai-move-source');cell.dataset.nodeId=node.instance_id;cell.innerHTML=`<span class="ai-formal-chip ${esc(node.node_type)}">${portHtml(def,chip.rotation)}${entry?'<span class="ai-entry-badge">START</span>':''}<b>${esc(def?.name||node.master_node_id)}</b>${summary?`<small>${esc(summary)}</small>`:''}<small class="ai-rotation">${chip.rotation}°</small></span>`;cell.onclick=()=>{if(!moveContext)openExistingConfig(node.instance_id);};}
      else if(extension){const severity=extensionSeverity.get(extension.id);cell.classList.add('occupied','ai-extension-cell');if(severity==='ERROR')cell.classList.add('ai-node-error');else if(severity==='WARNING')cell.classList.add('ai-node-warning');if(moveContext?.kind==='extension'&&moveContext.id===extension.id)cell.classList.add('ai-move-source');cell.dataset.extensionId=extension.id;cell.innerHTML=`<span class="ai-formal-chip extension">${extensionPortHtml(extension)}<b>${extension.shape==='straight'?'直線延長':'曲げ延長'}</b><small>${esc(extension.id)}</small><small class="ai-rotation">${extension.rotation}°</small></span>`;cell.onclick=()=>{if(!moveContext)openExistingExtensionConfig(extension.id);};}
    ...
    ui.undo.disabled=!activeSession.canUndo();ui.redo.disabled=!activeSession.canRedo();renderValidationState(evaluation);renderMovePreview();if(moveContext)ui.state.textContent=moveContext.destination?'移動プレビューを確認して「この位置へ移動」を押してください。':'移動先の空きマスをタップしてください。';ui.catalogNotice.classList.toggle('hidden',!!(catalog&&definitions(catalog).length));
  }
  ...
  function startMoveFromConfig(){if(!configContext||configContext.mode!=='existing')return;moveContext=configContext.kind==='extension'?{kind:'extension',id:configContext.extensionId,destination:null,evaluation:null}:{kind:'node',id:configContext.instanceId,destination:null,evaluation:null};configContext=null;candidateContext=null;showScreen('board');renderBoard();}
