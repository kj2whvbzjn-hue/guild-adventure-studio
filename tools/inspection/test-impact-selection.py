...
def plan(base,applied,out,integrity_report=None):
    env=os.environ.copy();env['PYTHONDONTWRITEBYTECODE']='1'
    cmd=[sys.executable,'-S','-B',str(PLAN),str(base),str(applied),'--json-output',str(out)]
    if integrity_report: cmd += ['--test-integrity-report',str(integrity_report)]
    r=subprocess.run(cmd,cwd=ROOT,env=env,text=True,capture_output=True,timeout=10)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='impact-selection-regression-') as td:
      ...
      # A machine-verified derived hash ledger sync is generated metadata, so it
      # must not force Full after Test Integrity has already cryptographically verified it.
      a=t/'machine-hash';shutil.copytree(base,a)
      crit=a/'shared/integrity/critical-runtime-manifest.json';crit.write_text('{"schema_version":1,"files":[]}\n',encoding='utf-8')
      (base/'shared/integrity/critical-runtime-manifest.json').write_text('{"schema_version":1,"files":[{"path":"x","size":1,"sha256":"0"}]}\n',encoding='utf-8')
      ti=t/'machine-hash-integrity.json';wj(ti,{'protected_changed_count':0,'machine_derived_hash_sync_paths':['shared/integrity/critical-runtime-manifest.json']})
      r,p=plan(base,a,t/'machine-hash.json',ti)
      if p.get('mode')!='impact' or 'shared/integrity/critical-runtime-manifest.json' in p.get('effective_changed_paths',[]):errors.append('MACHINE_HASH_SYNC_NOT_IGNORED '+json.dumps(p))
    ...
    print('IMPACT_SELECTION_REGRESSION_OK cases=8 fallback=full safety_critical=full scoped=impact build_only=ignored machine_hash_sync=ignored_after_verification stale_plan=blocked')
    return 0
