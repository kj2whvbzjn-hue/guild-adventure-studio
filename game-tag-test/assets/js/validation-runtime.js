/* Extracted without logic changes from game-tag-test/index.html — GA-B475 */
...
function makeCharacter(name,job){return{id:uid(),name,level:1,job,stats:Object.fromEntries(STATS.map(s=>[s,10])),skills:[],aiGraph:defaultAiGraph(),aiPolicy:'lowestHp',equipment:{weapon:null,armor:null,accessory:null},jobHistory:[{job,level:1,at:new Date().toISOString()}],growthHistory:[],createdAt:new Date().toISOString()}}
function normalize(raw){
 ...
 raw.characters.forEach(c=>{
  ...
  c.skills=Array.isArray(c.skills)?c.skills:[];c.aiGraph=normalizeAiGraph(c.aiGraph);
...
function render(){
 ...
 const c=data.characters.find(x=>x.id===selectedId);$('detailCard').classList.toggle('hidden',!c);if(!c)return;
 if($('changeJob'))$('changeJob').value=c.job;const currentA=JOBS[c.job];
 $('detail').innerHTML=`<div class="row"><div><div class="name">${escapeHtml(c.name)}</div><span class="tag">Lv ${c.level} / 50</span><span class="tag">現在職：${c.job}</span></div></div><div class="stats">${STATS.map(s=>`<div class="stat"><span class="small">${s}</span><b>${c.stats[s]}</b></div>`).join('')}</div><h3>装備</h3><div class="small">${Object.values(c.equipment||{}).filter(Boolean).join(' / ')||'なし'}（攻撃 +${equipmentBonus(c).attack} / HP +${equipmentBonus(c).maxHp} / AGI ${equipmentBonus(c).agi>=0?'+':''}${equipmentBonus(c).agi}）</div><h3>転職履歴</h3><div class="small">${c.jobHistory.map(h=>`Lv${h.level} ${h.job}`).join(' → ')}</div><h3>直近の成長</h3><div class="small">${c.growthHistory.slice(-5).reverse().map(g=>`Lv${g.toLevel}: ${g.gained.length?g.gained.join(', '):'能力上昇なし'}`).join('<br>')||'まだレベルアップしていません。'}</div>`;
...
const TAG_SKILL_TEST_BUILD='GA-B474 / Sprint 2.4 / DOT Defeat Verification';
const TAG_SKILLS=[
 {id:'SKL-TEST-ATTACK',name:'タグ攻撃テスト',tags:['ATTACK','敵','単体','物理','DAMAGE=100']},
 {id:'SKL-TEST-HEAVY',name:'強打テスト',tags:['ATTACK','敵','単体','物理','DAMAGE=150']},
 {id:'SKL-TEST-INVALID',name:'不正攻撃（DAMAGEなし）',tags:['ATTACK','敵','単体','物理']},
 {id:'SKL-TEST-POISON',name:'毒斬り（DOT実装確認）',tags:['ATTACK','DOT','敵','単体','物理','毒属性','DAMAGE=100','DOT_POWER=20','DOT_DURATION=1000','DOT_INTERVAL=100','STACK_GAIN=1']}
];
...
/* Sprint 4 console synchronized — GA-B486.4 */
...
function currentValidationGameBuild(){return window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN'}
...
function makeHealValidationCase({id,label,initialState,events,finalState,expectations,errors,result}){
 return{id,label,initial_state:initialState,events,final_state:finalState,expectations,result,passed:errors.length===0,errors};
}
function runHealDeviceJsonValidation(){
 pauseBattle();
 const cases=[];
 ...
 runCase('HEAL-SINGLE','単体回復',()=>{
  const actor=battle.units.find(x=>x.alive&&x.side==='味方');
  const target=battle.units.find(x=>x.alive&&x.side==='味方'&&x.id!==actor?.id);
  const skill=findTagSkill('SKL-TEST-HEAL-100'),errors=[];
  if(!actor||!target||!skill)errors.push('必要な使用者・対象・スキルがありません');
  if(errors.length)return makeHealValidationCase({id:'HEAL-SINGLE',label:'単体回復',initialState:null,events:[],finalState:null,expectations:{},result:null,errors});
  target.hp=Math.max(1,target.maxHp-50);
  const initial={actor:cloneHealValidationUnit(actor),target:cloneHealValidationUnit(target),skill:{id:skill.id,tags:[...(skill.tags||[])]}};
  const result=executeTaggedSkill(actor,target,skill),finalState={actor:cloneHealValidationUnit(actor),target:cloneHealValidationUnit(target)};
  if(!result.ok)errors.push(`実行失敗: ${result.reason||result.stage||'unknown'}`);
  if(finalState.target.hp!==finalState.target.max_hp)errors.push(`最大HP到達不一致: ${finalState.target.hp}/${finalState.target.max_hp}`);
  if(result.healResult?.healed!==50)errors.push(`実回復量不一致: ${result.healResult?.healed}/50`);
  if(result.healResult?.overheal!==50)errors.push(`超過回復量不一致: ${result.healResult?.overheal}/50`);
  return makeHealValidationCase({id:'HEAL-SINGLE',label:'単体回復',initialState:initial,events:[...(battle.validationEvents||[])],finalState,expectations:{healed:50,overheal:50,hp_capped_at_max:true},result,errors});
 });
 runCase('HEAL-ALL','味方全体回復',()=>{
  const allies=battle.units.filter(x=>x.alive&&x.side==='味方'),actor=allies[0],skill=findTagSkill('SKL-TEST-HEAL-ALL-60'),errors=[];
  if(!actor||allies.length<2||!skill)errors.push('必要な味方・使用者・スキルがありません');
  if(errors.length)return makeHealValidationCase({id:'HEAL-ALL',label:'味方全体回復',initialState:null,events:[],finalState:null,expectations:{},result:null,errors});
  allies.forEach((u,i)=>u.hp=Math.max(1,u.maxHp-(30+i*40)));
  const initial={actor:cloneHealValidationUnit(actor),targets:allies.map(cloneHealValidationUnit),skill:{id:skill.id,tags:[...(skill.tags||[])]}};
  const result=executeTaggedSkill(actor,allies[1],skill),finalState={targets:allies.map(cloneHealValidationUnit)};
  if(!result.ok)errors.push(`実行失敗: ${result.reason||result.stage||'unknown'}`);
  if(result.targets?.length!==allies.length)errors.push(`対象数不一致: ${result.targets?.length||0}/${allies.length}`);
  finalState.targets.forEach((u,i)=>{const expected=Math.min(initial.targets[i].max_hp,initial.targets[i].hp+60);if(u.hp!==expected)errors.push(`${u.id} HP不一致: ${u.hp}/${expected}`)});
  return makeHealValidationCase({id:'HEAL-ALL',label:'味方全体回復',initialState:initial,events:[...(battle.validationEvents||[])],finalState,expectations:{heal:60,target_count:allies.length,enemy_unchanged:true},result,errors});
 });
 runCase('HEAL-DEAD-REJECT','戦闘不能対象拒否',()=>{
  const actor=battle.units.find(x=>x.alive&&x.side==='味方');
  const target=battle.units.find(x=>x.alive&&x.side==='味方'&&x.id!==actor?.id);
  const skill=findTagSkill('SKL-TEST-HEAL-100'),errors=[];
  if(!actor||!target||!skill)errors.push('必要データがありません');
  if(errors.length)return makeHealValidationCase({id:'HEAL-DEAD-REJECT',label:'戦闘不能対象拒否',initialState:null,events:[],finalState:null,expectations:{},result:null,errors});
  target.hp=0;target.alive=false;
  const initial={actor:cloneHealValidationUnit(actor),target:cloneHealValidationUnit(target)};
  const result=executeTaggedSkill(actor,target,skill),finalState={target:cloneHealValidationUnit(target)};
  if(result.ok)errors.push('戦闘不能対象への通常回復が成功しました');
  if(target.hp!==0||target.alive)errors.push('戦闘不能対象の状態が変更されました');
  return makeHealValidationCase({id:'HEAL-DEAD-REJECT',label:'戦闘不能対象拒否',initialState:initial,events:[...(battle.validationEvents||[])],finalState,expectations:{execution_ok:false,hp:0,alive:false},result,errors});
 });
 runCase('HEAL-ENEMY-REJECT','敵対象拒否',()=>{
  const actor=battle.units.find(x=>x.alive&&x.side==='味方'),target=battle.units.find(x=>x.alive&&x.side==='敵'),skill=findTagSkill('SKL-TEST-HEAL-100'),errors=[];
  if(!actor||!target||!skill)errors.push('必要データがありません');
  if(errors.length)return makeHealValidationCase({id:'HEAL-ENEMY-REJECT',label:'敵対象拒否',initialState:null,events:[],finalState:null,expectations:{},result:null,errors});
  target.hp=Math.max(1,target.maxHp-50);const initial={actor:cloneHealValidationUnit(actor),target:cloneHealValidationUnit(target)};
  const result=executeTaggedSkill(actor,target,skill),finalState={target:cloneHealValidationUnit(target)};
  if(result.ok)errors.push('味方用回復が敵対象へ成功しました');
  if(finalState.target.hp!==initial.target.hp)errors.push('拒否後に敵HPが変化しました');
  return makeHealValidationCase({id:'HEAL-ENEMY-REJECT',label:'敵対象拒否',initialState:initial,events:[...(battle.validationEvents||[])],finalState,expectations:{execution_ok:false,hp_unchanged:true},result,errors});
 });
 runCase('HEAL-INVALID-DATA-REJECT','不正回復データ拒否',()=>{
  const errors=[],invalid={id:'SKL-INVALID-HEAL-0',name:'不正回復',tags:['HEAL','味方','単体','HEAL=0']},compiled=compileTaggedSkill(invalid);
  if(compiled.ok)errors.push('HEAL=0の不正データがコンパイルを通過しました');
  if(!compiled.errors.some(x=>x.includes('0より大きい')))errors.push('期待するHEAL値エラーがありません');
 ...
 const report={schema_version:'1.3.0',build:currentValidationGameBuild(),generated_at:new Date().toISOString(),test:{id:'TAG-HEAL-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunHealJson'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-heal-device-validation-${currentValidationGameBuild()}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);
...
function prepareShieldValidationFixture(){pauseBattle();resetBattle();battle.validationMode=true;battle.validationEvents=[];battle.tick=0;for(const fixture of SHIELD_VALIDATION_SKILLS){const i=TAG_SKILLS.findIndex(x=>x.id===fixture.id);const row={...fixture,source:'validation_fixture',environment:'validation'};if(i>=0)TAG_SKILLS.splice(i,1,row);else TAG_SKILLS.push(row)}const allies=ensureValidationTargets('味方',3),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.alive=true;u.hp=u.maxHp;u.gauge=0;u.reservedAction=null;u.shieldEffects=[];u.dotStacks=u.dotStacks||[];u.statusEffects=u.statusEffects||[];u.modifierStacks=u.modifierStacks||[]}return{allies,enemies,skills:{single:findTagSkill('SKL-TEST-SHIELD-100'),all:findTagSkill('SKL-TEST-SHIELD-ALL-60'),small:findTagSkill('SKL-TEST-SHIELD-40')}}}
...
function tagTestRunStatusJson(){
 ...
 const report={schema_version:'1.1.0',build:currentValidationGameBuild(),generated_at:new Date().toISOString(),test:{id:'TAG-STATUS-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunStatusJson'},design_decisions:{attack_hit_applies_status:true,application_rate:100,resistance_affects:'duration',resistance_cap_percent:75,duration_formula:'floor(base_duration * (1 - resistance/100))',minimum_duration_tick:1},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-status-device-validation-${currentValidationGameBuild()}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[STATUS DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}`;return report;
}
...
function tagTestRunCleanseJson(){
 ...
 const report={schema_version:'1.0.0',build:currentValidationGameBuild(),generated_at:new Date().toISOString(),test:{id:'TAG-CLEANSE-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunCleanseJson'},current_spec:{scope:'status_only',target_sides:['self','ally'],ranges:['single','all'],count:['CLEANSE_COUNT','CLEANSE_ALL'],order:'oldest',protected_is_not_removed:true,no_effect_is_success:true},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-cleanse-device-validation-${currentValidationGameBuild()}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[CLEANSE DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}`;return report;
}
...
function tagTestRunShieldJson(){const cases=[],allErrors=[];const run=(id,label,fn)=>{try{const c=fn();cases.push(c);if(!c.passed)allErrors.push(...c.errors.map(x=>`${id}: ${x}`))}catch(e){const msg=String(e?.message||e);cases.push(makeShieldCase({id,label,initialState:null,events:[],finalState:null,expectations:{},result:null,errors:[msg]}));allErrors.push(`${id}: ${msg}`)}};
 ...
 const report={schema_version:'1.3.0',build:currentValidationGameBuild(),generated_at:new Date().toISOString(),test:{id:'TAG-SHIELD-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunShieldJson'},design_decisions:{stacking:'additive individual instances',consumption:'FIFO by appliedAt',dot_consumes_shield:true,death_clears:true,battle_end_clears:true},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:allErrors.length===0,errors:allErrors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-shield-device-validation-${currentValidationGameBuild()}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[SHIELD DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;renderBattle();return report}
...
function tagTestRunReviveJson(){
 ...
 run('REVIVE-FIXED-REGRESSION','固定値蘇生の回帰維持',()=>{const f=prep();defeat(f.target);const result=executeTaggedSkill(f.actor,f.target,fixed(100)),er=[];if(f.target.hp!==100||result.reviveResult?.reviveMode!=='fixed')er.push('固定値蘇生の回帰に失敗しました');return{id:'REVIVE-FIXED-REGRESSION',label:'固定値蘇生の回帰維持',result,passed:!er.length,errors:er}});
 ...
 const report={schema_version:'1.1.0',build:currentValidationGameBuild(),generated_at:new Date().toISOString(),test:{id:'TAG-REVIVE-RATE-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunReviveJson'},current_spec:{scope:'revive_foundation_rate_hp_tag_validation',target_side:'ally',ranges:['single','all'],fixed_hp_tag:'REVIVE_HP',rate_hp_tag:'REVIVE_HP_RATE',rate_unit:'ratio_0_to_1',rate_formula:'max(1, floor(max_hp * REVIVE_HP_RATE))',fixed_and_rate:'mutually_exclusive',living_single_target:'INVALID_TARGET',death_resets_all_temporary_state:true,revive_restores_previous_effects:false,gauge_after_revive:0,same_tick_action:false,no_dead_all_is_success:true},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-revive-rate-device-validation-${currentValidationGameBuild()}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[REVIVE RATE DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}`;return report;
}
...
function tagTestRunAuraJson(){
 ...
 const report={schema_version:'1.0.0',build:'GA-B486.92',generated_at:new Date().toISOString(),test:{id:'TAG-AURA-DEVICE-001',mode:'device_validation',entrypoint,trigger:'tagTestRunAuraJson'},current_spec:{task_id:'P01-06',stage:'tag_validation',runtime_application:false,supported_effects:['BUFF','DEBUFF'],supported_stats:['ATK','DEF','MAGIC_WEAPON_BONUS','STATUS_RESIST'],value_tag:'AURA_VALUE',target_tag:'AURA_TARGET=<ally|enemy>',scope_tag:'AURA_SCOPE=<all|self_and_allies|allies_excluding_self>',stacking:'highest only',status_aura:'deferred',additive:'deferred',unique_source:'deferred'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-aura-device-validation-GA-B486.92-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[AURA DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.map(x=>' - '+x).join('\n'):''}\n[JSON] 出力完了`;return report;
}


async function runR04TriggerFoundationDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint=location.pathname.includes('game-tag-test')?'game-tag-test/index.html':'game/index.html',engine=globalThis.GKSTriggerEngine,bridge=window.GKSGenericSkillBridge,cases=[],rejectionCases=[],errors=[];
 ...
 if(!bridge?.loadRegistry)throw new Error('GKSGenericSkillBridge.loadRegistry is not loaded');
...
async function runR05GenericDirectRuntimeDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint='game-tag-test/index.html',cases=[],fixtures=[];
 const add=async(id,label,run)=>{try{const detail=await run(),errors=detail.errors||[];cases.push({id,label,...detail,passed:errors.length===0,errors})}catch(e){cases.push({id,label,passed:false,errors:[String(e?.message||e)]})}};
 const compileFixture=async(skill,{tamper}={})=>{
  if(!window.GKSGenericSkillBridge?.compileForLegacy)throw new Error('GKSGenericSkillBridge.compileForLegacy がありません');
  const generated=await window.GKSGenericSkillBridge.compileForLegacy(skill,{legacyCompile:compileTaggedSkill});
  if(!generated?.ok)throw new Error(`${skill.id} Generic compile failed: ${(generated?.errors||[]).map(x=>x.code||x.message||String(x)).join(', ')}`);
  const fixture={...generated.legacySkill,tags:[...(generated.legacySkill.tags||[])],source:'r05_current_contract_device_fixture',environment:'validation'};
  if(tamper)fixture.tags=fixture.tags.map(tamper);
  if(findTagSkill(fixture.id))throw new Error(`fixture id collision: ${fixture.id}`);
  TAG_SKILLS.push(fixture);fixtures.push(fixture.id);
  const compiled=compileTaggedSkill(fixture);if(!compiled.ok)throw new Error(`${skill.id} legacy compile failed: ${(compiled.errors||[]).join(', ')}`);
  return{skill,generated,fixture,compiled};
 };
 const prep=()=>{pauseBattle();resetBattle();battle.validationMode=true;battle.validationCaptureEvents=true;battle.validationEvents=[];battle.tick=0;const allies=ensureValidationTargets('味方',3),enemies=ensureValidationTargets('敵',2);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.reservedAction=null;u.statusEffects=[];u.dotStacks=[];u.modifierStacks=[];u.shieldEffects=[];u.coverEffects=[];u.maxMp=100;u.mp=50}return{actor:allies[0],ally:allies[1],ally2:allies[2],enemy:enemies[0],enemy2:enemies[1]}};
 const base=(id,target,effects)=>({schemaVersion:1,id,name:`R05 device ${id}`,trigger:{type:'ON_USE',scope:'SELF'},target,effects,resource:{mpCost:0,cooldown:0}});
 try{
  await add('R05-DEVICE-DAMAGE','R05-A DAMAGE: Legacy値改変後もGeneric契約値で実行',async()=>{const c=await compileFixture(base('R05-DEVICE-DAMAGE',{side:'ENEMY',range:'SINGLE'},[{type:'DAMAGE',power:73,damageType:'FIXED'}]),{tamper:t=>t==='DAMAGE=73'?'DAMAGE=999':t}),f=prep(),before=f.enemy.hp,r=executeTaggedSkill(f.actor,f.enemy,c.fixture,{origin:'base',suppressDerived:true}),events=battle.validationEvents.filter(x=>x.type==='generic_damage_executed'&&x.skill_id===c.fixture.id),errors=[];const dealt=before-f.enemy.hp;if(!r.ok)errors.push(`execute=${r.reason||r.stage}`);if(dealt!==73)errors.push(`damage=${dealt}`);if(events.length!==1)errors.push(`generic_damage_events=${events.length}`);return{source:'r05_current_contract_device_fixture',legacy_tag_tampered:true,damage:dealt,events:[...battle.validationEvents],errors}});
  await add('R05-DEVICE-HEAL','R05-B HEAL: Legacy値改変後もGeneric契約値で実行',async()=>{const c=await compileFixture(base('R05-DEVICE-HEAL',{side:'ALLY',range:'SINGLE'},[{type:'HEAL',power:63}]),{tamper:t=>t==='HEAL=63'?'HEAL=999':t}),f=prep();f.ally.hp=Math.max(1,f.ally.maxHp-100);const before=f.ally.hp,r=executeTaggedSkill(f.actor,f.ally,c.fixture,{origin:'base'}),healed=f.ally.hp-before,events=battle.validationEvents.filter(x=>x.type==='generic_heal_executed'&&x.skill_id===c.fixture.id),errors=[];if(!r.ok)errors.push(`execute=${r.reason||r.stage}`);if(healed!==63)errors.push(`heal=${healed}`);if(events.length!==1)errors.push(`generic_heal_events=${events.length}`);return{source:'r05_current_contract_device_fixture',legacy_tag_tampered:true,healed,events:[...battle.validationEvents],errors}});
  await add('R05-DEVICE-APPLY','R05-C APPLY: STATUS契約を本番Runtimeへ適用',async()=>{const c=await compileFixture(base('R05-DEVICE-APPLY',{side:'ENEMY',range:'SINGLE'},[{type:'APPLY',effectId:'STUN',duration:120}]),{tamper:t=>t==='DURATION=120'?'DURATION=999':t}),f=prep(),r=executeTaggedSkill(f.actor,f.enemy,c.fixture,{origin:'base'}),matches=(f.enemy.statusEffects||[]).filter(x=>x.statusId==='STATUS-STUN'),events=battle.validationEvents.filter(x=>x.type==='generic_apply_executed'&&x.skill_id===c.fixture.id),errors=[];if(!r.ok)errors.push(`execute=${r.reason||r.stage}`);if(matches.length!==1)errors.push(`STATUS-STUN=${matches.length}`);if(matches[0]&&matches[0].baseDurationTick!==120)errors.push(`duration=${matches[0].baseDurationTick}`);if(events.length!==1)errors.push(`generic_apply_events=${events.length}`);return{source:'r05_current_contract_device_fixture',legacy_tag_tampered:true,status_effects:f.enemy.statusEffects,events:[...battle.validationEvents],errors}});
  await add('R05-DEVICE-REMOVE','R05-D REMOVE: Generic count=1をLegacy改変から独立して実行',async()=>{const c=await compileFixture(base('R05-DEVICE-REMOVE',{side:'ALLY',range:'SINGLE'},[{type:'REMOVE',category:'STATUS',count:1}]),{tamper:t=>t==='CLEANSE_COUNT=1'?'CLEANSE_COUNT=9':t}),f=prep();f.ally.statusEffects=[{instanceId:'R05-OLD',sequence:1,statusId:'STATUS-X',appliedTick:1,expiresTick:100,removable:true},{instanceId:'R05-NEW',sequence:2,statusId:'STATUS-Y',appliedTick:2,expiresTick:100,removable:true}];const r=executeTaggedSkill(f.actor,f.ally,c.fixture,{origin:'base'}),events=battle.validationEvents.filter(x=>x.type==='generic_remove_executed'&&x.skill_id===c.fixture.id),errors=[];if(!r.ok)errors.push(`execute=${r.reason||r.stage}`);if(f.ally.statusEffects.length!==1||f.ally.statusEffects[0].instanceId!=='R05-NEW')errors.push(`remaining=${JSON.stringify(f.ally.statusEffects)}`);if(events.length!==1)errors.push(`generic_remove_events=${events.length}`);return{source:'r05_current_contract_device_fixture',legacy_tag_tampered:true,remaining_status_effects:f.ally.statusEffects,events:[...battle.validationEvents],errors}});
  await add('R05-DEVICE-RESOURCE-CHANGE','R05-E RESOURCE_CHANGE(MP): Generic契約でclamp実行',async()=>{const c=await compileFixture(base('R05-DEVICE-RESOURCE',{side:'ALLY',range:'SINGLE'},[{type:'RESOURCE_CHANGE',resource:'MP',amount:30}])),f=prep();f.ally.mp=80;const r=executeTaggedSkill(f.actor,f.ally,c.fixture,{origin:'base'}),events=battle.validationEvents.filter(x=>x.type==='generic_resource_change_executed'&&x.skill_id===c.fixture.id),errors=[];if(!r.ok)errors.push(`execute=${r.reason||r.stage}`);if(f.ally.mp!==100)errors.push(`mp=${f.ally.mp}`);if(events.length!==1)errors.push(`generic_resource_change_events=${events.length}`);return{source:'r05_current_contract_device_fixture',mp:f.ally.mp,events:[...battle.validationEvents],errors}});
  await add('R05-DEVICE-REVIVE','R05-F REVIVE: Legacy値改変後もGeneric hpRateで蘇生',async()=>{const c=await compileFixture(base('R05-DEVICE-REVIVE',{side:'ALLY',range:'SINGLE'},[{type:'REVIVE',hpRate:0.25}]),{tamper:t=>t.startsWith('REVIVE_HP')?t.replace(/=.*/,'=1'):t}),f=prep();f.ally.hp=0;f.ally.alive=false;const r=executeTaggedSkill(f.actor,f.ally,c.fixture,{origin:'base'}),expected=Math.max(1,Math.floor(f.ally.maxHp*0.25)),events=battle.validationEvents.filter(x=>x.type==='generic_revive_executed'&&x.skill_id===c.fixture.id),errors=[];if(!r.ok)errors.push(`execute=${r.reason||r.stage}`);if(!f.ally.alive||f.ally.hp!==expected)errors.push(`revive hp=${f.ally.hp}/${expected} alive=${f.ally.alive}`);if(events.length!==1)errors.push(`generic_revive_events=${events.length}`);return{source:'r05_current_contract_device_fixture',legacy_tag_tampered:true,hp:f.ally.hp,expected_hp:expected,events:[...battle.validationEvents],errors}});
  await add('R05-DEVICE-TARGET-CONTROL','R05-G TARGET_CONTROL(COVER): Generic契約でCOVER登録',async()=>{const effect={type:'TARGET_CONTROL',mode:'COVER',trigger:'DIRECT_ATTACK',priority:7,removable:true,lifetime:'USES',uses:2},c=await compileFixture(base('R05-DEVICE-TARGET-CONTROL',{side:'ALLY',range:'SINGLE'},[effect]),{tamper:t=>t.startsWith('COVER_PRIORITY=')?'COVER_PRIORITY=999':t}),f=prep(),r=executeTaggedSkill(f.actor,f.ally,c.fixture,{origin:'base'}),genericEvents=battle.validationEvents.filter(x=>x.type==='generic_target_control_executed'&&x.skill_id===c.fixture.id),coverEvents=battle.validationEvents.filter(x=>x.type==='cover_added'&&x.skill_id===c.fixture.id),errors=[];if(!r.ok)errors.push(`execute=${r.reason||r.stage}`);if(genericEvents.length!==1)errors.push(`generic_target_control_events=${genericEvents.length}`);if(coverEvents.length!==1)errors.push(`cover_added_events=${coverEvents.length}`);if(coverEvents[0]?.priority!==7)errors.push(`priority=${coverEvents[0]?.priority}`);if(coverEvents[0]?.remaining_uses!==2)errors.push(`uses=${coverEvents[0]?.remaining_uses}`);return{source:'r05_current_contract_device_fixture',legacy_tag_tampered:true,cover_event:coverEvents[0]||null,events:[...battle.validationEvents],errors}});
  await add('R05-DEVICE-SPECIAL-BOUNDARY','R05-H SPECIAL: 現行境界でGeneric compileを拒否',async()=>{const skill=base('R05-DEVICE-SPECIAL',{side:'ENEMY',range:'SINGLE'},[{type:'SPECIAL'}]),generated=await window.GKSGenericSkillBridge.compileForLegacy(skill,{legacyCompile:compileTaggedSkill}),errors=[];if(generated?.ok)errors.push('SPECIALがGeneric/Legacy実行へ流入しました');const codes=(generated?.errors||[]).map(x=>x.code||x.message||String(x));if(!codes.some(x=>String(x).includes('SPECIAL')))errors.push(`SPECIAL boundary error missing: ${codes.join(', ')}`);return{source:'r05_current_contract_device_fixture',compile_ok:!!generated?.ok,compile_errors:codes,errors}});
 }finally{battle.validationCaptureEvents=false;for(const id of fixtures){const i=TAG_SKILLS.findIndex(x=>x.id===id);if(i>=0)TAG_SKILLS.splice(i,1)}}
 const errors=cases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));
 return{schema_version:'1.0.0',build:gameBuild,generated_at:new Date().toISOString(),test:{id:'R05-GENERIC-DIRECT-RUNTIME-DEVICE-001',mode:'current_generic_contract_production_runtime_device_validation',entrypoint,trigger:'tagTestRunR05GenericDirectRuntimeJson'},provenance:{skill_source:'generated_at_test_time_from_current_generic_contract',fixture_source_label:'r05_current_contract_device_fixture',uses_studio_export:false,uses_legacy_demo_export:false,studio_export_acceptance_is_separate:true},scope:['R05-A DAMAGE','R05-B HEAL','R05-C APPLY','R05-D REMOVE','R05-E RESOURCE_CHANGE','R05-F REVIVE','R05-G TARGET_CONTROL(COVER)','R05-H SPECIAL BOUNDARY'],cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
}
async function tagTestRunR05GenericDirectRuntimeJson(){
 pauseBattle();const report=await runR05GenericDirectRuntimeDeviceValidation(),errors=report.summary.errors||[],build=report.build||'UNKNOWN';
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`r05-generic-direct-runtime-device-validation-${build}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[R05 GENERIC DIRECT RUNTIME] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[SOURCE] current Generic contract fixture (Studio Export非依存)\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;renderBattle();return report;
}


async function runR06LegacyGenericRuntimeDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',studioBuild=window.GA_PROJECT_CONFIG?.studioBuild||'UNKNOWN',entrypoint='game-tag-test/index.html',cases=[];
 const migration=window.GKSLegacySkillMigration,compare=window.GKSLegacyGenericRuntimeCompare,compiler=window.GKSGenericSkillCompiler,bridge=window.GKSGenericSkillBridge;
 if(!migration?.dryRunLegacySkill||!compare?.compareRuntimeExecution||!compiler?.compileGenericSkill||!bridge?.loadRegistry)throw new Error('R06 validation modules are not loaded');
 const registry=await bridge.loadRegistry({force:true});
 const prep=(kind)=>{pauseBattle();resetBattle();battle.validationMode=true;battle.validationCaptureEvents=true;battle.validationEvents=[];battle.tick=0;const allies=ensureValidationTargets('味方',3),enemies=ensureValidationTargets('敵',2);for(const u of battle.units){u.hp=u.maxHp;u.alive=true;u.gauge=0;u.reservedAction=null;u.statusEffects=[];u.dotStacks=[];u.modifierStacks=[];u.shieldEffects=[];u.coverEffects=[];u.maxMp=100;u.mp=100}const source=allies[0];let target=kind==='damage'?enemies[0]:allies[1];if(kind==='heal')target.hp=Math.max(1,target.maxHp-120);if(kind==='revive'){target.hp=0;target.alive=false}if(kind==='cleanse')target.statusEffects=[{instanceId:'R06-STATUS-1',sequence:1,statusId:'STATUS-X',sourceId:'E0',targetId:target.id,skillId:'STATUS-X',appliedTick:0,expiresTick:200,removable:true}];return{source,target}};
 const fixtures=[
  {kind:'damage',skill:{id:'R06-DEVICE-DAMAGE',name:'R06 Damage',tags:['ATTACK','敵','単体','固定','DAMAGE=120']}},
  {kind:'heal',skill:{id:'R06-DEVICE-HEAL',name:'R06 Heal',tags:['HEAL','味方','単体','HEAL=80']}},
  {kind:'revive',skill:{id:'R06-DEVICE-REVIVE',name:'R06 Revive',tags:['REVIVE','REVIVE_HP_RATE=0.25','味方','単体']}},
  {kind:'cover',skill:{id:'R06-DEVICE-COVER',name:'R06 Cover',tags:['COVER','COVER_TARGET=single_ally','COVER_TRIGGER=direct_attack','COVER_PRIORITY=4','COVER_REMOVABLE=true','COVER_LIFETIME=uses','COVER_USES=2','味方','単体']}},
  {kind:'cleanse',skill:{id:'R06-DEVICE-CLEANSE',name:'R06 Cleanse',tags:['CLEANSE','CLEANSE_CATEGORY=status','CLEANSE_ORDER=oldest','CLEANSE_COUNT=1','味方','単体']}}
 ];
 try{
  for(const row of fixtures){
   const dry=migration.dryRunLegacySkill(row.skill,{registry,legacyCompile:compileTaggedSkill,genericCompile:compiler.compileGenericSkill});const errors=[];
   if(!dry.ok)errors.push(`migration=${(dry.issues||[]).map(x=>x.code||x.message).join(',')}`);
   let runtime=null;
   if(dry.ok){const generated=compiler.compileGenericSkill(dry.genericSkill,registry,compileTaggedSkill);if(!generated.ok)errors.push(`generic_compile=${(generated.errors||[]).map(x=>x.code||x.message).join(',')}`);else{
    const genericSkill={...generated.legacySkill,tags:[...(generated.legacySkill.tags||[])],source:'r06_current_contract_device_fixture'},legacySkill={id:generated.legacySkill.id,name:generated.legacySkill.name,tags:[...(generated.legacySkill.tags||[])],source:'r06_legacy_fixture'};
    const run=(skill)=>{const state=prep(row.kind),result=executeTaggedSkill(state.source,state.target,skill,{origin:'base',suppressDerived:true});return{result,source:state.source,target:state.target}};
    runtime=compare.compareRuntimeExecution({skillId:row.skill.id,legacyRun:()=>run(legacySkill),genericRun:()=>run(genericSkill)});if(!runtime.ok)errors.push(`runtime=${(runtime.issues||[]).map(x=>x.code||x.message).join(',')}`);
   }}
   cases.push({id:`R06-DEVICE-${row.kind.toUpperCase()}`,label:`R06 ${row.kind} Legacy→Generic dry-run / execution compare`,source:'r06_current_contract_device_fixture',migration:{ok:dry.ok,mode:dry.mode,mutated:dry.mutated,issues:dry.issues||[],roundtrip_ok:dry.roundtripValidation?.ok===true},runtime_compare:runtime,passed:errors.length===0,errors});
  }
 }finally{battle.validationCaptureEvents=false}
 let studioExport={loaded:false,acceptance_ready:false,url:window.GA_PROJECT_CONFIG?.skillExportUrl||'../Export/skill/skills.json',expected:{game_build:gameBuild,studio_build:studioBuild},observed:null,errors:[]};
 try{const res=await fetch(`../Export/skill/skills.json?r06=${Date.now()}`,{cache:'no-store'});if(!res.ok)throw new Error(`HTTP ${res.status}`);const data=await res.json(),rows=Array.isArray(data?.data)?data.data:[];studioExport.loaded=true;studioExport.observed={data_version:data?.data_version||null,generated_by:data?.generated_by||null,generated_at:data?.generated_at||null,count:rows.length};const versionOk=String(data?.data_version||'').startsWith(gameBuild),studioOk=String(data?.generated_by||'').includes(studioBuild);if(!versionOk)studioExport.errors.push(`data_version is not current: ${data?.data_version||'(none)'}`);if(!studioOk)studioExport.errors.push(`generated_by is not current Studio: ${data?.generated_by||'(none)'}`);studioExport.acceptance_ready=versionOk&&studioOk;}catch(e){studioExport.errors.push(String(e?.message||e))}
 const mismatchRejectionCases=[];
 const addMismatchRejection=(id,label,legacyRun,genericRun)=>{const result=compare.compareRuntimeExecution({skillId:id,legacyRun,genericRun}),rejected=result?.ok===false&&Array.isArray(result?.issues)&&result.issues.some(x=>x.code==='RUNTIME_RESULT_MISMATCH'),errors=[];if(!rejected)errors.push('RUNTIME_RESULT_MISMATCH was not rejected');mismatchRejectionCases.push({id,label,expected:'REJECT',actual:result?.ok===false?'REJECT':'ACCEPT',rejection_code:result?.issues?.[0]?.code||null,compare_result:result,passed:errors.length===0,errors})};
 addMismatchRejection('R06-NEGATIVE-STATE-MISMATCH','不一致拒否: target HP差分をRUNTIME_RESULT_MISMATCHで拒否',()=>({result:{ok:true},source:{hp:100,maxHp:100,alive:true},target:{hp:80,maxHp:100,alive:true}}),()=>({result:{ok:true},source:{hp:100,maxHp:100,alive:true},target:{hp:79,maxHp:100,alive:true}}));
 addMismatchRejection('R06-NEGATIVE-EXECUTION-MISMATCH','不一致拒否: execution結果差分をRUNTIME_RESULT_MISMATCHで拒否',()=>({result:{ok:true,targetResults:[{attackResult:{ok:true,damage:20,beforeHp:80,afterHp:60}}]},source:{hp:100,maxHp:100,alive:true},target:{hp:60,maxHp:100,alive:true}}),()=>({result:{ok:true,targetResults:[{attackResult:{ok:true,damage:19,beforeHp:80,afterHp:61}}]},source:{hp:100,maxHp:100,alive:true},target:{hp:61,maxHp:100,alive:true}}));
 const errors=cases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));
 const rejectionErrors=mismatchRejectionCases.flatMap(x=>(x.errors||[]).map(e=>`${x.id}: ${e}`));
 const allErrors=[...errors,...rejectionErrors];
 return{schema_version:'1.1.0',build:gameBuild,generated_at:new Date().toISOString(),test:{id:'R06-LEGACY-GENERIC-RUNTIME-DEVICE-001',mode:'legacy_migration_and_generic_runtime_device_revalidation_with_negative_mismatch_rejection',entrypoint,trigger:'tagTestRunR06LegacyGenericRuntimeJson'},provenance:{fixture_source:'generated_at_test_time_from_r06_legacy_cases',uses_studio_export_for_fixture_cases:false,uses_legacy_demo_export:false,studio_export_acceptance_is_separate:true},scope:['R06-A Legacy→Generic dry-run','R06-B Legacy/Generic execution compare','R06-B intentional mismatch rejection','Current Studio Export freshness gate'],cases,mismatch_rejection_cases:mismatchRejectionCases,studio_export_acceptance:studioExport,summary:{fixture_case_count:cases.length,fixture_passed_count:cases.filter(x=>x.passed).length,fixture_failed_count:cases.filter(x=>!x.passed).length,fixture_runtime_passed:errors.length===0,mismatch_rejection_case_count:mismatchRejectionCases.length,mismatch_rejection_passed_count:mismatchRejectionCases.filter(x=>x.passed).length,mismatch_rejection_failed_count:mismatchRejectionCases.filter(x=>!x.passed).length,mismatch_rejection_passed:rejectionErrors.length===0,studio_export_acceptance_ready:studioExport.acceptance_ready,passed:allErrors.length===0,errors:allErrors}};
}
async function tagTestRunR06LegacyGenericRuntimeJson(){
 pauseBattle();const report=await runR06LegacyGenericRuntimeDeviceValidation(),errors=report.summary.errors||[],build=report.build||'UNKNOWN',studio=report.studio_export_acceptance;
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`r06-legacy-generic-runtime-device-validation-${build}-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[R06 LEGACY/GENERIC] ${report.summary.passed?'PASS':'FAIL'}\n[FIXTURE MATCH] ${report.summary.fixture_passed_count}/${report.summary.fixture_case_count}\n[MISMATCH REJECTION] ${report.summary.mismatch_rejection_passed_count}/${report.summary.mismatch_rejection_case_count}\n[STUDIO EXPORT] ${studio.acceptance_ready?'CURRENT / ACCEPTANCE READY':'STALE OR MISSING / ACCEPTANCE BLOCKED'}\n[EXPORT] ${studio.observed?.data_version||'(none)'} / ${studio.observed?.generated_by||'(none)'}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;renderBattle();return report;
}


function setupTagSkillTestUI(){
 const execute=$('tagTestExecute'),compile=$('tagTestCompile'),actor=$('tagTestActor'),run1000=$('tagTestRun1000'),runStackLimit=$('tagTestRunStackLimit'),runStaggered=$('tagTestRunStaggered'),runDefeat=$('tagTestRunDefeat'),runBuffHighest=$('tagTestRunBuffHighest'),runDebuffHighest=$('tagTestRunDebuffHighest'),runBuffAll=$('tagTestRunBuffAll'),runDebuffAll=$('tagTestRunDebuffAll'),runModifierTargetDeath=$('tagTestRunModifierTargetDeath'),runModifierSourceDeath=$('tagTestRunModifierSourceDeath'),runConditionalFollowUp=$('tagTestRunConditionalFollowUp'),runStudioBridge=$('tagTestRunStudioBridge'),runFormalRegression=$('tagTestRunFormalRegression'),runHealSingle=$('tagTestRunHealSingle'),runHealAll=$('tagTestRunHealAll'),runHealJson=$('tagTestRunHealJson'),runShieldJson=$('tagTestRunShieldJson'),runStatusJson=$('tagTestRunStatusJson'),runCleanseJson=$('tagTestRunCleanseJson'),runReviveJson=$('tagTestRunReviveJson'),runAuraJson=$('tagTestRunAuraJson'),runCounterJson=$('tagTestRunCounterJson'),runCoverJson=$('tagTestRunCoverJson'),runActionDisabledJson=$('tagTestRunActionDisabledJson'),runCooldownJson=$('tagTestRunCooldownJson'),runCostJson=$('tagTestRunCostJson'),runR04TriggerFoundationJson=$('tagTestRunR04TriggerFoundationJson'),runR05GenericDirectRuntimeJson=$('tagTestRunR05GenericDirectRuntimeJson'),runR06LegacyGenericRuntimeJson=$('tagTestRunR06LegacyGenericRuntimeJson'),exportJson=$('tagTestExportJson');if(!execute||execute.dataset.bound)return;
 ...
 if(runHealJson)runHealJson.onclick=runHealDeviceJsonValidation;
 ...
 if(runActionDisabledJson)runActionDisabledJson.onclick=tagTestRunActionDisabledJson;
 if(runCooldownJson)runCooldownJson.onclick=tagTestRunCooldownJson;if(runCostJson)runCostJson.onclick=tagTestRunCostJson;if(runR04TriggerFoundationJson)runR04TriggerFoundationJson.onclick=tagTestRunR04TriggerFoundationJson;if(runR05GenericDirectRuntimeJson)runR05GenericDirectRuntimeJson.onclick=tagTestRunR05GenericDirectRuntimeJson;if(runR06LegacyGenericRuntimeJson)runR06LegacyGenericRuntimeJson.onclick=tagTestRunR06LegacyGenericRuntimeJson;
...
function tagTestRunCooldownJson(){
 const defs=[
  {id:'COOLDOWN-FORMAL-300',label:'COOLDOWN=300正式形',tags:['ATTACK','敵','単体','物理','DAMAGE=100','COOLDOWN=300'],expected:true,expectCooldown:300},
  {id:'COOLDOWN-ZERO',label:'COOLDOWN=0を許可',tags:['ATTACK','敵','単体','物理','DAMAGE=100','COOLDOWN=0'],expected:true,expectCooldown:0},
  {id:'COOLDOWN-OMITTED',label:'未指定は0',tags:['ATTACK','敵','単体','物理','DAMAGE=100'],expected:true,expectCooldown:0},
  {id:'COOLDOWN-HEAL-120',label:'HEALでも共通メタデータとして保持',tags:['HEAL','味方','単体','HEAL=100','COOLDOWN=120'],expected:true,expectCooldown:120},
  {id:'COOLDOWN-BAD-NEGATIVE',label:'COOLDOWN=-1拒否',tags:['ATTACK','敵','単体','物理','DAMAGE=100','COOLDOWN=-1'],expected:false},
  {id:'COOLDOWN-BAD-DECIMAL',label:'COOLDOWN=1.5拒否',tags:['ATTACK','敵','単体','物理','DAMAGE=100','COOLDOWN=1.5'],expected:false}
 ];
 const cases=defs.map(x=>{const result=compileTaggedSkill({id:x.id,name:x.label,tags:x.tags});const errors=[];if(result.ok!==x.expected)errors.push(`expected ${x.expected} but got ${result.ok}`);if(x.expected&&result.definition?.parameters?.cooldown!==x.expectCooldown)errors.push(`cooldown=${result.definition?.parameters?.cooldown} expected=${x.expectCooldown}`);return{...x,result,passed:errors.length===0,errors}}),errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));
 const report={schema_version:'1.0.0',build:'GA-B486.92',generated_at:new Date().toISOString(),test:{id:'TAG-COOLDOWN-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunCooldownJson'},current_spec:{task_id:'P01-10',stage:'tag_validation',runtime_application:false,tag:'COOLDOWN=<integer>',unit:'tick',minimum:0,omitted_value:0,reservation_start:false,execution_eligibility_checks_cd:true,cast_interrupt_start:false,activation_established_starts_cd:true,execution_eligibility_check_once:true,cooldown_multiplier:'not_formalized_here'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-cooldown-device-validation-GA-B486.92-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COOLDOWN JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
function tagTestRunActionDisabledJson(){
 const base=['STATUS','STATUS_ID=STATUS-ACTION-DISABLED','ACTION_DISABLED=true','敵','単体','DURATION=400'];
 const defs=[
  {id:'ACTION-DISABLED-FORMAL',label:'行動不能正式形',tags:base,expected:true},
  {id:'ACTION-DISABLED-ATTACK-STATUS',label:'ATTACK付随STATUSでも正式payloadを保持',tags:['ATTACK',...base,'物理','DAMAGE=100'],expected:true},
  {id:'ACTION-DISABLED-NORMAL-STATUS',label:'通常STATUSはaction_disabledなしで継続利用可能',tags:['STATUS','STATUS_ID=STATUS-ACCURACY-DOWN','敵','単体','DURATION=400'],expected:true},
  {id:'ACTION-DISABLED-BAD-FALSE',label:'ACTION_DISABLED=false拒否',tags:base.map(x=>x==='ACTION_DISABLED=true'?'ACTION_DISABLED=false':x),expected:false},
  {id:'ACTION-DISABLED-BAD-TEXT',label:'ACTION_DISABLED=yes拒否',tags:base.map(x=>x==='ACTION_DISABLED=true'?'ACTION_DISABLED=yes':x),expected:false},
  {id:'ACTION-DISABLED-BAD-NUMERIC',label:'ACTION_DISABLED=1拒否',tags:base.map(x=>x==='ACTION_DISABLED=true'?'ACTION_DISABLED=1':x),expected:false},
  {id:'ACTION-DISABLED-BAD-NO-STATUS',label:'STATUSなしACTION_DISABLED拒否',tags:['ACTION_DISABLED=true','敵','単体'],expected:false},
  {id:'ACTION-DISABLED-BAD-NO-DURATION',label:'DURATIONなし拒否',tags:base.filter(x=>!x.startsWith('DURATION=')),expected:false},
  {id:'ACTION-DISABLED-BAD-NO-ID',label:'STATUS_IDなし拒否',tags:base.filter(x=>!x.startsWith('STATUS_ID=')),expected:false}
 ];
 const cases=defs.map(d=>{const result=compileTaggedSkill({id:d.id,name:d.label,tags:d.tags});const errors=[];if(result.ok!==d.expected)errors.push(`期待 ${d.expected?'VALID':'INVALID'} / 実際 ${result.ok?'VALID':'INVALID'}`);if(d.expected&&d.tags.includes('ACTION_DISABLED=true')&&result.definition.parameters.statusPayload?.action_disabled!==true)errors.push('statusPayload.action_disabled=true が保持されていません');if(d.id==='ACTION-DISABLED-NORMAL-STATUS'&&result.definition.parameters.statusPayload?.action_disabled===true)errors.push('通常STATUSへaction_disabledが混入しました');return{...d,result,passed:errors.length===0,errors};});
 const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));
 const report={schema_version:'1.0.0',build:'GA-B486.92',generated_at:new Date().toISOString(),test:{id:'TAG-ACTION-DISABLED-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunActionDisabledJson'},current_spec:{task_id:'P01-09',stage:'tag_validation',runtime_application:false,status_logic:'STATUS',payload_tag:'ACTION_DISABLED=true',compiled_payload:{action_disabled:true},execution_decision_point:'action_order_execution_eligibility_check',reservation_semantics:'presentation_only_no_execution_right',blocked_actions:['normal_action','skill_action','COUNTER','FOLLOW_UP'],continuous_effects_continue:['DOT','SHIELD','AURA','COVER'],cover_relation_stays_active:true,individual_status_names:'not_formalized_here'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-action-disabled-device-validation-GA-B486.92-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[ACTION DISABLED JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
...
function tagTestRunCounterJson(){
 ...
 const report={schema_version:'1.1.0',build:'GA-B486.92',generated_at:new Date().toISOString(),test:{id:'TAG-COUNTER-DEVICE-001',mode:'device_validation',entrypoint,trigger:'tagTestRunCounterJson'},current_spec:{task_id:'P01-07',stage:'data_driven_tag_validation',runtime_application:false,trigger_tag:'COUNTER_TRIGGER=hit',target_tag:'COUNTER_TARGET=attacker',limit_tag:'COUNTER_LIMIT=1',priority_tag:'COUNTER_PRIORITY=<integer>',require_alive_tag:'COUNTER_REQUIRE_ALIVE=true',allow_zero_damage_tag:'COUNTER_ALLOW_ZERO_DAMAGE=true',incoming_area_attack:false,attack_definition:'existing ATTACK',critical:'existing ATTACK',passive_effects:'existing ATTACK pipeline',counter_chain:false,derived_origin_trigger:false,multiple_execution:false,duplicate_policy:'skill configuration side',dedicated_counter_damage_formula:false},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-counter-device-validation-GA-B486.92-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COUNTER JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${report.summary.errors.length}${report.summary.errors.length?'\n'+report.summary.errors.join('\n'):''}`;return report;
}




function tagTestRunCostJson(){
 const defs=[
  {id:'COST-MP-20',label:'MP_COST=20正式形',tags:['ATTACK','敵','単体','物理','DAMAGE=100','MP_COST=20'],expected:true,expectMp:20},
  {id:'COST-MP-ZERO',label:'MP_COST=0許可',tags:['ATTACK','敵','単体','物理','DAMAGE=100','MP_COST=0'],expected:true,expectMp:0},
  {id:'COST-MP-OMITTED',label:'未指定はコストなし',tags:['ATTACK','敵','単体','物理','DAMAGE=100'],expected:true,expectMp:0},
  {id:'COST-MP-CD',label:'MPコストとCDを併用',tags:['ATTACK','敵','単体','物理','DAMAGE=100','MP_COST=20','COOLDOWN=300'],expected:true,expectMp:20},
  {id:'COST-MP-NEGATIVE',label:'MP_COST負数拒否',tags:['ATTACK','敵','単体','物理','DAMAGE=100','MP_COST=-1'],expected:false}
 ];const cases=defs.map(x=>{const r=compileTaggedSkill({id:x.id,name:x.label,tags:x.tags}),errors=[];if(r.ok!==x.expected)errors.push(`expected=${x.expected} actual=${r.ok}`);if(x.expected&&r.definition.parameters.mpCost!==x.expectMp)errors.push(`mpCost=${r.definition.parameters.mpCost}`);if(x.expected&&x.expectMp>0&&(r.definition.costs?.[0]?.type!=='mp'||r.definition.costs?.[0]?.amount!==x.expectMp))errors.push('共通costs構造不正');return{id:x.id,label:x.label,compiled_ok:r.ok,costs:r.definition.costs,errors,passed:errors.length===0}});const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));const report={schema_version:'1.0.0',build:'GA-B486.92',generated_at:new Date().toISOString(),test:{id:'TAG-COST-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunCostJson'},current_spec:{task_id:'P01-11',stage:'tag_validation',runtime_application:true,tag:'MP_COST=<number>',minimum:0,omitted_value:0,common_cost_shape:true,formal_runtime_type:'mp',future_types:['hp','action_gauge','stack','dedicated_resource','item','multi_cost'],reservation_consumes:false,activation_established_consumes:true,formal_integrated_spec:'戦闘仕様 正式統合版 v5'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-cost-device-validation-GA-B486.92-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COST JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report}

function tagTestRunActivationPriorityJson(){
 const defs=[
  {id:'ACTIVATION-PRIORITY-HIGH',label:'ACTIVATION_PRIORITY=100',tags:['ATTACK','敵','単体','物理','DAMAGE=10','ACTIVATION_PRIORITY=100'],expected:true,expectedPriority:100},
  {id:'ACTIVATION-PRIORITY-ZERO',label:'ACTIVATION_PRIORITY=0',tags:['ATTACK','敵','単体','物理','DAMAGE=10','ACTIVATION_PRIORITY=0'],expected:true,expectedPriority:0},
  {id:'ACTIVATION-PRIORITY-NEGATIVE',label:'ACTIVATION_PRIORITY=-100',tags:['ATTACK','敵','単体','物理','DAMAGE=10','ACTIVATION_PRIORITY=-100'],expected:true,expectedPriority:-100},
  {id:'ACTIVATION-PRIORITY-OMITTED',label:'未指定は0',tags:['ATTACK','敵','単体','物理','DAMAGE=10'],expected:true,expectedPriority:0},
  {id:'ACTIVATION-PRIORITY-DECIMAL',label:'小数拒否',tags:['ATTACK','敵','単体','物理','DAMAGE=10','ACTIVATION_PRIORITY=1.5'],expected:false,expectedPriority:null}
 ];
 const cases=defs.map(d=>{const result=compileTaggedSkill({id:d.id,name:d.label,tags:d.tags}),priority=result.definition?.parameters?.activationPriority??null,passed=result.ok===d.expected&&(d.expectedPriority==null||priority===d.expectedPriority);return{id:d.id,label:d.label,compiled_ok:result.ok,activation_priority:priority,compiler_errors:result.errors,passed,errors:passed?[]:[`期待 compiled=${d.expected}, priority=${d.expectedPriority} / 実際 compiled=${result.ok}, priority=${priority}`]}});
 const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));const report={schema_version:'1.0.0',build:'GA-B486.92',formal_candidate:'P01-12-FORMAL-1',generated_at:new Date().toISOString(),test:{id:'TAG-ACTIVATION-PRIORITY-001',mode:'device_tag_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunActivationPriorityJson'},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',default:0,higher_first:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-device-validation-GA-B486.92-P01-12-VAL2-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY TAG] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] NO\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
