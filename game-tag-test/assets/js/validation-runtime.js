...
/* Sprint 4 console synchronized — GA-B484.1 */
...
function runHealDeviceJsonValidation(){
 ...
 const report={schema_version:'1.3.0',build:'GA-B484.1',generated_at:new Date().toISOString(),test:{id:'TAG-HEAL-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunHealJson'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-heal-device-validation-GA-B484.1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);
...
function shieldUnitSnapshot(u){return{id:u.id,name:u.name,side:u.side,hp:u.hp,max_hp:u.maxHp,alive:u.alive,shield_total:shieldTotal(u),shield_effects:ensureShieldEffects(u).map(x=>({id:x.id,skill_id:x.skillId,amount:x.amount,remaining:x.remaining,applied_at:x.appliedAt,expires_at:x.expiresAt}))}}
