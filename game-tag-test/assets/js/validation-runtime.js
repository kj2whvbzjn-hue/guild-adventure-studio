...
function tagTestRunCoverJson(){
 const base=['COVER','COVER_TARGET=single_ally','COVER_TRIGGER=direct_attack','COVER_PRIORITY=0','COVER_REMOVABLE=true','味方','単体'];
 const casesRaw=[
  {id:'COVER-SINGLE',label:'単体COVER正式形',tags:base,expected:true},
  {id:'COVER-ALL-FOUNDATION',label:'味方全体COVER基盤',tags:['COVER','COVER_TARGET=all_allies','COVER_TRIGGER=direct_attack','COVER_PRIORITY=10','COVER_REMOVABLE=false','味方','全体'],expected:true},
  ...
  {id:'COVER-BAD-USES',label:'未確定USES拒否',tags:[...base,'COVER_USES=1'],expected:false},
  {id:'COVER-BAD-DURATION',label:'未確定DURATION拒否',tags:[...base,'DURATION=300'],expected:false},
  {id:'COVER-BAD-MIXED',label:'COVERとATTACK混在拒否',tags:[...base,'ATTACK','DAMAGE=10','物理'],expected:false}
 ];
 const cases=casesRaw.map(c=>{const result=compileTaggedSkill({id:c.id,name:c.label,tags:c.tags});const passed=result.ok===c.expected;return{...c,result,passed,errors:passed?[]:[`expected ${c.expected?'valid':'invalid'} but got ${result.ok?'valid':'invalid'}`]}});
 const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));
 const entrypoint=location.pathname.includes('game-tag-test')?'game-tag-test/index.html':'game/index.html';
 const report={schema_version:'1.0.0',build:'GA-B486.34',generated_at:new Date().toISOString(),test:{id:'TAG-COVER-DEVICE-001',mode:'device_validation',entrypoint,trigger:'tagTestRunCoverJson'},current_spec:{task_id:'P01-08',stage:'tag_validation',runtime_application:false,target_foundation:['single_ally','all_allies'],initial_formal_skill:'single_ally',area_attack_cover_limit_per_request:1,direct_attack_origins:['base','counter','follow_up'],standalone_dot:false,standalone_status:false,attached_dot_status:'follow_final_target',cover_chain:false,cover_increments_derived_generation:false,counter_after_cover:true,derived_generation_control:'runtime_context_only',removable_tag:'COVER_REMOVABLE=<true|false>',priority_tag:'COVER_PRIORITY=<integer>',priority_order_details:'deferred_P01-12_P01-13',cover_uses:'not_formal',cover_duration:'not_formal'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-cover-device-validation-GA-B486.34-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[COVER JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
