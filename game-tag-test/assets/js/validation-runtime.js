...
function ensureHealValidationFixtures(){
 ...
 for(const fixture of HEAL_VALIDATION_SKILLS){
  if(!findSkill(fixture.id))SKILLS.push({id:fixture.id,name:fixture.name,tags:[...fixture.tags]});
 }
 return{allies,enemies,skills:{single:findSkill('SKL-TEST-HEAL-100'),all:findSkill('SKL-TEST-HEAL-ALL-60')}};
}
...
function runHealDeviceJsonValidation(){
 ...
 runCase('HEAL-SINGLE','単体回復',()=>{
  const actor=battle.units.find(x=>x.alive&&x.side==='味方');
  const target=battle.units.find(x=>x.alive&&x.side==='味方'&&x.id!==actor?.id);
  const skill=findSkill('SKL-TEST-HEAL-100'),errors=[];
  ...
  const result=executeSkillRuntime(actor,target,skill),finalState={actor:cloneHealValidationUnit(actor),target:cloneHealValidationUnit(target)};
 ...
 runCase('HEAL-ALL','味方全体回復',()=>{
  const allies=battle.units.filter(x=>x.alive&&x.side==='味方'),actor=allies[0],skill=findSkill('SKL-TEST-HEAL-ALL-60'),errors=[];
  ...
  const result=executeSkillRuntime(actor,allies[1],skill),finalState={targets:allies.map(cloneHealValidationUnit)};
  if(!result.ok)errors.push(`実行失敗: ${result.reason||result.stage||'unknown'}`);
 ...
 runCase('HEAL-DEAD-REJECT','戦闘不能対象拒否',()=>{
  ...
  const result=executeSkillRuntime(actor,target,skill),finalState={target:cloneHealValidationUnit(target)};
 ...
 runCase('HEAL-ENEMY-REJECT','敵対象拒否',()=>{
  const actor=battle.units.find(x=>x.alive&&x.side==='味方'),target=battle.units.find(x=>x.alive&&x.side==='敵'),skill=findSkill('SKL-TEST-HEAL-100'),errors=[];
  if(!actor||!target||!skill)errors.push('必要データがありません');
 ...
 runCase('HEAL-INVALID-DATA-REJECT','不正回復データ拒否',()=>{
  const errors=[],invalid={id:'SKL-INVALID-HEAL-0',name:'不正回復',tags:['HEAL','味方','単体','HEAL=0']},compiled=compileSkillForRuntime(invalid);
...
function prepareShieldValidationFixture(){pauseBattle();resetBattle();battle.validationMode=true;battle.validationEvents=[];battle.tick=0;for(const fixture of SHIELD_VALIDATION_SKILLS){const i=SKILLS.findIndex(x=>x.id===fixture.id);const row={...fixture,source:'validation_fixture',environment:'validation'};if(i>=0)SKILLS.splice(i,1,row);else SKILLS.push(row)}const allies=ensureValidationTargets('味方',3),enemies=ensureValidationTargets('敵',1);for(const u of battle.units){u.alive=true;u.hp=u.maxHp;u.gauge=0;u.reservedAction=null;u.shieldEffects=[];u.dotStacks=u.dotStacks||[];u.statusEffects=u.statusEffects||[];u.modifierStacks=u.modifierStacks||[]}return{allies,enemies,skills:{single:findSkill('SKL-TEST-SHIELD-100'),all:findSkill('SKL-TEST-SHIELD-ALL-60'),small:findSkill('SKL-TEST-SHIELD-40')}}}
...
function tagTestRunReviveJson(){
 ...
 run('REVIVE-FIXED-REGRESSION','固定値蘇生の回帰維持',()=>{const f=prep();defeat(f.target);const result=executeSkillRuntime(f.actor,f.target,fixed(100)),er=[];if(f.target.hp!==100||result.reviveResult?.reviveMode!=='fixed')er.push('固定値蘇生の回帰に失敗しました');return{id:'REVIVE-FIXED-REGRESSION',label:'固定値蘇生の回帰維持',result,passed:!er.length,errors:er}});
...
async function runR04TriggerFoundationDeviceValidation(){
 const gameBuild=window.GA_PROJECT_CONFIG?.gameBuild||'UNKNOWN',entrypoint=location.pathname.includes('game-tag-test')?'game-tag-test/index.html':'game/index.html',engine=globalThis.GKSTriggerEngine,bridge=window.GKSSkillCompileService,cases=[],rejectionCases=[],errors=[];
...
async function runR05GenericDirectRuntimeDeviceValidation(){
 ...
 const compileFixture=async(skill,{tamper}={})=>{
  if(!window.GKSSkillCompileService?.compileSkill)throw new Error('GKSSkillCompileService.compileSkill がありません');
  const generated=await window.GKSSkillCompileService.compileSkill(skill,{legacyCompile:compileTaggedSkill});
  if(!generated?.ok)throw new Error(`${skill.id} Generic compile failed: ${(generated?.errors||[]).map(x=>x.code||x.message||String(x)).join(', ')}`);
  const fixture={...(generated.compiledSkill||generated.legacySkill),tags:[...((generated.compiledSkill||generated.legacySkill).tags||[])],source:'r05_current_contract_device_fixture',environment:'validation'};
  if(tamper)fixture.tags=fixture.tags.map(tamper);
  if(findSkill(fixture.id))throw new Error(`fixture id collision: ${fixture.id}`);
  SKILLS.push(fixture);fixtures.push(fixture.id);
  const compiled=compileSkillForRuntime(fixture);if(!compiled.ok)throw new Error(`${skill.id} legacy compile failed: ${(compiled.errors||[]).join(', ')}`);
...
function tagTestRunCooldownJson(){
 ...
 const cases=defs.map(x=>{const result=compileSkillForRuntime({id:x.id,name:x.label,tags:x.tags});const errors=[];if(result.ok!==x.expected)errors.push(`expected ${x.expected} but got ${result.ok}`);if(x.expected&&result.definition?.parameters?.cooldown!==x.expectCooldown)errors.push(`cooldown=${result.definition?.parameters?.cooldown} expected=${x.expectCooldown}`);return{...x,result,passed:errors.length===0,errors}}),errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));
...
function tagTestRunActivationPriorityJson(){
 ...
 const cases=defs.map(d=>{const result=compileSkillForRuntime({id:d.id,name:d.label,tags:d.tags}),priority=result.definition?.parameters?.activationPriority??null,passed=result.ok===d.expected&&(d.expectedPriority==null||priority===d.expectedPriority);return{id:d.id,label:d.label,compiled_ok:result.ok,activation_priority:priority,compiler_errors:result.errors,passed,errors:passed?[]:[`期待 compiled=${d.expected}, priority=${d.expectedPriority} / 実際 compiled=${result.ok}, priority=${priority}`]}});
