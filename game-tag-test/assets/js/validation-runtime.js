...
function tagTestRunReviveJson(){
 ...
 const single=(hp=100)=>({id:'SKL-REVIVE-SINGLE-100',name:'単体蘇生100',tags:['REVIVE','味方','単体',`REVIVE_HP=${hp}`]});
 const all=(hp=100)=>({id:'SKL-TEST-REVIVE-ALL-100',name:'味方全体蘇生100',tags:['REVIVE','味方','全体',`REVIVE_HP=${hp}`]});
 ...
 run('REVIVE-SINGLE-FIXED','死亡対象を固定100HPで蘇生',()=>{const f=prep();defeat(f.target);const result=executeTaggedSkill(f.actor,f.target,single()),er=[];if(!result.ok||!result.reviveResult?.ok)er.push('蘇生に失敗しました');if(f.target.hp!==100||!f.target.alive)er.push(`復帰状態不正:${f.target.hp}/${f.target.alive}`);if(f.target.gauge!==0)er.push('蘇生後ゲージが0ではありません');return{id:'REVIVE-SINGLE-FIXED',label:'死亡対象を固定100HPで蘇生',events:[...battle.validationEvents],final_state:{id:f.target.id,hp:f.target.hp,max_hp:f.target.maxHp,alive:f.target.alive,gauge:f.target.gauge},result,passed:!er.length,errors:er}});
 run('REVIVE-HP-CAP','最大HPを超える固定値は上限処理',()=>{const f=prep();f.target.maxHp=80;defeat(f.target);const result=executeTaggedSkill(f.actor,f.target,single(100)),er=[];if(f.target.hp!==80)er.push(`最大HP上限不一致:${f.target.hp}`);return{id:'REVIVE-HP-CAP',label:'最大HPを超える固定値は上限処理',events:[...battle.validationEvents],result,passed:!er.length,errors:er}});
 run('REVIVE-LIVING-REJECT','生存対象はINVALID_TARGET',()=>{const f=prep(),before=f.target.hp,result=executeTaggedSkill(f.actor,f.target,single()),er=[];if(result.ok||result.stage!=='target')er.push('生存対象が拒否されていません');if(f.target.hp!==before||!f.target.alive)er.push('生存対象が変更されました');return{id:'REVIVE-LIVING-REJECT',label:'生存対象はINVALID_TARGET',result,passed:!er.length,errors:er}});
 run('REVIVE-ALL-DEAD-ONLY','全体蘇生は死亡中の味方だけを復帰',()=>{const f=prep(),living=f.allies[0],dead1=f.allies[1],dead2=f.allies[2],livingHp=living.hp;defeat(dead1);defeat(dead2);const result=executeTaggedSkill(f.actor,f.actor,all(60)),er=[];if(dead1.hp!==60||dead2.hp!==60||!dead1.alive||!dead2.alive)er.push('死亡味方全員を蘇生できていません');if(living.hp!==livingHp||!living.alive)er.push('生存味方が変更されました');if(result.targets?.length!==2)er.push(`対象数不一致:${result.targets?.length}`);return{id:'REVIVE-ALL-DEAD-ONLY',label:'全体蘇生は死亡中の味方だけを復帰',events:[...battle.validationEvents],result,passed:!er.length,errors:er}});
 run('REVIVE-ALL-NONE-OK','死亡者0人の全体蘇生は正常終了',()=>{const f=prep(),result=executeTaggedSkill(f.actor,f.actor,all()),er=[];if(!result.ok||result.targets?.length!==0)er.push('対象0件の正常終了ではありません');return{id:'REVIVE-ALL-NONE-OK',label:'死亡者0人の全体蘇生は正常終了',events:[...battle.validationEvents],result,passed:!er.length,errors:er}});
 run('REVIVE-DEATH-RESET','死亡時に全一時効果をリセット',()=>{const f=prep(),reset=defeat(f.target),er=[];if(f.target.statusEffects.length||f.target.dotStacks.length||f.target.modifierStacks.length||f.target.shieldEffects.length)er.push('死亡時に効果が残っています');if(f.target.gauge!==0||f.target.reservedAction!==null)er.push('死亡時に行動状態が残っています');const result=executeTaggedSkill(f.actor,f.target,single()),post={statuses:f.target.statusEffects.length,dots:f.target.dotStacks.length,modifiers:f.target.modifierStacks.length,shields:f.target.shieldEffects.length,gauge:f.target.gauge,reserved_action:f.target.reservedAction},er2=Object.values(post).some(x=>x!==0&&x!==null);if(er2)er.push('蘇生後に死亡前の効果が復元されました');return{id:'REVIVE-DEATH-RESET',label:'死亡時に全一時効果をリセット',events:[...battle.validationEvents],reset_result:reset,result,final_state:post,passed:!er.length,errors:er}});
 run('REVIVE-INVALID-DATA','不正データ拒否',()=>{const a=compileTaggedSkill({id:'BAD-R1',tags:['REVIVE','味方','単体']}),b=compileTaggedSkill({id:'BAD-R2',tags:['REVIVE','味方','単体','REVIVE_HP=0']}),c=compileTaggedSkill({id:'BAD-R3',tags:['REVIVE','敵','単体','REVIVE_HP=100']}),d=compileTaggedSkill({id:'BAD-R4',tags:['REVIVE','味方','単体','REVIVE_HP_RATE=0.25']}),er=[];if(a.ok||b.ok||c.ok||d.ok)er.push('不正データを受理しました');return{id:'REVIVE-INVALID-DATA',label:'不正データ拒否',result:{missing_hp:a,zero_hp:b,enemy_target:c,rate_not_yet_supported:d},passed:!er.length,errors:er}});
 const report={schema_version:'1.0.0',build:'GA-B486.19',generated_at:new Date().toISOString(),test:{id:'TAG-REVIVE-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunReviveJson'},current_spec:{scope:'revive_foundation_fixed_hp',target_side:'ally',ranges:['single','all'],fixed_hp_tag:'REVIVE_HP',rate_hp_status:'future',living_single_target:'INVALID_TARGET',death_resets_all_temporary_state:true,revive_restores_previous_effects:false,gauge_after_revive:0,same_tick_action:false,no_dead_all_is_success:true},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-revive-device-validation-GA-B486.19-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);$('tagTestResult').textContent=`[REVIVE DEVICE JSON] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}`;return report;
}

function setupTagSkillTestUI(){
 const execute=$('tagTestExecute'),compile=$('tagTestCompile'),actor=$('tagTestActor'),run1000=$('tagTestRun1000'),runStackLimit=$('tagTestRunStackLimit'),runStaggered=$('tagTestRunStaggered'),runDefeat=$('tagTestRunDefeat'),runBuffHighest=$('tagTestRunBuffHighest'),runDebuffHighest=$('tagTestRunDebuffHighest'),runBuffAll=$('tagTestRunBuffAll'),runDebuffAll=$('tagTestRunDebuffAll'),runModifierTargetDeath=$('tagTestRunModifierTargetDeath'),runModifierSourceDeath=$('tagTestRunModifierSourceDeath'),runConditionalFollowUp=$('tagTestRunConditionalFollowUp'),runStudioBridge=$('tagTestRunStudioBridge'),runFormalRegression=$('tagTestRunFormalRegression'),runHealSingle=$('tagTestRunHealSingle'),runHealAll=$('tagTestRunHealAll'),runHealJson=$('tagTestRunHealJson'),runShieldJson=$('tagTestRunShieldJson'),runStatusJson=$('tagTestRunStatusJson'),runCleanseJson=$('tagTestRunCleanseJson'),runReviveJson=$('tagTestRunReviveJson'),exportJson=$('tagTestExportJson');if(!execute||execute.dataset.bound)return;
