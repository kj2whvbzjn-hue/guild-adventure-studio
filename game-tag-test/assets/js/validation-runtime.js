...
function tagTestRunActivationPriorityJson(){
 ...
 const errors=cases.flatMap(x=>x.errors.map(e=>`${x.id}: ${e}`));const report={schema_version:'1.0.0',build:'GA-B486.88',formal_candidate:'P01-12-FORMAL-1',generated_at:new Date().toISOString(),test:{id:'TAG-ACTIVATION-PRIORITY-001',mode:'device_tag_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunActivationPriorityJson'},current_spec:{task_id:'P01-12',stage:'formal_candidate_v1',formal_runtime_enabled:true,tag:'ACTIVATION_PRIORITY=<integer>',default:0,higher_first:true,tie_order:'deferred_P01-13'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-activation-priority-device-validation-GA-B486.88-P01-12-VAL2-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},0);const out=document.getElementById('tagTestResult');if(out)out.textContent=`[P01-12 ACTIVATION PRIORITY TAG] ${report.summary.passed?'PASS':'FAIL'}\n[CASES] ${report.summary.passed_count}/${report.summary.case_count}\n[FORMAL ENABLED] NO\n[ERRORS] ${errors.length}${errors.length?'\n'+errors.join('\n'):''}`;return report;
}
