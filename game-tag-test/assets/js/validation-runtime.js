...
/* Sprint 4 console synchronized — GA-B483.1 */
...
function runHealDeviceJsonValidation(){
 ...
 const report={schema_version:'1.3.0',build:'GA-B483.1',generated_at:new Date().toISOString(),test:{id:'TAG-HEAL-DEVICE-001',mode:'device_validation',entrypoint:'game-tag-test/index.html',trigger:'tagTestRunHealJson'},cases,summary:{case_count:cases.length,passed_count:cases.filter(x=>x.passed).length,failed_count:cases.filter(x=>!x.passed).length,passed:errors.length===0,errors}};
 const blob=new Blob([JSON.stringify(report,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`tag-heal-device-validation-GA-B483.1-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;a.click();URL.revokeObjectURL(a.href);
