/* Validation tag skill compiler/runtime — GA-B486.34 / P01-06 AURA source-dependent runtime v1 */
...
function compileTaggedSkill(skill){
 ...
 if(g.has('COVER')){
  ...
  if(n.COVER_USES)errors.push('COVER_USESは現時点で正式未確定のため指定できません');
  if(n.DURATION)errors.push('COVERのDURATIONは現時点で正式未確定のため指定できません');
 }
 if(g.has('COUNTER')){
