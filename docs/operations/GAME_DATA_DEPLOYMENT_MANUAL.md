...
### 6.5 GitHub版の既存IDを保持する安全モード

AIチップ追加など、既存Gameデータを削らず一部カテゴリだけ追加・差し替えする作業では、Gameデータ配置の **「GitHub版の既存IDを保持（追加・差し替えのみ）」** をONにする。

ONの場合は、今回の正式Exportに存在しないがGitHub版に存在する安定IDを、同じJSON配列へ引き継いでからmanifestを再生成する。これにより、部分更新で意図しない「除外」が発生することを防ぐ。同じIDが今回の正式Exportにも存在する場合は今回側を優先する。

- 部分追加・部分差し替え: ONを推奨
- 意図した除外: OFFにしてIDを個別確認
- ON中に除外が残った場合: 安全のため配置停止
- 保持処理後にFlag・Quest参照整合性を再検証する
- GitHub上のファイル削除は引き続き0件

この安全モードはStudio Project JSONへGitHub側データを逆輸入する機能ではない。Project JSONを正本とする原則は維持し、意図した正式削除はStudio側で整理したうえでOFFにして配置する。
