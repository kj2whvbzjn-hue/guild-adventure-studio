...
async function g07RevalidateSkillBatch(payload){
 ...
 for(let i=0;i<(Array.isArray(batch.skills)?batch.skills:[]).length;i++){
  ...
  entries.push({index:i,skillId:skill?.id||null,status:accepted?'ACCEPT':'REJECT',budgetResult:clone(budget),compilerWarnings:clone(compiled?.warnings||[]),issues});
 }
 return{
...
function renderPanel(){
 ...
 <div class="card skg-step"><h2>⑤ AI一括生成</h2><details class="skg-fold" open><summary>G05 Skill AI Request</summary><p class="small">AIは skillLevel / intent / effects / target / range / desiredStrength / searchMetadata の構造だけを指定します。power・duration等の完成戦闘数値はStudioがG04 Budget Rulesから決定します。</p><div class="toolbar"><button id="skgAiTemplate">G05 AI Requestテンプレート</button><button id="skgG06AiExport">G06 AI Request JSON出力</button></div><input id="skgAiFile" type="file" accept=".json,application/json" style="margin:10px 0"><details><summary><b>AI Request JSONを貼り付ける</b></summary><textarea id="skgAiJson" rows="12"></textarea></details><div class="toolbar"><button class="primary" id="skgAiBatchGenerate">Formal Skill一括生成</button></div><div id="skgAiBatchStatus" class="item">未実行</div><div class="toolbar"><button id="skgG06BatchExport" disabled>Batch Result JSON出力</button><button id="skgG06GenericExport" disabled>Skill JSON出力</button><button id="skgG06ValidationExport" disabled>Validation Report JSON出力</button><button id="skgG06RejectExport" disabled>REJECTのみ再入力JSON</button></div><details class="skg-fold" data-retired-skill-compat="g06-roundtrip" hidden><summary>G06 Batch Result再Import（退役待ち）</summary><input id="skgG06BatchFile" type="file" accept=".json,application/json" style="margin:10px 0"><textarea id="skgG06BatchJson" rows="10" placeholder="GKS_SKILL_AI_BATCH_RESULT JSON"></textarea><div class="toolbar"><button id="skgG06BatchRestore">Batch Resultを復元</button></div><div id="skgG06BatchRestoreStatus" class="item">未実行</div></details><details class="skg-fold" data-retired-skill-compat="g06-roundtrip" hidden><summary>G06 Validation Report → REJECT再入力（退役待ち）</summary><input id="skgG06ValidationFile" type="file" accept=".json,application/json" style="margin:10px 0"><textarea id="skgG06ValidationJson" rows="10" placeholder="GKS_SKILL_VALIDATION_REPORT JSON"></textarea><div class="toolbar"><button id="skgG06ValidationToReinput">REJECTだけAI Requestへ戻す</button></div><div id="skgG06ValidationReinputStatus" class="item">未実行</div></details><details class="skg-fold" data-retired-skill-compat="g06-roundtrip" hidden><summary>G06 Skill JSON再Import検証（退役待ち）</summary><input id="skgG06SkillFile" type="file" accept=".json,application/json" style="margin:10px 0"><textarea id="skgG06SkillJson" rows="10" placeholder="GKS_SKILL_BATCH JSON"></textarea><div class="toolbar"><button id="skgG06Revalidate">再Import・再Validation</button></div><div id="skgG06RevalidateStatus" class="item">未実行</div></details><p class="small">AI一括生成は正式GKS_SKILL_BATCHを作成し、そのままG07登録へ渡します。正式Schema / Budget / Compiler / runtimeContracts / Dry Run検査は登録時に一度だけ実行します。</p></details><details class="skg-fold" data-retired-skill-compat="batch-json" hidden><summary>旧一括JSON互換</summary><p class="small">旧skills配列の互換検査です。新規AI生成には上のG05 Skill AI Requestを使用してください。</p><div class="toolbar"><button id="skgTemplate">旧テンプレートJSON出力</button></div><input id="skgFile" type="file" accept=".json,application/json" style="margin:10px 0"><details><summary><b>旧JSONを貼り付ける</b></summary><textarea id="skgJson" rows="10"></textarea></details><div class="toolbar"><button id="skgDryGenerate">旧一括生成・受け入れ検査</button></div><div id="skgJsonStatus" class="item">未実行</div></details></div>
 <div class="card skg-step"><h2>⑥ 検査結果</h2><div id="skgResult"><div class="item">まだ生成されていません。</div></div></div>
 <div class="card skg-step"><h2>⑦ JSON出力・登録</h2><p class="small">G07: AI一括生成したGKS_SKILL_BATCHを登録時に正式Schema / Budget / Compilerで検査し、runtimeContracts生成→正式Skill Master契約→Data Exchange→Dry Run→Safe Applyの順で登録します。REJECT・不一致・既存ID競合は登録しません。</p><details class="skg-fold" open><summary>G07 正式Skill Master登録</summary><textarea id="skgG07SkillJson" rows="10" placeholder="GKS_SKILL_BATCH JSON"></textarea><div class="toolbar"><button id="skgG07UseLastSkill" type="button">直近AI Skill Batchをセット</button><button id="skgG07DryRun" type="button">G07登録Dry Run</button><button class="primary" id="skgG07Register" type="button">G07 Safe Apply</button><button id="skgG07Undo" type="button">直前登録をUndo</button></div><div id="skgG07Status" class="item">未実行</div></details><details class="skg-fold"><summary>旧Runtime互換登録</summary><div class="toolbar"><button id="skgExport" disabled>完成スキルJSON出力</button><button id="skgDryRun" disabled>部分JSON方式でDry Run</button><button class="primary" id="skgRegister" disabled>新規のみ安全登録</button></div><div id="skgApplyStatus" class="item">旧Runtime互換経路です。</div></details></div>`;if(!s.isConnected)workspace.appendChild(s);
 ...
 q('skgAiBatchGenerate').onclick=async()=>{try{const result=await generateSkillAiBatch(g06ImportJsonObject(JSON.parse(q('skgAiJson').value||'{}'),'GKS_SKILL_AI_BATCH_REQUEST'));renderAiBatch(result);const batch=clone(lastG06SkillBatch);if(batch&&q('skgG07SkillJson'))q('skgG07SkillJson').value=JSON.stringify(batch,null,2);q('skgG06BatchExport').disabled=false;q('skgG06GenericExport').disabled=result.summary.accepted<1;q('skgG06ValidationExport').disabled=false;q('skgG06RejectExport').disabled=result.summary.rejected<1;q('skgAiBatchStatus').innerHTML=`<b>${esc(result.aiGenerationRuleVersion)}</b> / Formal Draft ${result.summary.accepted}件<br><span class="small">正式なBudget / Compiler / runtimeContracts / Data Exchange検査はG07登録時に一度だけ実行します。</span>`;if(q('skgG07Status'))q('skgG07Status').textContent='AI一括生成結果を登録欄へセットしました。G07登録Dry Runで正式検査してください。';}catch(e){q('skgAiBatchStatus').textContent='AI一括生成エラー: '+e.message;}};
 ...
 q('skgG07DryRun').onclick=async()=>{try{const payload=JSON.parse(q('skgG07SkillJson').value||'{}');const out=await g07DryRunMasterRegistration(payload);q('skgG07Status').innerHTML=`<b>G07 Dry Run PASS</b> add ${out.dryRun.summary?.add||0} / conflict 0<br><span class="small">登録時の正式Schema・Budget・Compiler・runtimeContracts・正式Skill Master契約・Dry Run・Apply Plan検査を通過しました。まだMasterへ書き込みません。</span>`;}catch(e){q('skgG07Status').innerHTML=e.g07Blocker?g07FormatBlocker(e.g07Blocker):esc('G07 Dry Run REJECT ['+(e.code||'ERROR')+']: '+e.message);}};
