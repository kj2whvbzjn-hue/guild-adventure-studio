...
function buildFormalPassiveBatch(result=aiBatchPreview){const entries=Array.isArray(result?.entries)?result.entries:[],accepted=entries.filter(x=>x?.status==='ACCEPT'&&x?.passive).map(x=>({index:x.index,passive:clone(x.passive),modifierCandidate:clone(x.modifierCandidate||null),generation:clone(x.generation||{}),validation:{compilerWarnings:clone(x.validation?.compilerWarnings||[])}}));if(!accepted.length)throw Object.assign(new Error('Formal Batch対象のACCEPT Passiveがありません'),{code:'FORMAL_PASSIVE_BATCH_EMPTY'});return{schema:'GKS_PASSIVE_BATCH',version:SKILL_BATCH_VERSION,sourceSchema:result?.schema||'GKS_SKILL_AI_BATCH_RESULT',aiGenerationRuleVersion:result?.aiGenerationRuleVersion||'',passives:accepted};}
function assertJsonObjectSchema(payload,expectedSchema){if(!payload||typeof payload!=='object'||Array.isArray(payload))throw Object.assign(new Error('JSON objectが必要です'),{code:'JSON_OBJECT_REQUIRED'});if(payload.schema!==expectedSchema)throw Object.assign(new Error(`schema不一致: ${payload.schema||'(なし)'} / expected ${expectedSchema}`),{code:'SCHEMA_MISMATCH'});if(payload.version!==SKILL_BATCH_VERSION&&!(expectedSchema==='GKS_SKILL_AI_BATCH_REQUEST'&&payload.version==='1.0.0'))throw Object.assign(new Error(`version不一致: ${payload.version||'(なし)'}`),{code:'VERSION_MISMATCH'});if(expectedSchema==='GKS_SKILL_AI_BATCH_REQUEST'){const mode=String(payload.mode||'').trim().toUpperCase();if(!mode)throw Object.assign(new Error('AI Requestにはmode=ACTIVEまたはPASSIVEが必要です'),{code:'AI_MODE_REQUIRED'});if(!['ACTIVE','PASSIVE'].includes(mode))throw Object.assign(new Error(`未対応mode: ${mode}`),{code:'AI_MODE_INVALID'});if(!Array.isArray(payload.requests))throw Object.assign(new Error('AI Request requestsは配列が必要です'),{code:'AI_REQUESTS_REQUIRED'});payload=clone(payload);payload.mode=mode;}return clone(payload);}
...
const G07_FORMAL_PASSIVE_ID_RE=/^PAS-\d{4}$/;
const G07_FORMAL_MOD_ID_RE=/^MOD-\d{4}$/;
function g07AllocateIds(rows,re,prefix,count){const used=new Set((rows||[]).map(x=>String(x?.id||'')).filter(id=>re.test(id))),ids=[];for(let n=1;n<=9999&&ids.length<count;n++){const id=`${prefix}-${String(n).padStart(4,'0')}`;if(!used.has(id)){used.add(id);ids.push(id);}}if(ids.length!==count)throw Object.assign(new Error(`${prefix}採番枠が不足しています`),{code:'G07_FORMAL_ID_EXHAUSTED'});return ids;}
function g07NormalizePassiveBatch(payload){const batch=clone(payload);if(!batch||typeof batch!=='object'||Array.isArray(batch))throw Object.assign(new Error('G07 Passive Batch JSONがオブジェクトではありません'),{code:'G07_PASSIVE_BATCH_INVALID'});if(batch.schema!=='GKS_PASSIVE_BATCH')throw Object.assign(new Error(`G07 Passive Batch schemaが不正です: ${batch.schema||'(なし)'}`),{code:'G07_PASSIVE_BATCH_SCHEMA_INVALID'});if(batch.version!=='1.0.0')throw Object.assign(new Error(`G07 Passive Batch versionが不正です: ${batch.version||'(なし)'}`),{code:'G07_PASSIVE_BATCH_VERSION_INVALID'});return batch;}
async function g07RevalidatePassiveBatch(payload){if(!skillRegistry)await loadSkillDefinition();const batch=g07NormalizePassiveBatch(payload),rows=Array.isArray(batch.passives)?batch.passives:[];if(!rows.length)throw Object.assign(new Error('passivesを1件以上指定してください'),{code:'G07_PASSIVES_REQUIRED'});const passiveIds=g07AllocateIds(hostData()?.masters?.passives,G07_FORMAL_PASSIVE_ID_RE,'PAS',rows.length),modCount=rows.filter(r=>r?.modifierCandidate?.definition).length,modIds=g07AllocateIds(hostData()?.masters?.mods,G07_FORMAL_MOD_ID_RE,'MOD',modCount);let mi=0;const entries=[],compiledPassives=[],masterMods=[],balanceCandidates=[];for(let i=0;i<rows.length;i++){const row=rows[i],issues=[],src=clone(row?.passive);if(!src||typeof src!=='object'||Array.isArray(src)){issues.push({code:'G07_PASSIVE_OBJECT_REQUIRED',path:`passives[${i}].passive`,message:'正式Passive objectが必要です'});}let mod=null;if(!issues.length&&row?.modifierCandidate?.definition){mod=clone(row.modifierCandidate.definition);const old=String(mod.id||''),id=modIds[mi++];mod.id=id;src.params=src.params||{};src.params.mod_ids=(src.params.mod_ids||[]).map(x=>String(x)===old?id:String(x));masterMods.push(clone(mod));if(row.modifierCandidate.balance)balanceCandidates.push({...clone(row.modifierCandidate.balance),mod_id:id});}src.id=passiveIds[i];let compiled=null;if(!issues.length){try{compiled=formalPassiveCompilerApi()?.compilePassive?.(src,{registry:skillRegistry,skills:hostData()?.masters?.skills||[]});if(!compiled?.ok||!compiled?.compiledPassive?.runtimeContracts)issues.push({code:'G07_PASSIVE_COMPILE_REJECT',path:`passives[${i}].passive`,message:'Formal Passive compileに失敗しました'});}catch(e){issues.push({code:e.code||'G07_PASSIVE_COMPILE_REJECT',path:`passives[${i}].passive`,message:e.message||String(e)});}}if(!issues.length)compiledPassives.push(clone(compiled.compiledPassive));entries.push({index:i,status:issues.length?'REJECT':'ACCEPT',masterPassiveId:issues.length?null:src.id,issues});}
return{schema:'GKS_PASSIVE_REVALIDATION_REPORT',version:'1.0.0',batch,entries,compiledPassives,masterMods,balanceCandidates,summary:{total:entries.length,accepted:entries.filter(x=>x.status==='ACCEPT').length,rejected:entries.filter(x=>x.status==='REJECT').length,allAccepted:entries.every(x=>x.status==='ACCEPT')}};}
async function g07BuildPassiveRegistration(payload){const report=await g07RevalidatePassiveBatch(payload);if(!report.summary.allAccepted)throw Object.assign(new Error(`G07 Passive登録拒否: REJECT ${report.summary.rejected}`),{code:'G07_PASSIVE_REVALIDATION_REJECT',report});const root=clone(hostData()||{});root.project=root.project||{id:'PROJECT'};root.masters=root.masters||{};root.masters.mods=[...(root.masters.mods||[]),...report.masterMods];root.masters.passives=report.compiledPassives;const studioVersion=String(global.GKSSkillHost?.getBuild?.()||global.DISTRIBUTION_BUILD||'');const modEnvelope=report.masterMods.length?await global.GKSDataExchange.buildEnvelope({rootData:root,dataset:'mods',ids:report.masterMods.map(x=>x.id),dependencyMode:'direct',studioVersion}):null;const passiveEnvelope=await global.GKSDataExchange.buildEnvelope({rootData:root,dataset:'passives',ids:report.compiledPassives.map(x=>x.id),dependencyMode:'direct',studioVersion});return{report,modEnvelope,passiveEnvelope};}
async function g07DryRunPassiveRegistration(payload){const built=await g07BuildPassiveRegistration(payload),source=clone(hostData()||{});let working=clone(source),modDry=null,modPlan=null;if(built.modEnvelope){modDry=await global.GKSDataExchange.dryRunImport({rootData:working,envelope:built.modEnvelope});const blocker=g07DryRunBlocker(modDry);if(!modDry?.ok||blocker)throw Object.assign(new Error(blocker?.message||'G07 MOD Dry Run REJECT'),{code:blocker?.code||'G07_MOD_DRY_RUN_REJECT',dryRun:modDry,g07Blocker:blocker});modPlan=await global.GKSDataExchange.createApplyPlan({rootData:working,envelope:built.modEnvelope,dryRun:modDry,conflictChoices:{}});if(!modPlan?.can_apply)throw Object.assign(new Error('G07 MOD Apply Plan REJECT'),{code:'G07_MOD_APPLY_PLAN_REJECT'});working=(await global.GKSDataExchange.applySafeMerge({rootData:working,envelope:built.modEnvelope,plan:modPlan,dryRun:modDry})).nextRootData;}const passiveDry=await global.GKSDataExchange.dryRunImport({rootData:working,envelope:built.passiveEnvelope});const blocker=g07DryRunBlocker(passiveDry);if(!passiveDry?.ok||blocker)throw Object.assign(new Error(blocker?.message||'G07 Passive Dry Run REJECT'),{code:blocker?.code||'G07_PASSIVE_DRY_RUN_REJECT',dryRun:passiveDry,g07Blocker:blocker});const passivePlan=await global.GKSDataExchange.createApplyPlan({rootData:working,envelope:built.passiveEnvelope,dryRun:passiveDry,conflictChoices:{}});if(!passivePlan?.can_apply)throw Object.assign(new Error('G07 Passive Apply Plan REJECT'),{code:'G07_PASSIVE_APPLY_PLAN_REJECT'});return{...built,source,working,modDry,modPlan,passiveDry,passivePlan};}
async function g07SafeApplyPassiveBatch(payload){const checked=await g07DryRunPassiveRegistration(payload);let next=clone(checked.source);if(checked.modEnvelope)next=(await global.GKSDataExchange.applySafeMerge({rootData:next,envelope:checked.modEnvelope,plan:checked.modPlan,dryRun:checked.modDry})).nextRootData;next=(await global.GKSDataExchange.applySafeMerge({rootData:next,envelope:checked.passiveEnvelope,plan:checked.passivePlan,dryRun:checked.passiveDry})).nextRootData;if(global.GKSSkillHost?.backup?.('before-g07-formal-passive-safe-apply')===false)throw Object.assign(new Error('Backupに失敗しました'),{code:'G07_BACKUP_FAILED'});if(global.GKSSkillHost?.setData?.(next)===false)throw Object.assign(new Error('Passive統合データを反映できませんでした'),{code:'G07_COMMIT_FAILED'});if(global.GKSSkillHost?.persist?.(`G07 Formal Passive Safe Apply: passives=${checked.report.compiledPassives.length} mods=${checked.report.masterMods.length}`)===false){global.GKSSkillHost?.setData?.(clone(checked.source));throw Object.assign(new Error('Passive統合データを保存できませんでした'),{code:'G07_PERSIST_FAILED'});}return{...checked,applied:{passives:checked.report.compiledPassives.length,mods:checked.report.masterMods.length},balanceCandidates:clone(checked.report.balanceCandidates)};}
function g07BatchKind(payload){return String(payload?.schema||'')==='GKS_PASSIVE_BATCH'?'PASSIVE':'ACTIVE';}
async function g07DryRunRegistration(payload){return g07BatchKind(payload)==='PASSIVE'?g07DryRunPassiveRegistration(payload):g07DryRunMasterRegistration(payload);}
async function g07SafeApplyBatch(payload){return g07BatchKind(payload)==='PASSIVE'?g07SafeApplyPassiveBatch(payload):g07SafeApplySkillBatch(payload);}
...
function renderPanel(){
 ...
 <div class="card skg-step"><h2>⑤ AI一括生成</h2><details class="skg-fold" open><summary>G05 Skill AI Request</summary><div class="grid"><div class="field"><label>生成モード</label><select id="skgAiMode"><option value="ACTIVE">Active Skill</option><option value="PASSIVE">Passive Skill</option></select></div><div class="field"><label>Authority / Evidence</label><div id="skgAiModeHelp" class="small">ACTIVE専用入力だけを送信します。</div></div></div><p id="skgAiModeDescription" class="small">ACTIVEは skillLevel / statThresholds / abilityKind / target / range / trigger / conditions / resource を指定します。Effect完成値はCurrent SKL Threshold/Ability BudgetからStudioが決定します。未接続AbilityはFail Closedします。</p><div class="toolbar"><button id="skgAiTemplate">選択モードのRequestテンプレート</button><button id="skgAiRequestExport">AI Request JSON出力</button><button id="skgAiCandidateExport" type="button">Candidate結果JSON出力</button></div><input id="skgAiFile" type="file" accept=".json,application/json" style="margin:10px 0"><label class="small">RequestまたはCandidate結果JSONを再読込</label><input id="skgAiCandidateFile" type="file" accept=".json,application/json" style="margin:6px 0 10px"><details><summary><b>AI Request JSONを貼り付ける</b></summary><textarea id="skgAiJson" rows="12"></textarea></details><div class="toolbar"><button class="primary" id="skgAiBatchGenerate">Formal Skill一括生成 / Passive Candidate生成</button></div><div id="skgAiBatchStatus" class="item">未実行</div><p class="small">Candidate生成・JSON保存・再読込ではProduct Masterへ登録しません。正式登録は下のG07 Dry Run / Safe Applyだけが行います。</p></details></div>
 ...
 function aiNumericPreview(e){if(e?.skill){const budget=e.validation?.budgetResult||{},effects=(e.skill.effects||[]).map((x,i)=>({index:i,...x}));return JSON.stringify({effects,resource:e.skill.resource||{},budget:{cost:budget.totalCost??null,limit:budget.limit??null,ruleVersion:budget.budgetRuleVersion||''}},null,2);}if(e?.passive)return JSON.stringify({passive:e.passive,modifierCandidate:e.modifierCandidate||null,generation:e.generation||{}},null,2);return '';}
 function renderAiBatch(result){const r=q('skgResult'),entries=result?.entries||[],summary=result?.summary||{},passive=String(result?.mode||'').toUpperCase()==='PASSIVE';r.innerHTML=`<div class="item"><div class="skg-ai-summary"><b>G05 ${esc(result?.mode||'')} Batch</b><span class="skg-ai-count">TOTAL ${summary.total??entries.length}</span><span class="skg-ai-count">ACCEPT ${summary.accepted||0}</span><span class="skg-ai-count">REJECT ${summary.rejected||0}</span></div><div class="small">${esc(result?.aiGenerationRuleVersion||'')} / ${esc(result?.budgetRuleVersion||'')}</div><div class="skg-ai-filter" role="group" aria-label="AI一括生成結果フィルター"><button type="button" data-ai-batch-filter="ALL" aria-pressed="true">すべて</button><button type="button" data-ai-batch-filter="ACCEPT" aria-pressed="false">ACCEPTのみ</button><button type="button" data-ai-batch-filter="REJECT" aria-pressed="false">REJECTのみ</button></div></div>`+entries.map(e=>{const issues=e.validation?.issues||[],budget=e.validation?.budgetResult||{},numeric=aiNumericPreview(e),record=passive?e.passive:e.skill,id=record?.id||`request[${e.index}]`,name=record?.name||e.request?.intent||'',gate2=passive?'Formal Passive':'Budget',gate2ok=passive?!!e.validation?.compiler:!!e.validation?.budget;return `<details class="card skg-result-row skg-ai-result" data-ai-batch-status="${esc(e.status)}" ${e.status==='ACCEPT'?'':'open'}><summary><b>#${Number(e.index)+1} ${esc(id)}</b> ${esc(name)} ${e.status==='ACCEPT'?'✓ ACCEPT':'✗ REJECT'}</summary><div class="skg-ai-gates">${aiGateLabel('Registry',!!e.validation?.registry)}${aiGateLabel(gate2,gate2ok)}${passive?'':aiGateLabel('Compiler',!!e.validation?.compiler)}</div>${e.generation?`<div class="small">mode ${esc(e.generation.mode||result.mode||'')} / source ${esc(e.generation.source||'')} ${passive?`/ series ${esc(e.generation.passiveSeriesId||'')} / stage ${esc(e.generation.stage??'')}`:`/ budget ${esc(budget.totalCost??'-')} / limit ${esc(budget.limit??'-')}`}</div>`:''}${record?`<div class="small skg-tags">${passive?`passiveSeriesId: ${esc(record.passiveSeriesId||'')}`:`effects: ${esc((record.effects||[]).map(x=>x.type+(x.effectId?`:${x.effectId}`:'')).join(', '))}`}</div>`:'<div class="small">生成Recordなし（構造検査または生成前GateでREJECT）</div>'}${issues.length?`<div class="skg-ai-issues">${issues.map(x=>`<div class="skg-ai-issue"><code>${esc(x.code||'REJECT')}</code>${x.path?` <span class="small">@ ${esc(x.path)}</span>`:''}<div>${esc(x.message||'')}</div></div>`).join('')}</div>`:'<div class="small">個別検査PASS。</div>'}${numeric?`<details class="skg-fold"><summary>Studio生成数値 / Threshold trace / Candidate詳細</summary><pre class="skg-ai-numeric">${esc(numeric)}</pre></details>`:''}</details>`;}).join('');r.querySelectorAll('[data-ai-batch-filter]').forEach(btn=>btn.onclick=()=>{const mode=btn.dataset.aiBatchFilter;r.querySelectorAll('[data-ai-batch-filter]').forEach(x=>x.setAttribute('aria-pressed',String(x===btn)));r.querySelectorAll('[data-ai-batch-status]').forEach(row=>row.classList.toggle('skg-ai-hidden',mode!=='ALL'&&row.dataset.aiBatchStatus!==mode));});}
 function syncAiModeUi({resetTemplate=false}={}){const mode=String(q('skgAiMode')?.value||'ACTIVE').toUpperCase(),passive=mode==='PASSIVE';q('skgAiModeDescription').textContent=passive?'PASSIVEは passiveSeriesId / stage / requirement / category / effect・bindingを指定します。ActiveのBudget/Range/Resource fieldは送信できません。':'ACTIVEは skillLevel / statThresholds / abilityKind / target / range / trigger / conditions / resource を指定します。Passive専用fieldは送信できません。';q('skgAiModeHelp').textContent=passive?'PASSIVE専用入力だけを送信。RequirementはActive Ability Budgetへ変換しません。':'ACTIVE専用入力だけを送信。ThresholdをAbility Budgetとして扱います。';if(resetTemplate)q('skgAiJson').value=JSON.stringify(aiRequestTemplate(mode),null,2);}
 q('skgAiMode').onchange=()=>syncAiModeUi({resetTemplate:true});
 q('skgAiTemplate').onclick=()=>{syncAiModeUi({resetTemplate:true});q('skgAiBatchStatus').textContent='選択モードのAI Requestテンプレートを設定しました。';};
 q('skgAiRequestExport').onclick=()=>{try{const mode=String(q('skgAiMode').value||'ACTIVE').toUpperCase(),payload=exportAiRequest(JSON.parse(q('skgAiJson').value||JSON.stringify(aiRequestTemplate(mode))));if(String(payload.mode||'').toUpperCase()!==mode)throw Object.assign(new Error('画面の生成モードとRequest modeが一致しません'),{code:'AI_MODE_UI_MISMATCH'});download(`Skill_AI_Request_${mode}.json`,payload);q('skgAiBatchStatus').textContent='AI Request JSONを出力しました。';}catch(err){q('skgAiBatchStatus').textContent='出力エラー: '+err.message;}};
 q('skgAiCandidateExport').onclick=()=>{try{if(!aiBatchPreview)throw new Error('Candidate結果がありません');download(`Skill_AI_Candidate_${String(aiBatchPreview.mode||'UNKNOWN')}.json`,clone(aiBatchPreview));q('skgAiBatchStatus').textContent='Candidate結果JSONを出力しました。Product Masterは変更していません。';}catch(err){q('skgAiBatchStatus').textContent='Candidate出力エラー: '+err.message;}};
 q('skgAiFile').onchange=async e=>{try{const f=e.target.files?.[0];if(f){const payload=assertJsonObjectSchema(JSON.parse(await f.text()),'GKS_SKILL_AI_BATCH_REQUEST');q('skgAiMode').value=String(payload.mode||'ACTIVE').toUpperCase();syncAiModeUi();q('skgAiJson').value=JSON.stringify(payload,null,2);q('skgAiBatchStatus').textContent=f.name+' をAI Requestとして読み込みました。';}}catch(err){q('skgAiBatchStatus').textContent='読込REJECT ['+(err.code||'JSON_INVALID')+']: '+err.message;}};
 q('skgAiCandidateFile').onchange=async e=>{try{const f=e.target.files?.[0];if(!f)return;const payload=JSON.parse(await f.text());if(payload?.schema!=='GKS_SKILL_AI_BATCH_RESULT'||payload?.version!=='1.0.0')throw Object.assign(new Error('GKS_SKILL_AI_BATCH_RESULT 1.0.0が必要です'),{code:'AI_CANDIDATE_SCHEMA_INVALID'});aiBatchPreview=clone(payload);q('skgAiMode').value=String(payload.mode||'ACTIVE').toUpperCase();syncAiModeUi();renderAiBatch(aiBatchPreview);lastFormalSkillBatch=String(payload.mode||'').toUpperCase()==='PASSIVE'?buildFormalPassiveBatch(aiBatchPreview):buildFormalSkillBatch(aiBatchPreview);if(q('skgG07SkillJson'))q('skgG07SkillJson').value=JSON.stringify(lastFormalSkillBatch,null,2);q('skgAiBatchStatus').textContent=f.name+' をCandidate結果として再読込しました。Product Masterは変更していません。';}catch(err){q('skgAiBatchStatus').textContent='Candidate再読込REJECT ['+(err.code||'JSON_INVALID')+']: '+err.message;}};
 q('skgAiBatchGenerate').onclick=async()=>{try{const mode=String(q('skgAiMode').value||'ACTIVE').toUpperCase(),request=assertJsonObjectSchema(JSON.parse(q('skgAiJson').value||'{}'),'GKS_SKILL_AI_BATCH_REQUEST');if(String(request.mode||'').toUpperCase()!==mode)throw Object.assign(new Error('画面の生成モードとRequest modeが一致しません'),{code:'AI_MODE_UI_MISMATCH'});const result=await generateSkillAiBatch(request);renderAiBatch(result);const batch=clone(lastFormalSkillBatch);if(batch&&q('skgG07SkillJson'))q('skgG07SkillJson').value=JSON.stringify(batch,null,2);q('skgAiBatchStatus').innerHTML=`<b>${esc(result.mode||'')}</b> / <b>${esc(result.aiGenerationRuleVersion)}</b> / Candidate ${result.summary.accepted}件<br><span class="small">正式なBudget / Compiler / runtimeContracts / Data Exchange検査はG07登録時に実行します。Candidate生成ではProduct Masterを変更しません。</span>`;if(q('skgG07Status'))q('skgG07Status').textContent='Candidate結果を登録欄へセットしました。G07登録Dry Runで正式検査してください。';}catch(e){q('skgAiBatchStatus').textContent='AI一括生成エラー: '+e.message;}};
 q('skgG07UseLastSkill').onclick=()=>{try{let payload=lastFormalSkillBatch?clone(lastFormalSkillBatch):buildFormalSkillBatch();payload=g07BatchKind(payload)==='PASSIVE'?g07NormalizePassiveBatch(payload):g07NormalizeSkillBatch(payload);q('skgG07SkillJson').value=JSON.stringify(payload,null,2);q('skgG07Status').textContent='直近のAI一括生成結果をG07登録欄へセットしました。';}catch(e){q('skgG07Status').textContent='G07準備REJECT ['+(e.code||'ERROR')+']: '+e.message;}};
 q('skgG07DryRun').onclick=async()=>{try{const payload=JSON.parse(q('skgG07SkillJson').value||'{}');const out=await g07DryRunRegistration(payload);const dry=out.passiveDry||out.dryRun,rej=out.report?.summary?.rejected||out.revalidation?.summary?.rejected||0;q('skgG07Status').innerHTML=`<b>G07 Dry Run PASS</b> add ${dry?.summary?.add||0} / rejected ${rej} / conflict 0<br><span class="small">ACCEPT行だけを正式IDで採番し、Schema・Budget・Compiler・runtimeContracts・正式Skill Master契約・Dry Run・Apply Plan検査を通過しました。REJECT行は登録対象から除外しています。まだMasterへ書き込みません。</span>`;}catch(e){q('skgG07Status').innerHTML=e.g07Blocker?g07FormatBlocker(e.g07Blocker):esc('G07 Dry Run REJECT ['+(e.code||'ERROR')+']: '+e.message);}};
 q('skgG07Register').onclick=async()=>{try{const payload=JSON.parse(q('skgG07SkillJson').value||'{}');const out=await g07SafeApplyBatch(payload);const add=out.applied?.passives||out.plan?.add_count||out.dryRun?.summary?.add||0,rej=out.report?.summary?.rejected||out.revalidation?.summary?.rejected||0;q('skgG07Status').innerHTML=`<b>G07 Safe Apply 完了</b> add ${add} / rejected ${rej}<br><span class="small">REJECT行を除外し、backup / transaction / persist / Audit記録まで完了。直前SessionはUndoできます。</span>`;}catch(e){q('skgG07Status').innerHTML=e.g07Blocker?g07FormatBlocker(e.g07Blocker):esc('G07登録REJECT ['+(e.code||'ERROR')+']: '+e.message);}};
 ...
 syncAiModeUi({resetTemplate:!q('skgAiJson').value.trim()});renderSkillDynamic();document.dispatchEvent(new CustomEvent('gks:view-ready',{detail:{view:'skill-generator'}}));
}
