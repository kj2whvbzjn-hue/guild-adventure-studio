...
const VERSION='1.7.0';
let spec=null,skillRegistry=null,skillUiDefinition=null,budgetRules=null,aiGenerationRules=null,preview=null,aiBatchPreview=null,lastG06SkillBatch=null,lastEnvelope=null,lastDryRun=null;
...
async function generateSkillAiBatch(payload){if(!skillAiBatchEngineApi()?.generateBatch)throw new Error('G05 AI Batch Engineが読み込まれていません');if(!aiGenerationRules)await loadAiGenerationRules();aiBatchPreview=await skillAiBatchEngineApi().generateBatch(payload,{registry:skillRegistry,budgetRules, rules:aiGenerationRules,budgetEngine:global.GKSSkillBudgetEngine,compile:skill=>compileSkillDraft(skill),idPrefix:'G05-AI'});return clone(aiBatchPreview);}
const G06_JSON_VERSION='1.0.0';
function g06ExportAiRequest(payload){const src=payload==null?aiRequestTemplate():clone(payload);if(src?.schema!=='GKS_SKILL_AI_BATCH_REQUEST'||src?.version!=='1.0.0'||!Array.isArray(src?.requests))throw Object.assign(new Error('G06 AI Request JSON形式が不正です'),{code:'G06_AI_REQUEST_SCHEMA_INVALID'});return clone(src);}
function g06ExportSkills(result=aiBatchPreview){const entries=Array.isArray(result?.entries)?result.entries:[],accepted=entries.filter(x=>x?.status==='ACCEPT'&&x?.skill).map(x=>({index:x.index,skill:clone(x.skill),generation:clone(x.generation||{}),validation:{budgetResult:clone(x.validation?.budgetResult||null),compilerWarnings:clone(x.validation?.compilerWarnings||[])}}));if(!accepted.length)throw Object.assign(new Error('G06 Export対象のACCEPT Skillがありません'),{code:'G06_SKILL_EXPORT_EMPTY'});return{schema:'GKS_SKILL_BATCH',version:G06_JSON_VERSION,sourceSchema:result?.schema||'GKS_SKILL_AI_BATCH_RESULT',aiGenerationRuleVersion:result?.aiGenerationRuleVersion||'',budgetRuleVersion:result?.budgetRuleVersion||'',skills:accepted};}
function g06ExportValidationReport(result=aiBatchPreview){const entries=Array.isArray(result?.entries)?result.entries:[];if(!entries.length)throw Object.assign(new Error('G06 Validation Report対象がありません'),{code:'G06_VALIDATION_EXPORT_EMPTY'});return{schema:'GKS_SKILL_VALIDATION_REPORT',version:G06_JSON_VERSION,aiGenerationRuleVersion:result?.aiGenerationRuleVersion||'',budgetRuleVersion:result?.budgetRuleVersion||'',summary:clone(result?.summary||{}),entries:entries.map(x=>({index:x.index,status:x.status,skillId:x.skill?.id||null,request:clone(x.request||null),issues:clone(x.validation?.issues||[]),gates:{registry:!!x.validation?.registry,budget:!!x.validation?.budget,compiler:!!x.validation?.compiler}}))};}
function g06ExportBatchResult(result=aiBatchPreview){if(result?.schema!=='GKS_SKILL_AI_BATCH_RESULT'||result?.version!=='1.0.0'||!Array.isArray(result?.entries))throw Object.assign(new Error('G06 Batch Result対象がありません'),{code:'G06_BATCH_RESULT_INVALID'});return clone(result);}
function g06BatchResultIssues(payload){
 ...
 if(payload?.schema!=='GKS_SKILL_AI_BATCH_RESULT')issues.push({code:'G06_SCHEMA_MISMATCH',path:'schema',message:'GKS_SKILL_AI_BATCH_RESULTが必要です'});
...
function g06ValidationReportToReinput(payload){
 const report=g06ImportJsonObject(payload,'GKS_SKILL_VALIDATION_REPORT'),issues=[...g06UnknownFields(report,['schema','version','aiGenerationRuleVersion','budgetRuleVersion','summary','entries'],'$')];
...
function g06StrictSkillIssues(skill,index=0){
 ...
 for(const [i,e] of (Array.isArray(skill.effects)?skill.effects:[]).entries()){
  const type=String(e?.type||'').toUpperCase(),def=skillRegistry?.authoring?.effect_types?.[type],allowed=['type',...(def?.required_fields||[]),...(def?.optional_fields||[])];
  issues.push(...g06UnknownFields(e,[...new Set(allowed)],`${base}.effects[${i}]`));
 }
 return issues;
}
async function g06RevalidateSkillBatch(payload){
 if(!skillRegistry)await loadSkillDefinition();
 if(!skillRegistry?.authoring?.effect_types)throw Object.assign(new Error('Skill RegistryのEffect定義が未接続です'),{code:'G06_SKILL_REGISTRY_REQUIRED'});
 const batch=g06ImportJsonObject(payload,'GKS_SKILL_BATCH');
 ...
 for(let i=0;i<(Array.isArray(batch.skills)?batch.skills:[]).length;i++){
  ...
  if(!issues.length){
   try{
    budget=calculateSkillBudget(row.skill);
    compiled=await compileSkillDraft(row.skill);
 ...
 return{schema:'GKS_SKILL_REVALIDATION_REPORT',version:G06_JSON_VERSION,sourceSchema:batch.schema,registry:{loaded:true,phase:String(skillRegistry?.phase||''),effectTypeCount:Object.keys(skillRegistry?.authoring?.effect_types||{}).length},summary:{total:entries.length,accepted:entries.filter(x=>x.status==='ACCEPT').length,rejected:entries.filter(x=>x.status==='REJECT').length,rootIssueCount:rootIssues.length,allAccepted:rootIssues.length===0&&entries.length>0&&entries.every(x=>x.status==='ACCEPT')},rootIssues,entries};
}
...
async function g07RevalidateSkillBatch(payload){
 ...
 for(let i=0;i<(Array.isArray(batch.skills)?batch.skills:[]).length;i++){
  ...
  if(!issues.length){
   try{
    compiled=await compileSkillDraft(skill);
    if(!compiled?.ok){
...
function renderPanel(){
 ...
 <div class="card skg-step"><h2>⑤ AI一括生成</h2><details class="skg-fold" open><summary>G05 Skill AI Request</summary><p class="small">AIは skillLevel / intent / effects / target / range / desiredStrength / searchMetadata の構造だけを指定します。power・duration等の完成戦闘数値はStudioがG04 Budget Rulesから決定します。</p><div class="toolbar"><button id="skgAiTemplate">G05 AI Requestテンプレート</button><button id="skgG06AiExport">G06 AI Request JSON出力</button></div><input id="skgAiFile" type="file" accept=".json,application/json" style="margin:10px 0"><details><summary><b>AI Request JSONを貼り付ける</b></summary><textarea id="skgAiJson" rows="12"></textarea></details><div class="toolbar"><button class="primary" id="skgAiBatchGenerate">Generic一括生成・個別検査</button></div><div id="skgAiBatchStatus" class="item">未実行</div><div class="toolbar"><button id="skgG06BatchExport" disabled>Batch Result JSON出力</button><button id="skgG06GenericExport" disabled>Skill JSON出力</button><button id="skgG06ValidationExport" disabled>Validation Report JSON出力</button><button id="skgG06RejectExport" disabled>REJECTのみ再入力JSON</button></div><details class="skg-fold"><summary>G06 Batch Result再Import</summary><input id="skgG06BatchFile" type="file" accept=".json,application/json" style="margin:10px 0"><textarea id="skgG06BatchJson" rows="10" placeholder="GKS_SKILL_AI_BATCH_RESULT JSON"></textarea><div class="toolbar"><button id="skgG06BatchRestore">Batch Resultを復元</button></div><div id="skgG06BatchRestoreStatus" class="item">未実行</div></details><details class="skg-fold"><summary>G06 Validation Report → REJECT再入力</summary><input id="skgG06ValidationFile" type="file" accept=".json,application/json" style="margin:10px 0"><textarea id="skgG06ValidationJson" rows="10" placeholder="GKS_SKILL_VALIDATION_REPORT JSON"></textarea><div class="toolbar"><button id="skgG06ValidationToReinput">REJECTだけAI Requestへ戻す</button></div><div id="skgG06ValidationReinputStatus" class="item">未実行</div></details><details class="skg-fold"><summary>G06 Skill JSON再Import検証</summary><input id="skgG06SkillFile" type="file" accept=".json,application/json" style="margin:10px 0"><textarea id="skgG06SkillJson" rows="10" placeholder="GKS_SKILL_BATCH JSON"></textarea><div class="toolbar"><button id="skgG06Revalidate">再Import・再Validation</button></div><div id="skgG06RevalidateStatus" class="item">未実行</div></details><p class="small">G06: AI Request / Batch Result / Generic Skill / Validation ReportをJSONで往復できます。Validation ReportからREJECTだけを再入力し、Skill再Importではunknown field・不一致をstrict REJECTします。Master登録はG07まで無効です。</p></details><details class="skg-fold"><summary>旧一括JSON互換</summary><p class="small">旧skills配列の互換検査です。新規AI生成には上のG05 Skill AI Requestを使用してください。</p><div class="toolbar"><button id="skgTemplate">旧テンプレートJSON出力</button></div><input id="skgFile" type="file" accept=".json,application/json" style="margin:10px 0"><details><summary><b>旧JSONを貼り付ける</b></summary><textarea id="skgJson" rows="10"></textarea></details><div class="toolbar"><button id="skgDryGenerate">旧一括生成・受け入れ検査</button></div><div id="skgJsonStatus" class="item">未実行</div></details></div>
 <div class="card skg-step"><h2>⑥ 検査結果</h2><div id="skgResult"><div class="item">まだ生成されていません。</div></div></div>
 <div class="card skg-step"><h2>⑦ JSON出力・登録</h2><p class="small">G07: G06 Skill Batchを正式Compilerで再Validationし、正式Skill Master契約→Data Exchange→Dry Run→Safe Applyの順で登録します。REJECT・不一致・既存ID競合は登録しません。</p><details class="skg-fold" open><summary>G07 正式Skill Master登録</summary><textarea id="skgG07SkillJson" rows="10" placeholder="GKS_SKILL_BATCH JSON"></textarea><div class="toolbar"><button id="skgG07UseLastSkill" type="button">直近Skill JSONをセット</button><button id="skgG07DryRun" type="button">G07登録Dry Run</button><button class="primary" id="skgG07Register" type="button">G07 Safe Apply</button><button id="skgG07Undo" type="button">直前登録をUndo</button></div><div id="skgG07Status" class="item">未実行</div></details><details class="skg-fold"><summary>旧Runtime互換登録</summary><div class="toolbar"><button id="skgExport" disabled>完成スキルJSON出力</button><button id="skgDryRun" disabled>部分JSON方式でDry Run</button><button class="primary" id="skgRegister" disabled>新規のみ安全登録</button></div><div id="skgApplyStatus" class="item">旧Runtime互換経路です。</div></details></div>`;if(!s.isConnected)workspace.appendChild(s);
 const q=id=>s.querySelector('#'+id);
 const renderSkillRegistryStatus=()=>{const el=q('skgSkillRegistryStatus');if(!el)return;if(!skillUiDefinition){el.textContent='Skill Registry未接続';return;}const enabled=skillUiDefinition.effects.filter(x=>x.enabled).length,total=skillUiDefinition.effects.length;el.innerHTML=`<b>G01 Registry接続:</b> ${esc(skillUiDefinition.registryPhase)} / Effect ${enabled}/${total}有効 / Trigger ${skillUiDefinition.triggers.length} / Condition ${skillUiDefinition.conditions.length} <span class="small">（G03 Condition UI / G02 Effect UI / 旧Runtime UIは互換欄）</span>`;};
 ...
 q('skgG06GenericExport').onclick=()=>{try{download('G06_Generic_Skills.json',g06ExportSkills());}catch(e){q('skgAiBatchStatus').textContent='G06出力エラー: '+e.message;}};
 ...
 q('skgG06SkillFile').onchange=async e=>{try{const f=e.target.files?.[0];if(f){const payload=g06ImportJsonObject(JSON.parse(await f.text()),'GKS_SKILL_BATCH');lastG06SkillBatch=clone(payload);q('skgG06SkillJson').value=JSON.stringify(payload,null,2);q('skgG06RevalidateStatus').textContent=f.name+' をSkill Batchとして読み込みました。';}}catch(err){lastG06SkillBatch=null;q('skgG06RevalidateStatus').textContent='G06再Import REJECT ['+(err.code||'JSON_INVALID')+']: '+err.message;}};
 q('skgG06Revalidate').onclick=async()=>{try{const payload=g06ImportJsonObject(JSON.parse(q('skgG06SkillJson').value||'{}'),'GKS_SKILL_BATCH');const report=await g06RevalidateSkillBatch(payload);if(report.summary.allAccepted)lastG06SkillBatch=clone(payload);else lastG06SkillBatch=null;q('skgG06RevalidateStatus').innerHTML=`<b>${report.summary.allAccepted?'再Validation PASS':'再Validation REJECT'}</b> ACCEPT ${report.summary.accepted} / REJECT ${report.summary.rejected} / root issues ${report.summary.rootIssueCount}<br><span class="small">${report.entries.flatMap(x=>x.issues.map(i=>`${x.skillId||x.index}: ${i.code} @ ${i.path}`)).map(esc).join('<br>')||'Export時Budget結果と再Import結果が一致しました。'}</span>`;}catch(e){lastG06SkillBatch=null;q('skgG06RevalidateStatus').textContent='G06再Validation REJECT ['+(e.code||'JSON_INVALID')+']: '+e.message;}};
...
const api={VERSION,DEPENDENCY_TIMEOUT_MS,fetchJsonDependency,CONDITION_FIELDS,loadSpec,loadSkillDefinition,loadBudgetRules,loadAiGenerationRules,calculateSkillBudget,aiRequestTemplate,generateSkillAiBatch,g06ExportAiRequest,g06ExportSkills,g06ExportValidationReport,g06ExportRejectedRequests,g06ExportBatchResult,g06ImportBatchResult,g06ValidationReportToReinput,g06ImportJsonObject,g06StrictSkillIssues,g06RevalidateSkillBatch,g07AssertFormalMasterContract,g07NormalizeSkillBatch,g07RevalidateSkillBatch,g07BuildMasterEnvelopeFromSkillBatch,g07DryRunBlocker,g07FormatBlocker,g07AuditStorageKey,g07DryRunMasterRegistration,g07SafeApplySkillBatch,skillEffectRequirements,skillConditionRequirements,requiredFor,validateDraft,buildTags,generateRecord,generateBatch,requestTemplate,buildEnvelope,dryRun,safeRegisterNewOnly,compileSkillDraft,getSkillUiDefinition:()=>clone(skillUiDefinition),getBudgetRules:()=>clone(budgetRules),getAiGenerationRules:()=>clone(aiGenerationRules),getAiBatchPreview:()=>clone(aiBatchPreview),getPreview:()=>clone(preview)};global.GKSSkillGenerator=api;
