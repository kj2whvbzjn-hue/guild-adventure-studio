...
function renderPanel(){
 ...
 q('skgAiBatchGenerate').onclick=async()=>{try{const result=await generateSkillAiBatch(assertJsonObjectSchema(JSON.parse(q('skgAiJson').value||'{}'),'GKS_SKILL_AI_BATCH_REQUEST'));renderAiBatch(result);const batch=clone(lastFormalSkillBatch);if(batch&&q('skgG07SkillJson'))q('skgG07SkillJson').value=JSON.stringify(batch,null,2);q('skgAiBatchStatus').innerHTML=`<b>${esc(result.mode||'')}</b> / <b>${esc(result.aiGenerationRuleVersion)}</b> / Formal Draft ${result.summary.accepted}件<br><span class="small">正式なBudget / Compiler / runtimeContracts / Data Exchange検査はG07登録時に一度だけ実行します。</span>`;if(q('skgG07Status'))q('skgG07Status').textContent='AI一括生成結果を登録欄へセットしました。G07登録Dry Runで正式検査してください。';}catch(e){q('skgAiBatchStatus').textContent='AI一括生成エラー: '+e.message;}};
