...
function renderPanel(){
 ...
 for(const id of ['skgFormalMpCost','skgFormalCooldown','skgFormalActivationPriority','skgBudgetOverride','skgBudgetOverrideReason','skgLevel'])q(id)?.addEventListener('input',updateFormalReqStatus);
 ...
 function captureFormalValues(){for(const card of q('skgFormalEffectFields').querySelectorAll('[data-formal-effect]')){const type=card.dataset.formalEffect,values={};for(const el of card.querySelectorAll('[data-formal-field]')){let v=el.value;if(el.dataset.control==='number'&&v!=='')v=Number(v);if(el.dataset.control==='boolean')v=v==='true';values[el.dataset.formalField]=v;}formalDraftCache[type]=values;}}
