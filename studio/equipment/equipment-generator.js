...
function commit(){if(!preview)throw new Error('確認データがありません。');if(!preview.validation.ok)throw new Error('検証エラーがあるため保存できません。');if(!global.data?.masters?.equipment)throw new Error('装備マスターが利用できません。');const row=clone(preview.record);row.updated_at=stamp();if(!row.created_at)row.created_at=row.updated_at;writeEquipment(row);if(typeof global.persist==='function')global.persist('equipment generator commit');preview=null;return clone(row);}
...
function commitBatch(){if(!batchPreview)throw new Error('一括確認データがありません。');if(batchPreview.entries.some(x=>!x.validation.ok))throw new Error('検証エラーがあるため一括保存できません。');if(!global.data?.masters?.equipment)throw new Error('装備マスターが利用できません。');const rows=batchPreview.entries.map(x=>{const row=clone(x.record);row.updated_at=stamp();if(!row.created_at)row.created_at=row.updated_at;writeEquipment(row);return row;});if(typeof global.persist==='function')global.persist('equipment generator batch commit');batchPreview=null;return clone(rows);}
...
function renderPanel(){
  ...
  <div class="view-heading"><div><h1>装備生成</h1><p class="small">装備完全統合仕様 v1.1 / 基本装備生成パイプライン v1.2</p></div></div>
  <div class="card"><h2>単体生成</h2><div class="grid">
    ...
    <div class="field"><label>基本装備種別</label><select id="eqgType"></select></div><div class="field" id="eqgArmorSlotField"><label>防具部位</label><select id="eqgArmorSlot"></select></div>
    <div class="field"><label>アイテムレベル</label><input id="eqgIlv" type="number" value="1"></div><div class="field"><label>シード（追跡用）</label><input id="eqgSeed" value="0"></div>
    <div class="field"><label>装備ID</label><input id="eqgId" placeholder="EQP-0001"></div><div class="field"><label>名称</label><input id="eqgName" placeholder="空欄ならアイテムレベル名称＋種別"></div>
  </div><div class="toolbar"><button class="primary" id="eqgGenerate">生成して確認</button><button id="eqgCommit" disabled>保存</button></div><div id="eqgStatus" class="item">生成ルール／有効バランス設定を読み込んでいます。</div></div>
  <div class="card"><h2>一括試算／一括生成</h2><p class="small">同じ生成処理を使い、試算は保存しません。一括保存は確認対象の全件が検証合格の場合だけ可能です。</p><div class="grid">
    ...
    <div class="field"><label>基本装備種別（カンマ区切り／空欄＝全種）</label><input id="eqgBatchTypes"></div><div class="field"><label>防具部位（カンマ区切り／空欄＝全部位）</label><input id="eqgBatchSlots"></div>
    <div class="field"><label>アイテムレベル開始</label><input id="eqgBatchMin" type="number" value="1"></div><div class="field"><label>アイテムレベル終了</label><input id="eqgBatchMax" type="number" value="11"></div>
    <div class="field"><label>ID接頭辞</label><input id="eqgBatchPrefix" value="EQP-GEN"></div><div class="field"><label>シード接頭辞</label><input id="eqgBatchSeed" value="0"></div>
  </div><div class="toolbar"><button id="eqgSimulate">一括試算</button><button class="primary" id="eqgBatchGenerate">一括確認</button><button id="eqgBatchCommit" disabled>一括保存</button></div><div id="eqgBatchStatus" class="item">未実行</div></div>
  <div class="card"><h2>AI入力</h2><p class="small">AIは意図と候補だけを指定します。正式な数値項目の直接指定は拒否し、開発スタジオの装備生成処理が計算します。</p><textarea id="eqgAiRequest" rows="8" style="width:100%">{"kind":"weapon","base_item_types":["片手剣"],"item_level":{"min":1,"max":3},"id_prefix":"EQP-AI","seed":"ai"}</textarea><div class="toolbar"><button id="eqgAiPreview">AI入力を確認</button></div><div id="eqgAiStatus" class="item">未実行</div></div>
  <div class="card"><h2>検証結果／生成内容の確認</h2><pre id="eqgPreview" style="white-space:pre-wrap;overflow:auto;max-height:620px">まだ生成されていません</pre></div>`;
  ...
  section.querySelector('#eqgSimulate').addEventListener('click',()=>{try{const p=simulateBatch(batchRequest());pre.textContent=JSON.stringify(p,null,2);batchCommit.disabled=true;section.querySelector('#eqgBatchStatus').textContent=`試算完了: ${p.summary.count}件 / 有効 ${p.summary.valid}件 / 保存なし`;}catch(e){batchCommit.disabled=true;section.querySelector('#eqgBatchStatus').textContent='試算エラー: '+e.message;}});
  section.querySelector('#eqgBatchGenerate').addEventListener('click',()=>{try{const p=generateBatch(batchRequest());pre.textContent=JSON.stringify(p,null,2);batchCommit.disabled=p.summary.invalid>0;section.querySelector('#eqgBatchStatus').textContent=`確認準備完了: ${p.summary.count}件 / 有効 ${p.summary.valid}件 / 無効 ${p.summary.invalid}件`;}catch(e){batchCommit.disabled=true;section.querySelector('#eqgBatchStatus').textContent='一括生成エラー: '+e.message;}});
  batchCommit.addEventListener('click',()=>{try{const rows=commitBatch();batchCommit.disabled=true;section.querySelector('#eqgBatchStatus').textContent=`一括保存完了: ${rows.length}件`;if(typeof global.render==='function')global.render();}catch(e){section.querySelector('#eqgBatchStatus').textContent='一括保存失敗: '+e.message;}});
  section.querySelector('#eqgAiPreview').addEventListener('click',()=>{try{const req=JSON.parse(section.querySelector('#eqgAiRequest').value);const p=prepareAiRequest(req);pre.textContent=JSON.stringify(p,null,2);batchCommit.disabled=p.summary.invalid>0;section.querySelector('#eqgAiStatus').textContent=`AI入力の確認完了: ${p.summary.count}件 / 有効 ${p.summary.valid}件`;}catch(e){batchCommit.disabled=true;section.querySelector('#eqgAiStatus').textContent='AI入力エラー: '+e.message;}});
}
