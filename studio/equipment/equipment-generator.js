...
function validateConfig(){
  ...
  const values=[config.item_level?.min,config.item_level?.max,config.weapon?.performance?.attack_multiplier,config.weapon?.performance?.accuracy_multiplier,config.weapon?.performance?.weapon_critical_rate,config.weapon?.performance?.block_rate_base,config.weapon?.performance?.block_rate_per_item_level,config.weapon?.performance?.block_damage_cut_rate];
...
function saveBaseNameSet(kind,presetName,rows,{activate=true}={}){
  if(!['weapon','armor','accessory'].includes(kind))throw new Error('ベースアイテムセット区分が不正です。');
...
function generateArmor(input){
  ...
  const hp_bonus=required_vit*slotCoefficient*hpGrowth,mp_bonus=required_mnd*slotCoefficient*mpGrowth,evasion=required_agi*evasionGrowth,magic_resistance=required_agi+required_mnd;
  const trace=[`armor_category=${type}`,`armor_slot=${slot}`,`required_vit=${i}*${c.vit}=${required_vit}`,`required_mnd=${i}*${c.mnd}=${required_mnd}`,`required_agi=${i}*${c.agi}=${required_agi}`,`slot_coefficient=${slotCoefficient}`,`hp_bonus=${required_vit}*${slotCoefficient}*growth(${hpGrowth})=${hp_bonus}`,`mp_bonus=${required_mnd}*${slotCoefficient}*growth(${mpGrowth})=${mp_bonus}`,`evasion=${required_agi}*growth(${evasionGrowth})=${evasion}`,`magic_resistance=${required_agi}+${required_mnd}=${magic_resistance}`];
  const baseName=formalBaseName('armor',type,i);
  const out={id:String(input.id||''),name:String(input.name||`${baseName}${type}${slot}`),status:'draft',tags:Array.isArray(input.tags)?clone(input.tags):[],params:{},description:String(input.description||''),mod_ids:[],item_level:i,armor_category:type,armor_slot:slot,required_vit,required_mnd,required_agi,hp_bonus,mp_bonus,evasion,magic_resistance};
  out.generation=generationMeta({...input,base_item_type:type},trace);return out;
}
function generateAccessory(input){
  const i=Number(input.item_level),type=String(input.base_item_type||''),slot=ACCESSORY_SLOT_MAP[type];
  if(!slot)throw new Error('未定義のアクセサリ種別です: '+type);
  const preset=type==='ベルト'?'ベルト':'アミュレット・指輪',baseName=presetBaseName('accessory',preset,i);
  if(!baseName)throw new Error(`アクセサリ名称セットにiLv${i}がありません: ${preset}`);
  const trace=[`accessory_type=${type}`,`slot=${slot}`,`item_level=${i}`,`requirement=ilv_only`,`rarity=NORMAL`,`mod_count=0`];
  const out={id:String(input.id||''),name:String(input.name||`${baseName}${type}`),status:'draft',tags:Array.isArray(input.tags)?clone(input.tags):[],params:{slot,rarity:'NORMAL'},description:String(input.description||''),mod_ids:[],item_level:i};
  out.generation=generationMeta(input,trace);return out;
}
function validate(record,input={}){
  ...
  const kind=String(input.kind||record.generation?.generation_input?.kind||'');if(!['weapon','armor','accessory'].includes(kind))errors.push('装備区分が不正です。');
  if(kind==='accessory'){
    const type=String(input.base_item_type||record.generation?.generation_input?.base_item_type||''),slot=ACCESSORY_SLOT_MAP[type];
    if(!slot)errors.push('アクセサリ種別が不正です。');
    if(record.params?.slot!==slot)errors.push('アクセサリ装備先が仕様と一致しません。');
    if(record.params?.rarity!=='NORMAL')errors.push('ベースアクセサリはNORMAL固定です。');
    if(!Array.isArray(record.mod_ids)||record.mod_ids.length!==0)errors.push('ベースアクセサリはMODなしです。');
    for(const f of ['required_str','required_dex','required_int','required_vit','required_mnd','required_agi'])if(Object.prototype.hasOwnProperty.call(record,f))errors.push('アクセサリに能力値要求を設定できません: '+f);
  }else{
...
function expandBatchInputs(request={},reservedIds=[]){
  validateConfig();const kind=String(request.kind||'weapon');if(!['weapon','armor','accessory'].includes(kind))throw new Error('区分は武器・防具・アクセサリのいずれかです。');
  ...
  let types=normalizeList(request.base_item_types||request.base_item_type);if(!types.length)types=kind==='armor'?Object.keys(config.armor.requirement_coefficients):kind==='accessory'?Object.keys(ACCESSORY_SLOT_MAP):Object.keys(config.weapon.requirement_coefficients);
...
function requestTemplate(){return {schema:'GKS_EQUIPMENT_GENERATION_REQUEST',version:'1.0.0',requests:[{kind:'weapon',base_item_types:['片手剣','短剣'],item_level:{min:1,max:3},seed:'ai-weapon'},{kind:'armor',base_item_types:['重装'],armor_slots:['鎧','頭'],item_level:{min:1,max:3},seed:'ai-armor'},{kind:'accessory',base_item_types:['アミュレット','指輪','ベルト'],item_level:{min:1,max:3},seed:'ai-accessory'}]};}
...
function metricHtml(record){const kind=record.generation?.generation_input?.kind;if(kind==='accessory')return [['装備先',record.params?.slot||'-'],['装備条件','iLvのみ'],['レアリティ',record.params?.rarity||'-'],['MOD数',Array.isArray(record.mod_ids)?record.mod_ids.length:'-']].map(([k,v])=>`<div><span class="small">${k}</span><br><b>${v}</b></div>`).join('');const pairs=kind==='armor'?[['VIT要求',record.required_vit],['MND要求',record.required_mnd],['AGI要求',record.required_agi],['HP補正',record.hp_bonus],['MP補正',record.mp_bonus],['回避',record.evasion],['魔法抵抗',record.magic_resistance]]:[['STR要求',record.required_str],['DEX要求',record.required_dex],['INT要求',record.required_int],['攻撃力',record.attack],['命中力',record.accuracy],['魔法命中',record.magic_accuracy],['魔法上昇',record.magic_weapon_bonus],['武器クリ率',record.weapon_critical_rate],...(record.generation?.generation_input?.base_item_type==='盾'?[['Block率',record.block_rate],['Blockダメージカット率',record.block_damage_cut_rate]]:[])];return pairs.map(([k,v])=>`<div><span class="small">${k}</span><br><b>${Number.isFinite(Number(v))?Number(v):'-'}</b></div>`).join('');}
...
function renderPanel(){
  ...
  <div class="card eqg-step"><h2>① 生成設定</h2><div id="eqgConfigStatus" class="item"></div><div class="eqg-touch-stack">
    ...
    <details class="eqg-touch-details"><summary><b>防具ベースアイテムセット</b></summary><div class="eqg-detail-body"><p class="small">武器とは別の名前セットです。防具カテゴリ・部位の前に付けます。</p><div class="field"><label>プリセット</label><select id="eqgArmorPreset"></select></div><div class="toolbar"><button id="eqgArmorPresetUse">選択プリセットを使用</button></div><div id="eqgArmorNameRows"></div><button id="eqgArmorAddRow">＋ 追加</button><div class="grid" style="margin-top:12px"><div class="field"><label>新しいプリセット名</label><input id="eqgArmorPresetName" placeholder="例: 高級防具"></div></div><div class="toolbar eqg-actions"><button id="eqgArmorPresetSave">現在の内容をプリセット登録</button><button class="primary" id="eqgArmorSetConfirm">設定を確定</button></div><div id="eqgArmorSetStatus" class="item"></div></div></details>    <details class="eqg-touch-details"><summary><b>アクセサリ ベース名称セット</b></summary><div class="eqg-detail-body"><p class="small">正式仕様の「アミュレット・指輪」と「ベルト」の2セットを編集します。数値性能や能力値要求は付けません。</p><div class="field"><label>名称セット</label><select id="eqgAccessoryPreset"></select></div><div class="toolbar"><button id="eqgAccessoryPresetUse">選択セットを編集</button></div><div id="eqgAccessoryNameRows"></div><button id="eqgAccessoryAddRow">＋ 追加</button><div class="toolbar eqg-actions"><button class="primary" id="eqgAccessorySetConfirm">設定を確定</button></div><div id="eqgAccessorySetStatus" class="item"></div></div></details></div>
</div>
  </div>
  <div class="card eqg-step"><h2>② AI・JSON一括入力</h2><p class="small">武器・防具・アクセサリを分けて扱います。AIには種別・iLv帯・件数などを指定させ、装備IDはStudioがEQP-0001形式で自動採番し、攻撃力や要求値もStudioが計算します。</p>
    ...
    <details><summary><b>アクセサリを一括生成</b></summary><div class="eqg-kind-panel"><div class="eqg-file-picker"><input id="eqgAccessoryJsonFile" type="file" accept=".json,application/json" aria-label="アクセサリJSONを選択"></div><div class="toolbar eqg-actions eqg-template-row"><button id="eqgAccessoryTemplate">アクセサリAIテンプレート出力</button></div><details class="eqg-touch-details eqg-accessory-json-paste"><summary>JSONを貼り付ける</summary><textarea id="eqgAccessoryJsonText" rows="9"></textarea></details><div class="toolbar eqg-actions"><button id="eqgAccessorySimulate">アクセサリを試算（保存なし）</button><button class="primary" id="eqgAccessoryGenerate">アクセサリの生成内容を確定確認</button></div><div id="eqgAccessoryJsonStatus" class="item">未実行</div></div></details>
  </div>
  <div class="card"><details><summary><b>手動で1件作る</b></summary><div class="grid" style="margin-top:12px"><div class="field"><label>区分</label><select id="eqgKind"><option value="weapon">武器</option><option value="armor">防具</option><option value="accessory">アクセサリ</option></select></div><div class="field"><label>基本装備種別</label><select id="eqgType"></select></div><div class="field" id="eqgArmorSlotField"><label>防具部位</label><select id="eqgArmorSlot"></select></div><div class="field"><label>アイテムレベル</label><input id="eqgIlv" type="number"></div><div class="field"><label>装備ID</label><input id="eqgId" placeholder="EQP-0001"></div><div class="field"><label>名称（空欄ならセットから自動）</label><input id="eqgName"></div></div><div class="toolbar"><button id="eqgGenerate">生成して確認</button></div><div id="eqgStatus" class="item">未実行</div></details></div>
  ...
  let loadedWeaponPayload=null,loadedArmorPayload=null,loadedAccessoryPayload=null;
  ...
  function fillConfig(){q('eqgConfigVersion').value=config.config_version;q('eqgAttackMul').value=config.weapon.performance.attack_multiplier;q('eqgAccuracyMul').value=config.weapon.performance.accuracy_multiplier;q('eqgCrit').value=config.weapon.performance.weapon_critical_rate;q('eqgBlockRateBase').value=config.weapon.performance.block_rate_base;q('eqgBlockRatePerIlv').value=config.weapon.performance.block_rate_per_item_level;q('eqgBlockDamageCut').value=config.weapon.performance.block_damage_cut_rate;q('eqgCfgWeaponType').innerHTML=optionHtml(Object.keys(config.weapon.requirement_coefficients));q('eqgCfgArmorType').innerHTML=optionHtml(Object.keys(config.armor.requirement_coefficients));q('eqgCfgSlot').innerHTML=optionHtml(Object.keys(config.armor.slot_coefficients));syncWeaponCfg();syncArmorCfg();syncSlotCfg();refreshLevelRange();configStatus();}
  ...
  q('eqgSaveConfig').addEventListener('click',()=>{try{const next=clone(config),wt=q('eqgCfgWeaponType').value,at=q('eqgCfgArmorType').value,sl=q('eqgCfgSlot').value;next.config_version=q('eqgConfigVersion').value.trim()||next.config_version;next.weapon.performance.attack_multiplier=Number(q('eqgAttackMul').value);next.weapon.performance.accuracy_multiplier=Number(q('eqgAccuracyMul').value);next.weapon.performance.weapon_critical_rate=Number(q('eqgCrit').value);next.weapon.performance.block_rate_base=Number(q('eqgBlockRateBase').value);next.weapon.performance.block_rate_per_item_level=Number(q('eqgBlockRatePerIlv').value);next.weapon.performance.block_damage_cut_rate=Number(q('eqgBlockDamageCut').value);next.weapon.requirement_coefficients[wt]={str:Number(q('eqgCfgStr').value),dex:Number(q('eqgCfgDex').value),int:Number(q('eqgCfgInt').value)};next.armor.requirement_coefficients[at]={vit:Number(q('eqgCfgVit').value),mnd:Number(q('eqgCfgMnd').value),agi:Number(q('eqgCfgAgi').value)};next.armor.slot_coefficients[sl]=Number(q('eqgCfgSlotValue').value);saveActiveConfig(next);fillConfig();q('eqgConfigStatus').innerHTML='<b>生成基準を保存しました。</b><br>'+q('eqgConfigStatus').innerHTML;}catch(e){alert('生成設定を保存できません: '+e.message);}});
  ...
  const presetCap=kindName=>kindName==='weapon'?'Weapon':kindName==='armor'?'Armor':'Accessory';
  function rowsFromEditor(kindName){const root=q(`eqg${presetCap(kindName)}NameRows`);return [...root.querySelectorAll('.eqg-name-row')].map(row=>({level:Number(row.querySelector('[data-role="level"]').value),name:row.querySelector('[data-role="name"]').value.trim()}));}
  function renderNameEditor(kindName,rows){const root=q(`eqg${presetCap(kindName)}NameRows`);root.innerHTML=rows.map(r=>`<div class="eqg-name-row"><div class="field"><label>Lv</label><input data-role="level" type="number" min="1" value="${Number(r.level)}"></div><div class="field"><label>名前</label><input data-role="name" value="${String(r.name).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;')}"></div></div>`).join('');}
  function fillPresetSelect(kindName){const cap=presetCap(kindName),sel=q(`eqg${cap}Preset`),part=baseNameSets[kindName];sel.innerHTML=optionHtml(Object.keys(part.presets));sel.value=part.active_preset;renderNameEditor(kindName,part.presets[part.active_preset]);const status=q(`eqg${cap}SetStatus`);status.innerHTML=kindName==='accessory'?`編集対象: <b>${part.active_preset}</b> / ${part.presets[part.active_preset].length}段階`:`使用中: <b>${part.active_preset}</b> / ${part.presets[part.active_preset].length}段階`;}
  function addNameRow(kindName){const rows=rowsFromEditor(kindName),next=Math.max(0,...rows.map(r=>Number(r.level)||0))+1;rows.push({level:next,name:''});renderNameEditor(kindName,rows);q(`eqg${presetCap(kindName)}NameRows`).querySelector('.eqg-name-row:last-child [data-role="name"]')?.focus();}
  function usePreset(kindName){try{const cap=presetCap(kindName),sel=q(`eqg${cap}Preset`);activateBaseNamePreset(kindName,sel.value);fillPresetSelect(kindName);fillConfig();}catch(e){alert('プリセットを切り替えられません: '+e.message);}}
  function confirmSet(kindName){try{const cap=presetCap(kindName),part=baseNameSets[kindName],name=part.active_preset;saveBaseNameSet(kindName,name,rowsFromEditor(kindName),{activate:true});fillPresetSelect(kindName);fillConfig();q(`eqg${cap}SetStatus`).innerHTML=`<b>設定を確定しました。</b> 使用中: ${name} / 生成上限 iLv${levelRange().max}`;}catch(e){alert('ベースアイテムセットを確定できません: '+e.message);}}
  function registerPreset(kindName){try{const cap=presetCap(kindName),input=q(`eqg${cap}PresetName`),name=input.value.trim();saveBaseNameSet(kindName,name,rowsFromEditor(kindName),{activate:true});input.value='';fillPresetSelect(kindName);fillConfig();}catch(e){alert('プリセット登録できません: '+e.message);}}
  for(const k of ['weapon','armor','accessory']){const cap=presetCap(k);q(`eqg${cap}AddRow`).addEventListener('click',()=>addNameRow(k));q(`eqg${cap}PresetUse`).addEventListener('click',()=>usePreset(k));q(`eqg${cap}SetConfirm`).addEventListener('click',()=>confirmSet(k));const presetSave=q(`eqg${cap}PresetSave`);if(presetSave)presetSave.addEventListener('click',()=>registerPreset(k));fillPresetSelect(k);}
  const syncTypes=()=>{const armor=kind.value==='armor',accessory=kind.value==='accessory',vals=armor?Object.keys(config.armor.requirement_coefficients):accessory?Object.keys(ACCESSORY_SLOT_MAP):Object.keys(config.weapon.requirement_coefficients);type.innerHTML=optionHtml(vals);slotField.style.display=armor?'':'none';};kind.addEventListener('change',syncTypes);syncTypes();
  ...
  function templateFor(kindName){const req=kindName==='weapon'?{kind:'weapon',base_item_types:['片手剣','短剣'],item_level:{min:1,max:3},seed:'ai-weapon'}:kindName==='armor'?{kind:'armor',base_item_types:['重装'],armor_slots:['鎧','頭'],item_level:{min:1,max:3},seed:'ai-armor'}:{kind:'accessory',base_item_types:['アミュレット','指輪','ベルト'],item_level:{min:1,max:3},seed:'ai-accessory'};return {schema:'GKS_EQUIPMENT_GENERATION_REQUEST',version:'1.0.0',requests:[req]};}
  function ensureKindPayload(value,kindName){const reqs=normalizeRequestPayload(value);if(reqs.some(r=>r.kind!==kindName)){const label=kindName==='weapon'?'武器':kindName==='armor'?'防具':'アクセサリ';throw new Error(`${label}欄には${label}要求だけを入れてください。`);}return {schema:'GKS_EQUIPMENT_GENERATION_REQUEST',version:'1.0.0',requests:reqs};}
  function payloadRef(kindName){return kindName==='weapon'?loadedWeaponPayload:kindName==='armor'?loadedArmorPayload:loadedAccessoryPayload;}
  function setPayloadRef(kindName,value){if(kindName==='weapon')loadedWeaponPayload=value;else if(kindName==='armor')loadedArmorPayload=value;else loadedAccessoryPayload=value;}
  function bindJsonKind(kindName){const cap=kindName==='weapon'?'Weapon':kindName==='armor'?'Armor':'Accessory',file=q(`eqg${cap}JsonFile`),text=q(`eqg${cap}JsonText`),status=q(`eqg${cap}JsonStatus`);q(`eqg${cap}Template`).addEventListener('click',()=>downloadJson(`Equipment_AI_${cap}_Request_Template.json`,templateFor(kindName)));file.addEventListener('change',async e=>{try{const f=e.target.files?.[0];if(!f)return;const parsed=JSON.parse(await f.text()),normalized=ensureKindPayload(parsed,kindName);setPayloadRef(kindName,normalized);text.value=JSON.stringify(normalized,null,2);status.textContent=`${f.name} を読み込みました。試算してください。`;}catch(err){setPayloadRef(kindName,null);status.textContent='JSON読込エラー: '+err.message;}});const getPayload=()=>{const t=text.value.trim(),fallback=payloadRef(kindName);return ensureKindPayload(t?JSON.parse(t):fallback||templateFor(kindName),kindName);};q(`eqg${cap}Simulate`).addEventListener('click',()=>{try{const p=generateRequestPayload(getPayload()),sim={...p,mode:'simulation',commit_allowed:false};renderResult(section,sim);status.textContent=`試算完了: ${p.summary.count}件 / 正常 ${p.summary.valid} / エラー ${p.summary.invalid} / 保存なし`;q('eqgWorkExport').disabled=false;q('eqgFinalExport').disabled=true;updateExportStatus('simulation');}catch(e){status.textContent='試算エラー: '+e.message;}});q(`eqg${cap}Generate`).addEventListener('click',()=>{try{const p=generateRequestPayload(getPayload());renderResult(section,p);status.textContent=`生成確認完了: ${p.summary.count}件 / 正常 ${p.summary.valid} / エラー ${p.summary.invalid}`;q('eqgWorkExport').disabled=false;q('eqgFinalExport').disabled=p.summary.invalid>0;updateExportStatus(p.summary.invalid?'error':'confirmed');}catch(e){status.textContent='生成エラー: '+e.message;}});}
  bindJsonKind('weapon');bindJsonKind('armor');bindJsonKind('accessory');
