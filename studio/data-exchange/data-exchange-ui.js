(function(){
  ...
  const DATASET_LABELS={tags:'タグ',stats:'能力値',jobs:'職業',skills:'スキル',equipment:'装備',mods:'MOD',monsters:'モンスター',status_effects:'状態異常',tablets:'石板',ai_conditions:'AI条件',ai_targets:'AI対象',ai_actions:'AI行動',chapters:'ストーリー章',story_sections:'ストーリー節',story_scenes:'ストーリーシーン',story_dialogues:'ストーリー台詞'};
  const ORDER=['monsters','tags','skills','jobs','equipment','mods','stats','status_effects','tablets','ai_conditions','ai_targets','ai_actions','chapters','story_sections','story_scenes','story_dialogues'];
  ...
  function renderPicker(){const list=document.getElementById('dxPickerList');if(!list)return;const dataset=currentDataset(),set=selectedSet(dataset),visible=visibleRecords();list.innerHTML=visible.length?visible.map(row=>{const id=String(row?.id||''),sel=set.has(id),tags=Array.isArray(row?.tags)?row.tags.map(t=>typeof tagLabel==='function'?tagLabel(t):t).join(', '):'',parent=[row?.chapter_id,row?.section_id,row?.scene_id].filter(Boolean).join(' / ');return `<div class="dx-picker-row${sel?' selected':''}" role="button" tabindex="0" aria-pressed="${sel?'true':'false'}" onclick="GKSDataExchangeUI.toggleItem('${escA(id)}')" onkeydown="GKSDataExchangeUI.handleItemKey(event,'${escA(id)}')"><div class="dx-picker-row-name">${escText(row?.name||row?.title||row?.speaker||id)} <span class="badge">${escText(DATASET_LABELS[dataset]||dataset)}</span></div><div class="dx-picker-row-id">${escText(id)}</div><div class="dx-picker-row-meta">${escText(parent?`親: ${parent}`:(tags||row?.description||''))}</div></div>`}).join(''):'<div class="item">表示対象はありません。</div>';const count=document.getElementById('dxPickerCount');if(count)count.textContent=`選択 ${set.size}件 / 表示 ${visible.length}件 / 全 ${rows(dataset).length}件`;const btn=document.getElementById('dxPickerExport');if(btn){btn.disabled=set.size===0;btn.textContent=`選択した${set.size}件をJSON出力`;}}
  ...
  function openPicker(options={}){pickerDatasetFilter=Array.isArray(options.datasets)&&options.datasets.length?[...options.datasets]:null;pickerTitle=String(options.title||'部分データ入出力');initDatasetOptions();const p=document.getElementById('dataExchangePicker');if(!p)return;const title=document.getElementById('dataExchangePickerTitle');if(title)title.textContent=pickerTitle;document.getElementById('dxPickerSearch').value='';p.classList.remove('hidden');p.setAttribute('aria-hidden','false');document.body.classList.add('dx-picker-open');renderPicker();}
  function openStoryPicker(){openPicker({datasets:['chapters','story_sections','story_scenes','story_dialogues'],title:'シナリオ データ入出力'});}
  function closePicker(){const p=document.getElementById('dataExchangePicker');if(!p)return;p.classList.add('hidden');p.setAttribute('aria-hidden','true');document.body.classList.remove('dx-picker-open');pickerDatasetFilter=null;pickerTitle='部分データ入出力';}
