...
function project(){return {
  ...
  masters:{monsters:[],stats:[],status_effects:[],tablets:[],jobs:[],equipment:[],mods:[],
    ...
    ai_conditions:[{id:'AIC-0001',name:'Always',node_type:'condition',status:'active',tags:[],description:'',data_version:'1.0.0',evaluator:'condition.always',ports:{inputs:[],outputs:[{id:'true'}]},parameter_schema:{type:'object',properties:{}},unlock:{}}],
    ai_targets:[{id:'AIT-0001',name:'Enemy',node_type:'target',status:'active',tags:[],description:'',data_version:'1.0.0',evaluator:'target.enemy',ports:{inputs:[],outputs:[{id:'next'}]},parameter_schema:{type:'object',properties:{}},unlock:{}}],
    ai_actions:[{id:'AIA-0001',name:'Use Skill',node_type:'action',status:'active',tags:[],description:'',data_version:'1.0.0',evaluator:'action.skill',ports:{inputs:[],outputs:[{id:'next'}]},parameter_schema:{type:'object',properties:{}},unlock:{}}]},
  ai_programs:[{id:'AIP-0001',name:'Base AI',status:'valid',tags:['TAG-0003'],description:'',version:1,schema_version:'1.0.0',data_version:1,entry_node_id:'N1',nodes:[
    {instance_id:'N1',master_node_id:'AIC-0001',master_version:1,node_type:'condition',position:{x:0,y:0},parameters:{},comment:''},
    {instance_id:'N2',master_node_id:'AIT-0001',master_version:1,node_type:'target',position:{x:1,y:0},parameters:{},comment:''},
    {instance_id:'N3',master_node_id:'AIA-0001',master_version:1,node_type:'action',position:{x:2,y:0},parameters:{skill_id:'SKL-0001'},comment:''}
  ],edges:[],subroutines:[],compiled:null,updated_at:'TAG-0001'}]
}}
async function addEnvelope(base,id='AIP-0002'){
  const env=await dx.buildEnvelope({rootData:base,dataset:'ai_programs',ids:['AIP-0001'],dependencyMode:'recursive',studioVersion:'R9-B'});
...
async function main(){
  const base=project();
  const exported=await dx.buildEnvelope({rootData:base,dataset:'ai_programs',ids:['AIP-0001'],dependencyMode:'recursive',studioVersion:'R9-B'});
  ...
  assert.equal(exported.datasets.skills[0].id,'SKL-0001');
  ...
  let brokenIndex=0;
  for(const [nodeType,field,value] of [['action','skill_id','SKL-9999'],['condition','master_node_id','AIC-9999'],['target','master_node_id','AIT-9999'],['action','master_node_id','AIA-9999']]){
    const broken=await addEnvelope(base,'AIP-'+String(100+(brokenIndex++)).padStart(4,'0'));const node=broken.datasets.ai_programs[0].nodes.find(x=>x.node_type===nodeType);if(field==='skill_id')node.parameters.skill_id=value;else node[field]=value;
  ...
  const unknown=await addEnvelope(base,'AIP-9998');unknown.datasets.ai_programs[0].future_field=true;assert.equal((await dx.createApplyPlan({rootData:base,envelope:unknown})).can_apply,false);
  const deleted=await addEnvelope(base,'AIP-9999');deleted.operations={delete:{ai_programs:['AIP-0001']}};assert.equal((await dx.dryRunImport({rootData:base,envelope:deleted})).can_apply,false);
