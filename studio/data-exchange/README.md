...
### DE-16.2 Audit persistence / reload restoration
- Audit Sessionは独立localStorageキーだけに保存せず、Project正本の `data_exchange_audit_sessions` に保存する。
- 旧 `gks_data_exchange_audit_v1_<project>` が残る場合は初回読込時にProject正本へ移行する。
- Transaction `projectHash` はAudit運用メタデータを除外し、Audit保存自身でUndo hashが無効にならないようにする。
- Apply → Project保存 → Reload → Audit復元を回帰テスト化する。
