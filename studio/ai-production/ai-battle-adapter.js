(function(root,factory){
  ...
  const targetResolver=typeof module==='object'&&module.exports?require('../../assets/shared/js/formation-target-resolver.js'):root&&root.GKSFormationTargetResolver;
  const api=factory(engine,targetResolver);
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.GKSAIBattleAdapter=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(Engine,TargetResolver){
  'use strict';
  if(!Engine)throw new Error('GKSAIDecisionEngine is required');
  if(!TargetResolver)throw new Error('GKSFormationTargetResolver is required');
  const clone=(value)=>value==null?value:JSON.parse(JSON.stringify(value));
  function snapshot(input){
    const units=(input?.units||[]).map((unit)=>({id:String(unit.id),side:String(unit.side||''),alive:unit.alive!==false,hp:Number(unit.hp)||0,max_hp:Math.max(1,Number(unit.maxHp??unit.max_hp)||1),mp:Number(unit.mp)||0,max_mp:Math.max(0,Number(unit.maxMp??unit.max_mp)||0),defense:Number(unit.defense)||0,formationPosition:TargetResolver.normalizeFormationPosition(unit.formationPosition??unit.formation_position),statuses:clone(unit.statuses||[]),cooldowns:clone(unit.cooldowns||{}),skills:clone(unit.skills||[])}));
    return {battle_id:String(input?.battle_id||'studio-battle'),tick:Math.max(0,Number(input?.tick)||0),phase:input?.phase||'reservation',seed:input?.seed??0,actor_id:String(input?.actor_id||''),units,mp_cost_multiplier:Math.max(0,Number(input?.mp_cost_multiplier)||1),selected_action_id:String(input?.selected_action_id||'')};
  }
