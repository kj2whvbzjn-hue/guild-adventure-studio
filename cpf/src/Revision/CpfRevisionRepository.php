...
use GK\CPF\Core\{CpfException, CpfNodeManager, CpfProjectMutation, JsonStore};
...
final class CpfRevisionRepository
{
    ...
    {
        return $this->mutation->execute($projectDir, ['revisions/' . $nodeId, 'history'], function () use ($projectDir, $nodeId, $patch, $reason): array {
            ...
            $revisionId = sprintf(
                'REV_%s_%06d',
                preg_replace('/[^A-Za-z0-9_-]/', '_', $nodeId),
                $this->nextNumber($this->list($projectDir, $nodeId))
            );
            $now = date(DATE_ATOM);
    ...
    {
        return $this->mutation->execute($projectDir, ['nodes', 'revisions/' . $nodeId, 'history'], function () use ($projectDir, $nodeId, $revisionId, $approvedBy): array {
    ...
    {
        return $this->mutation->execute($projectDir, ['revisions/' . $nodeId, 'history'], function () use ($projectDir, $nodeId, $revisionId, $reason): array {
            $revision = $this->get($projectDir, $nodeId, $revisionId);
            if (($revision['revision_status'] ?? '') !== 'CANDIDATE') {
    ...
    private function nextNumber(array $revisions): int
    {
        $max = 0;
        foreach ($revisions as $revision) {
            if (preg_match('/_(\d+)$/', (string)($revision['revision_id'] ?? ''), $match)) {
