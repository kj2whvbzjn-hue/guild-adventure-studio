...
assert.strictEqual(generic.VERSION,'R05-A');
for(const runtimePath of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
  ...
  assert.deepStrictEqual(out.legacySkill.genericRuntime.effectContracts,[{type:'DAMAGE',power:120,damageType:'PHYSICAL'}]);
...
const fixedOut=generic.compileGenericSkill(fixed,registry,null);
assert.strictEqual(fixedOut.ok,true,JSON.stringify(fixedOut.errors));
assert.deepStrictEqual(fixedOut.legacySkill.genericRuntime.effectContracts,[{type:'DAMAGE',power:37,damageType:'FIXED'}]);
