...
function compile(ctx,skill){const out=generic.compileGenericSkill(skill,registry,ctx.compileTaggedSkill);ok(out.ok,`generic compile failed ${JSON.stringify(out.errors)}`);const c=ctx.compileTaggedSkill(out.legacySkill);ok(c.ok,`legacy compile failed ${JSON.stringify(c.errors)}`);return c}
