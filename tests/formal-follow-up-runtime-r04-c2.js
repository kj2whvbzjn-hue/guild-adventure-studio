...
assert.ok(/^R04-(?:C2|D\d+|E\d+)$/.test(trigger.VERSION),`unexpected trigger engine version ${trigger.VERSION}`);assert.ok(trigger.SUPPORTED.includes('ON_ALLY_ATTACK'));assert.strictEqual(condition.VERSION,'R04-C2');
let calls=0;const tc={type:'ON_ALLY_ATTACK',scope:'SELF',engineEvent:'ally_attack',dispatchMode:'FOLLOW_UP',priority:0};let r=trigger.dispatchCompiled(tc,'ally_attack',{},()=>{calls++;return{ok:true}});assert.strictEqual(r.ok,true);assert.strictEqual(calls,1);r=trigger.dispatchCompiled(tc,'hit_received',{},()=>{calls++});assert.strictEqual(r.ok,false);assert.strictEqual(calls,1);
