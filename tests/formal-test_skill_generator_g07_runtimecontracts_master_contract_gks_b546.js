...
(async()=>{
 ...
 assert.strictEqual(dry.summary.add,1,'runtimeContracts Skill must be a valid add in Dry Run');
 const plan=await dx.createApplyPlan({rootData,envelope:env,dryRun:dry,conflictChoices:{}});
 assert.strictEqual(plan.can_apply,true,`runtimeContracts must be accepted by Apply Plan: ${(plan.reasons||[]).join(' / ')}`);
 ...
 assert.deepStrictEqual(stored.runtimeContracts,skill.runtimeContracts,'runtimeContracts must be preserved in Master');
 ...
 assert.ok(html.includes('skill-generator.js?v=34'));
 assert.ok(html.includes('data-exchange-core.js?v=17'));
