...
assert.ok(/^R04-[BCDE]|^R0[5-9]-|^R[1-9][0-9]-/.test(registry.phase),`unexpected phase ${registry.phase}`);
assert.ok(/^R04-[BCDE]|^R0[5-9]-|^R[1-9][0-9]-/.test(generic.VERSION),`unexpected compiler version ${generic.VERSION}`);
...
for(const runtimePath of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
  ...
  ])assert.ok(out.compiledSkill.tags.includes(tag),`${runtimePath}: missing ${tag}`);
  const compiled=ctx.compileTaggedSkill(out.compiledSkill);
...
let bad=generic.compileSkill(badRange,registry);
...
bad=generic.compileSkill(noDamage,registry);
assert.strictEqual(bad.ok,false);
...
bad=generic.compileSkill(badLimit,registry);
assert.strictEqual(bad.ok,false);
