...
for(const legacyCompile of legacyCompilers){for(const c of cases){const r=generic.compileSkill(c.skill,registry,legacyCompile);ok(r.ok,`${c.id}: ${r.errors.map(x=>x.code+':'+x.message).join('|')}`);ok(r.legacyValidation&&r.legacyValidation.ok,`${c.id}: legacy validation missing`);}}
const invalid=[
...
for(const x of invalid){const r=generic.compileSkill(x.skill,registry,legacyCompilers[0]);ok(!r.ok,'invalid accepted: '+x.code);ok(r.errors.some(e=>e.code===x.code),'missing error '+x.code+': '+JSON.stringify(r.errors));}
