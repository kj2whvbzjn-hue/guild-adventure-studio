...
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 ...
 const compile=(id,power,duration)=>{const out=generic.compileGenericSkill(makeSkill(id,power,duration),registry,ctx.compileTaggedSkill);ok(out.ok,`${path}: generic compile failed ${JSON.stringify(out.errors)}`);const c=ctx.compileTaggedSkill(out.legacySkill);ok(c.ok,`${path}: legacy compile failed ${JSON.stringify(c.errors)}`);return c};
