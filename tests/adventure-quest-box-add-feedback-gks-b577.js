...
assert.ok(html.includes('class="quest-box-add-feedback" role="status" aria-live="polite"'),'zone header must expose accessible add feedback');
...
const stage={querySelector(selector){
  ...
  if(selector==='.quest-box-add-feedback')return feedback;
  return null;
}};
...
vm.runInContext(`
let questBoxPlacementFeedbackTimer=null;
...
`,ctx);
vm.runInContext(extractFunction('announceQuestBoxPlacementAdded'),ctx);
...
assert.strictEqual(feedback.textContent,'✓ Eventを追加しました','user must see immediate success feedback');
assert.ok(feedback.classList.contains('show'),'success feedback must be visible');
...
assert.strictEqual(delayed.length,1,'feedback cleanup must be scheduled');
delayed[0]();
assert.ok(!feedback.classList.contains('show'),'success feedback must clear after the brief acknowledgement');
...
console.log('PASS GKS-B577 Quest Box add feedback: immediate success status, auto-scroll, temporary highlight');
