...
for(const needle of ['function applyTaggedCover','function resolveCoverIntervention','function processCoverEffects','function clearAllCoverEffects','derivedGeneration=0','wasCovered=false',"reason:'AREA_ATTACK'","DERIVED_GENERATION_LIMIT","counter_chain_blocked"])if(!rt.includes(needle))errors.push(`runtime missing ${needle}`);
for(const needle of ['COVER-RUNTIME-BASE','COVER-RUNTIME-ATTACHED-STATUS','COVER-RUNTIME-ATTACHED-DOT','COVER-RUNTIME-DOT-ONLY-BLOCK','COVER-RUNTIME-STATUS-ONLY-BLOCK','COVER-RUNTIME-AREA-BLOCK','COVER-RUNTIME-USES','COVER-RUNTIME-DURATION','COVER-RUNTIME-COUNTER-COVER-COUNTER','COVER-RUNTIME-FOLLOW-UP'])if(!app.includes(needle))errors.push(`case missing ${needle}`);
