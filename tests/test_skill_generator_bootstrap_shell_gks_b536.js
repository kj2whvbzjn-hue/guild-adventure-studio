...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=600'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','600')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b600'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=600'"),'offline shell must advance');
