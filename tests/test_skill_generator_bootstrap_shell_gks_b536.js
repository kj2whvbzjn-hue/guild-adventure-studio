...
assert.ok(html.indexOf('id="view-skill-generator"') < html.indexOf('./skill/skill-generator.js?v=33'),'bootstrap shell must exist before dynamic script load');
...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=591'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','591')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b591'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=591'"),'offline shell must advance');
