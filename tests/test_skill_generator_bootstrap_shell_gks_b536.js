...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=577'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','577')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b577'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=577'"),'offline shell must advance');
