...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=607'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','607')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b607'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=607'"),'offline shell must advance');
