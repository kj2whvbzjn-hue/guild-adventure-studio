...
assert.ok(html.indexOf('id="view-skill-generator"') < html.indexOf('./skill/skill-generator.js?v=19'),'bootstrap shell must exist before dynamic script load');
...
assert.ok(html.includes("url.searchParams.set('appv','541')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b541'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=541'"),'offline shell must advance');
