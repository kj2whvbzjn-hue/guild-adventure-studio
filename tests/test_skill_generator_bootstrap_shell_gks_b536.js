...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=595'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','595')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b595'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=595'"),'offline shell must advance');
