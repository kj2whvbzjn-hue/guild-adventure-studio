...
assert.ok(html.includes("url.searchParams.set('appv','568')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b568'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=568'"),'offline shell must advance');
