...
assert.ok(html.includes("url.searchParams.set('appv','569')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b569'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=569'"),'offline shell must advance');
