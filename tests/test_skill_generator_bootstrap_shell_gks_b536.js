...
assert.ok(html.indexOf('id="view-skill-generator"') < html.indexOf('./skill/skill-generator.js?v=16'),'bootstrap shell must exist before dynamic script load');
...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=538'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','538')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b538'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=538'"),'offline shell must advance');
