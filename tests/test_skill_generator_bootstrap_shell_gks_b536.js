...
assert.ok(html.includes("url.searchParams.set('appv','593')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b593'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=593'"),'offline shell must advance');
