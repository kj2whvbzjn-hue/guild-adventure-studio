...
assert.ok(html.indexOf('id="view-skill-generator"') < html.indexOf('./skill/skill-generator.js?v=14'),'bootstrap shell must exist before dynamic script load');
...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=536'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','536')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b536'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=536'"),'offline shell must advance');
