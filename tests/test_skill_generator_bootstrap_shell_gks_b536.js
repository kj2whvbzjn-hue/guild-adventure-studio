...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=574'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','574')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b574'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=574'"),'offline shell must advance');
