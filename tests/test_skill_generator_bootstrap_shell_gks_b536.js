...
assert.ok(html.includes("url.searchParams.set('appv','608')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b608'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=608'"),'offline shell must advance');
