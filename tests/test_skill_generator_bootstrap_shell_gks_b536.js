...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=610'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','610')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b610'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=610'"),'offline shell must advance');
