...
assert.ok(html.indexOf('id="view-skill-generator"') < html.indexOf('./skill/skill-generator.js?v=21'),'bootstrap shell must exist before dynamic script load');
...
assert.ok(html.includes("url.searchParams.set('appv','543')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b543'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=543'"),'offline shell must advance');
