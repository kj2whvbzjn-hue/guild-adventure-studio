...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=578'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','578')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b578'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=578'"),'offline shell must advance');
