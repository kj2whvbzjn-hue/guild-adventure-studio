...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=567'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','567')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b567'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=567'"),'offline shell must advance');
