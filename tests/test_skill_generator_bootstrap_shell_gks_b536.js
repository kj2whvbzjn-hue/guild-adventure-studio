...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=571'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','571')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b571'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=571'"),'offline shell must advance');
