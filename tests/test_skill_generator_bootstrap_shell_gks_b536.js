...
assert.ok(html.includes("url.searchParams.set('appv','548')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b548'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=548'"),'offline shell must advance');
