...
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=579'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','579')"),'Studio app reload key must advance');
assert.ok(sw.includes('gks-studio-b579'),'Studio cache namespace must advance');
assert.ok(sw.includes("const OFFLINE_URL='./index.html?appv=579'"),'offline shell must advance');
