...
assert.ok(sg.includes("await global.GKSDataExchangeUI.undoSessionById(sessionId,{expectedDataset:'skills'})"),'G07 Undo must await exact Data Exchange session result');
for(const needle of ['G07 Undo 完了','success ${undone}','undone ${undone} / remain ${remain} / conflict ${conflict}','現在のSkill Masterは登録前の状態に戻っています。']){
