...
for(const runtimePath of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 ...
 for(const item of cases){
  ...
  const out=generic.compileGenericSkill(skill,registry,ctx.compileTaggedSkill);assert.strictEqual(out.ok,true,`${runtimePath}/${item.id}: ${JSON.stringify(out.errors)}`);
  const contract=out.legacySkill.genericRuntime.applyContracts[0];assert.ok(contract.values,`${item.id}: Generic values missing`);
  const tampered=JSON.parse(JSON.stringify(out.legacySkill));tampered.tags=tampered.tags.map(x=>x===item.legacy?item.tampered:x);
  ...
  const resolved=ctx.resolveGenericApplyDefinition(compiled,compiled.definition.genericRuntime.applyContracts[0]);assert.strictEqual(resolved.ok,true);
