...
ok(shared.VERSION==='R03-F1','shared lifecycle engine version mismatch');
...
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 ...
 ok(typeof ctx.getTaggedApplyLifecycleEngine==='function',`${path}: lifecycle engine facade missing`);const engine=ctx.getTaggedApplyLifecycleEngine();ok(engine.version==='R03-F1',`${path}: shared lifecycle engine not connected`);
...
for(const html of ['game/index.html','game-tag-test/index.html','studio/index.html']){const src=fs.readFileSync(html,'utf8');ok(src.includes('apply-lifecycle-engine.js?v=48673'),`${html}: shared lifecycle engine script missing`)}
