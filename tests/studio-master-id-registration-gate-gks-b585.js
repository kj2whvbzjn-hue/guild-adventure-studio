...
assert.strictEqual(build.studio_build,'GKS-B864');
...
const prefixes={tags:'TAG',stats:'STA',jobs:'JOB',skills:'SKL',equipment:'EQP',mods:'MOD',monsters:'MON',status_effects:'STS',tablets:'TBL',ai_searches:'AIS',ai_conditions:'AIC',ai_target_selectors:'ATS',ai_actions:'AIA',maps:'MAP',exploration_outcomes:'EXP',reward_tables:'RWD',adventure_settings:'ADV'};
...
assert.strictEqual(valid('ai_searches','AIS-0001'),true);
assert.strictEqual(valid('ai_conditions','AIC-0001'),true);
assert.strictEqual(valid('ai_target_selectors','ATS-0001'),true);
assert.strictEqual(valid('ai_actions','AIA-0001'),true);
assert.strictEqual(valid('ai_target_selectors','AIT-0001'),false,'legacy Target prefix must not be valid for Current ATS');
...
console.log('PASS R10 P2 Master registration/update gate enforces Current AI prefixes + four digits');
