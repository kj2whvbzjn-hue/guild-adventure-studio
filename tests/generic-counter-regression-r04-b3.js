...
assert.ok(/^R04-[BC]\d+/.test(String(registry.phase||'')),`unexpected registry phase ${registry.phase}`);
assert.ok(/^R04-[BC]\d+/.test(String(generic.VERSION||'')),`unexpected generic compiler version ${generic.VERSION}`);
assert.ok(/^R04-[BC]\d+/.test(String(triggerEngine.VERSION||'')),`unexpected trigger engine version ${triggerEngine.VERSION}`);
