...
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 const legacy=loadLegacy(path);for(const c of cases){
  ...
  const r=generic.compileGenericSkill(skill,registry,legacy);ok(r.ok,`${c.tag}: ${JSON.stringify(r.errors)}`);ok(r.legacySkill.tags.includes(c.tag),`${c.tag}: legacy tag missing`);const n=r.normalizedEffects[0];ok(n&&n.lifecycle&&n.lifecycle.stackRule!=='LEGACY',`${c.tag}: lifecycle not formalized`);
 }}
const broken=JSON.parse(JSON.stringify(registry));broken.effects.STUN.lifecycle.removeOnDeath='yes';
const bad=generic.compileGenericSkill({schemaVersion:1,id:'BAD',name:'bad',trigger:{type:'ON_USE'},target:{side:'ENEMY',range:'SINGLE'},effects:[{type:'APPLY',effectId:'STUN',duration:100}]},broken,null);
