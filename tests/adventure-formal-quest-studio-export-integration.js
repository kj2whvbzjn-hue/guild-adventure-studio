...
const retired=structuredClone(data);retired.chapters[0].available_monster_ids=[];retired.chapters[0].sections[0].enemy_budget=6;retired.quests[0].links={chapter_id:'CH-1'};
const retiredIssues=Export.collectRetiredStoryModelIssues(retired);assert(retiredIssues.some(x=>x.code==='RETIRED_CHAPTER_FIELD'&&x.field==='available_monster_ids'));assert(retiredIssues.some(x=>x.code==='RETIRED_SECTION_FIELD'&&x.field==='enemy_budget'));assert(retiredIssues.some(x=>x.code==='RETIRED_QUEST_FIELD'&&x.field==='links'));
