...
let a=Export.formalStoryQuestAssessment(data,q);assert.equal(a.is_formal,false);assert.equal(a.ready,false);assert.equal(a.legacy_runtime_ready,false);assert.equal(a.p5_runtime_ready,false);assert.equal(a.p6_runtime_ready,false);
...
a=Export.formalStoryQuestAssessment(data,q);assert.equal(a.is_formal,true);assert.equal(a.ready,true);assert.equal(a.p5_runtime_ready,true);assert.equal(a.p6_runtime_ready,true);assert.equal(a.legacy_runtime_ready,false);
...
data.quests=[ready];a=Export.formalStoryQuestAssessment(data,ready);assert.equal(a.ready,true);assert.equal(a.p5_runtime_ready,true);assert.equal(a.p6_runtime_ready,true);assert.equal(a.legacy_runtime_ready,true);assert.equal(Export.summarizeFormalStoryQuests(data).p5_runtime_ready_count,1);assert.equal(Export.summarizeFormalStoryQuests(data).p6_runtime_ready_count,1);assert.deepEqual(Export.summarizeFormalStoryQuests(data).ready_ids,['Q-READY']);
...
a=Export.formalStoryQuestAssessment(data,random);assert.equal(a.ready,true);assert.equal(a.p5_runtime_ready,false);assert(a.p5_runtime.issues.some(x=>x.code==='FORMAL_QUEST_P6_RANDOM_EVENT_PENDING'));assert.equal(a.p6_runtime_ready,true);
...
for(const needle of ['id="questFormalStatus"','id="questAdventureDuration"','function renderQuestFormalStatus()','P6 実行可能Quest','Quest.boxes正式Export契約','P6 Game Runtime','正式Export / P6 Runtime'])assert(html.includes(needle),`studio P6 formal integration missing: ${needle}`);
