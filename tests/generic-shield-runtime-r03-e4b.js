...
const legacySkill={id:'E4B-LEGACY-SHIELD',name:'Legacy Shield',tags:['SHIELD','味方','単体','SHIELD=100','DURATION=300']};
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 ...
 const out=generic.compileGenericSkill(skill,registry,ctx.compileTaggedSkill);ok(out.ok,`${path}: generic compile failed ${JSON.stringify(out.errors)}`);
 ...
 const legacyCompiled=ctx.compileTaggedSkill(legacySkill);ok(legacyCompiled.ok,`${path}: legacy SHIELD compile failed ${JSON.stringify(legacyCompiled.errors)}`);
