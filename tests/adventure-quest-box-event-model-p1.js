...
const quest=S.normalizeQuest({
 ...
 boxes:[{
  id:'BOX-LEGACY-ALIAS',name:'入口',order:'2',
  event_before_pre:[
   {event_id:'EVT-FIXED',order:1,failure_policy:'quest_fail',custom_note:'keep'},
   {kind:'random_event',order:2,filter:{event_type:'battle',group:'night',tags:[' rare ','']},allow_none:false,required:true}
  ],
  scenes:{pre_scene_id:'SCN-PRE',mid_scene_id:null,post_scene_id:'SCN-POST'},
  custom_box_field:{keep:true}
 }],
 custom_quest_field:'keep'
});
assert.equal(quest.custom_quest_field,'keep');
assert.deepEqual(quest.context.tags,['forest']);
assert.equal(quest.boxes[0].box_id,'BOX-LEGACY-ALIAS');
...
assert.deepEqual(quest.boxes[0].custom_box_field,{keep:true});
...
assert.equal(quest.boxes[0].event_zone_before_pre[0].custom_note,'keep');
...
const canonical=S.normalizeQuest({boxes:[{box_id:'BOX-001',event_zone_after_post:[{kind:'fixed_event',ref_id:'EVT-2'}]}]});
