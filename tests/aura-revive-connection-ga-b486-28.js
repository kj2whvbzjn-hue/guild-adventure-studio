...
const ctx={console,Math,Date,JSON,GKSFormationTargetResolver:formation,SKILLS:skills,GKSTriggerEngine:{dispatchCompiled(contract,event,payload,run){return{ok:true,result:run()}}},battle:{tick:0,units:[],log:[],validationMode:true,validationEvents:[]},recordValidationEvent:()=>{},queueSceneEvent:()=>{},finishIfNeeded:()=>{},renderBattle:()=>{}};
vm.createContext(ctx);
let src=fs.readFileSync('game/assets/js/tag-skill-runtime.js','utf8');
src += `\n(function(){
 function u(id){return {id,name:id,side:'味方',alive:true,hp:500,maxHp:500,formationPosition:'FRONTLINE',attack:100,gauge:0,modifierStacks:[],dotStacks:[],shieldEffects:[],statusEffects:[],coverEffects:[],cooldowns:{},auraSkillIds:[]}}
