...
assert.strictEqual(trigger.VERSION,'R04-C2');assert.ok(trigger.SUPPORTED.includes('ON_ALLY_ATTACK'));assert.strictEqual(condition.VERSION,'R04-C2');
...
const source=fs.readFileSync('game/assets/js/tag-skill-runtime.js','utf8');assert.ok(source.includes("engine.dispatchCompiled(triggerContract,'ally_attack'"));assert.ok(source.includes('conditionEngine.evaluateCompiled(conditionContract'));assert.ok(source.includes("else{\n    const result=runLegacyFollowUp()"),'Legacy FOLLOW_UP fallback missing');
