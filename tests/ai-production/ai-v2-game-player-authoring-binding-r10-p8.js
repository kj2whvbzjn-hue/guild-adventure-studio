...
(async()=>{
 ...
 const search=session.add('AIS-0001',{target_tag_id:'TAG-TGT-ENEMY',result_slot_id:resultSlot.slot_id,predicate:{logic:'ANY',clauses:[{predicate_master_id:'AIC-0001',params:{operator:'<',value:.5},negate:false}]}},0,0);
 const skill=session.add('AIA-0001',{skill_id:'SKL-P8'},2,0);session.updateTargetSelector(skill.instance_id,{selector_id:'ATS-0001',params:{}});session.updateTargetSource(skill.instance_id,{kind:'SEARCH_RESULT',result_slot_id:resultSlot.slot_id});
 ...
 const evaluation=session.evaluate();assert(evaluation.valid,JSON.stringify(evaluation.issues));assert.deepStrictEqual(evaluation.program.result_slots,[{slot_id:'ARS-0001',name:'低HPの敵',value_type:'UNIT_SET'}]);
 ...
 for(const text of ['検索結果の箱','対象候補の参照先','Predicate','Connection transition','接続を変更','Subroutine','Target Selector','判断プレビュー','構成要約'])assert(uiSource.includes(text),`Game AI authoring surface missing: ${text}`);
 ...
 console.log('AI_V2_GAME_PLAYER_AUTHORING_BINDING_R10_P8_OK player_authoring=1 game_save=1 developer_usage=1 developer_graph_edit=0 binding_single_authority=1 persistent_snapshot_id=0 connections=1 predicate=1 result_slot=1 target_source=1 ats=1 subroutine=1 datasets=5');
