...
(async()=>{
 ...
 const filters=UI.searchFilterBindingsFor(baseCatalog);assert.deepStrictEqual(filters.map(row=>({category_id:row.category_id,definition_id:row.definition.id,tag_field:row.tag_field.name,tags:row.tags.map(tag=>tag.id)})),[{category_id:'TGC-STATE',definition_id:'AIC-0001',tag_field:'state_tag_id',tags:['TAG-STATE-ALIVE','TAG-STATE-HP']}]);
 const search=session.add('AIS-0001',{target_tag_id:'TAG-TGT-ENEMY',predicate:{logic:'ALL',clauses:[{predicate_master_id:'AIC-0001',params:{state_tag_id:'TAG-STATE-HP',value_mode:'RATIO',operator:'<=',value:.5},negate:false}]}},0,0);
 ...
 const skill=session.add('AIA-0001',{skill_id:'SKL-P8'},2,0);session.updateTargetSelector(skill.instance_id,{selector_id:'ATS-0001',params:{}});session.updateTargetSource(skill.instance_id,{kind:'SEARCH_RESULT',result_slot_id:search.parameters.result_slot_id});
 const state=session.add('AIC-0001',{subject_scope:'SELF',predicate:{logic:'ALL',clauses:[{predicate_master_id:'AIC-0001',params:{state_tag_id:'TAG-STATE-ALIVE'},negate:false}]}},0,2);
 ...
 const evaluation=session.evaluate();assert(evaluation.valid,JSON.stringify(evaluation.issues));assert.deepStrictEqual(evaluation.program.result_slots,[{slot_id:'ARS-0001',name:'探索結果 AIN-0001',value_type:'UNIT_SET'}]);
 const previewRuntime=await Compiler.compile(evaluation.program,UI.projectData(baseCatalog));assert(previewRuntime.instructions.some(row=>row.op==='SEARCH'&&row.params.result_slot_id==='ARS-0001'));assert(previewRuntime.instructions.some(row=>row.op==='ACTION'&&row.target_source?.result_slot_id==='ARS-0001'));assert(previewRuntime.instructions.some(row=>row.op==='CALL'));assert(previewRuntime.instructions.some(row=>row.op==='RETURN'));assert.strictEqual(previewRuntime.instructions.some(row=>row.op==='TARGET'),false);
 ...
 for(const text of ['openCategoryDirect','searchFilterEditorHtml','searchStateCompareInlineHtml','絞り込み','候補','割合','現在値','以下','対象候補の参照先','Connection transition','接続を変更','Subroutine','Target Selector','構成要約'])assert(uiSource.includes(text),`Game AI authoring surface missing: ${text}`);assert(!uiSource.includes('data-ai-search-result-slot'),'Player Search must not expose result-slot storage control');assert(!uiSource.includes('検索結果の格納先'),'Player Search must not expose result-slot storage label');assert(!uiSource.includes('見方を選択してください。'),'Player Search must not expose the ambiguous 見方 wording');assert(uiSource.includes('data-ai-search-filter-scale=\"${mode===\'RATIO\'?\'PERCENT\':\'RAW\'}\"'),'RATIO input must use Player-facing percent scaling');assert(uiSource.includes("Number(raw)/100"),'Player-entered percentage must be converted to runtime ratio');assert(uiSource.includes("if(type==='search')ui.configBody.innerHTML")&&uiSource.includes('${searchFilterEditorHtml()}'),'Player Search must render the category/tag filter editor directly');assert(uiSource.includes("if(!currentCategory){ui.candidateList.innerHTML='';"),'Candidate screen must wait for the Player category click');
