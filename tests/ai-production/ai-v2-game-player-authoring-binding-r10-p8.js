...
(async()=>{
 ...
 const autoSlot=session.program().result_slots[0];assert.deepStrictEqual(autoSlot,{slot_id:'ARS-0001',name:'探索結果1',value_type:'UNIT_SET'},'Player Search must receive an internal result slot automatically');assert.strictEqual(session.program().nodes.find(row=>row.instance_id===search.instance_id).parameters.result_slot_id,autoSlot.slot_id);
 const skill=session.add('AIA-0001',{skill_id:'SKL-P8'},2,0);session.updateTargetSelector(skill.instance_id,{selector_id:'ATS-0001',params:{}});session.updateTargetSource(skill.instance_id,{kind:'SEARCH_RESULT',result_slot_id:autoSlot.slot_id});
 ...
 const evaluation=session.evaluate();assert(evaluation.valid,JSON.stringify(evaluation.issues));assert.deepStrictEqual(evaluation.program.result_slots,[{slot_id:'ARS-0001',name:'探索結果1',value_type:'UNIT_SET'}]);
 ...
 for(const text of ['条件','確認対象','検索結果は自動で保持します','対象候補','探索結果を選ぶと','状態確認','Connection transition','接続を変更','Subroutine','Target Selector','構成要約'])assert(uiSource.includes(text),`Game AI authoring surface missing: ${text}`);
 for(const hidden of ['検索結果の格納先','＋ 新しい検索結果の箱','<b>Predicate</b>','Predicateを選択'])assert(!uiSource.includes(hidden),`Player-internal authoring concept must be hidden: ${hidden}`);
 assert(uiSource.includes("if(currentCategory==='condition'){renderInlineStateCheck();return;}"),'State tab must render StateCheck controls directly below the category tabs');
 assert(uiSource.includes("String(row.evaluator||'')==='condition.state_compare'"),'Player StateCheck must resolve the formal state condition by evaluator, not concrete AIC id');
 const cleanupSession=UI.createSession(baseCatalog,{program_id:'AIP-CLEANUP',layout_id:'AIL-0001',data_version:dv,now:'2026-09-03T00:03:00Z'});const cleanupSearch=cleanupSession.add('AIS-0001',{target_tag_id:'TAG-TGT-ENEMY',predicate:{logic:'ALL',clauses:[{predicate_master_id:'AIC-0001',params:{operator:'<',value:.5},negate:false}]}},0,0),cleanupSlot=cleanupSession.program().result_slots[0],cleanupAction=cleanupSession.add('AIA-0001',{skill_id:'SKL-P8'},1,0);cleanupSession.updateTargetSelector(cleanupAction.instance_id,{selector_id:'ATS-0001',params:{}});cleanupSession.updateTargetSource(cleanupAction.instance_id,{kind:'SEARCH_RESULT',result_slot_id:cleanupSlot.slot_id});assert(cleanupSession.remove(cleanupSearch.instance_id));assert.deepStrictEqual(cleanupSession.program().result_slots,[],'Deleting Search must remove its hidden result slot');assert.strictEqual(cleanupSession.program().nodes.find(row=>row.instance_id===cleanupAction.instance_id).target_source,null,'Deleting Search must clear hidden Action target-source binding');
 ...
 console.log('AI_V2_GAME_PLAYER_AUTHORING_BINDING_R10_P8_OK player_authoring=1 player_predicate_internal=1 result_slot_auto=1 state_tab_direct=1 game_save=1 developer_usage=1 developer_graph_edit=0 binding_single_authority=1 persistent_snapshot_id=0 connections=1 target_source=1 ats=1 subroutine=1 datasets=5');
