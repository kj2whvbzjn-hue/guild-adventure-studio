...
const DV='DV-R5A';
const project = {project:{id:'PRJ-R5A',data_version:DV}, data_version:DV, masters:{ai_searches:[],ai_conditions:[],ai_target_selectors:[],ai_actions:[],skills:[]}, tags:[], ai_programs:[], ai_program_layouts:[], ai_program_runtime:[]};
let persistCount = 0;
global.GKSAIProductionHost = {getData:()=>project, getDataVersion:()=>DV, persist:()=>{ persistCount += 1; return true; }, now:()=> '2026-08-11T22:00:00Z'};
const UI = require('../../studio/ai-production/ai-production-ui.js');

const blank = Model.createProgram('AIP-0001', '2026-08-11T21:00:00Z', DV);
...
assert.strictEqual(copy.id, 'AIP-0003');
...
assert.strictEqual(created.id, 'AIP-0005');
...
assert.strictEqual(project.ai_programs.find((item)=>item.id==='AIP-0005').name, '回復優先AI');
assert.deepStrictEqual(project.ai_programs.find((item)=>item.id==='AIP-0005').tags, ['回復','安全']);
assert.strictEqual(UI.isDirty(), false);
assert.strictEqual(UI.openProgram('AIP-0005'), true);
UI.updateDraft('description', '未保存');
assert.strictEqual(UI.isDirty(), true);
assert.strictEqual(UI.revertDraft(), true);
assert.strictEqual(UI.isDirty(), false);
const duplicated = UI.duplicateDraft();
assert.strictEqual(duplicated.id, 'AIP-0007');
assert.strictEqual(UI.isDirty(), true, 'duplicate remains an explicit unsaved draft');
assert.strictEqual(project.ai_programs.some((item)=>item.id==='AIP-0007'), false, 'unsaved duplicate must not mutate project data');
...
assert(html.includes('R10 P7 Developer Authoring'), 'Studio must expose current Developer Authoring stage');
assert(html.includes('GKSAIProductionHost={getData:()=>data,persist:message=>persist(message),now:()=>now(),getDataVersion:()=>'));
