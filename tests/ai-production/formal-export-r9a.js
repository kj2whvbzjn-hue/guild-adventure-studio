...
const data=require('../e2e/minimum-data.js');
const root=path.resolve(__dirname,'../..');
(async()=>{
  assert.deepStrictEqual(RootCore.EXPORT_PATHS,StudioCore.EXPORT_PATHS);assert.strictEqual(RootCore.EXPORT_PATHS.length,24);
  ...
  const pkg=await RootCore.buildPackage(data,{dataVersion:'r9a-1.0.0',generatedAt:'2026-08-11T22:00:00Z',appVersion:'GKS-B527'});assert(pkg.files['ai/ai_programs.json']);assert(pkg.files['ai/ai_runtimes.json']);assert.strictEqual(pkg.manifest.files.length,24);assert(pkg.manifest.files.every((row)=>/^[a-f0-9]{64}$/.test(row.sha256)));
