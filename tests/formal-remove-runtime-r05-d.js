...
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 ...
 const out=generic.compileSkill(sample,registry,ctx.compileTaggedSkill);assert.strictEqual(out.ok,true,JSON.stringify(out.errors));
 ...
 const tampered=JSON.parse(JSON.stringify(out.compiledSkill));tampered.tags=tampered.tags.map(x=>x==='CLEANSE_COUNT=1'?'CLEANSE_COUNT=9':x);const compiled=ctx.compileTaggedSkill(tampered);assert.strictEqual(compiled.ok,true);
