...
const compiledSkill=(id,power)=>({id,name:id,tags:['SHIELD','味方','単体',`SHIELD=${power}`,'DURATION=300']});
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 ...
 const out100=generic.compileSkill(makeSkill('E4C-G100',100),registry,ctx.compileTaggedSkill),out40=generic.compileSkill(makeSkill('E4C-G40',40),registry,ctx.compileTaggedSkill);
 ok(out100.ok&&out40.ok,`${path}: generic compile failed`);
 const c100=ctx.compileTaggedSkill(out100.compiledSkill),c40=ctx.compileTaggedSkill(out40.compiledSkill);ok(c100.ok&&c40.ok,`${path}: generic legacy compile failed`);
 ...
 const l100=ctx.compileTaggedSkill(compiledSkill('E4C-L100',100)),l40=ctx.compileTaggedSkill(compiledSkill('E4C-L40',40));ok(l100.ok&&l40.ok,`${path}: legacy compile failed`);
