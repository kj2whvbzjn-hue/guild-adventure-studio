...
for(const f of ['assets/shared/js/skill-budget-engine.js','assets/shared/js/formal-contribution-resolver.js','assets/shared/js/skill-compiler.js','assets/shared/js/formal-passive-compiler.js','assets/shared/js/skill-ai-batch-engine.js'])vm.runInContext(fs.readFileSync(f,'utf8'),ctx,{filename:f});
(async()=>{
 ...
 const passive=await ctx.GKSSkillAiBatchEngine.generateBatch({schema:'GKS_SKILL_AI_BATCH_REQUEST',version:'1.0.0',mode:'PASSIVE',requests:[
  {intent:'HP自動回復',passiveSeriesId:'PASS-SER-14',stage:1,category:'Periodic',requirement:{VIT:20},effect:{kind:'PERIODIC_HP_REGEN',period_tick:20,max_hp_percent:1}},
  {intent:'最大HP上昇',passiveSeriesId:'PASS-SER-01',stage:1,category:'Modifier',requirement:{VIT:20},modifierChannel:'MAX_HP',effect:{channel:'MAX_HP',modifier_percent:5}},
  {intent:'属性耐性',passiveSeriesId:'PASS-SER-03',stage:1,category:'Modifier',requirement:{MND:20},modifierChannel:'ELEMENT_RESISTANCE:<ELEMENT>',effect:{channel:'ELEMENT_RESISTANCE:<ELEMENT>',modifier_percent:5}},
  {intent:'混在Reject',passiveSeriesId:'PASS-SER-X',stage:1,category:'Periodic',requirement:{VIT:20},skillLevel:2,effect:{kind:'PERIODIC_HP_REGEN',period_tick:20,max_hp_percent:1}}
 ]},{registry,rules:aiRules,compilePassive:ctx.GKSFormalPassiveCompiler.compilePassive,passiveSkills:[],validateFinal:true,passiveIdPrefix:'G05-PAS'});
 assert.deepStrictEqual(JSON.parse(JSON.stringify(passive.summary)),{total:4,accepted:1,rejected:3,allAccepted:false});assert.strictEqual(passive.mode,'PASSIVE');assert.strictEqual(passive.entries[0].status,'ACCEPT');assert.strictEqual(passive.entries[0].passive.passiveSeriesId,'PASS-SER-14');assert.deepStrictEqual(JSON.parse(JSON.stringify(passive.entries[0].passive.params.ability_conditions)),[{stat:'VIT',min:20}]);assert.deepStrictEqual(JSON.parse(JSON.stringify(passive.entries[0].passive.runtimeContracts.periodicContract)),{resource:'HP',intervalTicks:20,initialDelayTicks:20,recoveryRate:0.01});
 assert.ok(passive.entries[1].validation.issues.some(x=>x.code==='PASSIVE_BINDING_REQUIRED'));assert.ok(passive.entries[2].validation.issues.some(x=>x.code==='PASSIVE_PARAMETER_BINDING_REQUIRED'));assert.ok(passive.entries[3].validation.issues.some(x=>x.code==='AI_MODE_FIELD_MIXED'));
 const explicit=await ctx.GKSSkillAiBatchEngine.generateBatch({schema:'GKS_SKILL_AI_BATCH_REQUEST',version:'1.0.0',mode:'PASSIVE',requests:[{intent:'既存MOD参照',passiveSeriesId:'PASS-SER-01',stage:1,category:'Modifier',requirement:{VIT:20},modifierChannel:'MAX_HP',modifierRefs:{modIds:['MOD-EXPLICIT'],effectIds:[],combatCapabilities:[]}}]},{registry,rules:aiRules,validateFinal:false});assert.strictEqual(explicit.entries[0].status,'ACCEPT');assert.deepStrictEqual(JSON.parse(JSON.stringify(explicit.entries[0].passive.params.mod_ids)),['MOD-EXPLICIT']);
