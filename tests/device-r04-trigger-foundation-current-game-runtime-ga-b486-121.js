...
for(const html of [tagIndex,gameIndex]){
  assert.ok(html.includes('id="tagTestRunR04TriggerFoundationJson"'),'R04 Trigger foundation device button missing');
}
for(const src of [tagRuntime,gameRuntime]){
  assert.ok(src.includes('runR04TriggerFoundationDeviceValidation'),'R04 Trigger device runner missing');
  assert.ok(src.includes("'ON_USE','ON_HIT_RECEIVED','ON_DAMAGE_DEALT','ON_TURN_START','ON_TURN_END','ON_DEATH','ON_STATUS_APPLIED'"),'R04-A seven-trigger device scope missing');
  assert.ok(src.includes('TRIGGER_TYPE_UNSUPPORTED'),'unknown trigger rejection missing');
  assert.ok(src.includes('TRIGGER_ENGINE_EVENT_MISMATCH'),'event mismatch rejection missing');
...
console.log('DEVICE_R04_TRIGGER_FOUNDATION_CURRENT_GAME_RUNTIME_GA_B486_121_PASS');
