...
global.window=global;global.data={tags:[],masters:{equipment:[]}};let persistCount=0;global.persist=()=>{persistCount++};
...
(async()=>{
 ...
 if('commit' in GKSEquipmentGenerator||'commitBatch' in GKSEquipmentGenerator)throw new Error('legacy direct Equipment Master commit API remains');
 const sim=GKSEquipmentGenerator.simulateBatch({kind:'weapon',base_item_types:['片手剣','杖'],item_level:{min:1,max:2}});
 ...
 const batch=GKSEquipmentGenerator.generateBatch({kind:'armor',base_item_types:['重装'],armor_slots:['鎧','頭'],item_level_min:1,item_level_max:2});
 if(batch.summary.count!==4||batch.summary.invalid!==0)throw new Error('batch preview failed');
 if(data.masters.equipment.length!==0||persistCount!==0)throw new Error('generator preview must not write Equipment Master');
 const ai=GKSEquipmentGenerator.prepareAiRequest({kind:'weapon',base_item_types:['片手剣'],item_level:{min:1,max:3},seed:'x'});
 ...
 const grown=GKSEquipmentGenerator.generate({kind:'armor',base_item_type:'重装',armor_slot:'鎧',item_level:2,id:'EQP-0001'});
 if(grown.record.required_vit!==12||grown.record.hp_bonus!==360||grown.record.mp_bonus!==120||grown.record.evasion!==2)throw new Error('armor growth separation failed');
