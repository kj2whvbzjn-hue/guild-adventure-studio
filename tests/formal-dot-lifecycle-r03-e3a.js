...
const compiledSkill={id:'R03E3A-LEGACY-DOT',name:'Legacy DOT',tags:['DOT','敵','単体','DOT_POWER=12','DOT_DURATION=300','DOT_INTERVAL=100','STACK_GAIN=2']};
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 ...
 const out=generic.compileSkill(genericSkill,registry,ctx.compileTaggedSkill);ok(out.ok,`${path}: generic compile failed ${JSON.stringify(out.errors)}`);
 ...
 const legacyCompiled=ctx.compileTaggedSkill(compiledSkill);ok(legacyCompiled.ok,`${path}: legacy DOT compile failed`);
