...
for(const path of ['game/assets/js/tag-skill-runtime.js']){
 ...
 const out=formal.compileSkill(sample,registry);ok(out.ok,`${path}: formal compile failed ${JSON.stringify(out.errors)}`);
 const compiled=ctx.compileSkillForRuntime(out.compiledSkill);ok(compiled.ok,`${path}: formal runtime compile failed ${JSON.stringify(compiled.errors)}`);
 ...
 ok(src.includes("applyTaggedStatus(source,target,runtimeCompiled,lifecycleRef.formal?lifecycleRef.policy:null)"),`${path}: production STATUS path does not delegate policy`);
}
console.log('FORMAL_STATUS_RUNTIME_R03_E2B_PASS');
