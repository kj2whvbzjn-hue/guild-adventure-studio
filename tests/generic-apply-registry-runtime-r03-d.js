...
ok(/^R03-(D|E\d+[a-z]?|F\d+[a-z]?)$/.test(String(registry.phase||'')),'registry phase predates R03-D');
...
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){
 ...
 const compiled=legacy(out.legacySkill);ok(compiled.ok,`${path}: legacy compile rejected genericRuntime ${compiled.errors}`);
 ok(compiled.definition.genericRuntime?.applyContracts?.[0]?.effectId==='STUN',`${path}: compileTaggedSkill did not preserve contract`);
 const bad=JSON.parse(JSON.stringify(out.legacySkill));bad.genericRuntime.applyContracts=[];
...
const dup=generic.compileGenericSkill(duplicate,registry,null);ok(!dup.ok,'duplicate legacy APPLY logic accepted');ok(dup.errors.some(x=>x.code==='LEGACY_APPLY_LOGIC_DUPLICATE'),'duplicate apply logic error missing');
