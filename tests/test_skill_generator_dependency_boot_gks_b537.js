...
assert.strictEqual(build.studio_build,'GKS-B577');
...
assert.ok(html.includes('Game GA-B486.180 / Studio GKS-B577'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','577')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b577"'),'Studio cache namespace must advance');
