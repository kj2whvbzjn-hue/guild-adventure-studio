...
assert.ok(html.includes('Game GA-B486.121 / Studio GKS-B553'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','553')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b553"'),'Studio cache namespace must advance');
