...
assert.ok(html.includes('Game GA-B486.176 / Studio GKS-B562'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','562')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b562"'),'Studio cache namespace must advance');
