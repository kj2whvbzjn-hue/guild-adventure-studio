...
assert.ok(html.includes('Game GA-B486.182 / Studio GKS-B587'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','587')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b587"'),'Studio cache namespace must advance');
