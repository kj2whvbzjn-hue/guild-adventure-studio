...
assert.strictEqual(build.studio_build,'GKS-B605');
...
assert.ok(html.includes('Game GA-B486.190 / Studio GKS-B605'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','605')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b605"'),'Studio cache namespace must advance');
