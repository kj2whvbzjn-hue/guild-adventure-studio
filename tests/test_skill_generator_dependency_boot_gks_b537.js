...
assert.strictEqual(build.studio_build,'GKS-B584');
...
assert.ok(html.includes('Game GA-B486.181 / Studio GKS-B584'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','584')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b584"'),'Studio cache namespace must advance');
