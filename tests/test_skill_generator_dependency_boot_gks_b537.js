...
assert.strictEqual(build.studio_build,'GKS-B610');
...
assert.ok(html.includes('Game GA-B486.190 / Studio GKS-B610'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','610')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b610"'),'Studio cache namespace must advance');
