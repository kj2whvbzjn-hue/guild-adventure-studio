...
assert.strictEqual(build.studio_build,'GKS-B551');
...
assert.ok(html.includes('Game GA-B486.121 / Studio GKS-B551'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','551')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b551"'),'Studio cache namespace must advance');
