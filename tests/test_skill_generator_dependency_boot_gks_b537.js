...
assert.strictEqual(build.studio_build,'GKS-B559');
...
assert.ok(html.includes('Game GA-B486.175 / Studio GKS-B559'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','559')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b559"'),'Studio cache namespace must advance');
