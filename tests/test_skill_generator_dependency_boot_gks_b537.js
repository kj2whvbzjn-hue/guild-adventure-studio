...
assert.strictEqual(build.studio_build,'GKS-B591');
...
assert.ok(html.includes('Game GA-B486.183 / Studio GKS-B591'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','591')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b591"'),'Studio cache namespace must advance');
