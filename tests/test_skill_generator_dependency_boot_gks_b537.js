...
assert.ok(html.includes('Game GA-B486.188 / Studio GKS-B595'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','595')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b595"'),'Studio cache namespace must advance');
