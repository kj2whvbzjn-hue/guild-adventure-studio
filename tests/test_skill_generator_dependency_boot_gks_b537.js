...
assert.strictEqual(build.studio_build,'GKS-B558');
...
assert.ok(html.includes('Game GA-B486.175 / Studio GKS-B558'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','558')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b558"'),'Studio cache namespace must advance');
