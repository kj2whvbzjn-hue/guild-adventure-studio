...
assert.strictEqual(build.studio_build,'GKS-B607');
...
assert.ok(html.includes('Game GA-B486.190 / Studio GKS-B607'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','607')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b607"'),'Studio cache namespace must advance');
