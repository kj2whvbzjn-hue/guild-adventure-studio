...
assert.ok(html.includes('Game GA-B486.174 / Studio GKS-B556'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','556')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b556"'),'Studio cache namespace must advance');
