...
assert.strictEqual(build.studio_build,'GKS-B564');
...
assert.ok(html.includes('Game GA-B486.178 / Studio GKS-B564'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','564')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b564"'),'Studio cache namespace must advance');
