...
assert.strictEqual(build.studio_build,'GKS-B537');
...
assert.ok(html.includes('Game GA-B486.121 / Studio GKS-B537'),'Studio header build must not show the stale GKS-B527 label');
assert.ok(html.includes('./skill/skill-generator.js?v=15'),'Skill Generator cache key must advance');
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=537'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','537')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b537"'),'Studio cache namespace must advance');
console.log('PASS GKS-B537 Skill Generator bounded dependency boot / precache regression');
