...
assert.ok(html.includes('Game GA-B486.121 / Studio GKS-B545'),'Studio header build must not show the stale GKS-B527 label');
assert.ok(html.includes('./skill/skill-generator.js?v=23'),'Skill Generator cache key must advance');
assert.ok(html.includes("navigator.serviceWorker.register('./sw.js?v=545'"),'Studio service worker URL must advance');
assert.ok(html.includes("url.searchParams.set('appv','545')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b545"'),'Studio cache namespace must advance');
