...
assert.strictEqual(build.studio_build,'GKS-B575');
...
assert.ok(html.includes('Game GA-B486.180 / Studio GKS-B575'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','575')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b575"'),'Studio cache namespace must advance');
