...
assert.strictEqual(build.studio_build,'GKS-B560');
...
assert.ok(html.includes('Game GA-B486.175 / Studio GKS-B560'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','560')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b560"'),'Studio cache namespace must advance');
