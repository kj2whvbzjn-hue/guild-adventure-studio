...
assert.ok(html.includes('Game GA-B486.175 / Studio GKS-B561'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','561')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b561"'),'Studio cache namespace must advance');
