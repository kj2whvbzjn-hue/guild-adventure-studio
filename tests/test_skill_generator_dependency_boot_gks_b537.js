...
assert.strictEqual(build.studio_build,'GKS-B547');
...
assert.ok(html.includes('Game GA-B486.121 / Studio GKS-B547'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','547')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b547"'),'Studio cache namespace must advance');
