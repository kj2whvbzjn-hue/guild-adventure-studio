...
assert.strictEqual(build.studio_build,'GKS-B606');
...
assert.ok(html.includes('Game GA-B486.190 / Studio GKS-B606'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','606')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b606"'),'Studio cache namespace must advance');
