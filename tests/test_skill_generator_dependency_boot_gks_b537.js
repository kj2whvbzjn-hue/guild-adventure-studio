...
assert.strictEqual(build.studio_build,'GKS-B565');
...
assert.ok(html.includes('Game GA-B486.179 / Studio GKS-B565'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','565')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b565"'),'Studio cache namespace must advance');
