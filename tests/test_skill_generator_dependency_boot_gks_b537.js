...
assert.strictEqual(build.studio_build,'GKS-B572');
...
assert.ok(html.includes('Game GA-B486.180 / Studio GKS-B572'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','572')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b572"'),'Studio cache namespace must advance');
