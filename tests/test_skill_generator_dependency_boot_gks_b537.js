...
assert.strictEqual(build.studio_build,'GKS-B589');
...
assert.ok(html.includes('Game GA-B486.182 / Studio GKS-B589'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','589')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b589"'),'Studio cache namespace must advance');
