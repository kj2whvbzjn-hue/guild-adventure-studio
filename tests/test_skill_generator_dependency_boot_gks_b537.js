...
assert.strictEqual(build.studio_build,'GKS-B592');
...
assert.ok(html.includes('Game GA-B486.183 / Studio GKS-B592'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','592')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b592"'),'Studio cache namespace must advance');
