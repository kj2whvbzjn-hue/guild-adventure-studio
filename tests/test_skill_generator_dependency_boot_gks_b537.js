...
assert.ok(html.includes('Game GA-B486.187 / Studio GKS-B594'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','594')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b594"'),'Studio cache namespace must advance');
