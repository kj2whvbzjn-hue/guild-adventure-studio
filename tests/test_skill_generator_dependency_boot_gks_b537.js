...
assert.strictEqual(build.studio_build,'GKS-B563');
...
assert.ok(html.includes('Game GA-B486.177 / Studio GKS-B563'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','563')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b563"'),'Studio cache namespace must advance');
