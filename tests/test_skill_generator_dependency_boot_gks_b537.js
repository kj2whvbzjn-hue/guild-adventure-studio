...
assert.strictEqual(build.studio_build,'GKS-B599');
...
assert.ok(html.includes('Game GA-B486.189 / Studio GKS-B599'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','599')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b599"'),'Studio cache namespace must advance');
