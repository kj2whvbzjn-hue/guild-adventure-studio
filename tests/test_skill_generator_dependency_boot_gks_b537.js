...
assert.strictEqual(build.studio_build,'GKS-B576');
...
assert.ok(html.includes('Game GA-B486.180 / Studio GKS-B576'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','576')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b576"'),'Studio cache namespace must advance');
