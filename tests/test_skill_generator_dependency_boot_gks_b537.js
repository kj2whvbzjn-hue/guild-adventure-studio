...
assert.ok(html.includes('Game GA-B486.191 / Studio GKS-B613'),'Studio header build must not show the stale GKS-B527 label');
...
assert.ok(html.includes("url.searchParams.set('appv','613')"),'Studio reload appv must advance');
assert.ok(sw.includes('const CACHE_NAME="gks-studio-b613"'),'Studio cache namespace must advance');
