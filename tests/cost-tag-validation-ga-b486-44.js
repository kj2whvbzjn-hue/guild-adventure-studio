...
const data=JSON.parse(fs.readFileSync('Export/skill/skills.json','utf8'));assert.strictEqual(data.data_version,'FORMAL-SKILL-1');const prod=data.data.find(x=>x.id==='SKL-COST-MP-20-CD-300');assert(prod&&prod.environment==='production','production fixture missing');assert.strictEqual(prod.runtimeContracts?.resourceContract?.mpCost,20,'production mpCost contract mismatch');assert.strictEqual(prod.runtimeContracts?.resourceContract?.cooldown,300,'production cooldown contract mismatch');
