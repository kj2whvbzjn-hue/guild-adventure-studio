...
assert.ok(html.includes("if(isUnsafeDeployPath(relative))continue;"),'unsafe path protection must remain unconditional');
assert.ok(html.includes("if(!isDeployDeleteManifestAllowed(relative))continue;"),'only DELETE_MANIFEST paths may reach delete candidate classification');
assert.ok(html.includes("if(protectedSet.has(relative)&&!candidate.protectedDelete)continue;"),'legacy protected paths may not preempt an approved policy-protected delete');
assert.ok(!html.includes("if(protectedSet.has(relative)||isUnsafeDeployPath(relative))continue;"),'legacy protected set must not discard formal protected-delete candidates before dual approval');
