...
for(const [id,def] of Object.entries(registry.effects||{})){ok(def.lifecycle,`${id}: lifecycle missing`);for(const k of registry.apply_model.required_lifecycle_fields)ok(Object.prototype.hasOwnProperty.call(def.lifecycle,k),`${id}: ${k} missing`)}
...
for(const path of ['game/assets/js/tag-skill-runtime.js','game-tag-test/assets/js/tag-skill-runtime.js']){const r=generic.compileGenericSkill(skill,registry,loadLegacy(path));ok(r.ok,JSON.stringify(r.errors));const a=r.normalizedEffects.find(x=>x.type==='APPLY');ok(a&&a.effectId==='BURN','normalized APPLY missing');ok(a.lifecycle&&a.lifecycle.stackRule,'lifecycle normalization missing');ok(r.legacySkill.tags.includes('DOT'),'legacy DOT mapping missing')}
