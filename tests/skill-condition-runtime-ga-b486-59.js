...
function check(tags,expected,label){const compiled=ctx.compileTaggedSkill({id:'C',name:'C',tags:['ATTACK','敵','単体','DAMAGE=1',...tags]});if(!compiled.ok)throw new Error(label+' compile '+compiled.errors.join(','));ctx.compiled=compiled;ctx.actor=actor;const r=vm.runInContext('evaluateTaggedSkillConditions(actor,compiled)',ctx);if(r.ok!==expected)throw new Error(label+' expected '+expected+' got '+r.ok+' '+JSON.stringify(r.results));}
