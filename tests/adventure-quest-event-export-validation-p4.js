...
const exportIssues=Core.collectFormalQuestExportIssues(p4Only);assert(exportIssues.issues.some(x=>x.level==='WARNING'&&x.code==='FORMAL_QUEST_LEGACY_RUNTIME_NOT_READY'));
...
for(const token of ['GKExportCore.collectQuestEventContractIssues(data)','P4 Export契約 合格','警告 '+"'",'Quest.boxes正式Export契約','P5前の現行Game'])assert(html.includes(token),`Studio P4 integration missing ${token}`);
