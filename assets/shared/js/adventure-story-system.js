...
function simulateQuest(opts){
 ...
 for(const box of section.boxes){if(failed)break;const base={box_id:box.id,type:box.type,ref_id:box.ref_id};try{
   ...
   if(box.type==='random_battle'){const budget=typeof opts.enemyBudget==='function'?Number(opts.enemyBudget({quest,section,chapter,box}))||0:Number(opts.enemyBudget)||0;const generated=generateRandomBattle({budget,monsterIds:chapter.available_monster_ids,monsters,random,maxUnits:opts.maxRandomBattleUnits||100});const br=typeof opts.simulateBattle==='function'?opts.simulateBattle({formation:generated.formation,seed:Math.floor(random()*0xffffffff),box,quest}):{victory:true,formation:generated.formation,reward:{},playback_events:[]};assertBattlePlaybackEvents(br);battle_results.push({...clone(br),formation:clone(generated.formation),budget:generated.budget});mergeReward(br?.reward);processed.push({...base,result_index:battle_results.length-1});if(br?.victory===false)failed={reason:br.reason||'battle_lost',box_id:box.id};}
