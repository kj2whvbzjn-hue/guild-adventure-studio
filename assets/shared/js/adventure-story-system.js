...
function buildBattleResolverRequest({quest,event,box,placement,flags={},startCostResult={},tablets=[],encounterSeed=0,randomSelection=null}={}){
 const q=normalizeQuest(clone(quest||{})),e=normalizeEvent(clone(event||{})),context=q.context&&typeof q.context==='object'?clone(q.context):{},startResources=startCostResult?.cost?.resources&&typeof startCostResult.cost.resources==='object'?startCostResult.cost.resources:{};
 return{contract:'adventure_battle_encounter_request',contract_version:1,quest_id:String(q.id||''),event_id:String(e.id||''),box_id:String(box?.box_id||box?.id||''),placement_kind:String(placement?.kind||'fixed_event'),placement_order:Math.max(1,Math.floor(Number(placement?.order)||1)),failure_policy:QUEST_EVENT_FAILURE_POLICIES.has(placement?.failure_policy)?placement.failure_policy:'continue',quest_context:context,quest_difficulty:context.difficulty??null,event_intensity:String(e.intensity||''),generation_profile_ref:e.generation_profile_ref==null?null:String(e.generation_profile_ref),enemy_budget:resolveEnemyBudget({quest:q,startCostResources:startResources,tablets}),budget_policy:context.budget&&typeof context.budget==='object'&&!Array.isArray(context.budget)?clone(context.budget):null,flags:clone(flags||{}),encounter_seed:Number(encounterSeed)>>>0,random_selection:randomSelection?clone(randomSelection):null};
}
...
function simulateQuestBoxRuntime(opts,normalizedQuest){
 ...
 const seed=opts.seed??hashSeed(`${quest.id||''}|${Date.now()}`),random=rng(seed),workingFlags={...(opts.flags||{})},processed=[],battle_results=[],event_results=[],scene_snapshots=[],random_selections=[],rewardHistory=[];let failed=null;
 ...
 function runResolvedEvent(box,zoneKey,placement,event,{placementKind='fixed_event',randomSelectionIndex=-1,eventRandom=random}={}){
  ...
  try{
   ...
   if(String(event?.type||'')==='battle'&&typeof opts.resolveBattleEncounter==='function'){
    const encounterSeed=Math.floor(eventRandom()*0x100000000)>>>0,request=buildBattleResolverRequest({quest,event,box,placement,flags:workingFlags,startCostResult:opts.startCostResult,tablets:opts.tablets,encounterSeed,randomSelection:selection});
