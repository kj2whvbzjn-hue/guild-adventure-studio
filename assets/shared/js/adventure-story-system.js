...
function simulateQuestBoxRuntime(opts,normalizedQuest){
 ...
 function runPlacement(box,zoneKey,placement){
  if(failed)return;
  if(placement.kind==='random_event'){fail(new Error('Random Event Runtime is not available in P5'),box,{event_zone:zoneKey,placement_kind:'random_event',placement_order:placement.order});return;}
  const eventId=String(placement.event_id||'');const event=eventById.get(eventId);if(!event){fail(new Error(`Event not found: ${eventId}`),box,{event_id:eventId,event_zone:zoneKey,placement_kind:'fixed_event',placement_order:placement.order});return;}
  try{const er=typeof opts.resolveEvent==='function'?opts.resolveEvent({event:clone(event),flags:clone(workingFlags),random,quest:clone(quest),box:clone(box),placement:clone(placement),eventZone:zoneKey}):{success:true,reward:{},flags:{}};
  const eventFailed=Boolean(er?.failed||er?.success===false);event_results.push({event_id:event.id,type:String(event.type||'special'),failure_policy:placement.failure_policy,event_zone:zoneKey,placement_order:placement.order,...clone(er)});mergeReward(er?.reward);applyFlags(er?.flags);processed.push({box_id:box.box_id,type:'event',ref_id:eventId,resolved_ref_id:event.id,event_zone:zoneKey,placement_kind:'fixed_event',placement_order:placement.order,failure_policy:placement.failure_policy,result_index:event_results.length-1});
  if(eventFailed&&placement.failure_policy==='quest_fail')failed={reason:er?.reason||'event_failed',box_id:box.box_id,event_id:event.id,event_zone:zoneKey,placement_order:placement.order};}catch(error){fail(error,box,{event_id:eventId,event_zone:zoneKey,placement_kind:'fixed_event',placement_order:placement.order});}
 }
