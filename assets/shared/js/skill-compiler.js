...
function compileGenericSkill(skill,registry,legacyCompile){
 ...
 return result();
 function result(){const runtimeContracts={schemaVersion:1,registryPhase:String(registry?.phase||''),triggerContract:buildTriggerContract(skill,triggerDef),conditionContracts:conditionContracts.map(c=>({...c})),effectContracts:effectContracts.map(c=>({...c})),applyContracts:applyContracts.map(c=>({...c,lifecycle:{...c.lifecycle}})),auraEffectContract:auraEffectContract?{...auraEffectContract}:null};const compiledSkill={id:String(skill?.id||''),name:String(skill?.name||''),tags:[...tags],runtimeContracts,genericRuntime:runtimeContracts};return{ok:errors.length===0,version:VERSION,errors,warnings,normalizedEffects:[...normalizedEffects],compiledSkill,legacySkill:compiledSkill,legacyValidation}}
}
