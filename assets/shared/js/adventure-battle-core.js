...
function monsterStats(monster){const p=monster?.params&&typeof monster.params==='object'?monster.params:{},skillIds=normalizeSkillIds(p.skill_ids??p.skillIds??monster?.skill_ids??monster?.skillIds),monsterId=String(monster?.id||monster?.name||'Monster'),jobRef=String(p.job_code??p.job_id??p.job??monster?.job_code??monster?.job_id??monster?.job??'').trim(),levelValue=p.level??monster?.level,formalGenerated=!!jobRef&&Number.isFinite(Number(levelValue)),formalAiBinding=normalizeFormalAiBinding(monster?.formalAiBinding,{nullable:false});const defaultFormationPosition=normalizeFormationPosition(monster?.default_formation_position??p.default_formation_position,{nullable:false});if(formalGenerated){const maxHp=requiredMonsterCombatNumber(p.maxHp??p.max_hp??p.hp??monster?.maxHp??monster?.max_hp??monster?.hp,`${monsterId}.maxHp`,{min:1}),maxMp=requiredMonsterCombatNumber(p.maxMp??p.max_mp??p.mp??monster?.maxMp??monster?.max_mp??monster?.mp,`${monsterId}.maxMp`,{min:0}),attack=requiredMonsterCombatNumber(p.attack??p.atk??monster?.attack??monster?.atk,`${monsterId}.attack`,{min:0}),agi=requiredMonsterCombatNumber(p.agi??monster?.agi,`${monsterId}.agi`,{min:0});return{name:String(monster?.name||monster?.id||'Monster'),defaultFormationPosition,maxHp,maxMp,attack,agi,...(skillIds.length?{skillIds}:{}),formalAiBinding,formalGenerated:true};}throw Object.assign(new Error(`Non-Formal Monster is not accepted by Current Battle Runtime: ${monsterId}`),{code:'NON_FORMAL_MONSTER_NOT_ALLOWED',monster_id:monsterId});}
...
return{clone,FORMATION_POSITIONS,normalizeFormationPosition,normalizeFormation,normalizeFormalAiBinding,monsterStats,expandFormation,validationEventsToPlaybackEvents,buildBattleResult};
});
