...
function monsterStats(monster){const p=monster?.params&&typeof monster.params==='object'?monster.params:{},skillIds=normalizeSkillIds(p.skill_ids??p.skillIds??monster?.skill_ids??monster?.skillIds),requestedDefault=String((p.defaultSkillId??p.default_skill_id??monster?.defaultSkillId??monster?.default_skill_id)||'').trim(),defaultSkillId=skillIds.length?(skillIds.includes(requestedDefault)?requestedDefault:skillIds[0]):(requestedDefault||'SKL-TEST-ATTACK');const defaultFormationPosition=normalizeFormationPosition(monster?.default_formation_position??p.default_formation_position,{nullable:false});return{name:String(monster?.name||monster?.id||'Monster'),defaultFormationPosition,maxHp:Math.max(1,Math.floor(Number(p.maxHp??p.max_hp??p.hp??monster?.maxHp??monster?.max_hp??monster?.hp)||100)),attack:Math.max(0,Math.floor(Number(p.attack??p.atk??monster?.attack??monster?.atk)||10)),agi:Math.max(1,Math.floor(Number(p.agi??monster?.agi)||10)),aiPolicy:String((p.aiPolicy??p.ai_policy??monster?.aiPolicy??monster?.ai_policy)||'lowestHp'),...(skillIds.length?{skillIds}:{}),defaultSkillId};}
...
return{clone,FORMATION_POSITIONS,normalizeFormationPosition,normalizeFormation,monsterStats,expandFormation,validationEventsToPlaybackEvents,buildBattleResult,seededRandom,normalizePartySnapshot,simulateBasicBattle};
});
