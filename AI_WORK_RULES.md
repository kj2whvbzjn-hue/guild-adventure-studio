...
- Build更新時は`tools/release/sync-current-build-tokens.py --write`をmanifest再生成前に必須実行し、`run-approved-flow.sh`の`--check`でcurrent Build token driftをRelease前に拒否する。Build tokenだけの追随はTest Integrity承認対象にしない。
